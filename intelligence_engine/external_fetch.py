...
def _merge_rows(old: pd.DataFrame, new: pd.DataFrame, replace_tickers: set[str]) -> pd.DataFrame:
    if not old.empty and "ticker" in old.columns:
        old = old[~old["ticker"].astype(str).str.upper().isin(replace_tickers)]
    if old.empty:
...
def refresh_external_data(tickers: list[str], root: Path, *, ttl_hours: float = 20.0, max_tickers: int = 120) -> dict:
    ...
    for ticker in pending:
        cov = {key: False for key in FILES}
        status, error = "ok", ""
        try:
    ...
    for key, filename in FILES.items():
        ...
        replace = set(new["ticker"].astype(str).str.upper()) if not new.empty else set()
        _write(_merge_rows(_read(root / filename), new, replace), root / filename)
    _write(_merge_rows(_read(root / "provider_coverage.csv"), pd.DataFrame(coverage_rows), set(pending)), root / "provider_coverage.csv")
    return {
        "provider": "yfinance",
