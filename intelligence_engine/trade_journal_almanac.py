...
def _decision(report: JournalReport) -> dict[str, Any]:
    ...
    if not holdings.empty:
        if "event_risk" in holdings:
            event_count = int(holdings["event_risk"].fillna("").astype(str).str.strip().ne("").sum())
        if {"current_price", "stop_price"}.issubset(holdings.columns):
            ...
            stop_near = int((distance <= 0.02).fillna(False).sum())
...
(() => {
  ...
  const fmtPct = (v, signed=false, digits=1) => `${signed && v>0?'+':''}${(Number(v||0)*100).toFixed(digits)}%`;
  const fmtNum = (v, digits=2) => Number(v||0).toFixed(digits);
  const fmtYen = v => `${Number(v||0)<0?'-':''}¥${Math.abs(Math.round(Number(v||0))).toLocaleString('ja-JP')}`;
  const tone = v => Number(v||0)>0?'pos':Number(v||0)<0?'neg':'';
  ...
  function renderBars(rows, target, limit=99) {
    ...
    document.getElementById(target).innerHTML = list.map(r => {
      ...
      return `<div class="bar-row"><span>${name}</span><div class="bar-track"><i style="width:${Math.min(100,value/max*100).toFixed(0)}%"></i></div><b class="num">${fmtPct(value)}</b></div>`;
  ...
  document.getElementById('conc-list').innerHTML = holdings.map((h,i)=>`<div class="conc-row"><span class="conc-rank">${i+1}</span><span class="conc-ticker">${h.ticker}</span><span class="conc-track ${Number(h.unrealized_pct||0)>=0?'up':'down'}"><i style="width:${(Number(h.allocation||0)/maxAlloc*100).toFixed(0)}%"></i></span><span class="conc-pct num">${fmtPct(h.allocation)}</span><span class="conc-pnl num ${tone(h.unrealized_pct)}">${fmtPct(h.unrealized_pct,true)}</span></div>`).join('') || '<div class="stub">保有なし</div>';

  function stopDistance(h){ const c=Number(h.current_price||0), s=Number(h.stop_price||0); return c ? (c-s)/c : 0; }
  document.getElementById('holdings-cards').innerHTML = holdings.map(h=>`<details class="holding-card"><summary><div class="holding-main"><div><div class="holding-title">${h.ticker}${h.event_risk?'<span class="risk-badge">EVENT</span>':''}<small>${h.setup||'UNKNOWN'}｜${h.sector||'UNKNOWN'}</small></div><div class="holding-quick"><div><span>配分</span><b class="num">${fmtPct(h.allocation)}</b></div><div><span>保有</span><b class="num">${Number(h.hold_days||0)}日</b></div><div><span>Stop距離</span><b class="num ${stopDistance(h)<=.02?'neg':''}">${fmtPct(stopDistance(h),true)}</b></div></div></div><div class="holding-pnl num ${tone(h.unrealized_pct)}">${fmtPct(h.unrealized_pct,true)}<small style="display:block;color:var(--muted);font-size:7px;margin-top:2px">${fmtYen(h.unrealized_pnl_jpy)}</small></div></div></summary><div class="holding-detail"><div><span>現在値</span><b class="num">${fmtNum(h.current_price)}</b></div><div><span>Entry</span><b class="num">${fmtNum(h.entry_price)}</b></div><div><span>Stop</span><b class="num">${fmtNum(h.stop_price)}</b></div><div><span>Heat</span><b class="num">${fmtPct(h.heat_fraction)}</b></div><div><span>評価額</span><b class="num">${fmtYen(h.market_value_jpy)}</b></div><div><span>Theme</span><b>${h.theme||'UNKNOWN'}</b></div><div><span>Entry日</span><b class="num">${dateOnly(h.entry_date)}</b></div><div><span>Event</span><b>${h.event_risk||'なし'}</b></div></div></details>`).join('') || '<div class="stub">保有なし</div>';
  ...
  function renderTrades(reset=false){ if(reset) shown=20; const list=filteredTrades(); const visible=list.slice(0,shown); document.getElementById('j-count').innerHTML=`<span>${visible.length} / ${list.length}件</span><span>全${trades.length}件</span>`; document.getElementById('j-list').innerHTML=visible.map(t=>`<div class="trade-card"><div class="trade-top"><span class="tk">${t.ticker}<small>${t.setup||'UNKNOWN'}</small></span><span class="pnl num ${tone(t.net_pnl_jpy)}">${fmtPct(t.return_pct,true)}｜${fmtNum(t.r_multiple)}R</span></div><div class="trade-meta"><span>${dateOnly(t.entry_date)} → ${dateOnly(t.exit_date)}</span><span><i class="nq-dot nq-${t.nq_color}"></i>${t.nq_color}</span><span>${Number(t.hold_days||0)}日</span><span>${t.exit_reason||'—'}</span>${t.rule_followed===false?'<span class="rule-flag">規律逸脱</span>':''}<span class="num ${tone(t.net_pnl_jpy)}">${fmtYen(t.net_pnl_jpy)}</span></div></div>`).join('')||'<div class="stub">該当トレードなし</div>'; document.getElementById('j-more').hidden=shown>=list.length; }
  ...
  function renderEdge(){const list=tradesRange(...edgeRange),axis=document.getElementById('edge-axis').value,sort=document.getElementById('edge-sort').value,minn=Number(document.getElementById('edge-minn').value||1),groups=new Map();list.forEach(t=>{const k=axisFn[axis](t);if(!groups.has(k))groups.set(k,[]);groups.get(k).push(t)});let rows=[...groups].map(([k,v])=>({k,...stats(v)}));rows.sort((a,b)=>Number(b[sort]||0)-Number(a[sort]||0));const valid=rows.filter(r=>r.n>=minn),best=valid[0];document.getElementById('edge-callout').innerHTML=best?`<b>現時点の上位条件：${best.k}</b>n=${best.n}｜PF ${fmtNum(best.pf)}｜平均R ${fmtNum(best.r)}。サンプル数を優先して解釈してください。`:'<b>十分なサンプルなし</b>期間または最小件数を調整してください。';document.getElementById('rank-body').innerHTML=rows.map(r=>`<div class="rank-row ${r.n<minn?'low-n':''}" data-key="${String(r.k).replace(/"/g,'&quot;')}"><span class="lbl">${r.k}${r.n<minn?'<span class="tag-low">n少</span>':''}</span><b>${r.n}</b><b>${fmtPct(r.wr)}</b><b>${fmtNum(r.pf)}</b><b>${fmtNum(r.r)}</b></div>`).join('')||'<div class="stub">該当データなし</div>';document.querySelectorAll('.rank-row').forEach(row=>row.addEventListener('click',()=>{search.value=row.dataset.key.split(' × ')[0];location.hash='#journal';renderTrades(true)}));document.getElementById('rule-violations').innerHTML=list.filter(t=>t.rule_followed===false).slice(0,12).map(t=>`<div class="trade-card"><div class="trade-top"><span class="tk">${t.ticker}<small>${t.setup}</small></span><span class="pnl neg">${t.mistake_type||'RULE BREAK'}</span></div><div class="trade-meta"><span>${dateOnly(t.entry_date)}</span><span>${t.exit_reason||'—'}</span><span>${fmtNum(t.r_multiple)}R</span></div></div>`).join('')||'<div class="stub">規律違反なし</div>'}
...
<section class="tab-panel" id="assets"><div class="sec-head"><h2>資産推移</h2><p>期間を変えると実口座評価額ベースで再計算</p><div class="rule"></div></div><div class="period-bar" id="assets-period"></div><div class="custom-range" id="assets-custom" hidden><input type="date" id="assets-from"><span>〜</span><input type="date" id="assets-to"></div><div class="metrics"><div class="metric"><span>リターン</span><strong class="num" id="a-ret">—</strong></div><div class="metric"><span>PF</span><strong class="num" id="a-pf">—</strong></div><div class="metric"><span>勝率</span><strong class="num" id="a-wr">—</strong></div><div class="metric"><span>平均R</span><strong class="num" id="a-r">—</strong></div><div class="metric"><span>最大DD</span><strong class="num neg" id="a-dd">—</strong></div><div class="metric"><span>取引数</span><strong class="num" id="a-n">—</strong></div></div><div class="panel"><h3>Equity Curve</h3><div class="sub">入出金調整後の日次口座評価額</div><svg class="equity-svg" viewBox="0 0 600 190" id="equity-svg"></svg></div><div class="panel"><h3>月次リターン</h3><div class="sub">選択期間に含まれる月をすべて表示</div><div class="month-grid" id="month-grid"></div></div></section>
<section class="tab-panel" id="portfolio"><div class="sec-head"><h2>ポートフォリオ</h2><p>{len(report.holdings)}銘柄｜タップで詳細を展開</p><div class="rule"></div></div><div class="metrics"><div class="metric"><span>相関調整Heat</span><strong class="num">{_pct(risk.get('correlation_adjusted_heat'))}</strong></div><div class="metric"><span>名目Heat</span><strong class="num">{_pct(risk.get('nominal_heat'))}</strong></div><div class="metric"><span>Gross</span><strong class="num">{_pct(risk.get('gross_exposure'))}</strong></div><div class="metric"><span>最大クラスター</span><strong>{html.escape(str(risk.get('largest_cluster') or '—'))}</strong></div></div><div class="panel"><h3>集中度</h3><div class="sub">配分順。色は含み損益。Treemapより15〜30銘柄を比較しやすい表示</div><div id="conc-list"></div></div><div class="panel"><h3>セクター配分</h3><div id="sector-bars"></div></div><div class="panel"><h3>テーマ配分</h3><div id="theme-bars"></div></div><div class="panel"><h3>保有一覧</h3><div class="sub">一覧はコンパクト、必要な銘柄だけ詳細を展開</div><div id="holdings-cards"></div></div></section>
...
<section class="tab-panel" id="review"><div class="sec-head"><h2>振り返り</h2><p>数値から再発可能な問題だけを抽出</p><div class="rule"></div></div><div class="panel review">{review}</div><div class="panel"><h3>データ品質</h3><ul class="fineprint">{source_notes}</ul></div></section>
