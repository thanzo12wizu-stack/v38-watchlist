...
<section class="tab-panel" id="portfolio"><div class="sec-head"><h2>ポートフォリオ</h2><p>{len(report.holdings)}銘柄｜タップで詳細を展開</p><div class="rule"></div></div><div class="metrics"><div class="metric"><span>相関調整Heat</span><strong class="num">{_pct(risk.get('correlation_adjusted_heat'))}</strong></div><div class="metric"><span>名目Heat</span><strong class="num">{_pct(risk.get('nominal_heat'))}</strong></div><div class="metric"><span>Gross</span><strong class="num">{_pct(risk.get('gross_exposure'))}</strong></div><div class="metric"><span>最大クラスター</span><strong>{html.escape(str(risk.get('largest_cluster') or '—'))}</strong></div></div><div class="panel"><h3>集中度</h3><div class="sub">配分順。色は含み損益。Treemapより15〜30銘柄を比較しやすい表示</div><div id="conc-list"></div></div><div class="panel"><h3>セクター配分</h3><div id="sector-bars"></div></div><div class="panel"><h3>テーマ配分</h3><div id="theme-bars"></div></div><div class="panel"><h3>保有一覧</h3><div class="sub">一覧はコンパクト、必要な銘柄だけ詳細を展開</div><div id="holdings-cards"></div></div><div class="panel"><h3>相関上位ペア</h3><div class="sub">{html.escape(str(risk.get('method') or '相関計算なし'))}</div><div id="corr-pairs"></div></div></section>
