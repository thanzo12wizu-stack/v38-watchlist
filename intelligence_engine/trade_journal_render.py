...
def render_dashboard(report: JournalReport, path: Path) -> None:
    """Render one self-contained HTML with real tab pages.

    Only the selected tab panel is visible. Share images and post text are embedded
    so the generated index.html remains portable as a single file.
    """
