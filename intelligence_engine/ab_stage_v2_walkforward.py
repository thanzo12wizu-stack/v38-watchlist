...
def build_report(
    ...
    for _, row in summary.iterrows():
        lines.append(
            f"|{int(row['spec'])}. {row['spec_name']}|{row['portfolio_mode']}|"
            f"{number(row['mean_daily_spearman_ic'], 4)}|"
            f"{pct(row['mean_top10_excess_return'])}|"
            f"{pct(row['mean_hit_3r_rate'])}|"
            f"{pct(row['mean_early_failure_rate'])}|"
            f"{pct(row['mean_neither_rate'])}|"
            f"{pct(row['mean_brier_skill'])}|"
            f"{number(row['mean_profit_factor'], 2)}|"
            f"{pct(row['worst_max_drawdown'])}|"
            f"{int(row['qqq_excess_years'])}/{int(row['folds'])}|"
    ...
    for verdict in verdicts:
        ...
        lines.append(
            f"- **{verdict['increment']}**: {verdict['verdict']} — "
            f"改善: {improvements}。非劣化違反: {violations}。"
            f"取引数変化 {verdict['trade_count_change']:+d}。"
        )
    lines.extend(
        [
            "",
...
def run(config: ExperimentConfig) -> dict[str, Any]:
    ...
    fold_results.to_csv(
        config.output_dir / "walk_forward_v2_by_year.csv", index=False
    )
    ...
    (config.output_dir / "REPORT_V2_JA.md").write_text(
        build_report(summary, verdicts, metadata),
        encoding="utf-8",
    )
    return payload
...
def main() -> None:
    ...
    result = run(
        ExperimentConfig(
            ...
            start_year=max(2017, args.start_year),
