    ...
    for tag in tags:
        for row in _unit_rows(namespace.get(tag) or {}):
            ...
            if end is None or not np.isfinite(value):
                continue
            points.append(_Point(metric, _to_date(row.get("start")), end, value, str(row.get("form") or ""), _to_date(row.get("filed")), row.get("accn"), row.get("frame")))
...
def _quarterize(points: list[_Point]) -> list[_Point]:
    valid = [point for point in points if point.form in {"10-Q", "10-K", "20-F", "6-K"} and point.start is not None and point.duration is not None]
    direct = [point for point in valid if 70 <= int(point.duration or 0) <= 120]
    cumulative = [point for point in valid if 150 <= int(point.duration or 0) <= 380]
    ...
    for start, group in by_start.items():
        ...
        for cumulative_point in sorted(group, key=lambda p: (p.end, p.filed or date.min)):
            ...
            base = prior
            if base is None:
                candidates = [point for point in direct if point.start == start and point.end < cumulative_point.end]
                base = max(candidates, key=lambda p: p.end) if candidates else None
            if base is not None:
                derived = _Point(cumulative_point.metric, base.end, cumulative_point.end, cumulative_point.value - base.value, cumulative_point.form, cumulative_point.filed, cumulative_point.accession, cumulative_point.frame, True)
...
def extract_companyfacts_history(payload: dict[str, Any], ticker: str | None = None) -> pd.DataFrame:
    ...
    for metric, metric_tags in tags.items():
        ...
        points = _instant_points(raw) if metric in instant else _quarterize(raw)
        for point in points:
            available = point.filed or point.end
            observations.append(FactObservation(
                ticker=symbol, metric=metric, value=point.value,
                period_start=point.start.isoformat() if point.start else None,
                period_end=point.end.isoformat(), filed_at=point.filed.isoformat() if point.filed else None,
                available_at=available.isoformat(), form=point.form, accession=point.accession,
                accounting_standard=standard, derived_quarter=point.derived,
...
def _metric_series(available: pd.DataFrame, metric: str) -> pd.Series:
    ...
    return pd.Series(pd.to_numeric(subset["value"], errors="coerce").to_numpy(), index=pd.DatetimeIndex(subset["period_end"]))
...
def _snapshot(available: pd.DataFrame, available_at: pd.Timestamp) -> dict[str, Any]:
    ...
    if latest_period is not None and revenue_value not in (None, 0):
        ...
        if ocf_value is not None:
            ...
            if not ocf.empty:
                fcf_series = ocf - capex.reindex(ocf.index).fillna(0.0).abs()
                _, fcf_yoy, _, _ = _growth(fcf_series)
    shares_yoy = float(shares.iloc[-1] / shares.iloc[-5] - 1.0) if len(shares) >= 5 and shares.iloc[-5] != 0 else None
    ...
    evidence_values = [revenue_yoy, revenue_acceleration, eps_yoy, eps_acceleration, gross_margin_delta, operating_margin_delta, fcf_yoy, shares_yoy]
    evidence_count = sum(value is not None and np.isfinite(value) for value in evidence_values)
    accounting = available["accounting_standard"].dropna().iloc[-1] if "accounting_standard" in available and available["accounting_standard"].notna().any() else None
    return {
        "available_at": available_at, "latest_period_end": latest_period,
        "latest_filing_date": available_at.date().isoformat(), "accounting_standard": accounting,
        "revenue": revenue_value, "revenue_yoy": revenue_yoy, "revenue_acceleration": revenue_acceleration,
        "revenue_growth_stability": revenue_stability, "eps": eps_value, "eps_yoy": eps_yoy,
        "eps_acceleration": eps_acceleration, "eps_growth_stability": eps_stability,
        "gross_margin": gross_margin, "gross_margin_delta": gross_margin_delta,
        "operating_margin": operating_margin, "operating_margin_delta": operating_margin_delta,
        "free_cash_flow": fcf, "free_cash_flow_yoy": fcf_yoy, "fcf_margin": fcf_margin,
        "shares_yoy": shares_yoy, "cash": latest_cash, "debt": latest_debt, "net_cash": net_cash,
        "fundamental_evidence_count": evidence_count, "fundamental_confidence": evidence_count / len(evidence_values),
    }
...
def build_financial_snapshots(facts: pd.DataFrame) -> pd.DataFrame:
    ...
    for ticker, ticker_facts in work.groupby("ticker", sort=True):
        for available_at in sorted(ticker_facts["available_at"].unique()):
            snapshots.append({"ticker": ticker, **_snapshot(ticker_facts[ticker_facts["available_at"] <= available_at], pd.Timestamp(available_at))})
...
def merge_financial_snapshots(price_panel: pd.DataFrame, snapshots: pd.DataFrame) -> pd.DataFrame:
    ...
    if snapshots is None or snapshots.empty:
        out = price_panel.copy(); out["fundamental_confidence"] = 0.0; return out
    left = price_panel.copy(); left["date"] = pd.to_datetime(left["date"], errors="coerce")
    right = snapshots.copy(); right["available_at"] = pd.to_datetime(right["available_at"], errors="coerce")
    ...
    for ticker, group in left.groupby("ticker", sort=False):
        ...
        if history.empty:
            enriched = group.copy(); enriched["fundamental_confidence"] = 0.0
        else:
            enriched = pd.merge_asof(group.sort_values("date"), history.sort_values("available_at"), left_on="date", right_on="available_at", direction="backward", allow_exact_matches=True)
