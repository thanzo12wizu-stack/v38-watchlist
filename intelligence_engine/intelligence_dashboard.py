...
def build_html(payload: dict) -> str:
    ...
    stage_options = ''.join('<option>'+esc(s)+'</option>' for s in matrix.get('stage_order') or [])
    count_buttons = ''.join('<button onclick="jump(\''+esc(s)+'\')"><b>'+esc(s)+'</b><span>'+esc((summary.get("stage_counts") or {}).get(s,0))+'</span></button>' for s in matrix.get('stage_order') or [])
    matrix_view = f'''<section class="filters"><select id="af"><option>ALL</option><option>BUYABLE</option><option>WATCH</option><option>WAIT</option><option>TRIM</option><option>REDUCE</option><option>EXIT</option><option>AVOID</option></select><select id="sf"><option>ALL</option>{stage_options}</select><input id="qf" placeholder="Ticker / Sector / Industry"><button onclick="resetF()">解除</button></section>
    <div class="counts">{count_buttons}</div>{board(matrix)}'''
    ...
    buttons=''.join('<button class="'+('active' if i==0 else '')+'" data-tab="'+k+'">'+label+'</button>' for i,(k,label) in enumerate(tabs)); view_html=''.join('<main id="'+k+'" class="view '+('active' if i==0 else '')+'">'+views[k]+'</main>' for i,(k,_) in enumerate(tabs))
