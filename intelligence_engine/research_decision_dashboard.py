...
def _bar(label: str, value: Any) -> str:
    score = _score(value)
    return (
        f'<div class="bar"><span>{_esc(label)}</span>'
        f'<i><b style="width:{score:.1f}%"></b></i><em>{_num(value,0)}</em></div>'
    )
...
def _reasons(row: pd.Series) -> list[str]:
    ...
    return (reasons or ["候補条件は満たすが、根拠の重なり待ち"])[:3]


def _candidate(row: pd.Series) -> str:
    ...
    consistency = str(row.get("expectancy_consistency") or "UNAVAILABLE")
    reasons = "".join(f"<li>{_esc(text)}</li>" for text in _reasons(row))
    blocks = " / ".join(BLOCK_JA.get(code, code) for code in _blocks(row)) or "なし"
    bars = "".join(
        [
    ...
    quick = _kv(
        [
            ...
            ("期待値8年", _pct(row.get("expected_edge_10d_8y"), decimal=True, signed=True)),
    ...
    detail = "".join(
        [
            ...
            "</section><section><h4>Entry</h4>",
            _kv(
                [
                    ("Setup", _esc(setup)),
                    ("状態", _esc(row.get("entry_state"))),
                    ("EMA21 Low", _money(row.get("stop_ema21_low"))),
                    ("10MA", _money(row.get("stop_sma10"))),
                    ("出来高比", _num(row.get("volume_ratio_20d"), 2)),
                    ("50MA乖離ATR", _num(row.get("extension_atr"), 2)),
                ]
            ),
            "</section><section><h4>期待値</h4>",
            _kv(
                [
    ...
    <article class="candidate {status.lower()}" data-status="{_esc(status)}"
      data-archetype="{_esc(archetype)}" data-search="{_esc(ticker)} {_esc(archetype)} {_esc(setup)}">
      ...
      <div class="sub"><span>{_esc(archetype)}</span><span>{_esc(row.get('financial_phase'))}</span>
      <span>{_esc(row.get('rs_archetype'))}</span><span>{_esc(setup)}</span></div>
      <div class="scoreline"><strong>{_num(row.get('composite_rank_score'),1)}</strong><small>総合</small>
      <button onclick="copyTicker('{_esc(ticker)}')">コピー</button></div>
      <ul class="reasons">{reasons}</ul>{quick}<div class="bars">{bars}</div>
      <details><summary>財務・RS・Entry・期待値</summary><div class="detail-grid">{detail}</div>
      <p class="block">Hard block: {_esc(blocks)}</p></details>
    </article>
    """


def _window_map(expectancy: dict) -> dict[int, dict]:
...
def _group_rows(expectancy: dict, years: int) -> list[dict]:
    window = _window_map(expectancy).get(years) or {}
    rows = [
        item
        for item in window.get("groups") or []
        if item.get("group_type") == "archetype_setup"
        and int(item.get("horizon") or 0) == 10
    ]
    rows.sort(key=lambda item: (-(item.get("mean_excess_return") or -999), -int(item.get("samples") or 0)))
    return rows


def _expectancy_table(expectancy: dict) -> str:
    rows_by_window = {years: _group_rows(expectancy, years) for years in (8, 5, 3)}
    ...
    for archetype, setup in keys[:40]:
        ...
        for years in (8, 5, 3):
            match = next(
                (
                    row
                    for row in rows_by_window[years]
            ...
            if match:
                cells.append(
                    f'<td><b>{_pct(match.get("mean_excess_return"),decimal=True,signed=True)}</b>'
                    f'<small>n={_num(match.get("samples"),0)} / {_esc(match.get("qualification"))}</small></td>'
                )
            else:
                cells.append("<td>—</td>")
        body.append(
    ...
    return (
        '<div class="table-wrap"><table><thead><tr><th>型 × Setup</th>'
        "<th>8年・標準</th><th>5年</th><th>3年</th></tr></thead><tbody>"
        + "".join(body)
        + "</tbody></table></div>"
    )


def _validation(expectancy: dict) -> str:
    walk = [
        row
    ...
    loyo = [
        row
        for row in expectancy.get("leave_one_year_out") or []
        if int(row.get("horizon") or 0) == 10
    ]
    ...
    for row in walk[:12]:
        cards.append(
            f'<article class="mini"><h3>{_esc(row.get("test_year"))} Walk-forward</h3>'
            + _kv(
                [
                    ("採用型", _esc(row.get("selected_archetype"))),
                    ("学習n", _num(row.get("train_samples"), 0)),
                    ("検証n", _num(row.get("test_samples"), 0)),
                    ("検証超過", _pct(row.get("test_mean_excess"), decimal=True, signed=True)),
                    ("勝率", _pct(row.get("test_win_rate"), decimal=True)),
                ]
            )
            + "</article>"
        )
    for row in loyo[:8]:
        cards.append(
            f'<article class="mini"><h3>{_esc(row.get("omitted_year"))}除外</h3>'
            + _kv(
                [
                    ("標本", _num(row.get("samples"), 0)),
                    ("平均超過", _pct(row.get("mean_excess_return"), decimal=True, signed=True)),
                    ("勝率", _pct(row.get("win_rate"), decimal=True)),
                ]
            )
            + "</article>"
        )
...
def build_html(root: Path) -> str:
    ...
    cards = "".join(_candidate(row) for _, row in latest.head(100).iterrows())
    cards = cards or '<div class="empty">研究候補を生成中</div>'
    ...
    options = "".join(f'<option value="{_esc(value)}">{_esc(value)}</option>' for value in archetypes)
    date_range = f"{_esc(manifest.get('start_date'))} → {_esc(manifest.get('end_date'))}"
    primary_window = f"{_esc(expectancy.get('primary_window_years') or 8)}年"
    data_rows = [
        ("生成", _esc(generated)),
        ("期間", date_range),
        ("銘柄", _num(manifest.get("tickers"), 0)),
        ("Signals", _num(manifest.get("signal_rows"), 0)),
        ("Outcomes", _num(manifest.get("outcome_rows"), 0)),
        ("Rankings", _num(manifest.get("ranking_rows"), 0)),
        ("標準窓", primary_window),
        ("比較窓", _esc(expectancy.get("comparison_windows_years") or [10, 8, 5, 3])),
        ("警告", _esc(manifest.get("warnings") or ["なし"])),
    ]
    today = f"""
    <div class="hero"><div class="metric"><small>研究日</small><strong>{_esc(latest_text)}</strong></div>
    <div class="metric"><small>買える</small><strong>{int(counts.get('ACTIONABLE',0))}</strong></div>
    <div class="metric"><small>待つ</small><strong>{int(counts.get('READY',0))}</strong></div>
    <div class="metric"><small>避ける</small><strong>{int(counts.get('AVOID',0))}</strong></div>
    <div class="metric"><small>保存期間</small><strong>{_esc(manifest.get('years_retained') or 10)}年</strong></div></div>
    <section class="section"><h2>今日の複合候補</h2><div class="filters">
    <select id="statusFilter"><option value="ALL">全判断</option><option value="ACTIONABLE">買える</option>
    <option value="READY">待つ</option><option value="AVOID">避ける</option></select>
    <select id="archetypeFilter"><option value="ALL">全タイプ</option>{options}</select>
    <input id="search" placeholder="Ticker検索"><button onclick="resetFilters()">解除</button></div>
    <div class="candidate-grid">{cards}</div></section>
    """
    expectation = '<section class="section"><h2>型別10日期待値</h2><p class="muted">8年を標準、5年を最近相場、3年を現在レジームの確認に使用。3年だけ良くても順位補正しません。</p>' + _expectancy_table(expectancy) + "</section>"
    validation = '<section class="section"><h2>Walk-forward / 年除外</h2>' + f'<div class="mini-grid">{_validation(expectancy)}</div></section>'
    data_view = f'<section class="section"><h2>研究データ</h2>{_kv(data_rows)}</section>'
