...
def _failure_payload(returncode: int, log_path: Path) -> dict:
    if returncode in (-9, 137):
        error_type = "ProcessMemoryLimit"
        template = "Research worker was killed by SIGKILL, usually because the runner exceeded memory."
    elif returncode in (-15, 143):
        error_type = "ProcessTerminated"
        template = "Research worker was terminated before a clean exit."
    else:
        error_type = _exception_type_from_log(log_path) or "ResearchWorkerExit"
        template = f"Research worker returned exit code {returncode}."
    return {
        "schema_version": "2.0",
...
def main() -> None:
    ...
    command = [sys.executable, "-m", "intelligence_engine.research_pipeline.worker", *arguments]
    with log_path.open("w", encoding="utf-8") as handle:
        completed = subprocess.run(command, stdout=handle, stderr=subprocess.STDOUT, check=False)

    if completed.returncode != 0:
        _write_json(error_path, _failure_payload(completed.returncode, log_path))
        raise SystemExit(1)
    ...
    if missing or not signal_partitions or not ranking_partitions:
        _write_json(
            error_path,
            {
                ...
                "error_template": "Research worker exited cleanly but required outputs were not produced.",
    ...
    # The log remains local to the runner. Emit only a privacy-safe completion line.
    print(
        json.dumps(
            {
                "research_status": "PASS",
