...
def _latest_date(prices: dict[str, pd.DataFrame], ticker: str = "QQQ") -> pd.Timestamp:
    ...
    if frame is None or frame.empty:
        raise RuntimeError(f"{ticker} price history is required")
    index = pd.to_datetime(frame.index, errors="coerce")
    if getattr(index, "tz", None) is not None:
        index = index.tz_convert(None)
    latest = pd.Timestamp(index.max()).normalize()
    if pd.isna(latest):
        raise RuntimeError(f"{ticker} price history has no valid date")
    return latest
...
def _compact_columns(frame: pd.DataFrame) -> pd.DataFrame:
    columns = [
        "ticker", "date", "sector", "industry", "market_cap", "price", "adr_pct", "dollar_volume_20d",
        "volume_ratio_20d", "distance_52w_high_pct", "pivot_20d", "distance_pivot_pct", "stop_ema21_low",
        "stop_sma10", "stop_risk_pct", "reward_risk_raw", "extension_atr", "supply_risk_raw", "hard_block",
        "setup", "entry_state", "candidate_archetype", "financial_phase", "rs_archetype", "fundamental_quality",
        "fundamental_change", "leadership_quality", "entry_quality", "risk_fit", "research_confidence",
        "base_composite", "hard_blocks", "latest_filing_date", "available_at", "fundamental_evidence_count",
        ...
        "pct_rs_raw_63", "pct_rs_raw_126", "pct_rs_raw_189", "rs63_rank_change_21d",
        "rs126_rank_change_21d", "rs189_rank_change_21d", "rs126_top20_persistence_63d",
...
def _regime_series(panel: pd.DataFrame, qqq: pd.DataFrame) -> pd.Series:
    qclose = pd.to_numeric(qqq["close"] if "close" in qqq else qqq.iloc[:, 0], errors="coerce")
    qclose.index = pd.to_datetime(qclose.index, errors="coerce")
    qclose = qclose.sort_index()
    points = (
        (qclose > qclose.rolling(20).mean()).astype(int)
        + (qclose > qclose.rolling(50).mean()).astype(int)
        + (qclose > qclose.rolling(200).mean()).astype(int)
    )
...
def _research_summary(rankings: pd.DataFrame, expectancy: dict[str, Any], manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "rankings": latest[[column for column in columns if column in latest.columns]].head(100).to_dict(orient="records"),
    }
...
def _key_tuples(frame: pd.DataFrame) -> set[tuple[str, str, str, str]]:
    if frame is None or frame.empty:
        return set()
    result = set()
...
def _pending_signals(signals: pd.DataFrame, existing_outcomes: pd.DataFrame) -> pd.DataFrame:
    if signals.empty or existing_outcomes is None or existing_outcomes.empty or "outcome_ready_63" not in existing_outcomes:
        return signals.copy()
    completed = existing_outcomes[existing_outcomes["outcome_ready_63"].fillna(False).astype(bool)]
    completed_keys = _key_tuples(completed)
...
def _expectancy_for_training(training: pd.DataFrame, config: ResearchConfig) -> dict[str, Any]:
    return build_research_expectancy(
        training,
...
def _point_in_time_rankings(signals: pd.DataFrame, outcomes: pd.DataFrame, config: ResearchConfig) -> pd.DataFrame:
    if signals.empty:
        return signals.copy()
    work = signals.copy()
    work["date"] = pd.to_datetime(work["date"], errors="coerce")
    outcome_dates = (
        pd.to_datetime(outcomes["outcome_date_10"], errors="coerce")
        if outcomes is not None and not outcomes.empty and "outcome_date_10" in outcomes
        else pd.Series(pd.NaT, index=outcomes.index if outcomes is not None else [], dtype="datetime64[ns]")
    )
    pieces = []
    ...
    if pd.notna(latest):
        training = outcomes[outcome_dates < latest] if outcomes is not None and not outcomes.empty else pd.DataFrame()
        current = rank_signals(work[work["date"] == latest], _expectancy_for_training(training, config))
        if "date" in ranked:
            ranked = ranked[pd.to_datetime(ranked["date"], errors="coerce") != latest]
        if ranked.empty:
            ranked = current.copy()
        elif not current.empty:
            ranked = pd.concat([ranked, current], ignore_index=True, sort=False)
    return ranked.sort_values(
        ["date", "composite_rank_score", "research_confidence", "ticker"],
...
def build(
    *,
    ...
    facts = _load_facts(fundamental_provider, tickers)
    snapshots = build_financial_snapshots(facts)
    panel = merge_financial_snapshots(panel, snapshots)
    ...
    summary = _research_summary(ranked, expectancy, manifest)
    write_json(root / "manifest.json", manifest)
    write_json(root / "expectancy.json", expectancy)
    write_json(root / "current_rankings.json", summary)
    _attach_to_index(root.parent / "index.json", summary)
    return {
        **manifest,
