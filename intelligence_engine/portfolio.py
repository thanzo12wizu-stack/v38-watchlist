...
def load_positions(path: Path) -> pd.DataFrame:
    ...
    aliases = {
        ...
        "stop_method": ("stop_method", "trail", "撤退方法"),
    }
...
def build_portfolio_doctor(
    ...
    if positions.empty:
        return {
            ...
            "input_contract": "portfolio.csv: ticker, weight|shares, cost_basis, entry_date, stop_method",
        }
    ...
    for _, pos in positions.iterrows():
        ...
        stop = row.get("stop_sma10") if "10" in stop_method else row.get("stop_ema21_low")
        stop = pd.to_numeric(stop, errors="coerce")
        ...
        risk_contribution = weight * max(0.0, float(stop_distance or 0))
        phase = row.get("story_phase")
        setup = row.get("setup")
        theme_phase = row.get("theme_phase")
        hard_block = bool(row.get("hard_block", False))
        if hard_block or (stop_distance is not None and stop_distance <= 0):
            action = "EXIT"
        elif stop_distance is not None and stop_distance <= 2.0:
            action = "REDUCE"
        elif phase in {"DETERIORATING", "MARGIN_PRESSURE", "DILUTING"} or theme_phase == "WEAKENING":
            action = "REDUCE"
        elif setup in {"PULLBACK", "PRE_BREAKOUT"} and market_state.get("entry_gate") in {"ALLOW", "SELECTIVE"} and row.get("story_confirmed"):
            action = "ADD"
        else:
            action = "HOLD"
        ...
        records.append({
            ...
            "cost_basis": None if pd.isna(pos.get("cost_basis")) else float(pos.get("cost_basis")),
            "held_days": held_days,
    ...
    if sector_weights and max(sector_weights.values()) > 0.40:
        warnings.append("sector_concentration")
    if theme_weights and max(theme_weights.values()) > 0.35:
        warnings.append("theme_concentration")
    if correlation.get("average_pairwise_correlation") is not None and correlation["average_pairwise_correlation"] >= 0.65:
        warnings.append("correlation_concentration")
    if total_stop_risk > 4.0:
        warnings.append("portfolio_stop_risk_high")
    return {
        "status": "OK",
        "position_count": len(records),
        "concentration_hhi": round(hhi, 4),
        "effective_position_count": round(1 / hhi, 2) if hhi > 0 else None,
        "largest_position_weight": round(float(weights.max()), 4),
        "sector_weights": dict(sorted(((k, round(v, 4)) for k, v in sector_weights.items()), key=lambda x: (-x[1], x[0]))),
        "theme_weights": dict(sorted(((k, round(v, 4)) for k, v in theme_weights.items()), key=lambda x: (-x[1], x[0]))),
        "portfolio_adr_pct": round(avg_adr, 2),
        "portfolio_stop_risk_pct": round(total_stop_risk, 2),
        "correlation": correlation,
        "action_counts": pd.Series([r["action"] for r in records]).value_counts().to_dict(),
        "positions": sorted(records, key=lambda r: ({"EXIT": 0, "REDUCE": 1, "ADD": 2, "HOLD": 3}[r["action"]], -r["weight"], r["ticker"])),
        "warnings": warnings,
    }
