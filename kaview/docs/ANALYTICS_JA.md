...
## 既存Command Centerとの分離
### 読み取り可能なもの
Kaviewは既存workflowから呼び出さない。必要な場合だけ、生成済みのIntelligenceデータを読み取り専用入力として明示指定できる。
...
同期元として利用できるのは`data/intelligence/index.json`および互換コンポーネントJSON。Kaviewはこれらへ書き込まない。最終HTMLは`kaview/artifacts/index.html`へ生成する。
...
既存Secretや既存`equity.csv`は自動では読み込まない。CSVを`kaview/data`へ明示配置した場合のみ利用する。最新資産日が7日超前なら、その値を今日の残高へ複製せず`PARTIAL`として鮮度警告を残し、今日画面は「資産データ要更新」として新規発注可能を表示しない。
