...
def compute_metrics(W, order):
    ...
    for t in order:
        ...
        recs.append(dict(
            ...
            ret63=ret63, ret21=ret21, hi52=hi52, dist52=dist52, pos52=pos52,
...
def mri_auxiliary(mri, vals, metrics):
    ...
    last = vals.iloc[-1]
    # 12 bearish conditions (labeled, for display of which are lit)
    bvals = [
        ...
        ("VVIX>100",     last.get("vvix", np.nan) > 100),
    ...
    return dict(cur=cur, slope=slope_dir, bear_n=bear_n, bear_flags=bear_flags,
...
def build_buy_today(m, k=10):
    ...
    return [dict(t=t, rs=r["rs"], ext21=r["ext21s"], ext50=r["ext50s"], ext200=r["ext200s"])
...
def build_sector_rs(m, s2i, e2j):
    ...
    for t in m.index:
        ...
        if any(k in low for k in SECTOR_BLACKLIST_KEYS):
            continue
        ind.setdefault(nm, []).append((t, float(m.at[t, "rs"])))
    recs = []
    for nm, lst in ind.items():
        if len(lst) < 5:                       # >=5 members (was 3) for robustness
            continue
        rss = [r for _, r in lst]
        members = sorted(lst, key=lambda x: -(x[1] if x[1] == x[1] else -9))  # RS desc
        recs.append(dict(en=nm, ja=e2j.get(nm, nm), n=len(lst),
                         med=float(np.nanmedian(rss)), members=members))
    if not recs:
        return []
...
def build_distribution(macro):
    ...
    for tk in ["SPY", "QQQ"]:
        ...
        win = dist.iloc[-25:]
        n = int(win.sum())
        days = [d.strftime("%-m/%-d") for d, hit in win.items() if bool(hit)]
...
def _buy_today_card(buys):
    ...
    for b in buys:
        ...
        rows.append(
            f'<div class="bt">'
            f'<div class="bt-h"><span class="tk" data-tkone="{b["t"]}">{b["t"]}</span>'
    ...
    return (f'<div class="card hot-card"><div class="hdr"><h2>本日のピックアップ</h2>'
            ...
            f'<div class="sub">今日買うなら候補（ルール運用とは独立の裁量スキャン）。'
            f'過熱クライマックス（200日+100%超）は除外、乖離は表示情報（{len(buys)}銘柄）</div>'
            + "".join(rows) + '</div>')
...
def _buy_card(buys):
    ...
    for b in buys:
        rows.append(
            f'<div class="buy">'
            f'<div class="buy-h"><span class="tk" data-tkone="{b["t"]}">{b["t"]}</span>'
            f'<span class="rs">RS {b["rs"]:.0f}</span></div>'
    ...
    return (f'<div class="card"><div class="hdr"><h2>本日の押し目シグナル</h2>'
            f'{_cp([b["t"] for b in buys])}</div>'
            f'<div class="sub">ルールが押し目買いと判定したリーダーを<b>RS90以上</b>に厳選（正式な買い場）。エントリーレンジと+15%/+30%目標（{len(buys)}銘柄）</div>'
            + "".join(rows) + '</div>')
...
def _leaders_card(by_state):
    ...
    for code, label, cls in STATE_DEF:
        ...
        chips = "".join(f'<span class="chip {cls}" data-tkone="{x["t"]}">{x["t"]}</span>' for x in shown)
...
.bt{background:#0c1422;border:1px solid #233149;border-radius:10px;padding:9px 11px;margin-bottom:8px}
.bt-h{display:flex;align-items:baseline;gap:8px}
.bt-h .tk{font-size:17px;font-weight:800;color:#9ecbff}
...
.bt-g{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin-top:8px}
.bt-g>div{background:rgba(0,0,0,.25);border-radius:7px;padding:5px 7px}
.bt-g .k{display:block;font-size:10px;color:#7d8da1}
.bt-g .v{font-size:13px;font-weight:700}
...
.dov{width:100%;max-width:680px;background:#0f1623;border:1px solid #1c2533;border-bottom:none;border-radius:16px 16px 0 0;padding:16px 16px 28px;box-shadow:0 -8px 30px rgba(0,0,0,.5)}
...
.rowsec{font-size:9px;font-weight:400;color:#7d8da1;margin-top:1px;line-height:1.2}
td.tk[data-tkone]{cursor:pointer}
"""
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
           sectors, breadth, yields, asof, sar, mkt, leaders, s2i, e2j):
    ...
    # ---- MRI banner + ⑥ tappable breakdown panel
    drop_note = ""
    if dropped:
    ...
    _kj = {"qqq_50":"QQQ vs50MA","qqq_200":"QQQ vs200MA","spy_50":"SPY vs50MA","spy_200":"SPY vs200MA",
           ...
           "hyglqd_5d":"クレジット 5日","rsp_spy_20":"ブレッス RSP/SPY","iwm_spy_20":"小型株 IWM/SPY"}
    _gj = {"trend":"トレンド","vol":"ボラティリティ","credit":"信用","breadth":"ブレッドス"}
    def _mraw(k, v):
        if v is None or (isinstance(v, float) and np.isnan(v)): return "—"
        if k in ("vix", "vvix"): return f"{v:.1f}"
        if k in ("vix_ratio", "hyglqd_20", "rsp_spy_20", "iwm_spy_20"): return f"{v:.3f}"
    ...
    bear_sec = (f'<div class="mgrp">ベア警戒 点灯 {aux["bear_n"]}/12</div>'
    ...
    buytoday = build_buy_today(m)
    role_note = ('<div class="card"><div class="note" style="margin:0">'
                 '使い分け — <b>ピックアップ</b>=全市場の強モメンタムから過熱を除外した裁量参考'
                 '（ルール運用とは独立）。<b>押し目シグナル</b>=ルールが押し目買いと判定した'
                 'リーダー（正式な買い場）。</div></div>')
    ...
    nh_chips = "".join(
        f'<span class="chip{" hot" if m.at[t,"rs"]>=90 else ""}" data-tkone="{t}">{t}</span>' for t in nh_list)
    newhigh_card = (
    ...
    today = role_note + _buy_today_card(buytoday) + buy_card + leaders_card + newhigh_card
    ...
    # 次のRSリーダー（控え）= 採用8銘柄を除いた候補プール上位（次の月次で繰り上がる）
    picked_tk = {t for t, _, _ in picks}
    nextlead = cand[~cand.index.isin(picked_tk)].head(N_PORT)
    nlrows = []
    for rank, (t, r) in enumerate(nextlead.iterrows(), start=1):
        _ind = s2i.get(t); _indja = e2j.get(_ind, _ind) if _ind else "—"
        nlrows.append(
            f'<tr><td class="l mut">{rank}</td>'
            f'<td class="l tk" data-tkone="{t}">{t}'
            f'<div class="rowsec">{_indja}</div></td>'
            f'<td>{r["rs"]:.0f}</td><td>{r["rs_smooth"]:.0f}</td>'
            f'<td class="{color_pct(r["vs200"])}">{fmt_pct(r["vs200"])}</td>'
            f'<td class="{color_pct(r["dist52"])}">{fmt_pct(r["dist52"])}</td></tr>')
    next_lead_card = (
        f'<div class="card"><div class="hdr"><h2>次のRSリーダー（控え）</h2>{_cp(list(nextlead.index))}</div>'
        f'<div class="sub">次の月次入替で繰り上がる最有力・平滑RS順（200MA上×週足SAR非弱気）</div>'
        f'<table><tr><th class="l">#</th><th class="l">銘柄</th><th>RS</th>'
        f'<th>平滑RS</th><th>200MA乖離</th><th>52週高値差</th></tr>'
        + "".join(nlrows) + '</table></div>')
    port = port + next_lead_card
    ...
    ov_chips = "".join(
        f'<span class="chip ov" data-tkone="{t}" title="{"・".join(where[t])}">{t}'
    ...
    for nm, lst in setups.items():
        ...
        extra = len(lst) - len(shown)
        chips = "".join(
            f'<span class="chip{" hot" if (m.at[t,"rs"]>=90) else ""}" data-tkone="{t}">{t}</span>' for t in shown)
        more = f'<span class="more">+{extra}件</span>' if extra > 0 else ""
        setup_html.append(
    ...
    setup_card = (f'<div class="card"><h2>セットアップ別</h2>'
                  ...
                  + "".join(setup_html) + '</div>')
    deck = cand.iloc[N_PORT:N_PORT+22]
    wrows = []
    ...
    search_card = (
        ...
        '<div class="sub">ティッカー or 社名で全ユニバースを検索（タップで詳細）</div>'
        '<input id="tksearch" class="tksearch" type="text" inputmode="search" autocomplete="off" '
        'placeholder="例: NVDA / Nvidia" oninput="tkSearch(this.value)">'
        '<div id="tkresults" class="tkresults"></div></div>')
    watch = search_card + overlap_card + setup_card + deck_card
    ...
    tapset.update(list(nextlead.index))
    det_json = _det_json(m, names, set(m.index))   # 全ユニバース（検索＆タップ詳細）
    ...
    for i, s in enumerate(sectors, 1):
        ...
        for t, rs in mems:
            ...
            _ch.append(f'<span class="chip{_hot}" data-tkone="{t}">{t}'
                       f'<span class="rsb">{_rs}</span></span>')
        chips = "".join(_ch)
        srows.append(
            f'<tr class="secrow" onclick="secToggle(\'sec{i}\')">'
            f'<td class="l mut">{i}</td>'
            f'<td class="l tk" style="font-size:12px">{s["ja"]} <span class="secx">▾</span></td>'
    ...
    body = ('<nav>'
            ...
            '<button onclick="tab(\'t-today\',this)">今日</button>'
