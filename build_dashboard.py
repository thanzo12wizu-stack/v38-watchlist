...
def w30_breakout(c, low=None, p=W30X):
    """W30ブレイク判定(週足W-FRI)。lowに日足LowのSeriesを渡すと押し目タッチ判定が正確になる。"""
    ...
    for k in range(p["recent_weeks"] + 1):
        ...
        for kk in range(1, k + 1):
            ...
            maxprog = max(maxprog, cl / entry - 1)
        ...
        confirmed = not (k == 0 and c.index[-1].weekday() < 4)
...
def _equity_card(eq, at=None):
    setup = (
        ...
        '<div class="note">有効化: リポジトリに <b>equity.csv</b> を置く。記録は<b onclick="goTab(\'t-equity\')" style="cursor:pointer;text-decoration:underline">Equityタブ「記録」</b>の📋ボタンで1行コピー→貼り付け。'
    ...
    return (
        ...
        f'<div class="sub">口座資産（equity.csv・{eq["n"]}記録）と、その{eq.get("ema_lab","21日EMA")}（記録点ベース）。追記は<b onclick="goTab(\'t-equity\')" style="cursor:pointer;text-decoration:underline">Equityタブ「記録」</b>から。'
...
# ============================================================================= Overview投稿カード（実データから自動生成・Almanacライト）
#   Overview 1 = マーケット概略（個別銘柄なし・エッジ非掲載） / Overview 2 = セクター・ローテーション詳細。
#   1680x1080固定・iframe(srcdoc)埋め込み・全カード数値はビルド時点の実測値（サンプル数値は廃止）。
_OV_MICRO_EN = {
...
body{background:#DEDBD2;font-family:-apple-system,'Helvetica Neue',Arial,'Hiragino Sans','Noto Sans JP',sans-serif;overflow-x:hidden}
.stage{width:100%;overflow:hidden}.scaler{transform-origin:top left;width:1680px}
.card{width:1680px;height:1080px;background:#E9E7DF;color:#1B1D1C;padding:20px 26px 12px;display:flex;flex-direction:column;gap:10px;
  --ink:#1B1D1C;--mut:#727569;--line:#D5D1C6;--panel:#F3F1EA;--pos:#1E7A4D;--neg:#C2453A;--amb:#A16207;--acc:#2456A6}
header{display:flex;align-items:flex-end;justify-content:space-between;border-bottom:2px solid var(--ink);padding-bottom:10px}
.h-t{font-size:30px;font-weight:900;letter-spacing:.01em}
.h-en{font-size:13px;font-weight:800;letter-spacing:.22em;color:var(--mut);margin-left:12px}
.h-sub{font-size:13px;color:var(--mut);margin-top:4px}
.h-r{display:flex;align-items:center;gap:14px}
.rchip{display:inline-flex;align-items:center;gap:8px;font-size:15px;font-weight:800;padding:7px 16px;border-radius:999px;color:#fff}
.rchip i{width:9px;height:9px;border-radius:50%;background:#fff;display:inline-block}
.score{text-align:right}.score .v{font-size:40px;font-weight:900;line-height:1}.score .l{font-size:12px;color:var(--mut);font-weight:700}
.row{display:flex;gap:10px;min-height:0}
.t{background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:11px 13px 10px;display:flex;flex-direction:column;min-width:0}
.tt{font-size:14.5px;font-weight:800;display:flex;align-items:baseline;gap:8px;margin-bottom:6px}
.ten{font-size:10px;font-weight:800;letter-spacing:.14em;color:var(--mut)}
.tnote{font-size:10.5px;color:var(--mut);margin-top:auto;padding-top:5px}
.pos{color:var(--pos)}.neg{color:var(--neg)}.mut{color:var(--mut)}.amb{color:var(--amb)}
table{border-collapse:collapse;width:100%}
th{font-size:10.5px;color:var(--mut);font-weight:700;text-align:right;padding:2px 6px;border-bottom:1px solid var(--line)}
th.l,td.l{text-align:left;padding-left:2px}
td{font-size:12.5px;font-weight:700;text-align:right;padding:3px 6px;border-bottom:1px solid rgba(0,0,0,.05);font-variant-numeric:tabular-nums}
.big{font-size:26px;font-weight:900;line-height:1.05}
.kv{display:flex;justify-content:space-between;align-items:baseline;padding:3px 0;border-bottom:1px solid rgba(0,0,0,.05);font-size:12.5px}
.kv .k{color:var(--mut);font-weight:700;font-size:11.5px}.kv .v{font-weight:800;font-variant-numeric:tabular-nums}
.gg{position:relative;height:8px;border-radius:4px;background:linear-gradient(90deg,#c9d8cf,#e4dfc9,#dcc3bd);margin:6px 0 2px}
.gg i{position:absolute;top:-3px;width:3px;height:14px;background:var(--ink);border-radius:2px}
.ggl{display:flex;justify-content:space-between;font-size:9.5px;color:var(--mut);font-weight:700}
.bar{display:flex;align-items:center;gap:7px;font-size:11.5px;padding:1.5px 0}
.bar .n{width:118px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.bar .n em{font-style:normal;font-size:9px;color:var(--mut);font-weight:700;margin-left:4px}
.bar .tr{flex:1;height:9px;border-radius:3px;background:rgba(0,0,0,.05);position:relative;overflow:hidden}
.bar .tr i{position:absolute;top:0;bottom:0;border-radius:3px}
.bar .v{width:52px;text-align:right;font-weight:800;font-variant-numeric:tabular-nums}
.dv{display:flex;align-items:center;gap:7px;font-size:11.5px;padding:1px 0}
.dv .n{width:132px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.dv .n em{font-style:normal;font-size:9px;color:var(--mut);font-weight:700;margin-left:4px}
.dv .tr{flex:1;height:9px;position:relative;background:rgba(0,0,0,.05);border-radius:3px}
.dv .tr:before{content:'';position:absolute;left:50%;top:-2px;bottom:-2px;width:1px;background:rgba(0,0,0,.25)}
.dv .tr i{position:absolute;top:0;bottom:0}
.dv .v{width:46px;text-align:right;font-weight:800;font-variant-numeric:tabular-nums}
.rk{font-size:12px;font-weight:800;display:flex;gap:6px;align-items:baseline;padding:2.5px 0;border-bottom:1px solid rgba(0,0,0,.05)}
.rk .i{width:20px;color:var(--mut);font-size:10.5px;text-align:right}
.rk .nm{flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.rk .nm em{font-style:normal;font-size:9px;color:var(--mut);font-weight:700;margin-left:5px}
.rk .pc{font-variant-numeric:tabular-nums}
.rk .ar{width:26px;text-align:right;font-size:10.5px}
footer{display:flex;justify-content:space-between;font-size:10.5px;color:var(--mut);border-top:1px solid var(--line);padding-top:7px}
.cols{display:flex;gap:12px;min-height:0}
.col{flex:1;min-width:0}
.colh{font-size:11px;font-weight:800;color:var(--mut);letter-spacing:.06em;border-bottom:1.5px solid var(--ink);padding-bottom:3px;margin-bottom:3px}
"""
_OV_FIT_JS = ("function fit(){var s=document.getElementById('sc');var k=Math.min(1,window.innerWidth/1680);"
              "s.style.transform='scale('+k+')';document.body.style.height=(1080*k)+'px';}"
              "window.addEventListener('resize',fit);fit();")

def _ov_pct(x, d=1, signed=True):
    try:
...
def _ov_spark(ts, w=460, h=86, color="#2456A6", ref=None, lo=None, hi=None):
    """ts=[(date,val)] -> インラインSVGスパークライン。"""
    try:
        vs = [float(v) for _, v in ts if v == v]
        if len(vs) < 10:
            return '<div class="mut" style="font-size:11px">系列データ不足</div>'
        vlo = min(vs) if lo is None else lo
        vhi = max(vs) if hi is None else hi
        if vhi - vlo < 1e-9:
            vhi = vlo + 1
        pad = 4
        n = len(vs)
        def X(i): return pad + i * (w - 2*pad) / (n - 1)
        def Y(v): return pad + (1 - (v - vlo) / (vhi - vlo)) * (h - 2*pad)
        pts = " ".join(f"{X(i):.1f},{Y(v):.1f}" for i, v in enumerate(vs))
        refl = ""
        if ref is not None and vlo <= ref <= vhi:
            refl = f'<line x1="{pad}" y1="{Y(ref):.1f}" x2="{w-pad}" y2="{Y(ref):.1f}" stroke="rgba(0,0,0,.22)" stroke-dasharray="3,4" stroke-width="1"/>'
        area = f'<polygon points="{pad},{h-pad} {pts} {w-pad},{h-pad}" fill="{color}" opacity="0.10"/>'
        dot = f'<circle cx="{X(n-1):.1f}" cy="{Y(vs[-1]):.1f}" r="3.2" fill="{color}"/>'
        return (f'<svg width="100%" viewBox="0 0 {w} {h}" preserveAspectRatio="none" style="display:block">'
                f'{area}{refl}<polyline points="{pts}" fill="none" stroke="{color}" stroke-width="2"/>{dot}</svg>')
    except Exception:
        return ""

def _ov_gauge(v, labels=("低", "中", "高")):
    try:
        p = max(0.0, min(100.0, float(v)))
    except Exception:
        return ""
    ls = "".join(f"<span>{l}</span>" for l in labels)
    return f'<div class="gg"><i style="left:calc({p:.1f}% - 1px)"></i></div><div class="ggl">{ls}</div>'

def _ov_rets(c):
    """Close系列 -> 期間騰落 dict(r5,r21,r63,r126)。"""
    try:
        c = c.dropna()
        last = float(c.iloc[-1])
        def r(d):
            return (last / float(c.iloc[-d-1]) - 1) if len(c) > d else float("nan")
        return dict(r5=r(5), r21=r(21), r63=r(63), r126=r(126))
    except Exception:
        return dict(r5=float("nan"), r21=float("nan"), r63=float("nan"), r126=float("nan"))

def _ov_doc(title_jp, title_en, sub, header_right, rows_html, foot_left):
    return ("<!doctype html><html lang='ja'><head><meta charset='utf-8'>"
            "<meta name='viewport' content='width=device-width,initial-scale=1'>"
            f"<title>{title_jp}</title><style>{_OV_CSS}</style></head><body>"
            "<div class='stage'><div class='scaler' id='sc'><div class='card'>"
            f"<header><div><span class='h-t'>{title_jp}</span><span class='h-en'>{title_en}</span>"
            f"<div class='h-sub'>{sub}</div></div><div class='h-r'>{header_right}</div></header>"
            + rows_html +
            f"<footer><span>{foot_left}</span>"
            "<span>数値はビルド時点の実測値／個人の研究用・投資助言ではありません</span></footer>"
            "</div></div></div>"
            f"<script>{_OV_FIT_JS}</script></body></html>")

_OV_SAR = {"Blue": ("青・強い上昇", "#1d4ed8"), "Green": ("緑・上昇", "#16803c"),
           "Yellow": ("黄・注意", "#a16207"), "Red": ("赤・下落", "#b91c1c")}

def build_overview_cards(ctx):
    """実データ→Overview 1（マーケット概略）/ Overview 2（セクター・ローテーション）の2枚を生成。"""
    aux = ctx.get("aux") or {}
    ...
    m = ctx.get("m")
    asof_disp = ctx.get("asof_disp") or ""
    sar_lab, sar_hex = _OV_SAR.get(sar[0], ("判定なし", "#6b7280"))
    try:
        band_lab, _bcls = mri_band(aux.get("hl"))
    except Exception:
        band_lab = "—"
    header_right = (f'<span class="rchip" style="background:{sar_hex}"><i></i>トレンド {sar_lab}</span>'
                    f'<div class="score"><div class="v">{aux.get("cur", float("nan")):.0f}<span style="font-size:16px;font-weight:700">/100</span></div>'
                    f'<div class="l">MARKET STATUS・{band_lab}</div></div>')

    # ---------------- Overview 1: マーケット概略 ----------------
    perf = mkt.get("perf") or {}
    # 指数テーブル
    _prow = []
    for ix in (perf.get("indices") or []) + (perf.get("breadth_etf") or []):
        _prow.append(f'<tr><td class="l">{ix.get("label","—")}</td>'
                     + "".join(f'<td class="{_ov_cls(ix.get(k))}">{_ov_pct(ix.get(k))}</td>'
                               for k in ("d1", "w1", "m1", "ytd")) + "</tr>")
    vix = perf.get("vix")
    vc = perf.get("vixcurve") or {}
    vix_line = ""
    if vix is not None:
        _r = vc.get("ratio")
        _rt = f'　期間構造 1M/3M <b class="{"neg" if (_r or 0) >= 1 else "pos"}">{_r:.2f}</b>' if _r else ""
        vix_line = f'<div class="kv" style="border-bottom:0;margin-top:2px"><span class="k">VIX</span><span class="v">{vix:.1f}{_rt}</span></div>'
    tile_idx = ('<div class="t" style="flex:1.25"><div class="tt">指数パフォーマンス <span class="ten">PERFORMANCE</span></div>'
                '<table><tr><th class="l"></th><th>1日</th><th>1週</th><th>1カ月</th><th>年初来</th></tr>'
                + "".join(_prow) + "</table>" + vix_line + "</div>")
    # 地合いスコア推移
    mri_ts = mkt.get("mri_ts") or []
    tile_mri = ('<div class="t" style="flex:1.15"><div class="tt">地合いスコア推移（2年） <span class="ten">MARKET STATUS TREND</span></div>'
                + _ov_spark(mri_ts, color="#2456A6", ref=50, lo=0, hi=100)
                + f'<div class="tnote">トレンド・広がり・ボラ・信用の合成 0-100。現在 <b>{aux.get("cur", float("nan")):.0f}</b>・傾き {aux.get("slope","—")}・警戒 {aux.get("bear_n","—")}/4</div></div>')
    # ブレッドス
    bts = mkt.get("breadth_ts") or []
    tile_br = ('<div class="t" style="flex:1.15"><div class="tt">ブレッドス（市場の広がり） <span class="ten">BREADTH</span></div>'
               + _ov_spark(bts, color="#1E7A4D", ref=50, lo=0, hi=100)
               + f'<div class="kv"><span class="k">200日線上 / 50日線上</span><span class="v">{breadth.get("pa200", float("nan")):.0f}% / {breadth.get("pa50", float("nan")):.0f}%</span></div>'
               f'<div class="kv" style="border-bottom:0"><span class="k">騰落 / 52週高値圏</span><span class="v">{breadth.get("adv","—")}勝{breadth.get("dec","—")}敗 / {breadth.get("nh","—")}銘柄</span></div></div>')
    rowA = f'<div class="row" style="flex:1.12">{tile_mri}{tile_idx}{tile_br}</div>'

    # 売買代金 参加度
    dv = mkt.get("dollarvol_ts") or {}
    _ut = (dv.get("uni_trend") or {})
    _utdir = _ut.get("dir")
    _utlab = {"up": ("拡大（資金流入）", "pos"), "down": ("縮小（資金退潮）", "neg")}.get(_utdir, ("横ばい", "mut"))
    _dvsp = ""
    try:
        _s0 = (dv.get("series") or [None])[0]
        _vals = _s0.get("vals") if isinstance(_s0, dict) else None
        if _vals:
            _dvsp = _ov_spark(list(zip(dv.get("dates", []), _vals)), color="#A16207", ref=1.0)
    except Exception:
        pass
    tile_dv = ('<div class="t" style="flex:1"><div class="tt">売買代金 参加度 <span class="ten">PARTICIPATION</span></div>'
               + _dvsp
               + f'<div class="tnote">21日平滑÷自身の200日平均。トレンド: <b class="{_utlab[1]}">{_utlab[0]}</b></div></div>')
    # センチメント
    senti = mkt.get("senti") or {}
    _srows = ""
    for r in (senti.get("rows") or [])[:4]:
        try:
            lab, valtext, pct = r[0], r[1], r[2]
            _srows += (f'<div class="bar"><span class="n">{lab}</span><span class="tr">'
                       f'<i style="left:0;width:{max(2,min(100,pct)):.0f}%;background:#A16207;opacity:.55"></i></span>'
                       f'<span class="v" style="width:84px">{valtext}</span></div>')
        except Exception:
            pass
    tile_se = ('<div class="t" style="flex:1.15"><div class="tt">センチメント（群衆温度計） <span class="ten">SENTIMENT</span></div>'
               + (f'<div class="big">{senti.get("cur"):.0f}<span style="font-size:13px;font-weight:700;color:var(--mut)">/100・{senti.get("band","—")}</span></div>'
                  if senti.get("cur") is not None else '<div class="mut">取得不可</div>')
               + _ov_gauge(senti.get("cur") or 50, ("冷静", "平常", "過熱")) + _srows + "</div>")
    # リーダーの強さ
    lt = mkt.get("leader_temp") or {}
    _ltc = lt.get("cur")
    _ltlab = ("枯渇（歴史的低温）" if (_ltc is not None and _ltc <= 10) else
              "過熱" if (_ltc is not None and _ltc >= 90) else "平常")
    tile_lt = ('<div class="t" style="flex:1"><div class="tt">先導株の強さ <span class="ten">LEADER TEMP</span></div>'
               + (f'<div class="big">{_ltc:.0f}<span style="font-size:13px;font-weight:700;color:var(--mut)">%ile・{_ltlab}</span></div>'
                  if _ltc is not None else '<div class="mut">—</div>')
               + _ov_gauge(_ltc if _ltc is not None else 50, ("枯渇", "平常", "過熱"))
               + '<div class="tnote">先導株（189日RS上位10%）の63日リターンの過去分布内%タイル</div></div>')
    # 金利
    fred = mkt.get("macro_context") or {}
    def _fred_last(sid):
        d = fred.get(sid) or {}
        v = d.get("last")
        return float(v) if v is not None else None
    y10, y2, sp = _fred_last("DGS10"), _fred_last("DGS2"), _fred_last("T10Y2Y")
    if y10 is None:
        try:
            y10 = float(macro["^TNX"]["Close"].dropna().iloc[-1])
        except Exception:
            y10 = None
    def _macro_last(tk):
        try:
            return float(macro[tk]["Close"].dropna().iloc[-1])
        except Exception:
            return None
    dxy, mv = _macro_last("DX-Y.NYB"), _macro_last("^MOVE")
    _rows_yl = [("米10年金利", y10, "%")]
    if y2 is not None:
        _rows_yl.append(("米2年金利", y2, "%"))
    if sp is not None:
        _rows_yl.append(("10年-2年スプレッド", sp, "pt"))
    _rows_yl += [("ドル指数 DXY", dxy, ""), ("債券ボラ MOVE", mv, "")]
    _ykv = ""
    for lab, v, unit in _rows_yl[:4]:
        _ykv += f'<div class="kv"><span class="k">{lab}</span><span class="v">{("—" if v is None else f"{v:.2f}{unit}")}</span></div>'
    tile_yl = ('<div class="t" style="flex:1"><div class="tt">金利・マクロ <span class="ten">RATES & MACRO</span></div>'
               + _ykv + '<div class="tnote">FRED＋yfinance（ビルド時点の水準）</div></div>')
    rowB = f'<div class="row" style="flex:1">{tile_dv}{tile_se}{tile_lt}{tile_yl}</div>'

    # セクター騰落（週）Top/Bottom
    micro = ((mkt.get("sector_ranks") or {}).get("micro")) or []
    _mw = sorted([x for x in micro if x.get("w1") == x.get("w1")], key=lambda x: -x["w1"])
    def _sec_bar(x):
        v = x["w1"]; w = min(100, abs(v) / 0.06 * 100)
        col = "var(--pos)" if v >= 0 else "var(--neg)"
        side = "left:50%" if v >= 0 else f"right:50%"
        en = _OV_MICRO_EN.get(x["ja"], "")
        return (f'<div class="dv"><span class="n">{x["ja"]}<em>{en}</em></span>'
                f'<span class="tr"><i style="{side};width:{w/2:.0f}%;background:{col};opacity:.75"></i></span>'
                f'<span class="v {_ov_cls(v)}">{_ov_pct(v)}</span></div>')
    tile_sec = ('<div class="t" style="flex:1.3"><div class="tt">セクター騰落（1週間） <span class="ten">SECTOR MOVES 1W</span></div>'
                '<div class="cols"><div class="col"><div class="colh">上昇 TOP6</div>'
                + "".join(_sec_bar(x) for x in _mw[:6])
                + '</div><div class="col"><div class="colh">下落 BOTTOM6</div>'
                + "".join(_sec_bar(x) for x in _mw[-6:][::-1]) + "</div></div></div>")
    # サブテーマRS Top8（銘柄名は非掲載）
    _srs = ""
    for i, r in enumerate(sectors[:8]):
        _srs += (f'<div class="rk"><span class="i">{i+1}</span><span class="nm">{r.get("ja","—")}</span>'
                 f'<span class="pc mut" style="font-size:10.5px">{r.get("rot","")}</span>'
                 f'<span class="pc">RS {r.get("score", float("nan")):.0f}</span></div>')
    tile_srs = ('<div class="t" style="flex:1"><div class="tt">サブテーマRS 上位 <span class="ten">SUB-THEME RS TOP8</span></div>'
                + (_srs or '<div class="mut">—</div>')
                + '<div class="tnote">63日RSと189日RSの中央値ランク合成（0-100）</div></div>')
    # リーダー加速/失速
    lr = mkt.get("leader_run") or {}
    tile_lr = ('<div class="t" style="flex:1"><div class="tt">リーダーの勢い <span class="ten">MOMENTUM RUN</span></div>'
               + (f'<div class="big"><span class="pos">加速 {lr.get("acc","—")}</span>'
                  f'<span style="font-size:15px;color:var(--mut)"> vs </span>'
                  f'<span class="neg">失速 {lr.get("fade","—")}</span></div>'
                  f'<div class="kv" style="border-bottom:0"><span class="k">全体評価</span><span class="v">{lr.get("health","—")}</span></div>'
                  if lr else '<div class="mut">—</div>')
               + '<div class="tnote">RS189上位のうち直近21日で加速／失速している銘柄数（顔ぶれ非掲載）</div></div>')
    rowC = f'<div class="row" style="flex:1.18">{tile_sec}{tile_srs}{tile_lr}</div>'

    doc1 = _ov_doc("マーケット概略", "MARKET OVERVIEW", f"米国株の地合い・1枚まとめ｜{asof_disp}",
                   header_right, rowA + rowB + rowC, f"更新 {asof_disp}")

    # ---------------- Overview 2: セクター・ローテーション ----------------
    def _series_of(tk):
        if tk in extras:
            return extras[tk]
        d = macro.get(tk)
        try:
            return d["Close"] if d is not None else None
        except Exception:
            return None
    maj = []
    for tk, ja in SECTOR_ETFS:
        ...
        if c is None:
            continue
        d = _ov_rets(c); d.update(tk=tk, ja=ja, en=_OV_MAJOR_EN.get(ja, ""))
        maj.append(d)
    mic = []
    for tk, ja in MICRO_ETFS:
        c = _series_of(tk)
        if c is None:
            continue
        d = _ov_rets(c); d.update(tk=tk, ja=ja, en=_OV_MICRO_EN.get(ja, ""))
        mic.append(d)

    def _ranks(rows, key):
        vals = pd.Series({r["tk"]: r[key] for r in rows if r[key] == r[key]})
        return vals.rank(ascending=False)
    if maj:
        rk21, rk63, rk126 = _ranks(maj, "r21"), _ranks(maj, "r63"), _ranks(maj, "r126")
        for r in maj:
            r["rank21"] = rk21.get(r["tk"]); r["rank63"] = rk63.get(r["tk"]); r["rank126"] = rk126.get(r["tk"])
    _mrow = ""
    for r in sorted(maj, key=lambda x: (x.get("rank21") if x.get("rank21") == x.get("rank21") else 99)):
        _d = (r.get("rank63") or 0) - (r.get("rank21") or 0)
        ar = ('<span class="pos">▲%d</span>' % _d if _d > 0 else
              '<span class="neg">▼%d</span>' % -_d if _d < 0 else '<span class="mut">▬</span>')
        _mrow += (f'<tr><td class="l"><b>{r["ja"]}</b> <span class="mut" style="font-size:9.5px;font-weight:700">{r["en"]}</span></td>'
                  + "".join(f'<td class="{_ov_cls(r[k])}">{_ov_pct(r[k])}</td>' for k in ("r5", "r21", "r63", "r126"))
                  + f"<td>{ar}</td></tr>")
    tile_maj = ('<div class="t" style="flex:1.25"><div class="tt">大分類ランキング（1カ月騰落順） <span class="ten">MAJOR SECTORS</span></div>'
                '<table><tr><th class="l">セクター（等ウェイト）</th><th>1週</th><th>1カ月</th><th>3カ月</th><th>6カ月</th><th>順位Δ</th></tr>'
                + _mrow + "</table>"
                '<div class="tnote">Invesco等ウェイトETF（RSP系）・順位Δ=3カ月順位→1カ月順位の変化</div></div>')
    # 資金の流れ（短期順位% − 長期順位%）
    _flow = ""
    if maj:
        n_ = max(1, len(maj))
        for r in maj:
            r["_sp"] = ((r.get("rank126") or 0) - (r.get("rank21") or 0)) / n_ * 100
        for r in sorted(maj, key=lambda x: -x["_sp"]):
            v = r["_sp"]; w = min(100, abs(v) / 100 * 100)
            col = "var(--pos)" if v >= 0 else "var(--neg)"
            side = "left:50%" if v >= 0 else "right:50%"
            _flow += (f'<div class="dv"><span class="n">{r["ja"]}<em>{r["en"]}</em></span>'
                      f'<span class="tr"><i style="{side};width:{w/2:.0f}%;background:{col};opacity:.75"></i></span>'
                      f'<span class="v {_ov_cls(v)}">{v:+.0f}</span></div>')
    tile_flow = ('<div class="t" style="flex:1"><div class="tt">資金の流れ <span class="ten">MONEY FLOW</span></div>'
                 + (_flow or '<div class="mut">—</div>')
                 + '<div class="tnote">6カ月順位→1カ月順位の改善幅（+=資金流入・−=流出）</div></div>')
    rowA2 = f'<div class="row" style="flex:1.22">{tile_maj}{tile_flow}</div>'

    # 小分類 期間別ランキング（3列）
    def _rank_col(key, title, n=12):
        rs = sorted([x for x in mic if x[key] == x[key]], key=lambda x: -x[key])[:n]
        alt = "r63" if key == "r21" else "r126" if key == "r63" else "r126"
        alt_sorted = sorted([x for x in mic if x[alt] == x[alt]], key=lambda x: -x[alt])
        alt_pos = {x["tk"]: i for i, x in enumerate(alt_sorted)}
        out = f'<div class="col"><div class="colh">{title}</div>'
        for i, x in enumerate(rs):
            ap = alt_pos.get(x["tk"])
            d_ = (ap - i) if ap is not None else None
            ar = ('<span class="pos">▲</span>' if (d_ or 0) > 1 else
                  '<span class="neg">▼</span>' if (d_ or 0) < -1 else '<span class="mut">▬</span>')
            out += (f'<div class="rk"><span class="i">{i+1}</span><span class="nm">{x["ja"]}<em>{x["en"]}</em></span>'
                    f'<span class="pc {_ov_cls(x[key])}">{_ov_pct(x[key])}</span><span class="ar">{ar}</span></div>')
        return out + "</div>"
    tile_micro = ('<div class="t" style="flex:1.55"><div class="tt">テーマ別 期間ランキング <span class="ten">THEME LEADERS BY HORIZON</span></div>'
                  '<div class="cols">'
                  + _rank_col("r21", "1カ月 TOP12") + _rank_col("r63", "3カ月 TOP12") + _rank_col("r126", "6カ月 TOP12")
                  + '</div><div class="tnote">▲=より長い期間の順位から浮上（加速）・▼=後退（減速）</div></div>')
    # 加速/減速テーマ + 期間別先導株
    _acc = sorted([x for x in mic if x["r21"] == x["r21"] and x["r63"] == x["r63"]],
                  key=lambda x: -(x["r21"] - x["r63"]))
    def _spread_row(x):
        v = x["r21"] - x["r63"]
        return (f'<div class="rk"><span class="nm">{x["ja"]}<em>{x["en"]}</em></span>'
                f'<span class="pc {_ov_cls(v)}">{_ov_pct(v)}</span></div>')
    tile_ad = ('<div class="t" style="flex:1"><div class="tt">加速／減速テーマ <span class="ten">ACCEL / DECEL</span></div>'
               '<div class="cols"><div class="col"><div class="colh">加速 TOP5（1カ月−3カ月）</div>'
               + "".join(_spread_row(x) for x in _acc[:5])
               + '</div><div class="col"><div class="colh">減速 TOP5</div>'
               + "".join(_spread_row(x) for x in _acc[-5:][::-1]) + "</div></div>"
               '<div class="tnote">短期騰落が中期を上回る=資金の入り始め／下回る=抜け始め</div></div>')
    _ldr = ""
    try:
        if m is not None and len(m):
            for key, lab in (("rs63", "3カ月RS 上位"), ("rs189", "9カ月RS 上位")):
                top = m[key].dropna().sort_values(ascending=False).head(5)
                chips = "".join(f'<span style="display:inline-block;background:rgba(0,0,0,.06);border:1px solid var(--line);border-radius:7px;padding:2px 9px;margin:2px 4px 2px 0;font-size:11.5px;font-weight:800">{t}</span>'
                                for t in top.index)
                _ldr += f'<div class="colh" style="margin-top:5px">{lab}</div><div>{chips}</div>'
    except Exception:
        pass
    tile_ldr = ('<div class="t" style="flex:1"><div class="tt">期間別 先導株 <span class="ten">PERIOD LEADERS</span></div>'
                + (_ldr or '<div class="mut">—</div>')
                + '<div class="tnote">ユニバース内・相対力(RS)百分位の上位5銘柄</div></div>')
    rowB2 = f'<div class="row" style="flex:1.55">{tile_micro}</div>'
    rowC2 = f'<div class="row" style="flex:1">{tile_ad}{tile_ldr}</div>'

    doc2 = _ov_doc("セクター・ローテーション", "SECTOR ROTATION", f"大分類×テーマの資金の流れ｜{asof_disp}",
                   header_right, rowA2 + rowB2 + rowC2, f"更新 {asof_disp}")
    return doc1, doc2
...
def _post_iframe(doc):
    ...
    return ('<div class="postwrap"><button class="pfsbtn" onclick="pfs(this,1)">⛶ 全画面でスクショ</button>'
            f'<iframe class="postframe" srcdoc="{esc}"></iframe>'
            '<button class="pfsclose" onclick="pfs(this,0)">✕</button>'
            '<div class="postnote">全画面中はボタンが約3秒で自動的に消えます（画面タップで再表示）。横向きにすると大きく表示。</div></div>')
...
def _rank_in_badge(t, hold, selected):
    """選定トップ12の銘柄に「いつからランキング入りか」を表示。holdのed(継承された初回日)を使う。"""
    ...
    try:
        ...
        nn = ""
    return f'<span class="inb" title="トップ12に入り継続中の起点日">IN {ed}{("・"+nn) if nn else ""}</span>'
...
def build_rates_card(ctx, asof=None):
    ...
    for sid,cat,lab,unit,colored,risk_high in rows_def:
        ...
        cells.append(f'<div class="rrow"><div class="lft"><div class="nm">{lab}{alt}{badge}</div><div class="cat">{cat}</div></div>'
                     ...
                     f'<span class="hint">水準 {pct_txt}{speed_txt}</span></div>{sp}<div class="foot">観測 {eff_ld or "—"}{stale_txt}・{src_lab}{cache_txt}</div></div>')
...
def _spark(vals, asof=None, days=60, color="#6f93c9"):
    ...
    try:
        ...
        lx = P + (len(ys)-1)*(W-2*P)/(len(ys)-1); ly = H-P-(ys[-1]-lo)/rng*(H-2*P)
...
def _market_comment(aux, mkt, sar, breadth):
    ...
    return (
        ...
        f'{note_html}<div class="mkt-post-tools"><span>X共有用（結論のみ・銘柄/パラメータは載せない）</span>'
...
def _manual_holdings_card():
    ...
    return (
        ...
        '建値を入れると現在損益も出る。トップ12はシステムの候補、ここは<b>自分の実保有</b>。</div>'
        '<div class="calc-in">'
        '<div class="ci-row"><span class="ci-pre" style="font-size:12px">銘柄</span>'
...
.pulse .spark{width:100%;height:22px;margin:5px 0 0}
.pulse .pfoot{font-size:10px;margin-top:4px;border-top:1px solid rgba(255,255,255,.06);padding-top:3px}
.spark{grid-column:1 / -1;display:block;width:100%;height:22px;margin:5px 0 0;opacity:1}
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    _wk_concl = ('<div class="card" style="border:1.5px solid rgba(56,189,248,.35)">'
                 ...
                 '<div class="sub" style="margin:0">下の①〜⑤を読む前の要点：'
                 '②の警戒バッジが増えていないか → ③で来週の主戦場（改善→主導） → ④でリーダーが固定か流動か。'
                 'この3点で来週の基本姿勢（攻め/現状維持/守り）を決める。</div></div>')
    ...
    mkt_weekly = (
        _wk_concl
        + (mkt.get("wk_diff") or "")
        + _mkt_section("① 今週の相場", "地合いの推移・指数リターンで1週間を確認", en="Market Week")
        + '<div class="card"><h2>地合いの帯 <span class="h2en">Regime History</span></h2>'
        + _ribbon(mkt.get("trend_hist"))
        + '</div>'
        + _svg_mri(mkt.get("mri_ts", []))
        + _perf_card(mkt.get("perf", {}))
        + _mkt_section("② 金利とマクロ", "水準・変化・警戒バッジ", en="Rates & Macro")
        + (mkt.get("rates_card") or '<div class="card"><h2>金利レジーム</h2><div class="sub">FRED未取得</div></div>')
        + (mkt.get("pressure_card") or "")
        + _mkt_section("③ セクターの流れ", "来週の主戦場はどこか（改善→主導が狙い目）", en="Sector Rotation")
        + _rrg_card(mkt.get("rrg_etf"), None, mkt.get("etf_hier"), 100, "セクター・ローテーション（テーマETF）", _RRG_ETF_DESC)
        + _heatmap_card(mkt.get("sector_ranks"))
        + (mkt.get("wk_board") or "")
        + _mkt_section("④ 銘柄の入れ替わり", "リーダーの顔ぶれは固定か流動か（固定=トレンド継続・流動=物色転換）", en="Leadership Turnover")
        + _rs_continuity_card(mkt.get("rs_continuity"))
        ...
        + _mkt_section("⑥ 口座の答え合わせ", "カーブ×21EMA（入力・非常口はEquityタブ）", en="Account Review")
        + _equity_card(mkt.get("equity"), mkt.get("eq_attrib"))
        + (mkt.get("wk_acct") or ""))
    ...
    body = (build_today_action(sar, mkt, asof)
            ...
            '<button onclick="tab(\'t-alloc\',this)">Portfolio</button>'
            '<button onclick="tab(\'t-port\',this)">Top12</button>'
            '<button onclick="tab(\'t-today\',this)">Pickup</button>'
            '<button onclick="tab(\'t-w30\',this)">W30</button>'
            '<button onclick="tab(\'t-rs\',this)">RS</button>'
            '<button onclick="tab(\'t-rotation\',this)">Rotation</button>'
            '<button onclick="tab(\'t-sector\',this)">Industry</button>'
            '<button onclick="tab(\'t-weekly\',this)">Weekly</button>'
            '<button onclick="tab(\'t-equity\',this)">Equity</button>'
            '<button onclick="tab(\'t-rules\',this)">Rules</button>'
            '<button onclick="tab(\'t-post1\',this)">Overview 1</button>'
            '<button onclick="tab(\'t-post2\',this)">Overview 2</button></nav>'
            f'<section id="t-market" class="on">{market}</section>'
            f'<section id="t-weekly">{mkt_weekly}</section>'
            f'<section id="t-equity">{equity_tab}</section>'
            ...
            + _mkt_section("Overview 1 — マーケット概略", "実データから自動生成・全画面でスクショ投稿", en="Post Card — Market Overview")
            + '__POSTCARD1__</section>'
            f'<section id="t-post2">'
            + _mkt_section("Overview 2 — セクター・ローテーション", "実データから自動生成・全画面でスクショ投稿", en="Post Card — Sector Rotation")
...
def selftest(html, picks, setups, sectors, mkt=None, W=None, cutdate=None):
    ...
    for sid in ["t-today","t-market","t-weekly","t-equity","t-port","t-alloc","t-rs","t-rotation","t-sector","t-w30","t-rules"]:
    ...
    if "Pickup</button>" not in html:
        errs.append("pickup tab label missing")
    ...
    if "X共有用" not in html or "投稿文をコピー" not in html: errs.append("X-ready market digest missing")
...
def main():
    ...
    if do_self:
        ...
        if errs:
            ...
            sys.exit(1)
        bs_, bys_ = leaders
        print("SELFTEST OK (11 tabs (+W30), restored pickup layout, X-ready market digest, market health v2, emergency brake, RS compare+IN/OUT+continuity, N=%d, leaders=%d, buys=%d, sectors=%d, no None/nan)" %
