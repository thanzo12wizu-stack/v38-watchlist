...
def _equity_card(eq):
    setup = (
        ...
        '<div class="note">有効化: リポジトリに <b>equity.csv</b> を置く（環境変数 V38_EQUITY_CSV でパス指定可）。'
    ...
    return (
        ...
        f'<div class="sub">口座資産（equity.csv・{eq["n"]}記録）と、その21EMA（記録点ベース）。'
...
def build_lev_env(macro):
    ...
    if vpct is None:
        ...
        size, scls, snote = "薄く（半分目安）", "neg", "最悪−80%級のスイングが起こりうる帯。枚数を落として覚悟する"
    elif vpct >= 50:
        size, scls, snote = "標準〜やや薄め", "mut", "平常。無理な追いはしない"
    else:
        size, scls, snote = "フル可（配分どおり）", "pos", "静かな上昇はレバの追い風。減衰リスクが最も低い帯"
    return dict(up=up, gap=gap, streak=streak, vol20=vol20, vpct=vpct,
                size=size, scls=scls, snote=snote)

def _lev_env_card(le, rrg):
    if not le:
        return ('<div class="card"><h2>レバ玉エントリー環境（SOXL）</h2>'
                '<div class="sub">SOXX(半導体指数)データ取得不可。トレンド・ボラ判定を表示できず。</div></div>')
    # 半導体RRG象限（build_rrgのsemis点から）
    ...
    if semis:
        ...
        rot_note = {"主導": "SPY比で強く上向き", "改善": "弱かったが勢いが上向き＝先回り候補",
                    "弱化": "強いが勢いが鈍化", "停滞": "弱く勢いも下"}.get(semis["q"], "")
    else:
        rot, rot_note = '<span class="mut">—</span>', "SMH未取得"
    tcls = "st-good" if le["up"] else "st-bad"
    ttxt = (f'追い風（MA50&gt;MA100・乖離{le["gap"]*100:+.1f}%・連続{le["streak"]}日）' if le["up"]
            else f'逆風（MA50&lt;MA100・乖離{le["gap"]*100:+.1f}%・連続{le["streak"]}日）')
    trule = ("SOXXが50/100日線で上向き整列＝レバのトレンド地合いは良好。"
             if le["up"] else
             "SOXXが50日線割れ＝現行スペックのSOXL除外条件（→TQQQ寄せ）に該当する地合い。")
    vp = le["vpct"]
    ...
    return (
        f'<div class="card"><h2>レバ玉エントリー環境（SOXL）</h2>'
        f'<div class="sub">SOXLの新規サイズを測る3点。<b>表示専用・配分ロジックには接続していない</b>（'
        f'Fable検証で強度スロットルもボラサイジングも希釈型＝Sharpe不変と確定）。</div>'
        f'<div class="le-row"><span class="le-k">トレンド</span>'
        f'<span class="st {tcls}">{ttxt}</span></div>'
        f'<div class="note" style="margin:2px 0 8px">{trule}</div>'
        f'<div class="le-row"><span class="le-k">半導体ローテ</span>{rot}'
        f'<span class="le-sub">{rot_note}</span></div>'
        f'<div class="note" style="margin:2px 0 8px">RRG散布図の<b>■半導体</b>マーカーと連動。'
        f'※強度が強い＝厚く、ではない（実証で先行リターンの中央値は非単調）。向きを見るだけ。</div>'
        f'<div class="le-row"><span class="le-k">実現ボラ</span>'
        f'<span class="le-v">{le["vol20"]*100:.0f}%<span class="le-pct">（分位{vp:.0f}）</span></span>'
        f'<span class="le-size {le["scls"]}">サイズ: {le["size"]}</span></div>'
        f'{meter}'
        f'<div class="note">{le["snote"]}<br>'
        f'<span class="mut">※このDD削減効果は単体SOXLの数字（最悪−79→−61%）。確定構成の中ではNQ4色ゲートとMA除外が'
        f'同じ荒れ場を先に刈るため、実効の上乗せは小さい（≈−1.7pt）。メーターは配分の自動制御ではなく'
        f'「枚数を落とす覚悟」の目安として読む。</span></div></div>')
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    body = (build_today_action(sar, mkt, asof)
            ...
            f'<section id="t-sector">{_heatmap_card(mkt.get("sector_ranks"))}{_rrg_card(mkt.get("rrg"))}{_lev_env_card(mkt.get("lev_env"), mkt.get("rrg"))}{_sector_rank_card(mkt.get("sector_ranks"))}{sector}</section>'
...
def selftest(html, picks, setups, sectors):
    ...
    if "レバ玉エントリー環境" not in html: errs.append("lev-env panel missing")
    return errs
