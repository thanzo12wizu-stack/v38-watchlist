...
def build_buy_today(m, s2i, e2j, s2t, k=20, pick_set=None):
    ...
    base = ((close > sma200) & (close > sma50) & (m["rs"] >= 90)
            & (m["close"] >= 5) & (m["dist52"] >= -0.25))      # ステージ2・52週高値から25%以内
    sub = m[base].copy()
    if sub.empty:
        return []
    ...
    out = []
    for prio, _nr, t, r, tag, tcls in ranked[:k]:
...
def _buy_today_card(buys):
    ...
    return (f'<div class="card hot-card"><div class="hdr"><h2>本日のピックアップ</h2>'
            ...
            f'<div class="sub">RS90以上・ステージ2を実用セットアップで抽出（最大{len(buys)}・<b>タップで詳細</b>）。'
