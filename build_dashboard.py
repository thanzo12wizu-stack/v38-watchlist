...
def build_w30_breakouts(m, cap=12, e2j=None):
    ...
    warn_html = (f'<div class="mut" style="font-size:10.5px;font-weight:800;margin:10px 0 5px;color:#b23a2e">ギリ抜け（騙し率81%・原則見送り）</div>{warn}') if warn else ""
    tail = []
    if partm_list:
...
def _svg_senti(ts):
    ...
    zones = [(0,15,"#38bdf8"),(15,35,"#1e7a4d"),(35,65,"#64748b"),(65,85,"#8c6210"),(85,100,"#a8352a")]
    zr = "".join(f'<rect x="{pad}" y="{Y(z1):.1f}" width="{Wd-2*pad}" '
                 f'height="{max(0.0,Y(z0)-Y(z1)):.1f}" fill="{zc}" opacity="0.07"/>' for z0,z1,zc in zones)
    gl = "".join(f'<line x1="{pad}" y1="{Y(g):.1f}" x2="{Wd-pad}" y2="{Y(g):.1f}" stroke="#d6d2c7" stroke-width="1"/>'
                 f'<text x="{Wd-pad}" y="{Y(g)-2:.1f}" fill="#6e716b" font-size="20" font-weight="600" text-anchor="end">{g}</text>'
                 for g in (15,50,85))
...
def _sentiment_card(senti):
    ...
    cur = senti["cur"]
    col = "#b23a2e" if cur >= 85 else "#a6720f" if cur >= 65 else "#6b6e68" if cur >= 35 else "#2a9384" if cur > 15 else "#2e8a5a"
    comp_rows = []
...
def _svg_two(ys1, ys2, accent="#2e8a5a", accent2="#a6720f", gid="eq"):
    n = len(ys1)
    Wd, Ht, pad = 680, 200, 6
...
def _svg_expo(ups, n):
    ...
    for i, v in ups:
        ...
        col = "#38bdf8" if v >= 60 else "#2a9384" if v >= 25 else "#d6dae0"
        bars += f'<rect x="{X(i)-bw/2:.1f}" y="{Ht-6-h:.1f}" width="{bw:.1f}" height="{h:.1f}" rx="1.5" fill="{col}"/>'
    grid = (f'<line x1="{pad}" y1="{Ht-6}" x2="{Wd-pad}" y2="{Ht-6}" stroke="#d5dae2" stroke-width="1"/>'
            f'<line x1="{pad}" y1="{Ht-6-(Ht-14)}" x2="{Wd-pad}" y2="{Ht-6-(Ht-14)}" stroke="#d5dae2" stroke-width="1" stroke-dasharray="3,4"/>'
...
def _equity_card(eq, at=None):
    ...
    if expo:
        ...
        expo_html = (f'{expo}<div class="cap"><span class="mut">米株比率（青=高露出・空欄の日は非表示）</span>'
                     f'<span style="color:#2a9384;font-weight:700">直近 {lastu:.0f}%</span></div>')
    return (
        ...
        f'<span style="color:#2e8a5a;font-weight:700">実線=口座 ・ 破線={eq.get("ema_lab","21日EMA")}</span>'
...
_RRG_COL = {"主導": "#1e7a4d", "改善": "#2a9384", "弱化": "#a6720f", "停滞": "#b23a2e"}
...
def _rrg_svg(pts, cxv=50, period="now"):
    ...
    cx, cy = X(cxv), Y(0)
    quads = (f'<rect x="{cx}" y="{pad}" width="{Wd-pad-cx:.0f}" height="{cy-pad:.0f}" fill="#1e7a4d" opacity="0.08"/>'
             f'<rect x="{pad}" y="{pad}" width="{cx-pad:.0f}" height="{cy-pad:.0f}" fill="#1b7568" opacity="0.08"/>'
             f'<rect x="{cx}" y="{cy}" width="{Wd-pad-cx:.0f}" height="{Ht-pad-cy:.0f}" fill="#8c6210" opacity="0.08"/>'
             f'<rect x="{pad}" y="{cy}" width="{cx-pad:.0f}" height="{Ht-pad-cy:.0f}" fill="#a8352a" opacity="0.08"/>')
    QFS = 46                                   # 象限ラベルのフォント（大きく・薄く）
    qlab = (f'<text x="{Wd-pad-10}" y="{pad+QFS+2}" fill="#1e7a4d" fill-opacity="0.72" font-size="{QFS}" font-weight="800" text-anchor="end">主導</text>'
            f'<text x="{pad+10}" y="{pad+QFS+2}" fill="#2a9384" fill-opacity="0.72" font-size="{QFS}" font-weight="800">改善</text>'
            f'<text x="{Wd-pad-10}" y="{Ht-pad-12}" fill="#a6720f" fill-opacity="0.72" font-size="{QFS}" font-weight="800" text-anchor="end">弱化</text>'
            f'<text x="{pad+10}" y="{Ht-pad-12}" fill="#b23a2e" fill-opacity="0.72" font-size="{QFS}" font-weight="800">停滞</text>')
    FS = 15; CH = FS * 1.02
    placed = []
    def overlaps(bx):
        for q in placed:
    ...
    for p in order_pts:
        ...
        tx, ty, anc, bx = chosen
        placed.append(bx)
        lcol = "#565852" if (pq == "主導" or pcross) else "#6b6e68"
    ...
    return (f'<svg viewBox="0 0 {Wd} {Ht}" preserveAspectRatio="xMidYMid meet">{quads}'
            f'<line x1="{cx}" y1="{pad}" x2="{cx}" y2="{Ht-pad}" stroke="#d6dae0" stroke-width="1"/>'
            f'<line x1="{pad}" y1="{cy}" x2="{Wd-pad}" y2="{cy}" stroke="#d6dae0" stroke-width="1"/>'
            f'{qlab}{marks}{labels}'
            f'<text x="{Wd/2}" y="{Ht-6}" fill="#7c7f78" font-size="12" text-anchor="middle">→ SPYに対する相対力（右=強い）</text>'
            f'<text x="12" y="{Ht/2}" fill="#7c7f78" font-size="12" text-anchor="middle" transform="rotate(-90 12 {Ht/2})">↑ 相対力の勢い</text></svg>')
...
def build_share_html(aux, sar, mkt, picks, senti, asof_disp):
    color = sar[0]
    hexes = {"Blue": "#d1e6e3", "Green": "#16a34a", "Yellow": "#ca8a04", "Red": "#a8352a"}
...
body{{margin:0;background:#eceae2;font-family:'Hiragino Sans','Noto Sans JP',sans-serif}}
.fr{{width:1200px;height:675px;box-sizing:border-box;padding:44px 56px;display:flex;flex-direction:column;
background:radial-gradient(1200px 500px at 80% -10%,{glow}33,transparent),#eceae2;color:#565852}}
...
.ttl{{font-size:30px;font-weight:800;color:#1b7568}} .asof{{margin-left:auto;font-size:20px;color:#7c7f78}}
.mid{{display:flex;gap:40px;align-items:center;margin:34px 0 8px}}
.mri{{font-size:120px;font-weight:900;line-height:1}} .mrs{{font-size:26px;color:#6b6e68}}
.kv{{font-size:24px;color:#565852;line-height:1.9}} .kv b{{color:#fff}}
...
.rval{{font-size:19px;color:#565852;font-weight:700;margin-top:2px}}
...
.rrow .nm{{font-size:13px;color:#565852;font-weight:600;line-height:1.25}}
.rrow .cat{{font-size:10px;color:#64748b;letter-spacing:.03em}}
.rrow .rgt{{text-align:right;white-space:nowrap}}
.rrow .big{{font-size:19px;font-weight:700;color:#565852}}
.rrow .chg{{font-size:11px;margin-left:5px}}
.rrow .hint{{display:block;font-size:10.5px;color:#6e716b;margin-top:2px;letter-spacing:.02em}}
...
.rwarn{{color:#8c6210;font-size:11px;font-weight:700}}
...
.pk{{font-size:21px;font-weight:800;color:#1b7568;background:#d2d8e5;border-radius:9px;padding:5px 14px}}
...
def _theme_drill_card(themes):
    ...
    def tile(i, th):
        ...
        acol = "#1e7a4d" if th["mom"] > 1.5 else "#b23a2e" if th["mom"] < -1.5 else "#6b6e68"
    ...
    for i, th in enumerate(themes):
        chips = "".join(
            ...
            f'<span class="thstk-m" style="color:{"#1e7a4d" if x["mom"]>1.5 else "#b23a2e" if x["mom"]<-1.5 else "#7c7f78"}">'
...
def build_rates_insight(fred_ctx, asof=None):
    ...
    try:
        ...
        cur_html=('<div style="border:1.5px solid #38bdf8;background:rgba(56,189,248,.07);border-radius:12px;padding:12px 14px">'
                  f'<div style="font-size:12px;color:#94a3b8">現在の環境：<b style="color:#565852">{cur[0]} \u00d7 {cur[1]}</b></div>'
...
def build_dollarvol_ts(W, macro=None, lookback=CHART_LB, smooth=21):
    ...
    if macro:
        for key, color in (("QQQ", "#a78bfa"), ("SPY", "#2e8a5a")):
            d = macro.get(key)
            if d is None:
                continue
...
def _updown_state(v):
    ...
    if v >= 1.25: return ("集積（買い集め優勢）", "#2e8a5a", "accum")
    if v >= 1.00: return ("集積寄り", "#9ae6b4", "accum")
    if v >= 0.80: return ("分散寄り（注意）", "#9a6b14", "distrib")
    return ("分散（売り抜け優勢）", "#b23a2e", "distrib")
...
def _updown_vol_card(ts):
    ...
    zr = ""
    if hi2 > 1.25:
        zr += f'<rect x="{pad}" y="{Y(hi2):.1f}" width="{Wd-2*pad}" height="{max(0.0,Y(1.25)-Y(hi2)):.1f}" fill="#1e7a4d" opacity="0.06"/>'
    if lo2 < 0.80:
        zr += f'<rect x="{pad}" y="{Y(0.80):.1f}" width="{Wd-2*pad}" height="{max(0.0,Y(lo2)-Y(0.80)):.1f}" fill="#a8352a" opacity="0.06"/>'
    gl = ""
    for g in (0.8, 1.0, 1.25):
        if not (lo2 <= g <= hi2):
            continue
        is1 = abs(g - 1.0) < 1e-9
        stroke = "#3b4b63" if is1 else "#d6d2c7"
        dash = ' stroke-dasharray="4 3"' if is1 else ""
        gl += (f'<line x1="{pad}" y1="{Y(g):.1f}" x2="{Wd-pad}" y2="{Y(g):.1f}" stroke="{stroke}" stroke-width="1"{dash}/>'
               f'<text x="{Wd-pad}" y="{Y(g)-2:.1f}" fill="#6e716b" font-size="20" font-weight="600" text-anchor="end">{g:.2f}</text>')
...
def build_leader_breadth(m, macro=None):
    ...
    if s1 == "警戒" and s2 in ("警戒", "深刻"):
        mood, mcol = "最警戒（両方点灯・防御態勢へ）", "#b23a2e"
    elif s1 == "警戒":
        mood, mcol = "第1警戒（地殻変動の初動・身構える）", "#fb923c"
    elif s2 in ("警戒", "深刻"):
        mood, mcol = "第2警戒（痛みが本物・防御態勢へ移行）", "#fb923c"
    elif s1 == "注意":
        mood, mcol = "注意（内部が痛み始め）", "#8c6210"
    else:
        mood, mcol = "平常（リーダー健在）", "#1e7a4d"
...
def _transition_leaders_card(tl):
    if not tl or not tl.get("leaders"):
        return ""
    act = tl["active"]
    head = ("#1e7a4d" if act else "#6b6e68")
...
def _leader_breadth_card(lb):
    ...
    def bar(frac, thA, thB):
        ...
        col = "#b23a2e" if frac >= thB else "#8c6210" if frac >= thA else "#1e7a4d"
...
def build_leader_run(m, k=24):
    ...
    if acc >= fade * 2 and acc >= n * 0.3:
        health, hcol = "ラン拡大（先導株の勢いが伸びている）", "#1e7a4d"
    elif fade > acc:
        health, hcol = "ラン細り（勢いが失われつつある・防御寄り）", "#fb923c"
    else:
        health, hcol = "巡航（高値維持・方向感は中立）", "#6b6e68"
...
def _leader_temp_card(lt):
    ...
    if n < 3:
        ...
        def X(i): return pad + i * (Wd - 2*pad) / (n - 1)
        def Y(v): return pad + (1 - v/100) * (Ht - 2*pad)
        zones = [(0,10,"#1f6feb"),(10,30,"#1e7a4d"),(30,65,"#64748b"),(65,82,"#d29922"),(82,100,"#a8352a")]
        zr = "".join(f'<rect x="{pad}" y="{Y(z1):.1f}" width="{Wd-2*pad}" '
                     f'height="{max(0.0,Y(z0)-Y(z1)):.1f}" fill="{zc}" opacity="0.07"/>' for z0,z1,zc in zones)
        gl = "".join(f'<line x1="{pad}" y1="{Y(g):.1f}" x2="{Wd-pad}" y2="{Y(g):.1f}" stroke="#d6d2c7" stroke-width="1"/>'
                     f'<text x="{Wd-pad}" y="{Y(g)-2:.1f}" fill="#6e716b" font-size="20" font-weight="600" text-anchor="end">{g}</text>'
        ...
        svg = (f'<svg viewBox="0 0 {Wd} {Ht}" preserveAspectRatio="none">{zr}{gl}'
               f'<polyline points="{pts}" fill="none" stroke="#565852" stroke-width="2"/>'
               f'<circle cx="{X(n-1):.1f}" cy="{Y(ys[-1]):.1f}" r="3.5" fill="#565852"/></svg>')
        chart = (f'<div class="chart">{svg}'
                 f'<div class="cap cap-c"><span style="color:#565852;font-weight:700">現在 {cur:.0f}%（{zone}）・{span}</span></div>'
                 f'{_date_axis(ts)}</div>')
    return (
...
.rrow .big{font-size:19px;font-weight:700;color:#565852}
.rrow .chg{font-size:11px;margin-left:5px}
.rrow .hint{font-size:11px;color:#6b6e68;margin-top:2px;letter-spacing:.02em}
...
.pulse .pnm{font-size:13px;font-weight:700;color:#565852}
.pulse .pbig{font-size:20px;font-weight:800;color:#565852}
.pulse .pchg{font-size:11px}
.pulse .pcat{font-size:10.5px;color:#7e8ea3;margin-top:1px}
.pulse .pmeta{font-size:11px;color:#6b6e68;margin-top:4px;line-height:1.5}
...
.w30b{display:inline-block;margin-left:5px;font-size:10px;font-weight:800;border-radius:5px;padding:0 5px;background:#d5d9e2;color:#9fb2cc}
.w30b-full{background:#d1e6d9;color:#2e8a5a}
.w30b-part{background:#e5e5d1;color:#8c6210}
.w30b-weak{background:#e4d3d5;color:#b23a2e}
.w30row{padding:9px 2px;border-bottom:1px solid rgba(255,255,255,.06)}
.w30row-h{display:flex;align-items:baseline;gap:2px}
.w30row-h>b{font-size:14px;font-weight:800;color:#565852}
.w30row-m{font-size:11px;color:#6b6e68;margin-top:3px}
...
.w30hi td:nth-child(n+3){color:#a6720f;font-weight:800}
.w30t-s{background:#a16207;color:#856110}
.w30s{display:inline-block;margin-left:4px;font-size:10px;font-weight:700;border-radius:4px;padding:0 5px}
.w30s-new{background:#d1dae6;color:#3e9a8c}.w30s-adv{background:#d5d9e2;color:#9fb2cc}
.w30s-pb{background:#e5e5d1;color:#8c6210}.w30s-resume{background:#d1e6d9;color:#2e8a5a}
.w30s-stall{background:#ebeaec;color:#5b3fa6}.w30s-dead{background:#e4d3d5;color:#b23a2e}
.w30t-a{background:#d1e6d9;color:#2e8a5a}
.w30t-b{background:#d6dae0;color:#565852}
.w30new{display:inline-block;margin-left:4px;font-size:9px;font-weight:900;border-radius:4px;padding:0 5px;background:#d1e6e3;color:#dbeafe;vertical-align:middle}
.w30wk{margin-left:4px;font-size:10px;color:#8595aa}
.w30g-full{color:#2e8a5a}.w30g-part{color:#8c6210}.w30g-weak{color:#b23a2e}
.w30exit{display:grid;grid-template-columns:74px 1fr;gap:10px;padding:8px 2px;border-bottom:1px solid rgba(255,255,255,.06)}
.w30exit-t{font-size:11px;font-weight:800;color:#17685c}
.w30exit-b{font-size:11.5px;color:#565852;line-height:1.5}
.w30coh{font-size:11.5px;color:#565852;line-height:1.5;padding:8px 2px;border-bottom:1px solid rgba(255,255,255,.06)}
...
.lead-head{display:flex;justify-content:space-between;gap:10px;align-items:flex-end;margin:8px 0 6px}.lead-head b{font-size:15px}.lead-head span{font-size:10px;color:#7c7f78;white-space:nowrap}.lead-grid{display:grid;gap:5px}.lead-row{display:grid;grid-template-columns:58px 1fr;gap:8px;padding:7px 8px;border-radius:8px;background:rgba(255,255,255,.025)}.lead-row>span{font-size:10px;color:#7c7f78}.lead-row>b{font-size:11px;font-weight:600;line-height:1.35}
.mkt20{padding-bottom:12px}.mkt20-top{display:flex;justify-content:space-between;align-items:flex-start;gap:12px}.mkt20-top h2{margin-bottom:4px}.mkt20-verdict{font-size:14px;font-weight:800;line-height:1.45;color:#565852}.mkt20-score{min-width:74px;text-align:right;display:flex;flex-direction:column;align-items:flex-end}.mkt20-score>b{font-size:27px;line-height:1;color:#1b7568}.mkt20-score>span{font-size:10px;font-weight:800;color:#c8d5e6;margin-top:3px}.mkt20-score>em{font-size:9px;font-style:normal;color:#718096;margin-top:2px}.mkt20-label{font-size:9px;font-weight:800;letter-spacing:.12em;color:#6f8198;margin:9px 0 5px;text-transform:uppercase}.mkt20-grid{display:grid;gap:5px}.mkt-post-tools{display:flex;align-items:center;justify-content:space-between;gap:8px;margin:10px 0 6px;font-size:9.5px;font-weight:800;color:#70839a}.mkt-copy{margin-left:auto}.mkt-post{display:grid;gap:5px;padding:9px 10px;border-radius:10px;background:linear-gradient(180deg,rgba(30,50,76,.42),rgba(12,21,33,.55));border:1px solid rgba(91,127,176,.28)}.mkt-post-hook{font-size:12px;font-weight:900;line-height:1.42;color:#f0f5fb;padding-bottom:5px;border-bottom:1px solid rgba(122,148,177,.18)}.mkt-post-line{font-size:10.7px;line-height:1.48;color:#c8d5e6}.mkt-post-line>b{color:#17685c;margin-right:3px}.mkt-post-line:nth-child(2){font-size:11.2px;color:#565852}.mkt20-row{display:grid;grid-template-columns:44px minmax(0,1fr);gap:8px;padding:7px 9px;border-radius:8px;background:rgba(255,255,255,.028);border:1px solid rgba(255,255,255,.035)}.mkt20-k{font-size:10px;font-weight:800;color:#86a8cb;padding-top:1px}.mkt20-row b{display:block;font-size:11.5px;line-height:1.35;color:#565852;white-space:normal}.mkt20-row small{display:block;font-size:9.5px;line-height:1.35;color:#8593a6;margin-top:2px;white-space:normal}.mkt-fold{margin-top:8px;border-top:1px solid rgba(122,148,177,.18);padding-top:7px}.mkt-fold>summary{cursor:pointer;list-style:none;display:flex;justify-content:space-between;align-items:center;font-size:10.5px;font-weight:800;color:#17685c;padding:2px 1px}.mkt-fold>summary::-webkit-details-marker{display:none}.mkt-fold>summary:before{content:"▸";font-size:9px;margin-right:6px}.mkt-fold[open]>summary:before{content:"▾"}.mkt-fold-state:after{content:"タップで開く";font-size:9px;color:#63738a;font-weight:600}.mkt-fold[open] .mkt-fold-state:after{content:"折りたたむ"}.mkt-fold-body{display:grid;grid-template-columns:1fr 1fr;gap:5px;margin-top:7px}.mkt20-deep{padding:8px 9px;border-radius:8px;background:#d2dae5;border:1px solid #d4dbe2}.mkt20-deep>span{display:block;font-size:9px;color:#718096;margin-bottom:3px}.mkt20-deep>b{display:block;font-size:10.5px;line-height:1.45;color:#565852}.mkt20-deep:last-child:nth-child(odd){grid-column:1/-1}@media(max-width:620px){.mkt20-top{align-items:flex-start}.mkt20-verdict{font-size:13px}.mkt20-score>b{font-size:24px}.mkt-fold-body{grid-template-columns:1fr}.mkt20-deep:last-child:nth-child(odd){grid-column:auto}.mkt20-row{grid-template-columns:42px minmax(0,1fr);padding:7px 8px}}


html{background:#eceae2;color-scheme:dark}
body{font-family:-apple-system,BlinkMacSystemFont,'Helvetica Neue',sans-serif;background:#eceae2;color:#565852;font-size:14px;-webkit-text-size-adjust:100%}
...
.asof{color:#7c7f78;font-size:11px;margin-top:2px}
.banner{border-radius:14px;padding:14px 16px;margin:10px 0 4px;border:1px solid #d6d2c7;background:linear-gradient(135deg,#d4d9e3,#d4d9e3)}
.banner .lab{font-size:12px;color:#565852;opacity:.85}
...
.b-ovh .st{color:#2e8a5a}.b-bull .st{color:#1e7a4d}
.b-neu .st{color:#a6720f}.b-weak .st{color:#fb923c}.b-bear .st{color:#b23a2e}
.gauge{height:10px;border-radius:6px;margin:10px 0 4px;background:linear-gradient(90deg,#e6d1d1 0%,#e6d6d1 30%,#a16207 45%,#15803d 60%,#166534 100%);position:relative}
...
.sar .lab{font-size:11px;color:#565852;opacity:.9;letter-spacing:.04em}
...
.sar-blue{background:linear-gradient(135deg,#d1d8e6,#d1e6e3)}   .sar-blue .dot{background:#2a9384}
.sar-green{background:linear-gradient(135deg,#d1e6d9,#16a34a)}  .sar-green .dot{background:#1e7a4d}
.sar-yellow{background:linear-gradient(135deg,#e6dbd1,#ca8a04)} .sar-yellow .dot{background:#8c6210}
.sar-red{background:linear-gradient(135deg,#e6d1d1,#a8352a)}    .sar-red .dot{background:#b23a2e}
.sar-gray{background:linear-gradient(135deg,#d8dadf,#4b5563)}   .sar-gray .dot{background:#9ca3af}
...
.sar-badge.est{background:#a16207;color:#856110}
.sar-badge.none{background:#d6dae1;color:#565852}
/* distribution-day + performance widgets */
.dd{display:flex;gap:10px;flex-wrap:wrap}
.dd .box{flex:1 1 120px;background:#efece4;border:1px solid #d6d2c7;border-radius:10px;padding:10px 12px}
.dd .box .t{font-size:12px;color:#6b6e68;font-weight:700}
...
.st-good{background:#d1e6d9;color:#2e8a5a}.st-warn{background:#e6dbd1;color:#856110}.st-bad{background:#e6d1d1;color:#b23a2e}
.perf{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}
.ixn{font-size:13px;font-weight:800;color:#1b7568;margin:10px 0 6px}
.ixn:first-child{margin-top:0}
.perf-vix{margin-top:10px;font-size:13px;color:#6b6e68}
.perf-vix b{font-size:16px;color:#565852}
.perf-div{font-size:11px;font-weight:800;color:#6b6e68;margin:10px 0 4px;padding-top:8px;border-top:1px dashed #d6d2c7}
.vixc{margin-top:10px;font-size:12.5px;color:#6b6e68}
.vixc-l{font-weight:800;color:#565852;margin-right:4px}
.vixc b{font-size:15px;color:#565852}
.warnt{color:#a6720f;font-weight:700}
.ftd-row{display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-top:1px solid #d5dae2;font-size:13px}
.ftd-row:first-of-type{border-top:0}
.ftd-x{font-weight:800;color:#565852}
.perf .c{background:#efece4;border:1px solid #d6d2c7;border-radius:10px;padding:8px 10px;text-align:center}
.perf .c .k{font-size:11px;color:#7c7f78}.perf .c .v{font-size:16px;font-weight:800;margin-top:2px;font-variant-numeric:tabular-nums}
nav{position:sticky;top:0;z-index:9;background:rgba(11,15,23,.88);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);display:flex;gap:6px;overflow-x:auto;padding:9px 0;border-bottom:1px solid #d6d2c7;scrollbar-width:none}
nav::-webkit-scrollbar{display:none}
nav button{flex:0 0 auto;background:#d5d9e2;color:#6b6e68;border:1px solid #d5dae1;border-radius:18px;padding:8px 15px;font-size:13px;font-weight:700;transition:background .15s,color .15s}
nav button.on{background:#1f6feb;color:#fff;border-color:#1f6feb}
button:focus-visible,a:focus-visible{outline:2px solid #1b7568;outline-offset:2px}
section{display:none;padding-top:12px}
section.on{display:block}
.card{background:#f6f4ee;border:1px solid #d6d2c7;border-radius:13px;padding:12px 14px;margin-bottom:12px}
...
.setup-h .ct{font-size:11px;color:#7c7f78}
.chips{display:flex;flex-wrap:wrap;gap:6px}
.chip{background:#d4dae3;border:1px solid #d5dae2;border-radius:9px;padding:8px 12px;font-size:12.5px;font-weight:600;color:#565852;cursor:pointer}
.chip:active{background:#d4dae3}
.chip.hot{border-color:#2f81f7;color:#1b7568}
.chip.ov{border-color:#a16207;color:#856110;background:#e6e0d1}
.chip.ov .b{display:inline-block;margin-left:5px;background:#a16207;color:#eceae2;font-size:10px;font-weight:800;border-radius:5px;padding:0 5px}
/* ①〜⑤ state chips */
.chip.s-buy{background:#d1e6d8;border-color:#1f9d4d;color:#2e8a5a}
.chip.s-shape{background:#d5d9e1;border-color:#d6dae0;color:#aab6c8}
.chip.s-go{background:#d1e6da;border-color:#1f7a45;color:#2e8a5a}
.chip.s-wait{background:#e6e1d1;border-color:#a16207;color:#856110}
.chip.s-deep{background:#e6dad1;border-color:#c2660b;color:#a6720f}
.chip.s-break{background:#e3d3d6;border-color:#a13030;color:#b23a2e}
.qb{font-size:9px;font-weight:800;border-radius:5px;padding:1px 5px;white-space:nowrap;border:1px solid}
.qb-go{background:#d1e6d8;color:#2e8a5a;border-color:#1d7840}
.qb-wait{background:#d6dae1;color:#6b6e68;border-color:#d7dbdf}
.blkrow td{background:rgba(255,255,255,.018)}
.blkrow .tk{color:#aeb9c7}
/* 本日◎押し目 buy cards */
.buy{background:#d4e3da;border:1px solid #1f6d3c;border-radius:11px;padding:8px 11px;margin-bottom:6px}
...
.buy-h .rs{font-size:12px;font-weight:700;color:#7c7f78}
.buy-m{font-size:11px;color:#6b6e68;margin:2px 0 5px}
...
.buy-g .k{display:block;font-size:9px;color:#7c7f78}
.buy-g .v{font-size:12px;font-weight:700}
.cmt{background:linear-gradient(135deg,#d1d9e5,#d1dae6);border-color:#d3dbe4}
.cmt-b{font-size:14px;line-height:1.65;color:#565852}
.cmt-b b{color:#1b7568}
.chart{margin-top:4px}
.chart svg{width:100%;height:auto;display:block}
.chart .cap{display:flex;justify-content:space-between;font-size:11px;color:#7c7f78;margin-top:4px}
.empty{color:#5b6b80;font-size:12px;font-style:italic}
.more{display:inline-block;color:#7c7f78;font-size:11px;font-weight:700;padding:4px 2px;vertical-align:middle}
/* copy buttons */
.cp{background:#d2dae4;border:1px solid #d4dbe3;border-radius:7px;color:#1b7568;font-size:11px;
...
.pend-wait{background:rgba(239,68,68,.16);color:#b23a2e;border:1px solid rgba(239,68,68,.4)}
.pend-ok{background:rgba(34,197,94,.16);color:#1e7a4d;border:1px solid rgba(34,197,94,.4)}
.waitb{display:inline-block;font-size:9.5px;font-weight:800;border-radius:5px;padding:1px 6px;margin-left:5px;background:rgba(239,68,68,.16);color:#b23a2e;border:1px solid rgba(239,68,68,.4);vertical-align:middle}
.reg-card{border:1px solid #c9c4b8}
.reg-hdr{font-size:11px;font-weight:800;border-radius:6px;padding:3px 10px}
.reg-hdr-ok{background:rgba(67,201,138,.14);color:#1e7a4d;border:1px solid rgba(67,201,138,.35)}
.reg-hdr-warn{background:rgba(227,170,60,.15);color:#e3aa3c;border:1px solid rgba(227,170,60,.4)}
.reg-hdr-bad{background:rgba(229,100,94,.16);color:#b23a2e;border:1px solid rgba(229,100,94,.45)}
...
.reg-cell{border:1px solid #d6d2c7;border-radius:10px;padding:11px 12px;background:#d4d9e3}
...
.reg-k{font-size:10px;letter-spacing:.04em;color:#6b6e68;font-weight:700}
.reg-v{font-family:ui-monospace,Menlo,monospace;font-size:26px;font-weight:800;line-height:1.1;margin:2px 0}
.reg-ok .reg-v{color:#1e7a4d}.reg-warn .reg-v{color:#e3aa3c}.reg-bad .reg-v{color:#b23a2e}.reg-na .reg-v{color:#55627a}
.reg-l{font-size:11px;font-weight:700;margin-bottom:3px}
.reg-ok .reg-l{color:#1e7a4d}.reg-warn .reg-l{color:#e3aa3c}.reg-bad .reg-l{color:#b23a2e}
.reg-sub{font-size:9.5px;color:#7c7f78;line-height:1.4}
.reg-chips{margin-top:6px;display:flex;flex-wrap:wrap;gap:3px}
.rgchip{font-family:ui-monospace,Menlo,monospace;font-size:9px;font-weight:700;color:#17685c;background:#d4d9e3;border:1px solid #c9c4b8;border-radius:4px;padding:1px 4px}
...
.pqb.pq-a{background:rgba(67,201,138,.18);color:#1e7a4d;border:1px solid rgba(67,201,138,.5)}
.pqb.pq-b{background:rgba(143,179,255,.16);color:#17685c;border:1px solid rgba(143,179,255,.45)}
.pqb.pq-c{background:rgba(159,176,197,.12);color:#6b6e68;border:1px solid rgba(159,176,197,.35)}
...
.pqtag.pq-hi{background:rgba(67,201,138,.14);color:#1e7a4d}
.pqtag.pq-mid{background:rgba(143,179,255,.13);color:#17685c}
.pqtag.pq-lo{background:rgba(159,176,197,.1);color:#6b6e68}
.pqtag.pq-neg{background:rgba(229,100,94,.13);color:#b23a2e}
.pqbadge{font-size:9px;font-weight:800;color:#17685c;background:rgba(143,179,255,.13);border:1px solid rgba(143,179,255,.4);border-radius:5px;padding:1px 6px;margin-left:6px;vertical-align:middle}
.pqnum{font-family:ui-monospace,Menlo,monospace;font-size:14px;color:#1e7a4d}
.cp-one{font-size:9.5px;padding:1px 7px;margin-left:6px;vertical-align:middle;font-weight:700}
.cp.cp-done{background:#d1e6d8;border-color:#1f9d4d;color:#2e8a5a}
...
.inb{background:rgba(56,189,248,.14);border:1px solid rgba(56,189,248,.4);color:#2a9384;font-size:9px;font-weight:800;padding:1px 6px;border-radius:6px;margin-left:5px;white-space:nowrap}
.newb{display:inline-block;font-size:9px;font-weight:800;border-radius:5px;padding:1px 6px;margin-left:5px;vertical-align:middle;background:rgba(88,166,255,.16);color:#17685c;border:1px solid rgba(88,166,255,.45)}
...
.ppb-now{background:rgba(67,201,138,.18);color:#1e7a4d;border-color:rgba(67,201,138,.5)}
.ppx{font-size:10px;font-weight:800;color:#a78bfa;margin-left:5px}
.cfrow{display:flex;align-items:center;gap:10px;padding:7px 9px;border-radius:8px;background:#d4d9e3;border:1px solid #d6d2c7;margin-bottom:5px;cursor:pointer;flex-wrap:wrap}
...
.cfn{font-size:12px;font-weight:800;color:#d6dae1;background:#17685c;border-radius:5px;min-width:20px;text-align:center;padding:1px 0}
.cf-hi .cfn{background:#1e7a4d}
.cft{font-size:13px;font-weight:800}
.cfs{display:flex;gap:4px;flex-wrap:wrap}
.sgp{font-size:9px;font-weight:700;padding:1px 6px;border-radius:4px;background:#d4d9e2;color:#6b6e68;border:1px solid #d5dae1}
.cflg{margin-top:6px;font-size:10px;color:#6b7788;line-height:1.6}
.cfdet{margin-top:7px}
.cfdet summary{cursor:pointer;font-size:10.5px;color:#17685c;list-style:none;font-weight:700}
...
.ppd{font-size:9px;color:#7c7f78;margin-left:4px}
.hgrp{margin:10px 0 4px}
.hgrp-h{font-size:12px;font-weight:800;letter-spacing:.03em;display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:1px solid #d5dae2;margin-bottom:4px}
.hgrp-n{font-size:10px;font-weight:700;color:#7c7f78;background:#d4d9e2;border-radius:4px;padding:0 5px}
...
.entb2{display:inline-block;font-size:9.5px;font-weight:800;border-radius:5px;padding:1px 8px;margin-left:5px;vertical-align:middle;background:#d4d9e2;color:#7c7f78;border:1px solid #c9c4b8;cursor:pointer}
.entb2.ent-on{background:rgba(67,201,138,.16);color:#1e7a4d;border-color:rgba(67,201,138,.5)}
.hmper{display:inline-flex;gap:3px;margin-left:auto}
.hmp{font-size:11px;font-weight:700;padding:2px 10px;border-radius:6px;background:#d4d9e2;color:#7c7f78;border:1px solid #c9c4b8;cursor:pointer}
.hmp.on{background:rgba(88,166,255,.16);color:#17685c;border-color:rgba(88,166,255,.5)}
...
.msec-x .msec-g{display:none;margin-top:7px;padding-top:8px;border-top:1px solid #d5dae2;color:#93a1b5;font-size:11.5px;line-height:1.65}
.msec-x.open .msec-g{display:block}
.msec-more{font-size:10px;color:#5f6b7e;font-weight:700;margin-left:4px;white-space:nowrap}
.rrg-more{cursor:pointer;color:#17685c}
.rrg-desc{font-size:11.5px;color:#93a1b5;line-height:1.65;margin:2px 0 10px;padding:9px 11px;background:#efece4;border:1px solid #d5dae2;border-left:3px solid #c9c4b8;border-radius:8px}
.rrgview .chart{width:100%;max-width:100%}
.rrgview .chart svg{width:100%;height:auto;display:block}
.msec-x.open .msec-more{color:#17685c}
...
.bdry td{text-align:center;font-size:9.5px;color:#5f6b7e;padding:4px 0;border-top:1px dashed #c9c4b8;border-bottom:1px dashed #c9c4b8;background:#d4dae2}
.reg-role{font-size:9px;color:#7c7f78;line-height:1.4;margin:2px 0 4px}
.reg-kind{font-size:8.5px;font-weight:800;border-radius:4px;padding:1px 4px;margin-left:4px;vertical-align:middle}
.kind-t{background:rgba(88,166,255,.15);color:#17685c;border:1px solid rgba(88,166,255,.4)}
...
.on-bad{background:rgba(229,100,94,.16);color:#b23a2e;border:1px solid rgba(229,100,94,.4)}
.on-warn{background:rgba(240,136,62,.14);color:#f0883e;border:1px solid rgba(240,136,62,.38)}
.on-ok{background:rgba(67,201,138,.12);color:#1e7a4d;border:1px solid rgba(67,201,138,.32)}
.note p,.msec-g p{margin:0 0 7px;line-height:1.7}
.note p:last-child,.msec-g p:last-child{margin-bottom:0}
.note p.mut{color:#6b7788;font-size:10.5px;border-top:1px solid #d5dae2;padding-top:6px;margin-top:8px}
...
.tbchip{display:flex;flex-direction:column;gap:1px;padding:6px 9px;border-radius:8px;background:#d4d9e3;border:1px solid #d6d2c7;border-left:3px solid #f0b849;cursor:pointer}
.tbchip:hover{background:#d3d9e4}
.tbtk{font-weight:800;font-size:12.5px}
.tbmeta{font-family:ui-monospace,Menlo,monospace;font-size:9.5px;color:#7c7f78}
.def-card{border:1px solid rgba(229,100,94,.5);background:rgba(229,100,94,.05)}
.def-badge{font-size:9px;font-weight:800;color:#b23a2e;background:rgba(229,100,94,.15);border:1px solid rgba(229,100,94,.45);border-radius:5px;padding:1px 6px;margin-left:6px;vertical-align:middle}
.def-list{margin:8px 0 4px;padding-left:0;list-style:none}
.def-list li{font-size:12px;color:#c8d2df;line-height:1.5;padding:5px 0 5px 20px;position:relative;border-top:1px solid #d5dae1}
.def-list li:first-child{border-top:none}
.def-list li::before{content:"□";position:absolute;left:2px;color:#b23a2e;font-size:11px}
...
.mrchip{display:flex;align-items:center;gap:6px;padding:6px 9px;border-radius:8px;background:#d4d9e3;border:1px solid #d6d2c7;cursor:pointer}
.mrchip:hover{background:#d3d9e4}
.mrchip.run-acc{border-left:3px solid #1e7a4d}
.mrchip.run-cru{border-left:3px solid #17685c}
.mrchip.run-fade{border-left:3px solid #b23a2e}
.mrtk{font-weight:800;font-size:12px;min-width:40px}
.mrph{font-size:9px;font-weight:700;padding:1px 4px;border-radius:4px;background:#d4d9e3;color:#6b6e68;white-space:nowrap;flex-shrink:0}
.mrchip.run-acc .mrph{color:#1e7a4d}.mrchip.run-cru .mrph{color:#17685c}.mrchip.run-fade .mrph{color:#b23a2e}
.mrtr{font-family:ui-monospace,Menlo,monospace;font-size:9.5px;color:#7c7f78;margin-left:auto;white-space:nowrap;flex-shrink:0;letter-spacing:-0.2px}
.mrtr b{color:#565852}
.mr-acc{color:#1e7a4d}.mr-cru{color:#17685c}.mr-fad{color:#b23a2e}
.okb{display:inline-block;font-size:9.5px;font-weight:800;border-radius:5px;padding:1px 6px;margin-left:5px;background:rgba(34,197,94,.16);color:#1e7a4d;border:1px solid rgba(34,197,94,.4);vertical-align:middle}
.row-carry{background:rgba(96,165,250,.06)}
.coil{display:inline-block;width:52px;height:7px;border-radius:4px;background:#d6d2c7;overflow:hidden;vertical-align:middle;margin-right:4px}
.coil i{display:block;height:7px;border-radius:4px;background:linear-gradient(90deg,#1e7a4d,#84cc16)}
...
.r3b{display:inline-block;font-size:9px;font-weight:800;border-radius:5px;padding:1px 5px;margin-left:4px;background:rgba(34,197,94,.18);color:#1e7a4d;border:1px solid rgba(34,197,94,.45);vertical-align:middle}
...
.ent-good{background:rgba(34,197,94,.16);color:#1e7a4d;border-color:rgba(34,197,94,.45)}
.ent-ok{background:rgba(56,189,248,.14);color:#2a9384;border-color:rgba(56,189,248,.4)}
.ent-lo{background:rgba(148,163,184,.16);color:#6b6e68;border-color:rgba(148,163,184,.4)}
.ent-warn{background:rgba(234,179,8,.16);color:#8c6210;border-color:rgba(234,179,8,.42)}
.ent-bad{background:rgba(239,68,68,.16);color:#b23a2e;border-color:rgba(239,68,68,.45)}
...
.fade-lo{background:rgba(234,179,8,.14);color:#8c6210;border-color:rgba(234,179,8,.4)}
.fade-hi{background:rgba(239,68,68,.18);color:#b23a2e;border-color:rgba(239,68,68,.5)}
.rspbar{margin:6px 0 2px}
.rsper{background:#d2dae4;border:1px solid #d5dae2;color:#155e53;font-size:12px;font-weight:700;border-radius:8px;padding:6px 12px;cursor:pointer}
.rsper:active{background:#d3d9e4}
.slrow{padding:8px 0;border-top:1px solid #d5dae2}
.slrow:first-of-type{border-top:0}
.slname{font-size:13px;font-weight:700;color:#565852;margin-bottom:5px}
.rotb{display:inline-block;font-size:10px;font-weight:800;border-radius:5px;padding:1px 6px;border:1px solid;background:#efece4}
.rk-leg{margin:2px 0 8px}
.rk-leg summary{font-size:12px;color:#2a9384;cursor:pointer;padding:4px 0;list-style:none}
.rk-leg summary::-webkit-details-marker{display:none}
.rk-leg summary:before{content:"▸ ";color:#6e716b}
...
.lbk{font-size:13px;font-weight:700;color:#565852}
.lbv{font-size:15px;font-weight:800;color:#565852}
.lbbar{position:relative;height:8px;background:#d4dae2;border-radius:5px;margin:5px 0 2px;overflow:hidden}
...
.lbsub{font-size:11px;color:#6e716b;margin-bottom:4px}
.lbnote{font-size:11px;color:#6e716b;margin-top:10px;border-top:1px solid #d5dae2;padding-top:8px}
.lbchips{display:flex;flex-wrap:wrap;align-items:center;gap:5px;margin:2px 0 8px}
.lbcl{font-size:10px;font-weight:700;color:#6e716b;margin-right:2px}
.chip.warn{background:rgba(239,68,68,.12);border-color:rgba(239,68,68,.4);color:#f5b5b5}
.chip.tl-go{background:rgba(34,197,94,.16);border-color:rgba(34,197,94,.5);color:#1e7a4d}
.chip.tl-ok{background:rgba(56,189,248,.12);border-color:rgba(56,189,248,.4);color:#1b7568}
.chip.tl-wait{background:#d4dae2;border-color:#d5dae2;color:#6b6e68}
.chip.run-acc{background:rgba(34,197,94,.16);border-color:rgba(34,197,94,.5);color:#1e7a4d}
.chip.run-cru{background:rgba(56,189,248,.10);border-color:rgba(56,189,248,.35);color:#1b7568}
.chip.run-fade{background:rgba(234,88,12,.14);border-color:rgba(234,88,12,.45);color:#a6720f}
...
.rf-hi{background:rgba(239,68,68,.2);color:#b23a2e;border-color:rgba(239,68,68,.6)}
.rf-crit{background:#e6d1d1;color:#b23a2e;border-color:#a8352a;box-shadow:0 0 0 1px rgba(239,68,68,.4)}
.dov-flag-crit{background:rgba(127,29,29,.5);border:1px solid #a8352a;color:#b23a2e}
.rf-md{background:rgba(234,179,8,.16);color:#8c6210;border-color:rgba(234,179,8,.45)}
.dov-flag{border-radius:10px;padding:10px 12px;margin:2px 0 10px;font-weight:800;font-size:14px}
.dov-flag-hi{background:rgba(239,68,68,.14);border:1px solid rgba(239,68,68,.5);color:#b23a2e}
.dov-flag-md{background:rgba(234,179,8,.12);border:1px solid rgba(234,179,8,.45);color:#8c6210}
.dov-flag-n{font-weight:500;font-size:12px;color:#565852;margin-top:5px;line-height:1.5}
.dov-flag-s{font-weight:500;font-size:11px;color:#6e716b;margin-top:5px}
.patblk{padding:8px 0;border-top:1px solid #d5dae2}
.patblk:first-of-type{border-top:0}
.patname{font-size:13px;font-weight:700;color:#565852;margin-bottom:5px}
.swin{display:flex;flex-direction:column;gap:8px;margin:8px 0}
.swrow{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.swlab{flex:0 0 108px;font-size:12px;color:#6b6e68;font-weight:700}
.swin input,.swin select{flex:1;min-width:120px;background:#efece4;border:1px solid #d5dae2;color:#565852;border-radius:8px;padding:8px 10px;font-size:15px}
.swout{margin-top:6px}
.swkey{font-size:15px;font-weight:700;color:#565852;margin:6px 0 10px}
.swkey b{color:#2a9384;font-size:18px}
.swline{display:flex;justify-content:space-between;align-items:baseline;gap:8px;padding:4px 0;border-top:1px solid #d5dae2;font-size:13px}
.swline span:first-child{color:#6b6e68}
.swladder{display:flex;gap:6px;margin:8px 0 10px}
.swb{flex:1;border-radius:8px;padding:7px 4px;text-align:center;border:1px solid #d5dae2;background:#efece4}
.swbl{font-size:10px;color:#6b6e68;font-weight:700}
.swbp{font-size:14px;font-weight:800;color:#565852;margin:2px 0}
.swbc{font-size:11px;font-weight:700}
.swbs{font-size:10px;color:#6e716b;margin-top:2px}
.swb-stop{border-color:rgba(239,68,68,.5)}.swb-stop .swbc{color:#b23a2e}
.swb-ent{border-color:rgba(148,163,184,.5)}
.swb-t2{border-color:rgba(56,189,248,.45)}.swb-t2 .swbc{color:#2a9384}
.swb-t3{border-color:rgba(34,197,94,.5)}.swb-t3 .swbc{color:#1e7a4d}
.carryb{display:inline-block;font-size:9.5px;font-weight:800;border-radius:5px;padding:1px 6px;margin-left:5px;background:rgba(96,165,250,.16);color:#2a9384;border:1px solid rgba(96,165,250,.4);vertical-align:middle}
...
.hier-box{background:#d5d9e2;border:1px solid #d4dbe3;border-radius:14px;padding:18px;max-width:460px;width:100%;max-height:82vh;overflow:auto}
.hier-box h3{font-size:15px;margin-bottom:2px}
.hier-box .hb-sub{font-size:11px;color:#8b949e;margin-bottom:12px}
.hsub-row{border-bottom:1px solid #d6d2c7;padding:9px 0}
...
.hmem{background:#d3dae3;border:1px solid #d4dae2;border-radius:7px;padding:3px 8px;font-size:11px;cursor:pointer}
.hmem .rsb{margin-left:4px;color:#1b7568;font-weight:700}
.hback{background:#d2dae4;border:1px solid #d4dbe3;border-radius:7px;color:#1b7568;font-size:11px;padding:3px 10px;cursor:pointer;margin-bottom:10px}
.copybtn{background:#d2dae4;border:1px solid #d4dbe3;border-radius:7px;color:#1b7568;font-size:11px;font-weight:700;padding:3px 9px;cursor:pointer;vertical-align:middle;margin-left:8px}
.copybtn.done{background:#d1e6db;border-color:#238636;color:#3fb950}
...
.skip-note{font-size:11px;color:#8c6210;background:rgba(234,179,8,.08);border-radius:8px;padding:8px 10px;margin:8px 0}
.lagb{display:inline-block;font-size:9.5px;font-weight:800;border-radius:5px;padding:1px 6px;margin-left:4px;background:rgba(234,179,8,.16);color:#8c6210;border:1px solid rgba(234,179,8,.4)}
tr.row-excluded{opacity:.5}
tr.row-excluded td{text-decoration:line-through;text-decoration-color:rgba(139,148,158,.5)}
.cp.done{background:#d1e6d8;border-color:#1f9d4d;color:#2e8a5a}
.hdr{display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px}
.hdr h2{margin:0}
table{width:100%;border-collapse:collapse;font-size:12.5px;font-variant-numeric:tabular-nums}
th,td{text-align:right;padding:4px 5px;border-bottom:1px solid #d5dae2;white-space:nowrap}
td.l,th.l{white-space:normal}
th{color:#8595aa;font-weight:700;font-size:10.5px;letter-spacing:.05em}
td.l,th.l{text-align:left}
.tk{font-weight:700;color:#565852}
.pos{color:#1e7a4d}.neg{color:#b23a2e}.mut{color:#8595aa}
.bar{height:6px;border-radius:4px;background:#d4dae2;overflow:hidden;min-width:54px;display:inline-block;vertical-align:middle}
...
.a-ind{background:#1f6feb}.a-lev{background:#8957e5}.a-cash{background:#d8dbdf}
.note{font-size:11px;color:#7c7f78;line-height:1.5;margin-top:6px}
...
.kv{display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #d5dae2;font-size:12.5px}
.kv .k{color:#6b6e68}.kv .v{font-weight:700}
ul.rules{font-size:12.5px;line-height:1.7;padding-left:18px;color:#565852}
ul.rules li{margin-bottom:4px}
.rh{font-size:13px;font-weight:800;color:#1b7568;margin:14px 0 4px}
...
.nqt tr.hl td{font-weight:800;color:#565852}
.nqd{display:inline-block;width:10px;height:10px;border-radius:50%;margin-right:6px;vertical-align:middle}
.nqd.c-bl{background:#1b7568}.nqd.c-gr{background:#1e7a4d}
.nqd.c-yl{background:#8c6210}.nqd.c-rd{background:#a8352a}
.warn{background:#e6ddd1;border:1px solid #e6dfd1;border-radius:8px;padding:8px 10px;font-size:11.5px;color:#d6b56a;margin-top:8px}
/* ② distribution-day dates */
.dddays{font-size:10px;color:#7c7f78;margin-top:4px;line-height:1.5}
/* ⑤ index 200MA extension card */
.extbl{display:flex;flex-direction:column;gap:6px}
.exrow{display:flex;align-items:center;gap:10px;background:#efece4;border:1px solid #d6d2c7;border-radius:9px;padding:8px 10px}
.exk{flex:0 0 84px;font-size:13px;font-weight:700;color:#565852}
...
.ex-good{color:#1e7a4d}.ex-warn{color:#6b6e68}.ex-bad{color:#b23a2e}
/* ⑥ status breakdown panel */
.banner{cursor:pointer}
.cmt-sub{font-size:11px;color:#8b949e;margin:2px 0 10px;line-height:1.5}
.cmt-rot h2{font-size:15px}
.tap{font-size:10px;font-weight:700;color:#1b7568;opacity:.85;margin-left:6px}
...
.mbd-h{font-size:12px;font-weight:800;color:#565852;margin-bottom:6px}
.mgrp{font-size:10px;font-weight:700;color:#1b7568;letter-spacing:.04em;margin:8px 0 2px}
.mgrp:first-of-type{margin-top:0}
.mrow{display:flex;align-items:center;gap:8px;padding:3px 0}
.mk2{flex:0 0 116px;font-size:11px;color:#565852}
.mraw{flex:0 0 58px;font-size:11px;font-weight:700;text-align:right;color:#565852}
.mbar{flex:1;height:6px;border-radius:4px;background:#d4dae2;overflow:hidden}
.mbar i{display:block;height:100%;background:#2f81f7}
.mpts{flex:0 0 50px;font-size:10px;color:#6b6e68;text-align:right}
.mnote{font-size:10px;color:#7c7f78;text-align:center;margin-top:6px}
...
.bfl.on{background:#e5d2d2;color:#b23a2e;border:1px solid #e6d1d1}
.bfl.off{background:#d4d9e3;color:#5b6b80;border:1px solid #d6d2c7}
/* ⑧ 本日のピックアップ */
.hot-card{border-color:#2f81f7}
.bt{background:#d2d9e5;border:1px solid #d4d9e2;border-radius:10px;padding:7px 10px;margin-bottom:6px}
.bt-h{display:flex;align-items:baseline;gap:8px}
.bt-h .tk{font-size:15px;font-weight:800;color:#1b7568}
.bt-h .rs{margin-left:auto;font-size:12px;font-weight:700;color:#7c7f78}
.bt-tag{font-size:10px;font-weight:800;border-radius:6px;padding:1px 7px}
.st-break{color:#d1e6d8;background:#2e8a5a}
.st-base{color:#e6e1d1;background:#9a6b14}
.st-pull{color:#d1d8e6;background:#9cc2ff}
.st-high{color:#155e53;background:#d7d8e0}
.st-turn{color:#d8c9ff;background:#d8d3e4}
.st-fail{color:#b23a2e;background:#e6d1d4}
...
.scrtab th{font-size:10px;color:#7c7f78;font-weight:700;text-align:right;padding:3px 6px;border-bottom:1px solid #d6dae1}
.scrtab th.l{text-align:left}
.scr-row td{padding:5px 6px;border-bottom:1px solid #d5dae2;text-align:right}
.scr-tk{font-weight:800;color:#565852;text-align:left!important;white-space:nowrap}.scr-tk .bt-tag{margin-left:6px}.scr-rank{font-size:11px;font-weight:900;color:#17685c;text-align:center!important}
...
.scr-gn{font-size:11px;color:#6b6e68;margin-left:6px;font-weight:700}
.scr-gd{font-size:10px;color:#7c7f78;margin-left:6px}
.scr-ch td{font-size:10px;color:#7c7f78;font-weight:700;text-align:right;padding:1px 6px 5px;border-bottom:1px solid #d6dae1}
...
.ghb-base td:first-child{border-left:3px solid #9a6b14}
...
.posbar{display:inline-block;width:46px;height:7px;border-radius:4px;background:#d5dae2;vertical-align:middle;overflow:hidden;margin-right:6px}
.posfill{display:block;height:100%;background:linear-gradient(90deg,#5b7fb0,#9cc2ff);border-radius:4px}
.posn{font-size:11px;color:#6b6e68;vertical-align:middle;font-variant-numeric:tabular-nums}
.bt-held{font-size:9px;font-weight:700;color:#d6d9e1;background:#2e8a5a;border-radius:4px;padding:1px 5px;margin-left:5px}
.role-tag{font-size:10px;font-weight:700;border-radius:5px;padding:1px 7px;vertical-align:middle;margin-left:6px}
.rt-view{color:#9cc2ff;background:#d2d8e5}
.rt-act{color:#2e8a5a;background:#d1e6dc}
.bt-good{background:#d1e6d8;color:#2e8a5a}
.bt-warn{background:#e6e1d1;color:#856110}
.bt-bad{background:#e6dad1;color:#a6720f}
...
.bt-g .k{display:block;font-size:9px;color:#7c7f78}
.bt-g .v{font-size:12px;font-weight:700}
.bt-g .v.bt-good{background:none;color:#1e7a4d}
.bt-g .v.bt-warn{background:none;color:#6b6e68}
.bt-g .v.bt-bad{background:none;color:#b23a2e}
...
.dov{width:100%;max-width:680px;max-height:88vh;overflow-y:auto;background:#f6f4ee;border:1px solid #d6d2c7;border-bottom:none;border-radius:16px 16px 0 0;padding:16px 16px 28px;box-shadow:0 -8px 30px rgba(0,0,0,.5)}
.dov-h{display:flex;align-items:center;justify-content:space-between;margin-bottom:2px}
.dov-tk{font-size:20px;font-weight:800;color:#1b7568}
.dov-x{font-size:18px;color:#7c7f78;cursor:pointer;padding:4px 10px}
.dov-nm{font-size:12px;color:#7c7f78;margin-bottom:10px}
.dov-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}
.dov-status{margin:2px 0 12px}
.msec{margin:22px 0 8px;padding-top:14px;border-top:2px solid #d5dae2;scroll-margin-top:56px}
section>.msec:first-child,section>.card:first-child{margin-top:6px}
section>.msec:first-child{border-top:none;padding-top:4px}
.msec-l{font-size:16px;font-weight:800;color:#565852;letter-spacing:.02em}
.msec-q{font-size:12px;color:#a8b6c8;margin-top:2px;line-height:1.4}
.msec-g{font-size:11px;color:#93a3b8;line-height:1.6;margin-top:7px;padding:9px 11px;background:#efece4;border:1px solid #d5dae2;border-left:3px solid #c9c4b8;border-radius:8px}
...
.ci-pre{font-size:18px;color:#6e716b;font-weight:700}
.ci-row input,.ci-row2 input{flex:1;min-width:0;background:#efece4;border:1px solid #d5dae1;border-radius:9px;color:#565852;font-size:17px;padding:10px 12px;-webkit-appearance:none}
...
.ci-fxl{font-size:12px;color:#6e716b;font-weight:700}
.calc-sum{margin:8px 0}
.cs-row{display:flex;justify-content:space-between;align-items:center;padding:6px 2px;border-bottom:1px solid #d5dae2;font-size:13px;color:#5a5c56}
.cs-row b{color:#565852;font-size:14px}
.cs-cash b{color:#2e8a5a}
.lev-box{background:#efece4;border:1px solid #d6d2c7;border-radius:10px;padding:10px 12px;margin:8px 0}
.lev-h{font-size:12px;color:#6b6e68;margin-bottom:6px;font-weight:700}
...
.lev-r b{flex:0 0 auto;color:#565852}
.lev-r .mut{flex:1;text-align:right}
.soxl-tr{display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:6px 0 2px;font-size:12px;border-top:1px dashed #d6d2c7;margin-top:4px}
...
.calc-tab th{font-size:10px;color:#7f8da3;font-weight:700;text-align:right;padding:4px 6px;border-bottom:1px solid #d5dae1}
...
.sn-gauge{position:relative;height:10px;border-radius:6px;margin:6px 0 2px;
  background:linear-gradient(90deg,#1e7a4d 0 15%,#38bdf8 15% 35%,#64748b 35% 65%,#f97316 65% 85%,#a8352a 85% 100%);opacity:.9}
.sn-gauge i{position:absolute;top:-3px;width:4px;height:16px;background:#fff;border-radius:2px;box-shadow:0 0 6px #000}
.sn-gl{display:flex;justify-content:space-between;font-size:10px;color:#7c7f78;margin-bottom:8px}
.sn-ref{font-size:9px;font-weight:800;color:#7c7f78;background:#d4d9e2;border-radius:5px;padding:1px 5px;margin-left:5px}
.sn-note{font-size:10px;color:#5b6b80;margin:0 0 4px 0;padding-left:2px}
/* 要因分解 */
.attrib{margin-top:9px;background:#efece4;border:1px solid #d4d9e3;border-radius:10px;padding:9px 11px}
.attrib-h{font-size:11.5px;font-weight:800;color:#565852;margin-bottom:6px}
...
.atk{color:#6b6e68;font-size:11px}
...
.eqkv{font-size:12px;color:#6b6e68}.eqkv b{font-size:14px}
.ribwrap{margin:10px 0 12px}
.riblab{font-size:10px;color:#7c7f78;margin-bottom:4px;font-weight:700;letter-spacing:.03em}
.ribbon{display:flex;gap:1.5px}
.ribbon .rb{flex:1;height:16px;border-radius:2.5px;min-width:2px}
.rb.c-bl{background:#1b7568}.rb.c-gr{background:#1e7a4d}.rb.c-yl{background:#8c6210}.rb.c-rd{background:#a8352a}
.led{display:inline-flex;gap:2px;margin-left:8px;vertical-align:middle}
.led i{width:9px;height:12px;border-radius:2px;background:#d6d2c7;display:inline-block}
.led i.on{background:linear-gradient(180deg,#b23a2e,#a8352a);box-shadow:0 0 5px #a8352a88}
.ch-card{border-color:#d4dbe3}
ul.chlog{font-size:13px;line-height:1.8;padding-left:18px;color:#565852;margin:4px 0}
...
.hm-n{font-size:11px;font-weight:800;color:#eceae2;color:#fff;text-shadow:0 1px 2px rgba(0,0,0,.55);line-height:1.25}
...
.erb{font-size:9px;font-weight:800;color:#d6d9e1;background:#a6720f;border-radius:4px;padding:1px 6px;margin-left:5px;vertical-align:middle}
.rsd{font-size:10px;font-weight:800;margin-left:3px}
.rsd.up{color:#1e7a4d}.rsd.dn{color:#b23a2e}.rsd.fl{color:#4b5866}
.tday{font-size:9px;color:#7c7f78;font-weight:400}
.risknote{border-top:1px dashed #d6d2c7;padding-top:6px}
...
.dov-stage.stg1{background:linear-gradient(135deg,#d6dae1,#4b5563)}
.dov-stage.stg2{background:linear-gradient(135deg,#d1d8e6,#d1e6e3)}
.dov-stage.stg3{background:linear-gradient(135deg,#e6dbd1,#ca8a04)}
.dov-stage.stg4{background:linear-gradient(135deg,#e6d1d1,#a8352a)}

/* 要因分解 */
.thgrid{display:grid;grid-template-columns:repeat(2,1fr);gap:7px}
.thtile{text-align:left;border:1px solid #d4d9e3;border-radius:11px;padding:9px 11px;cursor:pointer;
        display:flex;flex-direction:column;gap:3px;color:#565852;transition:border-color .15s}
.thtile.on{border-color:#565852;box-shadow:0 0 0 1px #565852 inset}
.tht-n{font-size:13.5px;font-weight:800}
.tht-m{font-size:11px;color:#565852;font-variant-numeric:tabular-nums;display:flex;align-items:center;gap:6px}
.tht-m b{font-size:13px;color:#fff}
.tht-c{margin-left:auto;color:#6b6e68;font-weight:600}
.thpanel{margin-top:9px;background:#efece4;border:1px solid #d4d9e3;border-radius:11px;padding:10px 11px}
...
.thstk{display:inline-flex;align-items:center;gap:5px;background:#d3d9e4;border:1px solid #d4dae2;
       border-radius:8px;padding:5px 9px;font-size:12px;cursor:pointer;font-variant-numeric:tabular-nums}
.thstk:active{background:#d3dae3}
.thstk b{color:#565852;font-weight:800}
.thstk-rs{color:#6b6e68;font-size:10.5px;font-weight:700}
.thstk-m{font-size:10px}
.thmore{align-self:center;color:#7c7f78;font-size:11px;font-weight:700;padding:0 4px}
.rrgtog{display:flex;gap:4px}
.rtg{background:#d5d9e2;color:#6b6e68;border:1px solid #d5dae1;border-radius:14px;padding:4px 11px;font-size:11px;font-weight:700;cursor:pointer}
.rtg.on{background:#1f6feb;color:#fff;border-color:#1f6feb}
.rrgseed{font-size:12.5px;color:#bfe3ff;background:#d1dae6;border:1px solid #d2dce5;border-radius:9px;padding:7px 10px;margin:8px 0 4px}
...
.shl{font-size:10px;font-weight:900;color:#1e7a4d}
.shl-n{color:#b23a2e;margin-left:6px}
.chip.shp{border-color:#d1e6d9;color:#a7f3d0}
.chip.shn{border-color:#e6d1d1;color:#b23a2e}
.le-row{display:flex;flex-wrap:wrap;align-items:center;gap:8px;margin:6px 0 2px}
.le-k{font-size:11px;font-weight:900;color:#6b6e68;min-width:76px;letter-spacing:.03em}
.le-v{font-size:15px;font-weight:800;color:#565852}
.le-pct{font-size:11px;color:#6b6e68;font-weight:600;margin-left:3px}
.le-rot{font-size:14px;font-weight:900}
.le-sub{font-size:11px;color:#7c7f78}
.le-size{font-size:12px;font-weight:800;margin-left:auto;padding:2px 9px;border-radius:7px;background:#d4d9e2}
.le-size.pos{color:#2e8a5a}.le-size.neg{color:#b23a2e}.le-size.mut{color:#565852}
.le-vg{margin:6px 0 2px}


.calc-tab th.l{text-align:left}
.calc-tab td{text-align:right;padding:7px 6px;border-bottom:1px solid #d5dae2;font-size:13px;vertical-align:top}
.calc-tab td.l{text-align:left}
.entIn{width:74px;background:#efece4;border:1px solid #d5dae1;border-radius:7px;color:#565852;font-size:13px;padding:5px 7px;text-align:right;-webkit-appearance:none}
...
.memx{background:#efece4;border:1px solid #d5dae1;color:#cdd9ea;border-radius:8px;font-size:12px;padding:6px 10px}
.memx:active{background:#d2d9e5}
.tkbtn{display:block;margin-top:5px;width:74px;background:#efece4;border:1px solid #e6ddd1;color:#d9b06a;border-radius:7px;font-size:10px;padding:4px 0;text-align:center}
.tkbtn.on{background:#e6ddd1;border-color:#ca8a04;color:#9a6b14;font-weight:700}
.tkbtn:active{opacity:.7}
.redep{background:#efece4;border:1px solid #d4d9e3;border-radius:10px;padding:10px 12px;margin:8px 0}
.todayact{border:1px solid #d5dae1;border-left:4px solid #6b7280;border-radius:11px;background:#d4dae2;padding:9px 12px;margin:0 0 10px}
.todayact.ta-blue{border-left-color:#4d9fff}.todayact.ta-green{border-left-color:#34d39c}
.todayact.ta-yellow{border-left-color:#a6720f}.todayact.ta-red{border-left-color:#b23a2e}.todayact.ta-gray{border-left-color:#6b7280}
...
.todayact.ta-yellow .ta-col{color:#a6720f}.todayact.ta-red .ta-col{color:#b23a2e}.todayact.ta-gray .ta-col{color:#9aa4b2}
.ta-expo{font-size:12.5px;color:#565852}
.ta-est{font-size:10px;color:#a6720f;border:1px solid #5b4a1f;border-radius:5px;padding:1px 6px}
.ta-act{font-size:12.5px;font-weight:600;line-height:1.4;padding:6px 9px;border-radius:7px;background:#efece4}
.ta-act.ta-alert{color:#b23a2e;background:#e2d4d6;border:1px solid #e6d1d1}
.ta-act.ta-change-up{color:#bfdbfe;background:#d1dae5}
.ta-act.ta-change-dn{color:#856110;background:#e6dfd1}
.ta-act.ta-normal{color:#9aa4b2;background:transparent;padding-left:0}
.ta-rebal{margin-top:6px;font-size:12px;color:#155e53;background:#d4d7e2;border-radius:7px;padding:6px 9px}
.ta-go{font-size:11.5px;font-weight:700;color:#a5c8ff;cursor:pointer;margin-left:auto;white-space:nowrap}
.ta-rec{margin-top:7px;display:flex;flex-wrap:wrap;gap:5px;align-items:center;font-size:11.5px;color:#6b6e68}
.ta-reclab{color:#7c7f78}
.nqb{font-size:11.5px;font-weight:800;border:1px solid #d5dae1;background:#d5d9e2;color:#565852;border-radius:6px;padding:2px 9px;cursor:pointer}
...
.nqb.b-clear{color:#7c7f78;font-weight:700}
.ta-manual{font-size:10px;font-weight:800;background:#dbe7ff;color:#d1dae6;border-radius:5px;padding:1px 7px;margin-left:6px}
.ta-recst{color:#7c7f78;font-size:11px}
...
.cmt-pos{color:#9be8b8;background:#d3e4dc;border:1px solid #d3e4db}
.cmt-neg{color:#b23a2e;background:#e2d4d6;border:1px solid #e6d1d1}
.secrs th.sortable{cursor:pointer}
.rottbl th.rsort{cursor:pointer;user-select:none}
.rottbl th.rsort:active{opacity:.55}
.rottbl th.rsort.act{color:#565852;text-decoration:underline}
...
.secrs th.sortable.act{color:#565852;text-decoration:underline}
.secrs th .so{font-size:9px;color:#7f8da3}
.rc-h{font-size:12px;color:#5a5c56;margin-bottom:8px;font-weight:700}
.rc-sec{margin:8px 0}
.rc-l{font-size:11px;font-weight:700;margin-bottom:4px}
.rc-sl{color:#b23a2e}.rc-ad{color:#2e8a5a}
.rc-row{display:flex;justify-content:space-between;align-items:center;padding:6px 9px;border-radius:7px;background:#efece4;border:1px solid #d5dae2;margin-bottom:4px;font-size:13px}
.rc-row b{color:#565852}
.rc-okall{font-size:12.5px;color:#2e8a5a;background:#d3e4dc;border-radius:8px;padding:9px 11px}
.dov-meta{display:flex;flex-wrap:wrap;align-items:center;gap:8px;margin:0 0 8px}
.dov-sec{font-size:12.5px;font-weight:700;color:#1b7568}
.dov-cap{font-size:11px;font-weight:800;color:#565852;background:#d5dae1;border:1px solid #c9c4b8;border-radius:6px;padding:1px 7px}
.dov-loc{display:flex;flex-wrap:wrap;align-items:center;gap:5px;margin:0 0 12px}
.dov-loc-l{font-size:10px;font-weight:800;color:#7c7f78;margin-right:2px}
.dov-loc-b{font-size:11px;font-weight:700;border-radius:6px;padding:2px 8px}
.loc-p{background:#d1e6d8;border:1px solid #1f9d4d;color:#2e8a5a}
.loc-d{background:#d3e4de;border:1px solid #1f7a55;color:#7fe0c0}
.loc-k{background:#e6dad1;border:1px solid #c2660b;color:#a6720f}
.loc-h{background:#d1dce6;border:1px solid #2f81f7;color:#1b7568}
.loc-w{background:#d5dae1;border:1px solid #c9c4b8;color:#565852}
.dov-st{display:flex;align-items:center;gap:8px;flex-wrap:wrap;border-radius:10px;padding:9px 12px;border:1px solid #d6d2c7;background:#efece4}
.dov-st.lead{border-left:4px solid #1f6feb}
.dov-st.nolead{border-left:4px solid #d8dbdf;opacity:.85}
.dov-st-pos{font-size:14px;font-weight:800;color:#565852}
.dov-st-nu{font-size:11px;font-weight:800;border-radius:6px;padding:2px 8px;margin-left:auto}
.dov-st-nu.pos{background:#d3e4d9;color:#2e8a5a;border:1px solid #1d7840}
.dov-st-nu.neg{background:#e5d2d2;color:#b23a2e;border:1px solid #e6d1d1}
.dov-st-nu.mut{background:#d4d9e3;color:#6b6e68;border:1px solid #d6d2c7}
.dov-st-note{font-size:11px;color:#6b6e68;margin-top:6px;line-height:1.45}
...
.dov-st.st-⑤{border-left-color:#a8352a}
.dov-c{background:#efece4;border:1px solid #d6d2c7;border-radius:9px;padding:8px 10px}
.dov-k{font-size:11px;color:#7c7f78}
.dov-v{font-size:16px;font-weight:800;margin-top:2px}
.dov-note{font-size:10px;color:#7c7f78;text-align:center;margin-top:12px}
/* 銘柄検索 */
.tksearch{width:100%;background:#efece4;border:1px solid #d5dae2;border-radius:9px;color:#565852;font-size:15px;padding:10px 12px;margin-top:4px}
.tksearch:focus{outline:none;border-color:#2f81f7}
.tkresults{margin-top:8px;display:flex;flex-direction:column;gap:5px}
.tkr{display:flex;align-items:center;gap:8px;background:#efece4;border:1px solid #d6d2c7;border-radius:8px;padding:8px 10px;cursor:pointer}
.tkr-s{font-size:14px;font-weight:800;color:#1b7568;flex:0 0 62px}
.tkr-n{font-size:11px;color:#6b6e68;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.tkr-rs{font-size:11px;color:#7c7f78;flex:0 0 auto}
.tkno{color:#7c7f78;font-size:12px;padding:4px}
.secrow{cursor:pointer}
.secx{font-size:9px;color:#7c7f78}
.secsub{display:none}
...
.rowsec{font-size:9px;font-weight:400;color:#7c7f78;margin-top:1px;line-height:1.15}
.dov-since{font-size:11.5px;color:#6b6e68;margin-top:5px}
.dov-since b{color:#565852}
tr[data-tkone]{cursor:pointer}
tr[data-tkone]:active{background:#d2d9e5}
.bt:active,.buy:active,.tkr:active{background:#d1dae5}
.capb{font-size:9px;font-weight:800;border-radius:5px;padding:1px 4px;white-space:nowrap}
.cap-micro{background:#e5d2d2;color:#b23a2e;border:1px solid #e6d1d1}
.cap-small{background:#e5ddd2;color:#9a6b14;border:1px solid #78531d}
.cap-mid{background:#d2d9e5;color:#3e9a8c;border:1px solid #d1dce6}
.cap-large{background:#d3e4d9;color:#2e8a5a;border:1px solid #1d7840}
.cap-mega{background:#d1e5e5;color:#5eead4;border:1px solid #156464}
.cap-none{background:#d4d9e3;color:#5b6b80;border:1px solid #d6d2c7}
.stb{font-size:9px;font-weight:800;border-radius:5px;padding:1px 5px;white-space:nowrap;border:1px solid #d6d2c7;background:#efece4}
.stb b{font-weight:800}
.stb-③{color:#2e8a5a;border-color:#1d7840}.stb-②{color:#3e9a8c;border-color:#d1dce6}
.stb-①{color:#9a6b14;border-color:#78531d}.stb-④{color:#a6720f;border-color:#9a4a1d}
.stb-⑤{color:#b23a2e;border-color:#e6d1d1}.stb-none{color:#7c7f78}
...
.rk-note{background:#d1d9e6;border:1px solid #d1dbe6;border-radius:9px;padding:8px 10px;color:#6b6e68}
...
.liqlab{font-size:11px;font-weight:800;color:#6b6e68;display:flex;flex-direction:column;line-height:1.1;margin-right:3px}
.liqsub{font-size:8px;font-weight:600;color:#5b6b80}
.lqf-b{font-size:12px;font-weight:700;padding:6px 10px;border-radius:8px;background:#d4d9e3;color:#6b6e68;border:1px solid #d6d2c7;cursor:pointer}
.lqf-b.active{background:#d1e6e3;color:#fff;border-color:#d1e6e3}
.lqf-rec{border-color:#1d7840}
.lqf-tag{font-size:8px;font-weight:800;margin-left:3px;color:#2e8a5a;vertical-align:top}
.lqf-rec.active .lqf-tag{color:#1e7a4d}
.sectog{display:flex;gap:5px;align-items:center;flex-wrap:wrap;margin:2px 0 8px}
.stg-g,.stg-t{font-size:12px;font-weight:700;padding:4px 10px;border-radius:7px;background:#d4d9e3;color:#6b6e68;border:1px solid #d6d2c7;cursor:pointer}
.stg-g.active{background:#d1e6e3;color:#fff;border-color:#d1e6e3}
.stg-t.active{background:#d6dae0;color:#fff;border-color:#475569}
.stg-sep{color:#d8dbdf;font-weight:400;margin:0 2px}
.secsub td{background:#efece4;color:#6b6e68;font-size:10px;font-weight:800;padding:3px 4px}
...
.subgrp-n{font-size:9px;font-weight:700;color:#5b6b80;background:#d4d9e3;border:1px solid #d6d2c7;border-radius:4px;padding:0 4px}

/* ランク番号＋急落局面フラグ（配分calc） */
.rkb{display:inline-block;min-width:22px;text-align:center;font-size:10.5px;font-weight:800;color:#eceae2;background:#93b7df;border-radius:5px;padding:1px 4px;margin-right:2px;font-variant-numeric:tabular-nums}
...
.kf-half{background:rgba(234,179,8,.18);color:#8c6210;border:1px solid rgba(234,179,8,.4)}
.kf-skip{background:rgba(239,68,68,.16);color:#b23a2e;border:1px solid rgba(239,68,68,.4)}
.kf-skiptxt{color:#b23a2e;font-weight:800;font-size:12px}
.kf-ok{background:rgba(34,197,94,.16);color:#1e7a4d;border:1px solid rgba(34,197,94,.4)}
.kf-oktxt{color:#1e7a4d;font-weight:800;font-size:12px}
.hf{color:#8c6210;font-weight:800;font-size:11px;margin-left:3px}
.row-skip{opacity:.72}
...
.lt-big{font-size:32px;font-weight:800;color:#8c6210;font-variant-numeric:tabular-nums;line-height:1}
.lt-big span{font-size:13px;font-weight:700;color:#8494ab;margin-left:3px}
.lt-lv{font-size:13px;font-weight:700;color:#565852}
.lt-hist{display:flex;justify-content:space-between;gap:4px;margin-top:9px}
.lt-h{flex:1;text-align:center;background:#efece4;border:1px solid #d6d2c7;border-radius:8px;padding:5px 2px}
...
.beta-read{font-size:13px;font-weight:700;color:#565852}
...
.bt-k{color:#6b6e68;min-width:120px}
...
/* emergency brake */
.emergency{border-width:1px}.emergency.em-on{border-color:#e6d1d1;background:linear-gradient(135deg,#e3d4d7,#dcd7df)}
.emergency.em-off{border-color:#d4e2d9}.emergency.em-na{border-color:#d8dadf}
.em-mult{font-size:12px;font-weight:900;border-radius:7px;padding:4px 8px;background:#d4d8e3;color:#e5e7eb}
.em-on .em-mult{background:#e6d1d1;color:#b23a2e}.em-off .em-mult{background:#d1e6d9;color:#1e7a4d}
.em-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:10px}
.em-grid>div{background:#efece4;border:1px solid #d6d2c7;border-radius:9px;padding:9px 10px}
.em-grid span{display:block;color:#6b6e68;font-size:10.5px}.em-grid b{display:block;font-size:20px;margin:2px 0}.em-grid small{display:block;color:#66758a;font-size:9px;line-height:1.35}
.em-foot{display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap;margin-top:8px;color:#6b6e68;font-size:10px}.em-foot span:last-child{color:#65758a}
.em-note{margin-top:6px;padding:6px 8px;border-radius:7px;background:#e3d4d7;color:#b23a2e;font-size:10.5px;font-weight:700}
/* RS persistence */
.rs-cont{margin-top:10px}.rsc-groups{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin-top:10px}
.rscg{background:#efece4;border:1px solid #d6d2c7;border-radius:8px;padding:7px 8px;min-width:0}.rscg>b{font-size:11px}.rscg>span{margin-left:6px;font-weight:800;color:#1b7568}.rscg .chips{margin-top:4px}.rscg .chip{font-size:9px;padding:1px 5px;margin:1px}
.rsc-list{margin-top:10px}.rsc-row{border-top:1px solid #d5d9e2;padding:8px 0}.rsc-row:first-child{border-top:0}
.rsc-head{display:flex;align-items:center;gap:6px;min-width:0}.rsc-rk{font-size:10px;color:#1b7568;width:27px}.rsc-head>b{font-size:13px}.rsc-rs{margin-left:auto;font-size:10px;color:#6b6e68}
.rsc-tag{font-size:8.5px;font-weight:800;border-radius:5px;padding:1px 5px}.rsc-tag.stable{background:#d2e4d8;color:#2e8a5a}.rsc-tag.surge{background:#d1dce6;color:#3e9a8c}.rsc-tag.return{background:#d8d4e3;color:#5b3fa6}.rsc-tag.fade{background:#e4d3d3;color:#b23a2e}.rsc-tag.spike{background:#e6dcd1;color:#a6720f}.rsc-tag.hold{background:#d5dae1;color:#6b6e68}
.rsc-metrics{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:3px 8px;padding-left:33px;margin-top:4px;font-size:9.5px;color:#7c7f78}.rsc-metrics b{color:#565852}.rsc-slopes{padding-left:33px;margin-top:3px;font-size:9px;color:#65758a}.rsc-slopes b{color:#6b6e68}
.rsx-overlap{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:10px}
.rsx-stat{background:#efece4;border:1px solid #d6d2c7;border-radius:9px;padding:8px 10px;min-width:0}
.rsx-stat-h{display:flex;align-items:center;gap:5px}
.rsx-stat-h>span{font-size:10.5px;color:#6b6e68}.rsx-stat-h>b{font-size:19px}.rsx-stat-h .cp{margin-left:auto;flex:0 0 auto}
.rsx-stat .chips{margin-top:5px}.rsx-stat .chip{font-size:10px;padding:2px 6px;margin:1px}
.swdiv{height:1px;background:#d6d2c7;margin:14px 0 10px}
.swhold-h{font-size:13px;font-weight:800;color:#565852;margin-bottom:8px}
.rsx-signals{display:grid;grid-template-columns:1fr;gap:7px;margin-top:9px}
.rsx-sig{background:#d4dae3;border:1px solid #d5dae1;border-radius:9px;padding:8px 10px}
.rsx-sig>b{font-size:12px;color:#565852;margin-right:7px}.rsx-sig>span{font-size:10.5px;color:#7c7f78}
...
.rsx-card{margin-bottom:0}.rsx-item{border-top:1px solid #d5d9e2;padding:7px 0}
.rsx-item:first-of-type{border-top:0}.rsx-row{display:flex;align-items:center;gap:7px}
.rsx-rk{width:22px;height:22px;border-radius:6px;background:#d4d9e3;color:#1b7568;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;flex:0 0 auto}
...
.rsx-badge.sel{background:#d3e4d9;color:#2e8a5a;border:1px solid #287a45}.rsx-badge.ok{background:#d1dce6;color:#3e9a8c;border:1px solid #245b85}
.rsx-badge.ex{background:#e4d3d3;color:#b23a2e;border:1px solid #e6d1d1}.rsx-badge.watch{background:#d5dae1;color:#6b6e68;border:1px solid #c9c4b8}
.rsx-new{background:#e6dcd1;color:#a6720f;border:1px solid #8a520d;border-radius:4px;padding:1px 4px;font-size:8.5px;font-weight:800}
...
.rsx-sub b{color:#565852}.rsx-sub span{color:#66758a}
.rsx-history .sub b{color:#565852}.rsx-hgrid{display:grid;grid-template-columns:1fr;gap:9px;margin-top:10px}
.rsx-hblock{background:#efece4;border:1px solid #d6d2c7;border-radius:10px;overflow:hidden}
.rsx-hhead{display:flex;align-items:center;gap:7px;padding:8px 10px;background:#d4dae3;border-bottom:1px solid #d6d2c7}
.rsx-hhead>b{font-size:13px;color:#565852}.rsx-hhead>span{font-size:9.5px;color:#718197;flex:1}.rsx-hhead .cp{margin-left:auto}
.rsx-hrow{display:grid;grid-template-columns:74px minmax(0,1fr);gap:8px;padding:8px 10px;border-top:1px solid #d5d9e2}
...
.rsx-hwhen em{display:block;font-size:8.5px;color:#6b6e68;font-style:normal;margin-top:4px}.rsx-hflow{min-width:0;display:grid;gap:5px}
...
.rsx-hflow .hin{background:#d2e4d8;color:#2e8a5a;border:1px solid #287a45}.rsx-hflow .hout{background:#e4d3d3;color:#b23a2e;border:1px solid #e6d1d1}
.rsx-hflow .chips{margin:0;line-height:1.3}.rsx-hflow .chip{font-size:9px;padding:1px 5px;margin:0 2px 2px 0}
.rsx-hflow .chip.in{border-color:#287a45;color:#2e8a5a;background:#d2e4d8}.rsx-hflow .chip.out{border-color:#e6d1d1;color:#b23a2e;background:#e3d3d3}
...
function showThemeHier(el){
  ...
  var QC={'主導':'#1e7a4d','改善':'#2a9384','弱化':'#a6720f','停滞':'#b23a2e'};
...
function showDet(tk){
  ...
  if(sr){ meta+='<span class="dov-rot" style="border-color:'+(sr.c||'#6b6e68')+';color:'+(sr.c||'#6b6e68')+'">ローテ: '+sr.rot+'（#'+sr.rk+'/'+sr.n+'・RS'+Math.round(sr.med)+'・週'+((sr.d1w>=0?'+':'')+sr.d1w.toFixed(1))+'）</span>'; }
...
function calcRun(){
  ...
  if(has){
    ...
    if((C.emergency_mult||1)<1){
      s+='<div class="cs-row" style="color:#b23a2e"><span>非常口 発動中</span><b>レバ枠を自動で50%へ縮小</b></div>';
    }
    if(freed>0){
    ...
    sum.innerHTML=s;
    if(C.color==='Yellow'){
      sum.innerHTML+='<div class="cs-row cs-cash" style="color:#8c6210"><span>⚠ 黄＝新規停止</span><b>下の株数は既存保有の管理用。新規に買い増さない</b></div>';
    } else if(C.color==='Red'){
      sum.innerHTML+='<div class="cs-row cs-cash" style="color:#b23a2e"><span>⚠ 赤＝撤退</span><b>個別0%・全て現金</b></div>';
    }
...
def _svg_inner(ys, accent, gid, gridvals, suffix, lo_pad=4, hi_pad=4, ymin=0, ymax=100, zero_ref=False):
    ...
    gl = "".join(f'<line x1="{pad}" y1="{Y(g):.1f}" x2="{Wd-pad}" y2="{Y(g):.1f}" '
                 f'stroke="#d6d2c7" stroke-width="1"/>'
                 f'<text x="{Wd-pad}" y="{Y(g)-2:.1f}" fill="#6e716b" font-size="20" font-weight="600" '
...
def _svg_breadth(ts, n_uni=None):
    ...
    return (f'<div class="card"><h2>ブレッドス推移（200日線上の割合）</h2>'
            ...
            f'<div class="chart">{svg}'
            f'<div class="cap cap-c"><span style="color:#1b7568;font-weight:700">現在 {last:.0f}%</span></div>'
            f'{_date_axis(ts)}</div></div>')
...
def _dollarvol_card(dv):
    ...
    legend = "".join(
        ...
        f'<span style="color:#155e53;font-size:11px">{s["name"]}</span></span>' for s in dv["series"])
    uni = next((s for s in dv["series"] if s["name"] == "ユニバース合算"), dv["series"][0])
    ut = dv.get("uni_trend") or {}
    tcol = "#2e8a5a" if ut.get("dir") == "拡大" else "#b23a2e" if ut.get("dir") == "縮小" else "#6b6e68"
    ...
    return (f'<div class="card"><h2>売買代金 参加度（200日平均比）</h2>'
            ...
            f'<div class="chart">{svg}'
            f'<div class="cap cap-c"><span style="color:#2a9384;font-weight:700">'
...
def _trend_state(ys, up_label, dn_label, win=50, slope_lb=10):
    ...
    if above and slope >= 0:
        return (up_label, "#2e8a5a")
    if (not above) and slope < 0:
        return (dn_label, "#b23a2e")
    return ("中立", "#9aa4b2")
...
def _svg_vixterm_card(ts):
    ...
    svg = _svg_inner(ys, "#5b3fa6", "vt", [0.90, 0.95, 1.00, 1.05], "",
                     lo_pad=0.03, hi_pad=max(0.03, 1.02 - dhi), ymin=0, ymax=100)
    if last < 0.95:
        state, col = "コンタンゴ（平穏）", "#2e8a5a"
    elif last <= 1.00:
        state, col = "フラット（要観察）", "#9a6b14"
    else:
        state, col = "バックワーデーション（ストレス）", "#b23a2e"
...
def _svg_mri(ts):
    ...
    def X(i): return pad + i * (Wd - 2*pad) / (n - 1)
    def Y(v): return pad + (1 - (v - lo)/rng) * (Ht - 2*pad)
    zones = [(0, 25, "#a8352a"), (25, 40, "#f97316"), (40, 55, "#64748b"),
             (55, 70, "#1e7a4d"), (70, 100, "#16a34a")]
    zr = ""
    ...
    return (f'<div class="card"><h2>マーケットステータス推移</h2>'
            ...
            f'<div class="chart">{svg}'
            f'<div class="cap cap-c"><span style="color:#2e8a5a;font-weight:700">現在 {last:.0f}（{band_lab}）</span></div>'
            f'{_date_axis(ts)}</div></div>')
...
def _rotation_card(sectors, m):
    ...
    scol = {"初動": "#1e7a4d", "改善": "#2a9384", "強い継続": "#a78bfa", "監視": "#6b6e68", "弱い": "#6b7280"}
    def arr(v):
    ...
    rows = ""
    for s in ranked[:12]:
        rot = s.get("rot", "弱い"); col = scol.get(rot, "#6b6e68")
        leads = []
        ...
        if C is not None and len(C) >= 6:
            ...
            def _line(k, v, n):
                ...
                return (f'<div style="display:flex;justify-content:space-between;padding:4px 2px;'
                        f'border-bottom:1px solid rgba(255,255,255,.05)"><span style="font-size:12px;color:#565852">{k}'
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    _rotcol = {"初動": "#1e7a4d", "改善": "#2a9384", "強い継続": "#a78bfa", "監視": "#6b6e68", "弱い": "#6b7280"}
    srows = []
    for i, s in enumerate(sectors, 1):
        chips = "".join(_chip(t, rs) for t, rs in s.get("members", []))
        _rot = s.get("rot", "弱い"); _rc = _rotcol.get(_rot, "#6b6e68")
    ...
    market = (
        ...
        + _svg_ratio_card(mkt.get("credit_ts", []),
                          ...
                          "リスクオン", "信用悪化・警戒", "#a6720f", "cg", zero_ref=True, unit="σ")
    ...
    if cur_jp:
        ...
        cur_line = (f'<div class="sub" style="margin-top:8px">現在の地合い：<b style="color:#565852">{cur_jp}</b>'
                    f' → 個別 {em[0]} ／ レバ {em[1]}</div>')
    rules = (
        f'<div class="card"><h2>システムルール（v2・確定）</h2>'
        f'<div class="sub" style="color:#155e53">相場の方向でリスク量を4段階に調整し、その中で最も強い{N_PORT}銘柄を等分で持つ。負けは早く小さく、勝ちは伸ばす。ルールは事前に固定し、その日の気分で動かさない。</div>'
    ...
    body = (build_today_action(sar, mkt, asof)
            ...
            '<button onclick="tab(\'t-rules\',this)">Rules</button><button onclick="tab(\'t-post1\',this)">Overview 1</button><button onclick="tab(\'t-post2\',this)">Overview 2</button></nav>'
            ...
            f'<section id="t-post1">' + _mkt_section("投稿カード：概略", "地合いの全体感・スクショして投稿", en="Overview 1") + _post_iframe(POST1_B64) + '</section>'
            f'<section id="t-post2">' + _mkt_section("投稿カード：詳細", "セクター・ローテーション・スクショして投稿", en="Overview 2") + _post_iframe(POST2_B64) + '</section>')
    ...
    if asof:
        ...
        asof_disp = ""
    _thex = {"Blue": "#d1e6e3", "Green": "#16a34a", "Yellow": "#ca8a04", "Red": "#a8352a"}.get(sar[0], "#eceae2")
    html = ("<!doctype html><html lang='ja'><head><meta charset='utf-8'>"
            "<meta name='viewport' content='width=device-width,initial-scale=1'>"
            "<meta name='theme-color' content='" + _thex + "'>"
            "<meta name='apple-mobile-web-app-capable' content='yes'>"
            "<meta name='apple-mobile-web-app-status-bar-style' content='black-translucent'>"
            "<title>Command Center</title><meta name=\"color-scheme\" content=\"dark\"><style>" + CSS + POST_CSS + LIGHT_FIX + "</style></head><body>"
            ...
            + "<script>" + JS + POST_JS + "</script></body></html>")
...
def _lightpass(html):
    import re as _re, colorsys as _cs
    def _h2(h):
        h=h.lstrip('#'); r,g,b=[int(h[i:i+2],16)/255 for i in (0,2,4)]
        hh,l,s=_cs.rgb_to_hls(r,g,b); return hh*360,s,l
    def _mk(hh,s,l):
        r,g,b=_cs.hls_to_rgb((hh%360)/360.0,max(0.0,min(1.0,l)),max(0.0,min(1.0,s)))
    ...
    def _ct(a,b):
        la,lb=_lm(a),_lm(b); return (max(la,lb)+0.05)/(min(la,lb)+0.05)
    def _warm(hx):
        hh,s,l=_h2(hx)
        if s<0.09: return _mk(40,0.07,l)
        if (190<=hh<=290) and s<0.42 and l>0.55: return _mk(40,0.13,l)
        return hx
    _stash=[]
    def _st(m):
        _stash.append(m.group(0)); return '\x00L'+str(len(_stash)-1)+'\x00'
    html=_re.sub(r'<iframe class="postframe".*?</iframe>', _st, html, flags=_re.S)
    def _fixcolor(hx, ebg):
        if hx.lower()=='#ffffff':
            return hx if _lm(ebg)<0.5 else '#24261f'
        hh,s,l=_h2(hx)
        if _ct(hx,ebg)>=3.2:
            if s<0.16 and (190<=hh<=290): return _mk(38,0.06,l)
            return hx
        bgl=_h2(ebg)[2]
        cands=[0.42,0.38,0.34,0.30,0.26,0.22,0.18] if bgl>0.5 else [0.80,0.85,0.90,0.94]
        for nl in cands:
            cand=_mk(hh,(0.06 if s<0.14 else min(s,0.62)),nl)
            if _ct(cand,ebg)>=4.0: return cand
        return '#20221d' if bgl>0.5 else '#f6f4ee'
    def _rule(mr):
        sel,body=mr.group(1),mr.group(2)
        body=_re.sub(r'(background(?:-color)?:)(#[0-9a-fA-F]{6})', lambda m:m.group(1)+_warm(m.group(2)), body)
        body=_re.sub(r'((?:linear|radial)-gradient\([^)]*\))', lambda g:_re.sub(r'#[0-9a-fA-F]{6}', lambda h:_warm(h.group(0)), g.group(1)), body)
        bgm=_re.search(r'background(?:-color)?:(#[0-9a-fA-F]{6})', body)
        if not bgm: bgm=_re.search(r'(?:linear|radial)-gradient\([^)]*?(#[0-9a-fA-F]{6})', body)
        ebg=bgm.group(1) if bgm else '#ece9e1'
        body=_re.sub(r'color:(#[0-9a-fA-F]{6})', lambda m:'color:'+_fixcolor(m.group(1),ebg), body)
        return sel+'{'+body+'}'
    def _style(ms):
        return '<style>'+_re.sub(r'([^{}]+)\{([^{}]*)\}', _rule, ms.group(1))+'</style>'
    html=_re.sub(r'<style>(.*?)</style>', _style, html, flags=_re.S)
    def _inl(mi):
        seg=mi.group(0)
        seg=_re.sub(r'(background(?:-color)?:)(#[0-9a-fA-F]{6})', lambda m:m.group(1)+_warm(m.group(2)), seg)
        seg=_re.sub(r'color:(#[0-9a-fA-F]{6})', lambda m:'color:'+_fixcolor(m.group(1),'#ece9e1'), seg)
        return seg
    html=_re.sub(r'style="[^"]*"', _inl, html)
    i=0
    for b in _stash:
        html=html.replace('\x00L'+str(i)+'\x00', b); i+=1
    return html
