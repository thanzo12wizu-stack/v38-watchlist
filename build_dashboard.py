...
FMP_REFERENCE_CACHE = os.environ.get("V38_FMP_REFERENCE_CACHE", "fmp_reference_cache.json")
try:
    FMP_REFERENCE_BUDGET = max(0, min(80, int(os.environ.get("V38_FMP_REFERENCE_BUDGET", "40"))))
except (TypeError, ValueError):
    FMP_REFERENCE_BUDGET = 40
FMP_REFERENCE_TTL_DAYS = 30
FMP_REFERENCE_NEGATIVE_TTL_DAYS = 7

N_PORT = 12
# サニタイズで隔離した価格欠損を前日値で埋める最大連続日数。
# 3営業日を超える連続欠損は「本当にデータが無い」とみなして埋めない（捏造しない）。
SANITIZE_FFILL_LIMIT = 3
# サニタイズで銘柄を残す最低有効本数。取得側 _extract(minbars=30) と揃える。
# これ未満は指標が計算できないので除外するが、これ以上あれば「上場して間もない正常な銘柄」として残す。
MIN_HISTORY_BARS = 30
...
# F1(リーダー脱落率)の観測率しきい値。分母=20日前の上位24のうち、今日も観測できた数の比率。
#   FULL以上   : 通常表示
#   MIN〜FULL  : 参考値として算出は続ける（数銘柄の欠測でキモの警報を落とさない）
#   MIN未満    : さすがに代表性が無いので無効化(DATA_INCOMPLETE)
F1_COV_FULL             = 0.90   # これ以上なら完全な観測
F1_COV_MIN              = 0.70   # これ未満は無効化。間は PARTIAL(参考値)
...
def _reset_build_globals():
    ...
    global _SPARK_SEQ
    global _ANALYSIS_ASOF, _SESSION_INDEX, _LAST_FETCH_QUALITY
    _ALLOW_NETWORK = True
    _PERSIST_CACHES = True
...
_VALID_FTD_STATES = {"NO_CORRECTION", "CORRECTION", "RALLY_ATTEMPT", "FTD_ACTIVE", "FTD_FAILED"}
MARKET_CYCLE_IDXS = (("QQQ", "NASDAQ100"), ("SPY", "S&P500"))
MARKET_CYCLE_TICKERS = tuple(tk for tk, _ in MARKET_CYCLE_IDXS)

def _validate_market_cycle_history(v):
    """FTD/DDの状態遷移＋件数変化ログ。日次スナップショットではなく、状態/DD集合が変わった時だけ保存する。"""
    if v is None:
        return True
    if not isinstance(v, list) or len(v) > 240:
        return False
    prev_d = None
    for rec in v:
        ...
        if not isinstance(inds, dict):
            return False
        for tk, x in inds.items():
            if tk not in MARKET_CYCLE_TICKERS or not isinstance(x, dict):
                return False
            st = x.get("state")
            if st is not None and st not in _VALID_FTD_STATES:
                return False
            if not _is_iso_or_none(x.get("ftd_iso")):
                return False
            for k in ("dd", "stall"):
                arr = x.get(k, [])
                if not isinstance(arr, list) or len(arr) > 25:
                    return False
                if not all(_is_iso_date(z) and z <= d for z in arr):
                    return False
    return True
...
def _sanitize_market_cycle_history(v):
    ...
    for rec in v:
        ...
        if not isinstance(inds, dict):
            continue
        for tk in MARKET_CYCLE_TICKERS:
            x = inds.get(tk)
            ...
            if fi is not None and not _is_iso_date(str(fi)):
                fi = None
            dd = [z for z in (x.get("dd") or []) if isinstance(z, str) and _is_iso_date(z) and z <= d][:25]
            stall = [z for z in (x.get("stall") or []) if isinstance(z, str) and _is_iso_date(z) and z <= d][:25]
            clean_inds[tk] = dict(state=st, ftd_iso=fi, dd=dd, stall=stall)
        if clean_inds:
            by_date[d] = {"date": d, "indices": clean_inds}
    out = [by_date[d] for d in sorted(by_date)][-120:]
    return out
...
def _validate_state_schema(obj):
    ...
    if not _validate_state_common(obj, allow_nested=True):
        return False
    if "market_cycle_history" in obj and not _validate_market_cycle_history(obj.get("market_cycle_history")):
        return False
...
def _sanitize_state(obj):
    ...
    if isinstance(obj.get("prevday"), dict):
        ...
        if _pd:
            out["prevday"] = _pd
    # FTD/DD状態遷移ログ（壊れた行だけ除去。履歴全体を捨てない）
    _mch = _sanitize_market_cycle_history(obj.get("market_cycle_history"))
    if _mch:
        out["market_cycle_history"] = _mch
...
def _sanitize_price_body(W, min_coverage=0.20, min_tickers=1):
    ...
    # 重要: 充足率は「全期間」でなく「上場後(最初の有効値以降)」で見る。
    #   3年枠に対する比率で見ると、上場したばかりの正常な銘柄(IPO/スピンオフ。例: 上場49本)が
    #   一律 劣化データ扱いで消える。実際にCerebras/Quantinuum/FedEx Freight等が丸ごと落ちていた。
    #   上場後に歯抜けが多い銘柄だけを除外し、履歴が短いだけの銘柄は残す(最低本数で担保)。
    _first = C2.apply(lambda s: s.first_valid_index())
    _cov_post, _bars = {}, {}
    for _c in C2.columns:
        _fv = _first.get(_c)
        if _fv is None or pd.isna(_fv):
            _cov_post[_c], _bars[_c] = 0.0, 0
            continue
        _post = C2[_c].loc[_fv:]
        _cov_post[_c] = float(_post.notna().mean()) if len(_post) else 0.0
        _bars[_c] = int(_post.notna().sum())
    cov = pd.Series(_cov_post)
    bars = pd.Series(_bars)
    keep = [c for c in C2.columns
            if float(cov.get(c, 0)) >= min_coverage and int(bars.get(c, 0)) >= MIN_HISTORY_BARS]
    ...
    # 局所異常をNaN化しただけだと、rolling(200)等は窓内に1セルでもNaNがあると値を返さないため、
    # 1セルの隔離が ma200 を200日ぶん消滅させ、F1(リーダー脱落率)の observable から銘柄が丸ごと落ちる。
    # → 前に有効値がある「孤立欠損」だけを前日値で補完し、系列の連続性を回復する。
    #   価格4本を同じ日付で揃えて前日値にするのでOHLC整合性は保たれる。
    #   上場前など「前に有効値が無い」欠損は ffill されないので、履歴の捏造にはならない。
    _filled = 0
    for _k in ("Open", "High", "Low", "Close", "Volume"):
        _before = int(clean[_k].isna().to_numpy().sum())
        clean[_k] = clean[_k].ffill(limit=SANITIZE_FFILL_LIMIT)
        _filled += _before - int(clean[_k].isna().to_numpy().sum())
    report = dict(bad_price_cells=n_bad_price, incoherent_cells=n_incoh,
                  dropped_tickers=dropped, kept=len(keep), filled_cells=_filled)
...
def load_inputs(use_cache=True):
    ...
    if use_cache and os.path.exists(CACHE):
        ...
        pk = fetch_live(order, names=names)          # CI path: no pre-existing cache
    ...
    _W = pk["W"]
    # FMPは価格源にしない。FMP検索で明確なticker変更を確認し、その新tickerでYahoo価格が
    # 実際に取れた場合だけ、ユニバースの軸を安全に読み替える。
    _aliases = ((pk.get("fetch_quality") or {}).get("symbol_aliases") or {})
    if _aliases:
        names, order, s2i, s2t, _W = _apply_universe_aliases(
            names, order, s2i, s2t, _W, _aliases)
    global _LAST_FETCH_QUALITY
    _LAST_FETCH_QUALITY = dict(pk.get("fetch_quality") or {})
...
def _symbol_equiv(a, b):
    import re as _re
    return _re.sub(r"[^A-Z0-9]", "", str(a or "").upper()) == _re.sub(r"[^A-Z0-9]", "", str(b or "").upper())

def _company_name_norm(value):
    """会社名比較専用。法人接尾辞と記号だけを落とし、弱い社名一致をticker変更と誤認しない。"""
    import re as _re
    text = str(value or "").upper().replace("&", " AND ")
    text = _re.sub(r"[^A-Z0-9 ]+", " ", text)
    stop = {"INC", "INCORPORATED", "CORP", "CORPORATION", "CO", "COMPANY", "LTD", "LIMITED",
            "PLC", "LP", "LLC", "SA", "NV", "GROUP", "HOLDING", "HOLDINGS", "THE"}
    return " ".join(x for x in text.split() if x not in stop)

def _company_name_score(left, right):
    import difflib
    a, b = _company_name_norm(left), _company_name_norm(right)
    if not a or not b:
        return 0.0
    sa, sb = set(a.split()), set(b.split())
    jac = len(sa & sb) / max(1, len(sa | sb))
    seq = difflib.SequenceMatcher(None, a, b).ratio()
    return max(jac, 0.65 * seq + 0.35 * jac)

_FMP_US_EXCHANGES = {"NASDAQ", "NYSE", "AMEX", "NYSEARCA", "NYSEAMERICAN", "BATS"}

def _fmp_get(endpoint, params, api_key, timeout=15):
    """FMP stable API。URL/API keyはログへ出さず、payloadと安全なstatusだけを返す。"""
    if not api_key:
        return None, "no_key"
    if not _net_ok():
        return None, "offline"
    try:
        ...
        url = "https://financialmodelingprep.com/stable/%s?%s" % (
            str(endpoint).strip("/"), _up.urlencode(query))
        req = _ur.Request(url, headers={"Accept": "application/json", "User-Agent": "dashboard-builder/1.0"})
        with _ur.urlopen(req, timeout=timeout) as response:
            payload = json.loads(response.read().decode("utf-8"))
        return payload, "ok"
    except _ue.HTTPError as exc:
        sys.stderr.write("[fmp-reference] HTTP %s (%s)\n" % (getattr(exc, "code", "?"), endpoint))
        return None, "http_%s" % getattr(exc, "code", "error")
    except Exception as exc:
        sys.stderr.write("[fmp-reference] request failed (%s): %s\n" % (endpoint, type(exc).__name__))
        return None, "error"

def _fmp_ref_rows(payload):
...
def _fmp_us_row(row):
    ex = str(row.get("exchangeShortName") or row.get("exchange") or "").upper().replace(" ", "")
    cur = str(row.get("currency") or "USD").upper()
    return (not ex or ex in _FMP_US_EXCHANGES or ex.startswith("NASDAQ") or ex.startswith("NYSE")) and cur in ("", "USD")

def _fmp_ref_cache_load(path):
    try:
...
def _fmp_ref_fresh(rec, today):
    if not isinstance(rec, dict) or not rec.get("checked_at"):
        return False
    try:
        age = (today - pd.Timestamp(rec["checked_at"]).normalize()).days
    except Exception:
        return False
    status = rec.get("status")
    ttl = FMP_REFERENCE_TTL_DAYS if status in ("confirmed", "alias") else FMP_REFERENCE_NEGATIVE_TTL_DAYS
    if status in ("error", "offline", "no_key"):
        ttl = 1
    return 0 <= age < ttl

def _fmp_reference_record(ticker, company_name, api_key, budget):
    """無料枠で実測済みのsearch-symbolだけを使う。価格・一覧・screener APIは呼ばない。"""
    ticker = str(ticker or "").strip().upper()
    checked_at = str(pd.Timestamp.today().normalize().date())
    if budget.get("remaining", 0) <= 0:
        return {"status": "budget", "checked_at": checked_at}, False

    def search(query):
        budget["remaining"] -= 1
        budget["used"] += 1
        payload, status = _fmp_get("search-symbol", {"query": query}, api_key)
        return [r for r in _fmp_ref_rows(payload) if _fmp_us_row(r)], status

    rows, api_status = search(ticker)
    if api_status != "ok":
        return {"status": api_status, "checked_at": checked_at}, True

    exact = next((r for r in rows if _symbol_equiv(r.get("symbol"), ticker)), None)
    if exact:
        symbol = str(exact.get("symbol") or ticker).upper().strip()
        if not _valid_ticker(symbol):
            return {"status": "invalid", "checked_at": checked_at}, True
        return {
            "status": "confirmed", "canonical": symbol, "yahoo_symbol": _yf_symbol(symbol),
            "name": str(exact.get("name") or exact.get("companyName") or company_name or ""),
            "exchange": str(exact.get("exchangeShortName") or exact.get("exchange") or ""),
            "checked_at": checked_at,
        }, True

    if not company_name or budget.get("remaining", 0) <= 0:
        return {"status": "not_found", "checked_at": checked_at}, True

    rows, api_status = search(company_name)
    if api_status != "ok":
        return {"status": api_status, "checked_at": checked_at}, True
    ranked = []
    for row in rows:
        ...
        if not symbol or not _valid_ticker(symbol):
            continue
        row_name = str(row.get("name") or row.get("companyName") or "")
        ranked.append((_company_name_score(company_name, row_name), symbol, row_name, row))
    ranked.sort(key=lambda x: (-x[0], x[1]))
    if not ranked:
        return {"status": "not_found", "checked_at": checked_at}, True
    best = ranked[0]
    runner = ranked[1][0] if len(ranked) > 1 else 0.0
    if best[0] < 0.88 or best[0] - runner < 0.08:
        return {"status": "ambiguous", "checked_at": checked_at,
                "candidate": best[1], "name_score": round(best[0], 3)}, True
    row = best[3]
    return {
        "status": "alias", "canonical": best[1], "yahoo_symbol": _yf_symbol(best[1]),
        "name": best[2], "exchange": str(row.get("exchangeShortName") or row.get("exchange") or ""),
        "name_score": round(best[0], 3), "checked_at": checked_at,
    }, True

def _fmp_resolve_missing(tickers, names, api_key, cache_path=FMP_REFERENCE_CACHE, budget_limit=FMP_REFERENCE_BUDGET):
    today = pd.Timestamp.today().normalize()
    cache = _fmp_ref_cache_load(cache_path)
    budget = {"remaining": max(0, int(budget_limit)), "used": 0}
    results, cache_hits = {}, 0
    changed = False
    for ticker in tickers:
        ticker = str(ticker or "").upper().strip()
        rec = cache.get(ticker)
        if _fmp_ref_fresh(rec, today):
            results[ticker] = dict(rec)
            cache_hits += 1
            continue
        if not api_key or budget["remaining"] <= 0:
            continue
        rec, attempted = _fmp_reference_record(ticker, (names or {}).get(ticker, ""), api_key, budget)
        if attempted:
            cache[ticker] = rec
            results[ticker] = dict(rec)
            changed = True
    ...
    if changed:
        _defer_cache_write(cache_path, {
            "schema_version": 1, "updated_at": str(today.date()), "symbols": cache})

    catalog = {}
    for rec in cache.values():
        if not isinstance(rec, dict) or rec.get("status") not in ("confirmed", "alias"):
            continue
        symbol = str(rec.get("canonical") or "").upper().strip()
        if symbol and _valid_ticker(symbol):
            catalog[symbol] = {"name": rec.get("name") or "", "exchange": rec.get("exchange") or "", "source": "FMP確認"}
    return results, {
        "queries": budget["used"], "cache_hits": cache_hits,
        "confirmed": sum(1 for x in results.values() if x.get("status") == "confirmed"),
        "aliases": sum(1 for x in results.values() if x.get("status") == "alias"),
        "catalog": catalog,
    }

def _apply_universe_aliases(names, order, s2i, s2t, W, aliases):
    names, order, s2i, s2t = dict(names), list(order), dict(s2i), dict(s2t)
    existing = set(order)
    applied = {}
    for old, new in (aliases or {}).items():
        old, new = str(old).upper(), str(new).upper()
        if old not in existing or not _valid_ticker(new) or (new in existing and new != old):
            continue
        order = [new if x == old else x for x in order]
        existing.discard(old); existing.add(new)
        if old in names: names[new] = names.pop(old)
        if old in s2i: s2i[new] = s2i.pop(old)
        if old in s2t: s2t[new] = s2t.pop(old)
        applied[old] = new
    if applied and isinstance(W, dict):
        for frame in W.values():
            if hasattr(frame, "rename"):
                frame.rename(columns=applied, inplace=True)
    return names, order, s2i, s2t, W

def fetch_live(order, names=None, period="3y", chunk=100, retries=3):
    ...
    for i in range(0, len(yfs), chunk):
        ...
        for attempt in range(retries):
            try:
                df = yf.download(pending, period=period, progress=False, auto_adjust=True,
                                 group_by="ticker", threads=True)
                got = _extract(df, pending)
                store.update(got)
                missing = [t for t in pending if t not in store]
                # 失敗分だけを再取得する（chunk全体を投げ直すより成功率が高く、レート制限にも優しい）
    ...
    _batch_have = len(store)
    # Yahooを主系統のまま強化する。batchで落ちた銘柄だけTicker.history経路を試す。
    _serial_recovered = 0
    _serial_started = time.time()
    for y in [x for x in yfs if x not in store][:120]:
        if time.time() - _serial_started > 150:
            break
        try:
            hist = yf.Ticker(y).history(period=period, auto_adjust=True)
            got = _extract(hist, [y])
            if y in got:
                store[y] = got[y]
                _serial_recovered += 1
    ...
    # それでも取れないtickerだけFMP無料検索で確認する。FMPから価格は取得しない。
    _remaining_base = [y2b.get(y, y) for y in yfs if y not in store]
    _fmp_results, _fmp_diag = _fmp_resolve_missing(
        _remaining_base, names or {}, os.environ.get("FMP_API_KEY", "").strip())
    _symbol_aliases = {}
    _alias_suggestions = []
    _reference_recovered = 0
    _original = set(order)
    for old, rec in _fmp_results.items():
        if not isinstance(rec, dict):
            continue
        status = rec.get("status")
        new = str(rec.get("canonical") or "").upper().strip()
        if status == "ambiguous" and rec.get("candidate"):
            _alias_suggestions.append({"from": old, "to": rec.get("candidate"), "status": "要確認"})
            continue
        if status != "alias" or not new or new in _original:
            continue
        _alias_suggestions.append({"from": old, "to": new, "status": "FMP一致"})
        yahoo_symbol = str(rec.get("yahoo_symbol") or _yf_symbol(new))
        try:
            hist = yf.Ticker(yahoo_symbol).history(period=period, auto_adjust=True)
            got = _extract(hist, [yahoo_symbol])
            if yahoo_symbol not in got:
                continue
            store[yahoo_symbol] = got[yahoo_symbol]
            y2b[yahoo_symbol] = new
            _symbol_aliases[old] = new
            _reference_recovered += 1
        except Exception:
            continue

    # どの銘柄が取れなかったかを明示する。上場廃止・ティッカー変更・API失敗の切り分けと
    # universe.csv のメンテナンス（＝観測率低下の根本対処）に必要。
    _missed = [y2b.get(y, y) for y in yfs if y not in store and y2b.get(y, y) not in _symbol_aliases]
    if _missed:
        sys.stderr.write("[fetch_live] 未取得 %d件（上場廃止/ティッカー変更/取得失敗の可能性・universe.csv要確認）: %s\n"
                         % (len(_missed), ",".join(_missed[:60])))
    ...
    _fetch_quality = {
        "yf_requested": len(yfs), "yf_batch_have": _batch_have,
        "yf_serial_recovered": _serial_recovered, "yf_reference_recovered": _reference_recovered,
        "missing_count": len(_missed), "missing_tickers": _missed[:60],
        "fmp_mode": "reference_only", "fmp_queries": _fmp_diag.get("queries", 0),
        "fmp_cache_hits": _fmp_diag.get("cache_hits", 0),
        "fmp_confirmed": _fmp_diag.get("confirmed", 0), "fmp_aliases": _fmp_diag.get("aliases", 0),
        "fmp_reference_catalog": _fmp_diag.get("catalog", {}),
        "alias_suggestions": _alias_suggestions, "symbol_aliases": _symbol_aliases,
        "fmp_paid_endpoints_used": False,
    }
    return {"W": W, "macro": macro, "asof": str(pd.Timestamp.now("UTC")), "fetch_quality": _fetch_quality}
...
def build_w30_breakouts(m, cap=12, e2j=None):
    ...
    signal_card = (f'<div class="card"><div class="hdr"><h2>ブレイク一覧 <span class="h2en">Signals</span></h2>{_cp([t for _,_,_,_,t,_ in rows[:cap]], [t for _,_,_,_,t,_ in rows])}</div>'
...
def _vwap_features(c, h, l, v, window=None, allow_expanding=False):
    """Return current VWAP location and actionable interaction flags.

    Rolling VWAP uses HLC3 and volume.  The expanding variant is intentionally
    disabled unless the symbol started after the common download window; this
    prevents a three-year fetch from being mislabeled as an old company's
    all-time VWAP.
    """
    empty = dict(value=np.nan, dist=np.nan, slope5=np.nan, near=False,
                 support=False, reclaim=False, valid=False)
    try:
        frame = pd.concat(
            [c.rename("c"), h.rename("h"), l.rename("l"), v.rename("v")],
            axis=1,
        ).replace([np.inf, -np.inf], np.nan).dropna()
        frame = frame[frame["v"] > 0]
        minimum = 60 if window is None else int(window)
        if len(frame) < minimum or (window is None and not allow_expanding):
            return empty

        pv = ((frame["h"] + frame["l"] + frame["c"]) / 3.0) * frame["v"]
        if window is None:
            vw = pv.expanding(min_periods=minimum).sum() / frame["v"].expanding(min_periods=minimum).sum()
        else:
            vw = pv.rolling(window, min_periods=minimum).sum() / frame["v"].rolling(window, min_periods=minimum).sum()
        vw = vw.replace([np.inf, -np.inf], np.nan).dropna()
        if len(vw) < 6:
            return empty

        aligned = frame.reindex(vw.index)
        value = float(vw.iloc[-1])
        close = float(aligned["c"].iloc[-1])
        prev_close = float(aligned["c"].iloc[-2])
        prev_vwap = float(vw.iloc[-2])
        dist = close / value - 1.0 if value > 0 else np.nan
        slope5 = value / float(vw.iloc[-6]) - 1.0 if float(vw.iloc[-6]) > 0 else np.nan
        recent = aligned.iloc[-3:]
        recent_vw = vw.reindex(recent.index)
        touched = bool((recent["l"] <= recent_vw * 1.0075).any())
        # "Near" is a watch location only. Support/reclaim are the actionable
        # reactions used by the entry board.
        ...
        return dict(value=value, dist=dist, slope5=slope5, near=near,
                    support=support, reclaim=reclaim, valid=True)
    except Exception:
        return empty
...
def compute_metrics(W, order, s2i=None, macro=None):
    ...
    for t in order:
        ...
        ema21_10ago = float(ema21s.iloc[-11]) if len(c) >= 32 else np.nan  # 10日前の21EMA（傾き用）
        ema21_touch3 = False
        ema21_touch_days = np.nan
        try:
            _e3 = ema21s.reindex(l.index).iloc[-3:]
            _l3 = l.reindex(_e3.index)
            _tm = (_l3 <= _e3 * 1.005).to_numpy()
            if _tm.any() and close >= ema21 * 0.99:
                ema21_touch3 = True
                ema21_touch_days = float(len(_tm) - 1 - np.where(_tm)[0][-1])
        except Exception:
            ema21_touch3 = False
            ema21_touch_days = np.nan
        ...
        # Multi VWAP. 63/252 are rolling; expanding("all-time") is shown only
        # for symbols whose first bar is clearly after the common fetch start,
        # i.e. listings whose full history exists inside this dataset.
        _common_start = idx.min() if len(idx) else c.index.min()
        try:
            _recent_listing = bool(c.index.min() > (_common_start + pd.Timedelta(days=35)))
        except Exception:
            _recent_listing = False
        _vw63 = _vwap_features(c, h, l, v, window=63)
        _vw252 = _vwap_features(c, h, l, v, window=252)
        _vwall = _vwap_features(c, h, l, v, window=None, allow_expanding=_recent_listing)
        ...
        atr14 = float(tr.iloc[-14:].mean()) if len(tr) >= 14 else np.nan
        ext50_atr = ((close - sma50) / atr14
                     if np.isfinite(close) and np.isfinite(sma50) and np.isfinite(atr14) and atr14 > 0
                     else np.nan)
        ...
        recs.append(dict(
            **_w30,
            ...
            vwap63_near=_vw63["near"], vwap63_support=_vw63["support"], vwap63_reclaim=_vw63["reclaim"],
            vwap252=_vw252["value"], vwap252_dist=_vw252["dist"], vwap252_slope5=_vw252["slope5"],
            vwap252_near=_vw252["near"], vwap252_support=_vw252["support"], vwap252_reclaim=_vw252["reclaim"],
            vwap_all=_vwall["value"], vwap_all_dist=_vwall["dist"], vwap_all_slope5=_vwall["slope5"],
            vwap_all_near=_vwall["near"], vwap_all_support=_vwall["support"],
            vwap_all_reclaim=_vwall["reclaim"], vwap_all_valid=_vwall["valid"],
...
def _market_cycle_snapshot(asof_date, ftd, distrib):
    """FTD状態と現サイクルDD集合の小さなスナップショット。将来の状態遷移＋件数変化Timeline用。"""
    ftd_by = {x.get("tk"): x for x in (ftd or []) if isinstance(x, dict) and x.get("tk")}
    inds = {}
    for tk in MARKET_CYCLE_TICKERS:
        f = ftd_by.get(tk, {})
        d = (distrib or {}).get(tk, {}) if isinstance(distrib, dict) else {}
        ev = d.get("events") or []
        dd = [e.get("date") for e in ev if isinstance(e, dict) and e.get("cycle") != "prior"
...
def _quality_card(q):
    ...
    _pcov = q.get("price_cov")
    _pcovtxt = ("—" if not _finite(_pcov) else f'{_pcov*100:.1f}%')
    _yr = q.get("yf_requested")
    _yahoo = ("価格キャッシュ使用" if _yr is None else
    ...
    rows = [
        ...
        ("Yahoo取得経路", _yahoo),
        ("最終未取得ticker", "・".join(q.get("missing_tickers") or []) or "なし"),
        ("FMP無料参照", (f'API {q.get("fmp_queries", 0)}回・cache {q.get("fmp_cache_hits", 0)}件・確認 {q.get("fmp_confirmed", 0)}件・変更候補 {q.get("fmp_aliases", 0)}件'
                        if q.get("fmp_mode") else "未使用（価格キャッシュ）")),
...
def _confluence_facts(r, new_entry=None):
    ...
    if flag("vcp_ready"):
        setups.append("VCP")
    if flag("cup_ready"):
        setups.append("カップ・ハンドル")
    if flag("ema21_touch3") and ok(r.get("ema21_10ago")) and ok(r.get("ema21")) and r.get("ema21") > r.get("ema21_10ago"):
        setups.append("21EMAタッチ")

    triggers = []
    if flag("true_breakout"):
        triggers.append("ブレイク")
    if flag("cup_breakout"):
        triggers.append("カップ・ハンドルブレイク")
    pp = r.get("pp_days")
    if ok(pp):
        triggers.append("PP本日" if int(pp) == 0 else f"PP {int(pp)}日前")
    for key, lab in (("vwap63", "63VWAP"), ("vwap252", "252VWAP"), ("vwap_all", "上場来VWAP")):
        if flag(key + "_reclaim"):
            triggers.append(lab + "下から回復")
        elif flag(key + "_support"):
            triggers.append(lab + "タッチ後上")

    confirms = []
    rs189, rs63 = r.get("rs189"), r.get("rs")
    if ok(rs189):
        confirms.append(f"RS189 {float(rs189):.0f}")
    if ok(rs63):
        confirms.append(f"RS63 {float(rs63):.0f}")
    if flag("rsline_newhigh"):
        confirms.append("RSライン新高値")
    if ok(r.get("uvdv20")):
        confirms.append(f"U/D {float(r.get('uvdv20')):.2f}")
    if isinstance(new_entry, str) and new_entry:
        confirms.append("RS新規参入")

    locations = []
    if flag("stage2"):
        locations.append("ステージ2")
    if ok(r.get("pivot_dist")):
        locations.append(f"ピボット差 {float(r.get('pivot_dist'))*100:+.1f}%")
    for key, lab in (("vwap63", "63VWAP"), ("vwap252", "252VWAP"), ("vwap_all", "上場来VWAP")):
        if key == "vwap_all" and not flag("vwap_all_valid"):
            continue
        if flag(key + "_near") and ok(r.get(key + "_dist")):
            locations.append(f"{lab} {float(r.get(key + '_dist'))*100:+.1f}%")
    if ok(r.get("ext50_atr")):
        locations.append(f"50MA {float(r.get('ext50_atr')):+.1f}ATR")
    if ok(r.get("ret5")):
        locations.append(f"5日 {float(r.get('ret5'))*100:+.1f}%")
    if ok(r.get("adr")):
        locations.append(f"ADR {float(r.get('adr'))*100:.1f}%")
...
def _signals_of(r, new_entry=None):
    """Compatibility helper used by the displayed-location index."""
    a = _confluence_facts(r, new_entry)
...
def build_confluence_watch(m, cand=None, cap=20, min_rs=CONFLUENCE_RS189_MIN):
    ...
    for t, r in pool.iterrows():
        ...
        if not (a["setup"] or a["trigger"] or a["location"]):
            continue
        if a["has_setup"] and a["has_trigger"]:
            state, order = "セットアップ＋発火", 0
        elif a["has_trigger"]:
            state, order = "発火観測", 1
        elif a["has_setup"]:
            state, order = "セットアップ観測", 2
        else:
            state, order = "位置観測", 3
        rows.append((order, float(r.get("rs189") or 0), t, r, a, state))
    det = ('<details class="cfdet"><summary>判定の定義</summary>'
           '<div class="cflg">既存指標を<b>セットアップ・発火・確認・位置/警戒</b>に分けて表示。'
           '総合点、合格点、発注合否は作らない。算出できない上場来VWAPも減点しない。'
           '<b>52週高値への近さは判定に使わない</b>。</div></details>')
    if not rows:
        return (f'<div class="card"><h2>コンフルエンス・ボード</h2>'
                f'<div class="sub">RS189≥{min_rs}の対象に、表示するセットアップ・発火・VWAP近接は現在なし。{det}</div></div>')
    rows.sort(key=lambda x: (x[0], -x[1], x[2]))
    total = len(rows)
    rendered = []
    for order, rs, t, r, a, state in rows:
        pills = (''.join(f'<span class="sgp">SETUP {_h(x)}</span>' for x in a["setup"])
                 + ''.join(f'<span class="sgp">TRIGGER {_h(x)}</span>' for x in a["trigger"])
                 + ''.join(f'<span class="sgp">CONFIRM {_h(x)}</span>' for x in a["confirm"])
                 + ''.join(f'<span class="sgp">LOCATION {_h(x)}</span>' for x in a["location"])
                 + ''.join(f'<span class="sgp neg">WARNING {_h(x)}</span>' for x in a["warning"]))
        cls = "cf-fire" if a["has_trigger"] else ""
        rendered.append(f'<div class="cfrow {cls}" data-liq="{(r.get("dvol") or 0)/1e6:.1f}" data-tkone="{t}">'
                        f'<div class="cfl"><span class="cfstate">{_h(state)}</span>'
                        f'<b class="cft">{_h(t)}</b><span class="mut" style="font-size:10.5px">RS189 {int(round(rs))}</span></div>'
                        f'<div class="cfs">{pills}</div></div>')
    body = ''.join(rendered[:cap])
    more = (f'<details class="cfdet cfmore"><summary>残り{total-cap}件</summary>'
            f'{"".join(rendered[cap:])}</details>') if total > cap else ""
    _cptk = [t for _order, _rs, t, _r, _a, _state in rows[:cap]]
    _cpall = [t for _order, _rs, t, _r, _a, _state in rows]
    return (f'<div class="card"><div class="hdr"><h2>コンフルエンス・ボード</h2>{_cp(_cptk, _cpall)}</div>'
            f'<div class="sub">RS189≥{min_rs}の対象を「セットアップ＋発火」「発火観測」「セットアップ観測」「位置観測」の順に表示。'
            f'同区分内はRS189順。総合点と発注判定はなし。{det}</div>'
            f'{body}{more}</div>')
...
def build_pocket_pivots(m, cap=10):
    ...
    secs = ""; _ppshown = []; _ppall = []
    for d_lo, d_hi, lab in ((0, 0, "本日"), (1, 3, "直近3日"), (4, 10, "4〜10日前")):
        sub0 = pp[(pp["pp_days"] >= d_lo) & (pp["pp_days"] <= d_hi)].sort_values("pp_ratio", ascending=False)
        if sub0.empty:
            continue
        n = len(sub0)
        _ppall.extend(list(sub0.index))
        sub = sub0.head(cap)
        _ppshown.extend(list(sub.index))
        chips = ""
        for t, r in sub.iterrows():
            rt = r.get("pp_ratio")
            rs = r.get("rs189")
    ...
    if not secs:
        return ""
    return (f'<div class="card"><div class="hdr"><h2>ポケットピボット（10D）</h2>{_cp(_ppshown, _ppall)}</div>'
...
def build_new_entrants(cand):
    ...
    for lab in ("今日", "今週", "今月"):
        ...
        if not rows:
            continue
        _alltk.extend(t for _, t, _ in rows)
        chips = "".join(
    ...
    return (f'<div class="card"><div class="hdr"><h2>新規参入（ポート候補{NEW_ENTRY_OUT_RANK}位圏）</h2>{_cp(_alltk)}</div>'
...
def build_vcp(m, s2i, e2j, s2t, k=12):
    """VCP observations shown as raw shape/volume/pivot facts."""
    try:
    ...
    if sub.empty:
        return []
    sub = sub.sort_values(["rs189"], ascending=[False])
    out = []
    for t, r in sub.head(k).iterrows():
        ind = s2i.get(t); indja = e2j.get(ind, ind) if ind else "—"
        _a = _confluence_facts(r)
        out.append(dict(t=t, rs=float(r["rs189"]),
                        ...
                        trigger_state=("発火あり" if _a["trigger"] else "未発火"),
                        triggers=_a["trigger"],
                        theme=theme_of(t, s2t), ind=subtheme_of(t, s2t, indja),
                        tier_lab=r.get("tier_lab", "—"), tier_key=r.get("tier_key", "none")))
    return out
...
def compute_regime_history(W, days=112):
    ...
    try:
        ...
        if len(C) < 230:
            return None
        # min_periods: 窓内に少数のNaN(局所異常の隔離跡)が残っても指標が丸ごと消えないようにする。
        #   これが無いと、1セルの欠損が ma200 を200日分NaNにし、F1の observable から銘柄が脱落して
        #   観測率が閾値(90%)を割り、F1自体が DATA_INCOMPLETE で無効化される。
        ma50 = C.rolling(50, min_periods=45).mean(); ma200 = C.rolling(200, min_periods=180).mean()
        ...
        # 観測率がFULL(90%)未満でも、MIN(70%)以上なら参考値として算出を続ける。
        # 数銘柄の欠測でリーダー脱落率という最速の警報を丸ごと失う方が実害が大きいため。
        f1 = f1.where(coverage >= F1_COV_MIN)                      # MIN未満のみ無効化(DATA_INCOMPLETE)
        ...
        try:
            ...
            _cov_last = coverage.iloc[-1]
            out["f1_status"]   = ("DATA_INCOMPLETE" if pd.isna(f1.iloc[-1])
                                  else ("OK" if (pd.notna(_cov_last) and _cov_last >= F1_COV_FULL) else "PARTIAL"))
...
def compute_regime_state(m, hist=None):
    ...
    try:
        ...
        if hist and hist.get("f1_status") in ("OK", "PARTIAL"):
...
def build_regime_alerts(m, st=None, collapsible=False, hist=None):
    ...
    try:
        ...
        cp1, cp2, cp3 = _cp(f1_names), _cp(f2_names), _cp(f3_names)
        _cov = st.get("f1_coverage")
        _f1st = st.get("f1_status")
        if _f1st == "DATA_INCOMPLETE":
            _f1meta = f'<div class="reg-onset"><span class="onset on-warn">データ不足で無効化（観測率 {(_cov or 0)*100:.0f}%）</span></div>'
        elif _f1st == "PARTIAL" and _cov is not None and _cov == _cov:
            # 観測率がやや低いが代表性はある。算出は続け、参考値であることを明示する。
            _f1meta = (f'<div class="reg-onset"><span class="onset on-warn">参考値（観測率{_cov*100:.0f}%'
                       f'・分母{st.get("f1_den","?")}中{st.get("f1_den",0)-len(st.get("f1_unknown") or [])}銘柄で算出）</span></div>')
...
def _theme_drill_card(themes):
    ...
    for i, th in enumerate(themes):
        ...
        _showtk = [x["t"] for x in th["members"][:24]]
        _alltk = [x["t"] for x in th["members"]]
        panels += (f'<div class="thpanel" id="thp-{i}" style="display:none">'
                   f'<div class="thpanel-h">{th["name"]}・{th["n"]}銘柄 <span class="mut">（RS強い順・タップで詳細）</span>{_cp(_showtk, _alltk)}</div>'
                   f'<div class="thstks">{chips}{more}</div></div>')
...
def build_distribution(macro, ftd=None):
    """IBD式の売り抜け日（QQQ/SPY代理）。
    FTD_ACTIVE がある場合は、そのFTD成立日より後のDDだけを「現サイクル」として件数化する。
    FTD以前のDDは前サイクル履歴として保持するが、現在件数には入れない。
    FTD当日は上昇率条件上Classic DD/Stallと定義上両立しないため、d <= ftd_d の境界は防御的なもの。
    Early DDの警告閾値は作らず、FTD後何セッション目に発生したかを素で表示する。

    Classic = 前日比 −0.2%以下 かつ 出来高が前日超
    Stall   = 上昇したのに +0.4%以下で止まり、出来高は前日超、レンジ下半分で引け、
              かつ高値圏(20日高値の-3%以内)
    ★点灯(4/6)に使うのは Classic のみ。Stall は表示専用。
    直近25営業日で集計し、その日の終値から+5%上昇したDDは解消扱い。
    """
    out = {}
    ftd_by = {}
    for r in (ftd or []):
        if isinstance(r, dict) and r.get("state") == "FTD_ACTIVE" and r.get("ftd_iso"):
            try:
                ftd_by[r.get("tk")] = pd.Timestamp(r["ftd_iso"])
            except Exception:
                pass

    for tk in reversed(MARKET_CYCLE_TICKERS):  # 表示順は従来どおり SPY→QQQ
        if tk not in macro:
            continue
        ...
        for k, (d, ok) in enumerate(hit.items()):
            ...
            cd = float(c.loc[d])
            # as-of安全性を明示：参照範囲は分析データの最終確定日まで。
            future = c.loc[(c.index > d) & (c.index <= c.index[-1])]
            washed = bool(len(future) and float(future.max()) >= cd * 1.05)

            ftd_session = None
            cycle = "no_ftd"
            if ftd_d is not None:
                if d <= ftd_d:
                    cycle = "prior"
                else:
                    cycle = "current"
                    # FTD翌営業日=FTD+1。FTD当日は数えない。
                    ftd_session = int(((c.index > ftd_d) & (c.index <= d)).sum())

            iso = pd.Timestamp(d).strftime("%Y-%m-%d")
            events.append(dict(date=iso, kind=("stall" if is_st else "classic"),
                               washed_out=washed, cycle=cycle, ftd_session=ftd_session))
            if washed:
                continue

            lab = pd.Timestamp(d).strftime("%-m/%-d") + ("s" if is_st else "")
            if cycle == "prior":
                prior_days.append(lab)
                continue

            # FTDなし、またはFTD後の現サイクルだけを現在件数へ。
            if ftd_session is not None:
                lab += f"(FTD+{ftd_session})"
            n_st += int(is_st)
            n_cl += int(not is_st)
            n_hv += int(bool(heavy.loc[d]))
            if k >= len(idx25) - 10:
                n_r10 += 1
            days.append(lab)
        ...
        out[tk] = dict(
            n=n_alert, alert_basis="classic_only",
            cl=n_cl, stall=n_st, heavy=n_hv, r10=n_r10,
            total=n_cl + n_st, st=st_, cls=cls, days=days,
            prior_days=prior_days,
            ftd_from=(ftd_d.strftime("%-m/%-d") if ftd_d is not None else None),
            ftd_iso=(ftd_d.strftime("%Y-%m-%d") if ftd_d is not None else None),
            events=events,
        )
    return out
...
def build_ftd(macro, idxs=MARKET_CYCLE_IDXS):
    ...
    for tk, lab in idxs:
        ...
        rec = dict(tk=tk, lab=lab,
                   ...
                   ftd_iso=(dates[ftd_i].strftime("%Y-%m-%d") if ftd_i is not None else None),
...
def _dd_card(dd):
    ...
    def _box(tk, d):
        days = d.get("days") or []
        prior = d.get("prior_days") or []
        dline = (f'<div class="dddays">{"・".join(days)}</div>') if days else ""
        ftd_note = (f'<span class="pos">FTD {d["ftd_from"]}以降</span>・' if d.get("ftd_from") else "")
        bd = (f'<div class="dddays mut">{ftd_note}売り抜け{d.get("cl",0)}・ストール{d.get("stall",0)}'
              f'・直近10日{d.get("r10",0)}</div>')
        prior_line = (f'<div class="dddays mut" style="opacity:.62">前サイクル（FTDでリセット）: '
                      f'{"・".join(prior)}</div>') if prior else ""
    ...
    return (f'<div class="card"><h2>ディストリビューション・デイ（直近25営業日／FTD以降）</h2>'
            ...
            f'FTD後のDDは「(FTD+N)」で発生セッションをそのまま表示し、Early DDの恣意的な警告閾値は置かない。</div></details>'
...
def build_categorized_commentary(mkt, m, breadth, sar, aux):
    ...
    def _dd_cycle_text(k, v):
        base = f"{k} {(v.get('n') or 0)}日"
        if v.get("ftd_from"):
            base += f"（FTD {v['ftd_from']}以降・前サイクル分リセット済み）"
        return base
    dd_detail = "・".join(_dd_cycle_text(k, v) for k, v in distrib.items()) or "—"
    _dd_reset_active = any(v.get("ftd_from") for v in distrib.values())
    ftd = mkt.get("ftd") or []
    ...
    blocks.append(("指数", "Indices", [
        ...
        ("週次", f'QQQ {pct(qqq.get("w1"))}・SPY {pct(spy.get("w1"))}。' + (f'売り抜けは{dd_detail}＝' + ("6日以上で内部劣化・戻りは慎重" if (qqq_dd or 0) >= 6 or (spy_dd or 0) >= 6 else ("新サイクルの蓄積を観察" if _dd_reset_active else "許容範囲で押し目は機能")) + '。' if qqq_dd is not None else '')),
...
def build_tenbagger_l1(m):
    ...
    try:
        ...
        chips = "".join(
            ...
            for x in rows)
        return (f'<div class="card"><div class="hdr"><h2>テンバガー・レーダー <span class="tbbadge">宝くじ枠</span></h2>{_cp([x["t"] for x in rows], [x["t"] for x in all_rows])}</div>'
                f'<div class="sub"><b>3年内10倍の生息地</b>を機械抽出（該当{len(all_rows)}銘柄）。'
...
def _transition_leaders_card(tl):
    ...
    return (f'<div class="card"><div class="hdr"><h2>転換初動リーダーボード</h2>{_cp([r["t"] for r in tl["leaders"][:16]], [r["t"] for r in tl["leaders"]])}</div>'
...
def _leader_breadth_card(lb):
    ...
    return (f'<div class="card"><h2>リーダー・ブレッドス警戒灯</h2>'
            ...
            f'<div class="lbchips"><span class="lbcl">脱落した銘柄</span>{_cp([t for t, *_ in dl])}{drop_chips}</div>'
            ...
            f'<div class="lbchips"><span class="lbcl">勢い細りの銘柄</span>{_cp([t for t, *_ in fl])}{fade_chips}</div>'
...
def build_structure_pivot_screen(m, s2t=None):
    ...
    _alltk = [t for t, _ in rows]
    hd = (f'<div class="card"><div class="hdr"><h2>底打ち（構造ピボット） <span class="h2en">Structure Pivot</span></h2>{_cp(_alltk[:24], _alltk)}</div>')
...
def build_movers_tab(m, s2t=None):
    ...
    def _col(ser, cls, head):
        ...
        _tks = list(ser.index)
        head_html = f'<div class="mv-col-h {cls}"><span>{head}</span>{_cp(_tks[:10], _tks)}</div>'
        if len(rows) <= 10:
    ...
    _up3 = sorted(up3); _dn3 = sorted(dn3)
    mom_block = ('<div class="mv-sec-h">② 3窓一致（1日・1週・1ヶ月すべてで上位/下位20＝相対的に一貫して強い/弱い）</div>'
                 f'<div class="mv-mom"><div class="mv-mom-l pos">3窓上位 {_cp(_up3)}</div><div class="mv-mom-b">{_chips(up3)}</div></div>'
                 f'<div class="mv-mom"><div class="mv-mom-l neg">3窓下位 {_cp(_dn3)}</div><div class="mv-mom-b">{_chips(dn3)}</div></div>')
...
def _vcp_card(rows):
    """VCP table with observed shape, volume contraction and trigger facts."""
    head = ('<div class="card"><div class="hdr"><h2>圧縮コイル（VCP）</h2>'
            + (_cp([r["t"] for r in rows]) if rows else "") + '</div>'
            '<div class="sub">VCP形状、収縮回数、安値切上げ、出来高枯れ、ATR収縮、ピボット差、'
            'PP/ブレイク/VWAP反応の有無を表示。総合点と発注合否はなし。</div>')
    if not rows:
    ...
    for r in rows:
        ...
        if int(r.get("touches") or 0): _fl.append(f'ピボット接触{int(r.get("touches") or 0)}')
        flags = " ".join(_fl)
        _state = r.get("trigger_state") or "未発火"
        _scls = "pos" if r.get("triggers") else "mut"
        _trig = " / ".join(r.get("triggers") or []) or "発火待ち"
        trs += (f'<tr data-liq="{(r.get("dvol") or 0)/1e6:.1f}" data-tkone="{r["t"]}">'
                ...
                f'<span class="mut" style="font-size:9px">RS189 {r["rs"]:.0f}</span></div></td>'
                f'<td><b class="{_scls}">{_state}</b><div class="mut" style="font-size:9.5px">{_h(_trig)}</div></td>'
                f'<td><span class="mut" style="font-size:9.5px">{flags}<br>Vol {vd} / ATR {at}</span></td>'
                f'<td>{pdist}</td></tr>')
    return (head + '<table class="vcptbl"><tr><th class="l">銘柄</th><th>発火</th><th>VCP観測</th><th>ピボット差</th></tr>'
            + trs + '</table></div>')
...
def build_multi_vwap_card(m, cap=24):
    ...
    if m is None or m.empty or not all(x in m.columns for x in needed):
        return ""
    # RSや上場年次で配点/選別せず、共通の最低流動性・価格・時価総額条件のみ。
    elig = setup_eligible(m)
    any_loc = pd.Series(False, index=m.index)
    for key in needed:
        for suffix in ("_near", "_support", "_reclaim"):
            if key + suffix in m.columns:
                any_loc |= m[key + suffix].fillna(False).astype(bool)
    sub = m[elig & any_loc].copy()
    if sub.empty:
    ...
    def _action_rank(r):
        has_reclaim = any(bool(r.get(k + "_reclaim")) for k in needed)
        has_support = any(bool(r.get(k + "_support")) for k in needed)
        state = 0 if has_reclaim else (1 if has_support else 2)
        dists = [abs(float(r.get(k + "_dist"))) for k in needed if pd.notna(r.get(k + "_dist"))]
    ...
    def _cell(r, key):
        if key == "vwap_all" and not bool(r.get("vwap_all_valid")):
            return '<span class="mut">対象外</span>'
        dist = r.get(key + "_dist")
        d = "—" if pd.isna(dist) else f'{float(dist)*100:+.1f}%'
        if bool(r.get(key + "_reclaim")):
            return f'<b class="pos">下から回復</b><div class="mut" style="font-size:9px">{d}</div>'
        if bool(r.get(key + "_support")):
            return f'<b class="pos">タッチ反発</b><div class="mut" style="font-size:9px">{d}</div>'
        if bool(r.get(key + "_near")):
            return f'<span class="warnc">近接</span><div class="mut" style="font-size:9px">{d}</div>'
        return f'<span class="mut">{d}</span>'

    rows = []
    tks = []
    for t, r in ordered:
        tks.append(t)
        ath_break = bool(r.get("vwap_all_valid")) and bool(r.get("vwap_all_reclaim"))
        ath = ('<b class="pos">発生</b>' if ath_break else
...
def _patterns_card(pats):
    ...
    return (f'<div class="card"><div class="hdr"><h2>テクニカル・パターン別</h2>{_cp(_pttk[:30], _pttk)}</div>'
...
def _swing_planner_card():
    ...
    return (
        ...
        '<input id="swPick" type="text" autocomplete="off" autocapitalize="characters" spellcheck="false" '
        'placeholder="ティッカー入力 例 NVDA" list="swList" oninput="swPickInput()" '
        'onclick="tickerSuggest(\'swPick\',\'swPickSuggest\',\'plan\')"><datalist id="swList"></datalist></div>'
        '<div id="swPickSuggest" class="tksuggest"></div>'
        ...
        '<div class="mut" style="font-size:11px;margin:-4px 0 8px">Core 12（システム選定）＋レバETF（TQQQ/SOXL）＋裁量スイングを含む<b>実際の保有</b>。ティッカーは候補から選び、誤入力を防止。</div>'
        '<div class="calc-in">'
        '<div class="ci-row"><span class="ci-pre" style="font-size:12px">銘柄</span>'
        '<input id="hldTk" type="text" autocomplete="off" autocapitalize="characters" spellcheck="false" '
        'placeholder="例 NVDA / TQQQ / SOXL" list="hldTkList" oninput="hldTickerInput()" '
        'onclick="tickerSuggest(\'hldTk\',\'hldTkSuggest\',\'hold\')"></div>'
        '<datalist id="hldTkList"></datalist>'
        '<div id="hldTkSuggest" class="tksuggest"></div><div id="hldTkHint" class="tkhint"></div>'
        ...
        '<button type="button" class="stratbtn" data-strat="lev" onclick="hldStratPick(this)">レバETF</button>'
        ...
        '<option>リーダー継続</option><option>レバETFゲート</option><option>裁量</option><option>その他</option></select>'
    ...
    for lab, col, _pcol, _retcol, period, _desc, hist_specs in specs:
        ...
        for win, asof_txt, hcol in hist_specs:
            ...
            hrows.append(
                ...
                f'<div class="rsx-hflow"><div><strong class="hin">IN {len(ins)}</strong>{_cp(ins)}<span class="chips">{_chips(ins, "in")}</span></div>'
                f'<div><strong class="hout">OUT {len(outs)}</strong>{_cp(outs)}<span class="chips">{_chips(outs, "out")}</span></div></div>'
                '</div>'
            )
        hist_blocks.append(
...
.cpwrap{display:inline-flex;gap:5px;align-items:center;flex-wrap:wrap}
.cp-all{background:#172a1d;border-color:#315f3e;color:#8de5a9}
...
.cfrow.cf-fire{border-color:rgba(143,179,255,.42);background:rgba(143,179,255,.05)}
.cfl{display:flex;align-items:center;gap:7px;min-width:132px}
.cfstate{font-size:9px;font-weight:800;color:#cdd9ea;background:#1b2637;border-radius:5px;padding:2px 5px;white-space:nowrap}
...
.vcptbl,.vwtbl{table-layout:fixed;width:100%}.vcptbl th,.vcptbl td,.vwtbl th,.vwtbl td{white-space:normal;overflow-wrap:anywhere;vertical-align:top;line-height:1.25;padding:5px 3px}.vcptbl th:nth-child(1){width:28%}.vcptbl th:nth-child(2){width:27%}.vcptbl th:nth-child(3){width:31%}.vcptbl th:nth-child(4){width:14%}.vwtbl{font-size:11px}.vwtbl th:nth-child(1){width:20%}.vwtbl th:nth-child(n+2){width:20%}
...
.thpanel-h{font-size:12.5px;font-weight:800;color:#eef3fa;margin-bottom:8px;display:flex;align-items:center;gap:6px;flex-wrap:wrap}
.thpanel-h .mut{font-weight:500}.thpanel-h .cpwrap,.thpanel-h>.cp{margin-left:auto}
...
@media (max-width:520px){
  .vcptbl{font-size:10px}.vwtbl{font-size:9.5px}
  .vcptbl th,.vcptbl td,.vwtbl th,.vwtbl td{padding:4px 2px}
  .vwtbl .mut{font-size:8px!important}
}
...
function showDet(tk){
  ...
  var rows=[['RS(63日)',_f(d.rs),'mut'],
            ...
            ['VCP形状',_f(d.vcpq),'mut'],
            ['63 VWAP',(d.vws63||'—')+(d.vw63===null||d.vw63===undefined?'':' '+_f(d.vw63,'%')),(d.vws63==='下から回復'||d.vws63==='タッチ反発')?'pos':'mut'],
            ['252 VWAP',(d.vws252||'—')+(d.vw252===null||d.vw252===undefined?'':' '+_f(d.vw252,'%')),(d.vws252==='下から回復'||d.vws252==='タッチ反発')?'pos':'mut'],
            ['上場来VWAP',(d.vwsall||'—')+(d.vwall===null||d.vwall===undefined?'':' '+_f(d.vwall,'%'))+(d.vwab?' ブレイク':''),d.vwab?'pos':'mut'],
...
/* ---- 共通ティッカー辞書：個別株の全ユニバース＋運用対象レバETF ---- */
var LEV_TICKERS={
  TQQQ:{label:'NASDAQ100 3倍レバETF',priceKey:'tqqq'},
  SOXL:{label:'半導体 3倍レバETF',priceKey:'soxl'}
};
function _isLevTicker(t){return !!LEV_TICKERS[String(t||'').trim().toUpperCase()];}
function _tickerData(t){
  t=String(t||'').trim().toUpperCase(); if(!t)return null;
  var z=LEV_TICKERS[t]; if(z){var C=window.CALC||{},p=Number(C[z.priceKey]);return {t:t,kind:'lev',label:z.label,d:{},px:(p>0?p:null)};}
  var D=window.DET||{}; if(D[t])return {t:t,kind:'stock',label:(D[t].sth||D[t].sec||'個別株'),d:D[t],px:D[t].px};
  var R=window.TICKER_REF||{}; if(R[t])return {t:t,kind:'ref',label:(R[t].name||R[t].exchange||'FMP確認銘柄'),d:{},px:null};
  return null;
}
function _tickerPrice(t){var x=_tickerData(t);return x&&x.px!=null?Number(x.px):null;}
function _tickerItems(mode){
  var seen={},out=[];
  if(mode==='plan'){
    (window.SWING||[]).forEach(function(x){var t=String(x.t||'').toUpperCase();if(t&&!seen[t]){seen[t]=1;out.push({t:t,kind:'stock',label:'裁量候補'});}});
  }else{
    Object.keys(LEV_TICKERS).forEach(function(t){if(!seen[t]){seen[t]=1;out.push({t:t,kind:'lev',label:LEV_TICKERS[t].label});}});
    Object.keys(window.DET||{}).forEach(function(t){if(!seen[t]){seen[t]=1;var d=window.DET[t]||{};out.push({t:t,kind:'stock',label:(d.sth||d.sec||'個別株')});}});
    Object.keys(window.TICKER_REF||{}).forEach(function(t){if(!seen[t]){seen[t]=1;var r=window.TICKER_REF[t]||{};out.push({t:t,kind:'ref',label:(r.name||r.exchange||'FMP確認銘柄')});}});
  }
  return out;
}
function _tickerMatches(q,mode,limit){
  q=String(q||'').trim().toUpperCase(); if(!q)return [];
  return _tickerItems(mode).filter(function(x){return x.t.indexOf(q)>-1;}).sort(function(a,b){
    var pa=a.t.indexOf(q)===0?0:1,pb=b.t.indexOf(q)===0?0:1;if(pa!==pb)return pa-pb;
    if(a.kind!==b.kind){var rank={lev:0,stock:1,ref:2},ra=(rank[a.kind]!=null?rank[a.kind]:9),rb=(rank[b.kind]!=null?rank[b.kind]:9);return ra-rb;}return a.t<b.t?-1:1;
  }).slice(0,limit||8);
}
function tickerSuggest(inputId,boxId,mode){
  var el=document.getElementById(inputId),box=document.getElementById(boxId);if(!el||!box)return;
  var q=(el.value||'').trim().toUpperCase();if(!q){box.classList.remove('on');box.innerHTML='';return;}
  var a=_tickerMatches(q,mode,8);if(!a.length){box.innerHTML='<div class="tkno" style="padding:8px 10px">候補なし（ティッカーを確認）</div>';box.classList.add('on');return;}
  box.innerHTML=a.map(function(x){return '<button type="button" class="tksug" onclick="tickerPick(\''+mode+'\',\''+x.t+'\')">'
    +'<span class="tksug-t">'+x.t+'</span><span class="tksug-n">'+_escH(x.label||'')+'</span>'
    +(x.kind==='lev'?'<span class="tksug-k">レバETF</span>':(x.kind==='ref'?'<span class="tksug-k">FMP確認</span>':''))+'</button>';}).join('');box.classList.add('on');
}
function tickerPick(mode,t){
  var id=mode==='plan'?'swPick':'hldTk',bid=mode==='plan'?'swPickSuggest':'hldTkSuggest',el=document.getElementById(id),box=document.getElementById(bid);
  if(el)el.value=t;if(box){box.classList.remove('on');box.innerHTML='';}if(mode==='plan')swPickFill();else hldTickerInput(true);
}
function swPickInput(){tickerSuggest('swPick','swPickSuggest','plan');swPickFill();}
function _hldSetStrat(s){var seg=document.getElementById('hldStratSeg');if(!seg)return;[].forEach.call(seg.querySelectorAll('.stratbtn'),function(b){b.classList.toggle('on',b.getAttribute('data-strat')===s);});}
function hldTickerInput(fromPick){
  var el=document.getElementById('hldTk'),hint=document.getElementById('hldTkHint');if(!el)return;
  var t=(el.value||'').trim().toUpperCase();el.value=t;if(!fromPick)tickerSuggest('hldTk','hldTkSuggest','hold');
  var x=_tickerData(t);if(x&&x.kind==='lev')_hldSetStrat('lev');
  if(hint){hint.className='tkhint'+(x?' ok':(t?' warn':''));hint.textContent=!t?'':(x?(x.kind==='lev'?x.label+'として確認済み・戦略を「レバETF」に設定':(x.kind==='ref'?'FMPで銘柄確認済み（Yahoo価格は未取得・建値は手入力）':'ティッカー確認済み')):'候補から選択してください（未確認のまま登録不可）');}
}
function tickerGoPosition(t){goTab('t-alloc');var el=document.getElementById('hldTk');if(el){el.value=t;hldTickerInput(true);try{el.scrollIntoView({behavior:'smooth',block:'center'});}catch(e){}}}
function tkSearch(q){
  var box=document.getElementById('tkresults'); if(!box) return;
  q=(q||'').trim().toUpperCase();
  if(q.length<1){ box.innerHTML=''; return; }
  var D=window.DET||{}, out=_tickerMatches(q,'hold',20);
  if(out.length===0){ box.innerHTML='<div class="tkno">該当なし</div>'; return; }
  box.innerHTML=out.map(function(x){
    var d=D[x.t]||{};
    if(x.kind==='lev')return '<div class="tkr" onclick="tickerGoPosition(\''+x.t+'\')"><span class="tkr-s">'+x.t+'</span><span class="tkr-rs">レバETF・Positionsへ</span></div>';
    if(x.kind==='ref')return '<div class="tkr" onclick="tickerGoPosition(\''+x.t+'\')"><span class="tkr-s">'+x.t+'</span><span class="tkr-rs">FMP確認・Positionsへ</span></div>';
    return '<div class="tkr" data-tkone="'+x.t+'"><span class="tkr-s">'+x.t+'</span>'
...
function swInit(){
  ...
  if(hl && !hl._done){ hl._done=1;
    _tickerItems('hold').map(function(x){return x.t;}).sort().forEach(function(t){ var o=document.createElement('option'); o.value=t; hl.appendChild(o); });
  }
...
function _hldStratMeta(s){
  if(s==='lev')return {lab:'レバETF',cls:'strat-lev',exit:'NQゲート（青100／緑50／黄赤0）＋SOXLはSOXX条件'};
  return s==='swing'?{lab:'裁量',cls:'strat-swing',exit:'+25%で1/3利確 → 残りは21EMA Low/10日線でトレール'}:{lab:'Core 12',cls:'strat-core',exit:'ピーク×0.70 または 建値×0.75（途中利確しない）'};
}
...
function _hldEnv(tk){var C=window.CALC||{},x=_tickerData(tk)||{},d=x.d||{};return {envColor:(C.color||''),eRS63:(d.rs!=null?d.rs:null),eRS189:(d.rs189!=null?d.rs189:null),theme:(x.kind==='lev'?'レバETF':(d.sth||d.sec||'')),entryPx:(x.px!=null?x.px:null),envAsOf:_hldToday()};}
function _hldClearNote(){['hldSetup','hldConv','hldMemo'].forEach(function(id){var e=document.getElementById(id);if(e)e.value='';});}
function _hldNewTrade(tk,strat,date){ return {tradeId:_hldNewId(),schemaVersion:HLD_SCHEMA,t:String(tk||'').toUpperCase(),strat:(strat==='core'||strat==='lev'?strat:'swing'),status:'open',dt:date||_hldToday(),fills:[],exits:[],leg2:'none',filled:false,coreSold:false,remainingShares:0,remainingCost:0,filledSharesTotal:0,entryCostTotal:0,entryAvg:null,avgPx:null,px:null,initialStop:null,initialRiskTotal:null,stop:null,stopOverride:null,partialDone:false}; }
...
function _hldNormalize(src){
  ...
  h.t=String(h.t).toUpperCase(); if(_isLevTicker(h.t))h.strat='lev';else if(h.strat!=='core'&&h.strat!=='swing')h.strat='swing'; if(h.status!=='closed') h.status='open';
...
function currentSwingStop(h){ var x=_tickerData(h.t)||{},d=x.d||{}; if(h.stopOverride!=null) return h.stopOverride; if(h.exitType==='e21l'&&d.e21l!=null)return d.e21l; if(h.exitType==='s10'&&d.s10!=null)return d.s10; return h.stop!=null?h.stop:null; }
function hldAdd(){
  var tkEl=document.getElementById('hldTk'); if(!tkEl)return; var tk=(tkEl.value||'').trim().toUpperCase(); if(!tk)return;
  var info=_tickerData(tk);if(!info){var near=_tickerMatches(tk,'hold',3).map(function(x){return x.t;});alert('「'+tk+'」はティッカー候補で確認できません。'+(near.length?'候補: '+near.join(' / '):'入力を確認してください。'));return;}
  ...
  if(info.kind==='lev'){strat='lev';_hldSetStrat('lev');}else if(strat==='lev'){alert('レバETF戦略で登録できるのは TQQQ / SOXL です。個別株はCore 12か裁量スイングを選んでください。');return;}
  ...
  else if(strat==='core') stop=px*0.75;
  ...
  var ni=_hldNoteIn();if(strat==='lev'&&!ni.setup)ni.setup='レバETFゲート';
  var h=_hldInitFilled(_hldNewTrade(tk,strat,date),px,sh,date); h.initialStop=stop;h.stop=stop;h.initialRiskTotal=_hldRiskTotal(h,stop);h=Object.assign(h,ni,_hldEnv(tk));
...
function hldFromPlan(){
  ...
  if(_isLevTicker(tk)){alert(tk+' はレバETFです。裁量スイングとして誤登録しないため、下の「保有中」からレバETF戦略で追加してください。');return;}
...
function hldTakePartial(ev,i){
  if(ev&&ev.stopPropagation)ev.stopPropagation();var a=hldLoad();if(i<0||i>=a.length)return;var h=a[i],e=_hldEff(h),defShares=_hldTpRemaining(h),cur=_tickerPrice(h.t);if(cur==null)cur='';
...
function hldClose(ev,i){
  if(ev&&ev.stopPropagation)ev.stopPropagation();var a=hldLoad();if(i<0||i>=a.length)return;var h=a[i],rem=h.remainingShares||0,cur=_tickerPrice(h.t);if(cur==null)cur='';
...
function hldEdit(ev,i){
  ...
  try{
    ...
    if(h.strat==='lev'){h.initialStop=null;h.stop=null;h.initialRiskTotal=null;}
    else {var iv=window.prompt('【'+h.t+'】初期ストップ＝R計算基準（訂正。空欄可）',h.initialStop!=null?h.initialStop:'');if(iv===null)return;var ist=parseFloat(iv);h.initialStop=ist>0?ist:null;h.initialRiskTotal=_hldRiskTotal(h,h.initialStop);}
    ...
    else if(h.strat==='core'){var cv=window.prompt('【'+h.t+'】現在ストップ（空欄可）',h.stop!=null?h.stop:'');if(cv===null)return;var cn=parseFloat(cv);h.stop=cn>0?cn:null;}
...
function _mdRow(h){
  h=_hldNormalize(h)||h;var e=_hldEff(h),avg=h.status==='closed'?h.entryAvg:e.avg,sh=h.status==='closed'?h.filledSharesTotal:e.sh,orig=_hldOrigSh(h),cur=_tickerPrice(h.t),closed=h.status==='closed',L=[],hm=_hldStratMeta(h.strat);
  L.push('## '+h.t+'（'+hm.lab+'）'+(h.dt?'  ['+h.dt+']':'')+(closed?'  ✅決済済み':''));var l1=[];if(h.setup)l1.push('セットアップ: '+h.setup);if(h.conv)l1.push('確信度: '+h.conv);if(h.envColor)l1.push('環境(NQ): '+h.envColor);if(l1.length)L.push('- '+l1.join(' ／ '));
...
function _hldOpenRow(h,i){
  h=_hldNormalize(h)||h;var e=_hldEff(h),avg=e.avg,sh=e.sh,orig=_hldOrigSh(h),days=_hldDays(h.dt),cur=_tickerPrice(h.t),pl=(cur!=null&&avg!=null)?(cur/avg-1)*100:null,plc=pl==null?'mut':(pl>=0?'pos':'neg'),plHtml=pl!=null?'<div class="'+plc+'" style="font-size:10px">'+(pl>=0?'+':'')+pl.toFixed(1)+'%'+(cur!=null?' ・現在$'+cur:'')+'</div>':'',meta=_hldStratMeta(h.strat),swx='';
  ...
  else if(h.strat==='lev'){var C=window.CALC||{},lc=C.color||'未取得',le=Math.round((Number(C.e_lev)||0)*100),ll=['現在 '+lc+'・レバ露出 '+le+'%'];if(h.t==='SOXL'){if(!C.soxl_trend_ok)ll.push('<b class="neg">SOXXトレンド不適格 → 売却対象</b>');else if(C.soxl_entry_ok)ll.push('<b class="pos">SOXL投入帯</b>');else ll.push('<b class="warnc">投入帯外 → 既存分のみ・買い増し停止</b>');}swx+='<div class="mut hld-plan">'+ll.join('<br>')+'</div>';}
...
def _cp(tickers, all_tickers=None):
    """Ticker copy UI. When all_tickers is supplied, displayed and full-list copies are separated."""
    shown = list(dict.fromkeys(str(t) for t in (tickers or []) if t))
    allt = list(dict.fromkeys(str(t) for t in (all_tickers or []) if t)) if all_tickers is not None else shown
    if not allt:
        return ""
    # all_tickers may intentionally omit nothing; ensure every visible ticker remains copyable.
    for t in shown:
        if t not in allt:
            allt.append(t)
    if len(allt) <= len(shown):
        data = ",".join(allt)
        return (f'<button class="cp" data-tk="{data}" onclick="copyTk(event,this)">'
                f'コピー <span class="n">{len(allt)}</span></button>')
    return ('<span class="cpwrap">'
            f'<button class="cp" data-tk="{",".join(shown)}" onclick="copyTk(event,this)">'
            f'表示 <span class="n">{len(shown)}</span></button>'
            f'<button class="cp cp-all" data-tk="{",".join(allt)}" onclick="copyTk(event,this)">'
            f'全 <span class="n">{len(allt)}</span></button></span>')
...
def _det_json(m, names, tapset, extra=None):
    ...
    for t in sorted(tapset):        # #13 set反復順はPYTHONHASHSEED依存→sortedで決定的に
        ...
        def _vw_state(key):
            if key == "vwap_all" and not bool(r.get("vwap_all_valid")):
                return "対象外"
            if bool(r.get(key + "_reclaim")): return "下から回復"
            if bool(r.get(key + "_support")): return "タッチ反発"
        ...
        out[t] = {"rs": _r(r.get("rs"), 0), "rs189": _r(r.get("rs189"), 0),
                  ...
                  "vw63": _p(r, "vwap63_dist", 1), "vws63": _vw_state("vwap63"),
                  "vw252": _p(r, "vwap252_dist", 1), "vws252": _vw_state("vwap252"),
                  "vwall": _p(r, "vwap_all_dist", 1), "vwsall": _vw_state("vwap_all"),
                  "vwab": 1 if r.get("vwap_all_reclaim") else 0,
...
def _rotation_card(sectors, m):
    ...
    return (f'<div class="card"><div class="hdr"><h2>ローテーション時間軸（どこに資金が来ているか）</h2>{_cp(_rot_tk)}</div>'
...
def build_ema21_touch(m, W, s2t, cap=12, out=None):
    """21EMA pullback card built only from normalized per-symbol metrics.

    The prior implementation recomputed a wide-frame EMA/ATR inside the card
    and could turn a duplicate ticker column into a DataFrame, then fail with
    ``'DataFrame' object is not callable``.  All required values are now
    calculated once in ``compute_metrics`` and consumed here as scalars.
    """
    try:
        if m is None or m.empty or "ema21_touch3" not in m.columns:
            return ""
        adr = pd.to_numeric(m.get("adr"), errors="coerce")
        adr = adr.where(adr < 1, adr / 100.0)
        mask = (setup_eligible(m)
                & (m["rs"] >= LEADER_RS)
                & (m["rs189"] >= 90)
                & (m["close"] > m["sma200"])
                & (adr >= 0.03)
        ...
        if lead.empty:
            return ('<div class="card"><h2>21EMAタッチ <span class="h2en">21EMA Touch</span></h2>'
                    '<div class="sub">条件を満たす銘柄なし（リーダーが上向きEMA21まで押していない）。</div></div>')
        lead["_dev"] = lead["close"] / lead["ema21"] - 1.0
        lead["_adr"] = adr.reindex(lead.index)
        lead = lead[(lead["_dev"] >= -0.01)
                    & (lead["_dev"] <= lead["_adr"].clip(lower=0.02))]
        if lead.empty:
            return ('<div class="card"><h2>21EMAタッチ <span class="h2en">21EMA Touch</span></h2>'
                    '<div class="sub">タッチ後の終値位置が適正範囲の銘柄なし。</div></div>')
        lead = lead.sort_values(["rs189", "_dev"], ascending=[False, True]).head(cap)
        rows, tks = [], []
        for t, r in lead.iterrows():
            tks.append(t)
            am = r.get("ext50_atr")
            am_txt = "—" if pd.isna(am) else f"{float(am):.1f}×"
            am_cls = "neg" if pd.notna(am) and float(am) > 8 else "mut"
            td = int(r.get("ema21_touch_days")) if pd.notna(r.get("ema21_touch_days")) else 0
            tdl = "今" if td == 0 else f"{td}d"
            rows.append(
                f'<tr data-rs="{float(r.get("rs189")):.0f}" data-dev="{float(r.get("_dev"))*100:.2f}" '
                f'data-td="{td}" data-atr="{float(am) if pd.notna(am) else 999:.2f}" data-tkone="{t}">'
                f'<td><b>{_h(t)}</b></td>'
                f'<td class="l" style="font-size:10.5px;color:#94a3b8;white-space:normal;max-width:120px">{_h(theme_of(t, s2t))}</td>'
                f'<td>{float(r.get("rs189")):.0f}/{float(r.get("rs")):.0f}</td>'
                f'<td>{float(r.get("_dev"))*100:+.1f}%</td><td class="{am_cls}">{am_txt}</td>'
                f'<td class="mut" style="font-size:10.5px">{tdl}</td></tr>')
        if out is not None:
            out.extend(tks)
        return (f'<div class="card"><div class="hdr"><h2>21EMAタッチ <span class="h2en">21EMA Touch</span></h2>{_cp(tks)}</div>'
                f'<div class="sub">RS63≥{LEADER_RS}・RS189≥90・時価総額≥$1B・$vol≥$10M・ADR≥3%のリーダーで、'
                '<b>上向きEMA21に直近3日以内にタッチし、終値が回復</b>。50MAから8ATR超は伸び切り警戒。</div>'
                '<table class="tbl"><thead><tr><th>銘柄</th><th>テーマ</th><th>RS189/63</th>'
                '<th>EMA21乖離</th><th>50MA(ATR)</th><th>タッチ</th></tr></thead><tbody>'
                + ''.join(rows) + '</tbody></table></div>')
    except Exception as e:
        import traceback as _tb
        sys.stderr.write("[ema_touch] %s\n" % _tb.format_exc(limit=4))
        return ""
...
def build_weekly_unique(W, m, s2t, picks, eq_s, macro, mkt, rates_ctx, press_ctx):
    ...
    try:
        ...
        if C is not None and len(C) >= 6:
            ...
            r5 = r5.dropna()
            th_map = {t: theme_of(t, s2t) for t in r5.index}
            import collections
            ...
            try:
                ...
                if tks:
                    ...
                    hold = (f'<div class="mut" style="font-size:10.5px;margin:8px 0 4px;font-weight:800">保有12の週次 {_cp([t for t, _ in hs])}</div>'
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    try:
        _vw = pd.Series(False, index=m.index)
        for _vk in ("vwap63", "vwap252", "vwap_all"):
            for _vs in ("_near", "_support", "_reclaim"):
                if _vk + _vs in m.columns:
                    _vw |= m[_vk + _vs].fillna(False).astype(bool)
        _wadd(list(m.index[_elig & (m["rs189"] >= 80) & _vw]), "Multi VWAP")
    except Exception: pass
    try:
    ...
    html = ("<!doctype html><html lang='ja'><head><meta charset='utf-8'>"
            ...
            + "<script>window.TICKER_REF=" + _json_for_script(mkt.get("ticker_ref") or {}) + ";</script>"
...
def selftest(html, picks, setups, sectors, mkt=None, W=None, cutdate=None):
    ...
    if ("兆候が4つ以上そろった銘柄" in html or "52週高値−15%以内" in html
            or "ENTRY候補だけ" in html or "スコア65以上" in html):
        errs.append("旧/未検証コンフルエンス（個数数え/高値圏加点/配点合否）が残存")
    if CONFLUENCE_RS189_MIN != 95:
        errs.append("confluence facts: RS189表示下限が95から変更")
    try:
        _base = dict(vcp_ready=True, ema21_touch3=False, stage2=True, higher_lows=True,
                     contractions=2, pivot_dist=-0.02, true_breakout=False, pp_days=0,
                     rvol_pre=1.1, pchg=0.01, close_pos=0.7, rs189=94, rs=91,
                     rsline_newhigh=True, uvdv20=1.2, ext50_atr=4.5, adr=0.05,
                     ret5=0.01, breakout_failure=False, vwap63_support=False,
                     vwap63_reclaim=False, vwap252_support=False, vwap252_reclaim=False,
                     vwap_all_support=False, vwap_all_reclaim=False)
        ...
        _near = dict(_base)
        _near.update(vcp_ready=False, pp_days=None, rsline_newhigh=False, uvdv20=0.9,
                     vwap63_near=True, vwap63_dist=0.01,
                     vwap63_support=False, vwap63_reclaim=False, rvol_pre=1.0)
    ...
    if mkt:
        ...
        for _tk, _d in (mkt.get("distrib") or {}).items():
            ...
            _fi = _d.get("ftd_iso")
            if _fi:
                _leak = [e for e in (_d.get("events") or [])
                         if e.get("cycle") == "current" and e.get("date") <= _fi]
                if _leak:
                    errs.append(f"{_tk}: pre-FTD day leaked into current cycle")
                if any((e.get("ftd_session") or 0) < 1 for e in (_d.get("events") or [])
                       if e.get("cycle") == "current"):
                    errs.append(f"{_tk}: ftd_session must start at 1")
        _rh = mkt.get("reg_hist") or {}
        if _rh:
            if "f1_now" not in _rh:
                errs.append("F1 history did not expose f1_now")
            if _rh.get("f1_status") in ("OK", "PARTIAL"):
                # 分子は観測できた脱落のみ（unknownが混入していないこと）。状態に関わらず常に成立すべき。
                ...
                _fcov = _rh.get("f1_coverage") or 0
                if _rh.get("f1_status") == "OK" and _fcov < F1_COV_FULL:
                    errs.append("F1 reported OK below full coverage")
                if _rh.get("f1_status") == "PARTIAL" and not (F1_COV_MIN <= _fcov < F1_COV_FULL):
                    errs.append("F1 reported PARTIAL outside the partial coverage band")
        ...
        if shutil.which("node"):
            ...
            if _js.strip():
                ...
                if _r.returncode != 0:
                    ...
                    __JS__
                    try{
                      window.CALC={color:"Yellow",fx:150,e_ind:1.0,e_lev:0.5,alloc_ind:70,alloc_lev:30,n:12,tqqq:88.25,soxl:42.50,
                        names:[{t:"AAA",px:100,rs:95,rk:1,d52:-5,r5:1.2,drop:false,clx:false},
                               {t:"BBB",px:50,rs:90,rk:2,d52:-12,r5:-1.0,drop:true,clx:false}]};
                      window.DET={NVDA:{px:180,rs:95,sth:"半導体"}};
                      window.TICKER_REF={NEWX:{name:"Alpha Systems",exchange:"NASDAQ",source:"FMP確認"}};
                      if(typeof calcRun==="function"){calcRun();}
                      // Positionsティッカー回帰: TQQQ/SOXLを候補・価格・専用戦略として保持する
                      const tq=_tickerData("tqqq"),sx=_tickerData("SOXL");
                      if(!tq||tq.kind!=="lev"||Math.abs(tq.px-88.25)>1e-9) throw new Error("TQQQ catalog");
                      if(!sx||sx.kind!=="lev"||Math.abs(sx.px-42.5)>1e-9) throw new Error("SOXL catalog");
                      if(!_tickerMatches("TQ","hold",8).some(x=>x.t==="TQQQ")) throw new Error("TQQQ suggest");
                      const rx=_tickerData("NEWX");
                      if(!rx||rx.kind!=="ref"||rx.px!==null) throw new Error("FMP reference catalog");
                      if(!_tickerMatches("NEW","hold",8).some(x=>x.t==="NEWX")) throw new Error("FMP reference suggest");
                      const lh=_hldNormalize(_hldNewTrade("TQQQ","lev","2026-07-01"));
                      if(lh.strat!=="lev"||_hldStratMeta(lh.strat).lab!=="レバETF") throw new Error("leveraged strategy");
                      ...
                      console.log("CALCRUN_OK"); console.log("TRADELEDGER_OK"); console.log("LEVETF_OK");
                    ...
                    if "LEVETF_OK" not in _out:
                        errs.append("PositionsレバETF回帰テスト失敗: 候補/価格/戦略分類")
...
# ----------------------------------------------------------------------------- main
def _scard(label, fn, *args, default=None, **kwargs):
    """表示カード/派生データのビルダーを安全に呼ぶ。スケール時の想定外データ形状で個別に失敗しても、
       その1カードだけ default に落として全体ビルドは継続する（本体HTMLは既に計算済み・可用性優先）。"""
    try:
        return fn(*args, **kwargs)
    except Exception as _e:
        sys.stderr.write("[card:%s] failed, degrading: %r\n" % (label, repr(_e)[:200]))
        return default
...
def main():
    ...
    if _san.get("bad_price_cells") or _san.get("incoherent_cells") or _san.get("dropped_tickers"):
        _dp = _san.get("dropped_tickers") or []
        sys.stderr.write("[sanitize] 局所異常を隔離: 非正/非有限セル=%d, OHLC不整合セル=%d, 補完セル=%d, 除外銘柄=%d (%s), 残銘柄=%d\n" % (
            _san.get("bad_price_cells", 0), _san.get("incoherent_cells", 0), _san.get("filled_cells", 0),
            len(_dp), ",".join(_dp[:8]), _san.get("kept", 0)))
        if _dp:
            sys.stderr.write("[sanitize] 除外理由=上場後の有効本数が%d本未満、または上場後の充足率が%.0f%%未満\n"
                             % (MIN_HISTORY_BARS, 0.20 * 100))
    ...
    _ftd = _scard("ftd", build_ftd, macro, default=[])
    _distrib = _scard("distrib", build_distribution, macro, _ftd, default={})
    mkt = {"distrib": _distrib, "ftd": _ftd,
           "perf": _scard("perf", build_perf, macro, _extras),
           "sector_ranks": _scard("sector_ranks", build_sector_ranks, macro, _extras),
           "breadth_ts": _scard("breadth_ts", build_breadth_ts, W),
           "dollarvol_ts": _scard("dollarvol_ts", build_dollarvol_ts, W, macro, lookback=CHART_LB),
           "updown_ts": _scard("updown_ts", build_updown_vol_ts, W, macro),
           "credit_ts": (_scard("credit_ts", build_ratio_ts, macro, "HYG", ["IEI"]) if (macro.get("IEI") is not None) else _scard("credit_ts", build_ratio_ts, macro, "HYG", ["LQD"])),
           "credit_den": ("IEI" if (macro.get("IEI") is not None) else "LQD"),
           "defensive_ts": _scard("defensive_ts", build_ratio_ts, macro, "RSPD", ["RSPS", "RSPU"]),
           "vixterm_ts": _scard("vixterm_ts", build_vix_term_ts, macro),
           "expmove": _scard("expmove", build_expected_move, macro, m, picks),
           "adline_ts": _scard("adline_ts", build_adline_ts, W),
           "trend_prev": _tprev,
           "trend_hist": _thist,
           "asof_bar": asof_bar,
           ...
           "transition_leaders": _scard("transition_leaders", build_transition_leaders, m, macro)}
    mkt["rrg"] = _scard("rrg", build_rrg, m, s2t)
    mkt["macro_context"] = macro_context
    mkt["fred_all"] = _fred_all            # OVの値注入・データ健全性バナーの統一データ源（全系列）
    mkt["rates_card"] = _scard("rates_card", build_rates_card, macro_context, asof=asof_bar, default="")
    mkt["macro_pulse"] = _scard("macro_pulse", build_macro_pulse, macro_context, macro_pressure_ctx, asof=asof_bar, default="")
    mkt["rates_card"] += (_scard("rates_insight", build_rates_insight, macro_context, asof=asof_bar, default="") or "")
    mkt["macro_pressure_ctx"] = macro_pressure_ctx
    mkt["pressure_card"] = _scard("pressure_card", build_macro_pressure, macro_pressure_ctx, macro, asof=asof_bar, default="")
    ...
    mkt["reg_hist"] = _scard("reg_hist", compute_regime_history, W)
    mkt["regime_state"] = _scard("regime_state", compute_regime_state, m, hist=mkt["reg_hist"])
    try:
    ...
    mkt["rs_continuity"] = _scard("rs_continuity", build_rs_continuity, W, m, top_n=CONTINUE_TOP_N)
    mkt["lead_pulse"] = _scard("lead_pulse", build_leadership_pulse, m, mkt["rs_continuity"], s2t)
    mkt["rrg_etf"] = _scard("rrg_etf", build_rrg_etf, macro, _extras, themes=MICRO_ETFS)
    mkt["etf_hier"] = _scard("etf_hier", build_etf_hier, m, s2t)
    ...
    mkt["calc"] = _scard("calc", build_calc_extras, picks, sar[0], cand)
    ...
    mkt["quality"] = dict(
        ...
        yf_reference_recovered=_fq.get("yf_reference_recovered", 0),
        fmp_mode=_fq.get("fmp_mode"), fmp_queries=_fq.get("fmp_queries", 0),
        fmp_cache_hits=_fq.get("fmp_cache_hits", 0), fmp_confirmed=_fq.get("fmp_confirmed", 0),
    ...
    _pending_state = dict(date=_today, gate=sar[0], mri=round(aux["cur"], 1),
                        market_cycle_history=_cycle_hist,
    ...
    if production_run:
        ...
        if _critical_fail:
            # HTML/shareは既に書けている。state/trend/logはトランザクション(全か無か+ロールバック)なので、
            # 失敗しても部分コミットは残らず旧ファイルは無傷。次回実行で再試行される。
            # PermissionError/OSError等の環境要因(read-only/権限)や一時的失敗で、成功した本体ビルドを
            # 赤くしない。警告は目立つ形で残す。
            _is_env = any(("PermissionError" in e or "OSError" in e or "ReadOnly" in e
                           or "Read-only" in e or "No space" in e or "Errno 13" in e or "Errno 30" in e)
                          for e in _critical_fail)
            print("[commit] WARNING: state/trend/log を書き込めませんでした（トランザクションはロールバック済み・旧ファイル保持）:")
            for e in _critical_fail:
                print("  -", e)
            if _is_env:
                print("[commit] 環境要因（権限/read-only/容量）と判断。ダッシュボード本体は生成済みのためビルドは成功とします。")
                # exit 0（本体成功）。
            else:
                # 環境要因でない失敗も、本体が出力済みなら致命にしない（個人用自動ダッシュボードの可用性優先）。
                # ただしログには残す。厳格に止めたい場合は V38_STRICT_COMMIT=1 で従来どおり非0にできる。
                if os.environ.get("V38_STRICT_COMMIT") == "1":
                    sys.exit(2)
                print("[commit] 本体は生成済みのためビルドは成功として継続します（厳格化は V38_STRICT_COMMIT=1）。")
