...
Command Center — dashboard builder (v2・確定)
  A. 個別スリーブ(40%): N=15, 189日RS上位 × 50日SMA>200日SMA × 出来高金額$10M/日 × 株価$5以上,
     各1/12均等(テーマ上限なし)。継続=50>200日SMA&189日RS上位24(2N)。週足SARはアブレーションで撤廃。
     ピーク×0.70トレールで利を伸ばし割れたら全売り, 初期ストップ建値×0.70(-30%)。ストップ後クールダウンなし（検証: 撤廃版がCAGR+3.6〜4.3pt・Sharpe+0.08で優位・DD悪化なし・年間エントリー数不変）。
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    port = _liq_bar() + _mkt_section("① 現在の構成", "個別株スリーブ（N=12・等ウェイト）") + port + _mkt_section("② 控え", "次の入替で上がってくる候補") + deck_card
    search_card = (
    ...
    watch = cap_filter_bar + _mkt_section("① 銘柄を調べる", "ティッカー検索 → タップで詳細") + search_card + _mkt_section("② セットアップで絞る", "複数条件ヒット＝注目度が高い") + overlap_card + setup_card + _mkt_section("③ 今日の値動き", "新高値圏＝需給が強い") + newhigh_card
    ...
    rules = (
        ...
        f'<li><b>初期ストップ</b>：建値<b>×0.70（−30%）</b>で全売り。<b>クールダウンなし</b>——切られてもRS上位12・適格なら次の隔週で即復帰（バックテストで撤廃が優位・DD悪化なし）。</li>'
        f'</ul>'
