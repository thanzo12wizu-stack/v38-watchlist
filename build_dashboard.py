...
def build_confluence_watch(m, cand=None, cap=20, min_rs=95):
    ...
    _cpall = [t for _n, _rs, t, _r, _sg in rows]
...
def build_ema21_touch(m, W, s2t, cap=12, out=None):
    ...
    try:
        ...
        if C is None or L is None or len(C) < 30 or m is None or m.empty:
            return ""
        # スケール時の防御: 重複index/列があると df[t] が Series でなく DataFrame を返し、後段の
        # float(...)や比較が 'DataFrame' object is not callable 等で落ちる。先頭で重複を排除する。
        if getattr(m.index, "has_duplicates", False):
            m = m[~m.index.duplicated(keep="first")]
        def _dedup_cols(df):
            if df is not None and hasattr(df, "columns") and df.columns.has_duplicates:
                return df.loc[:, ~df.columns.duplicated(keep="first")]
            return df
        W = {k: _dedup_cols(v) for k, v in W.items()} if isinstance(W, dict) else W
        C = W.get("Close"); L = W.get("Low")
