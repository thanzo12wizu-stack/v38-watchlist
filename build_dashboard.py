...
def _spark(vals, asof=None, days=60, color="#5b7fb0"):
...
def build_macro_pulse(rates_ctx, press_ctx, asof=None):
    ...
    def _one(d, lab, cat, is_bp, note, metrics_fn):
        ...
        return (f'<div class="rrow"><div class="lft"><div class="nm">{lab}{badge}</div>'
                ...
                f'<span class="hint">水準 {pct_txt}{spd_txt}</span>'
                f'<span class="hint">{note}</span></div>{sp}'
                f'<div class="foot">観測 {eff or "—"}・{src_lab}</div></div>')
    ...
    if not rows:
        return ""
    return (f'<div class="card"><h2>マクロの脈拍 <span class="h2en">Macro Pulse</span></h2>'
            f'<div class="sub">毎日見るのはこの2本だけ。金利・マクロの全量はWeeklyタブ。</div>'
            f'{"".join(rows)}</div>')
...
def build_macro_pressure(fred_ctx, macro, asof=None):
    ...
    def _row(cat, lab, vals, ld, src, unit, cached_at=None, is_bp=False,
        ...
        return (f'<div class="rrow"><div class="lft"><div class="nm">{lab}{badge}</div>'
                ...
                f'<div class="rgt"><span class="big">{lvl_txt}</span>'
                f'<span class="chg mut">{chg_txt}<span class="mut">/20日</span></span>'
                f'<span class="hint">{pct3_hint} {pct_txt}{spd_txt}</span>{note}</div>{sp}'
...
def _rs_continuity_card(cont):
    ...
    return (
        ...
        '<div class="sub">Top10滞在、Top24滞在率、連続日数、順位・RS変化。</div><details class="mut" style="margin:2px 0 6px"><summary style="font-size:11px;cursor:pointer">分類の定義</summary><div class="sub">直近21営業日のRS189順位で機械分類（正式選定には使わない）：<b>定着</b>=Top10に15日以上×Top24滞在率75%以上×長期RS悪化なし／<b>継続</b>=Top24圏内を維持／<b>失速中</b>=順位が21日で8以上悪化、またはRS63・126がともに低下／<b>再浮上</b>=一度圏外→直近でTop10に5日以上／<b>新規急浮上</b>=順位を21日で10以上上げて浮上直後／<b>一日急騰型</b>=滞在2日以下の瞬間芸（定着未確認）。</div></details>'
...
.rrow .na{font-size:13px;color:#5f6b7e}
.spark{grid-column:1 / -1;display:block;margin:2px 0 0;opacity:.85}
...
def build_ema21_touch(m, W, s2t, cap=12):
    ...
    try:
        ...
        return (f'<div class="card"><h2>21EMAタッチ <span class="h2en">21EMA Touch</span></h2>'
                f'<div class="sub">エントリー基準（RS63≥{LEADER_RS}・RS189≥90・時価総額≥$1B・$vol≥$10M・ADR%≥3%・200日線上）を満たすリーダーのうち、<b>上向きEMA21の定位置に押した</b>銘柄。'
                f'条件: 直近3営業日内に安値がEMA21タッチ・終値はEMA21〜<b>+1×ADR%</b>（銘柄のボラに適応）。50MA(ATR)=50日線からのATR%倍率〔(50MA比%上昇)÷(ATR÷現在値)〕。作者(jfsrev)は<b>7〜10倍超で利確開始</b>＝伸び切りの目安。<b>基本のエントリーはこのレンジで行う</b>（執行ゾーンの観測・正式選定には使わない）。</div>'
...
def build_weekly_unique(W, m, s2t, picks, eq_s, macro, mkt, rates_ctx, press_ctx):
    ...
    try:
        ...
        if len(th) >= 6 and th[-1] and th[-6]:
            c0, c5 = th[-1][1], th[-6][1]
            items.append(f"NQ色 {_COLOR_JP.get(c5, c5)} → <b>{_COLOR_JP.get(c0, c0)}</b>" if c0 != c5
                         else f"NQ色 <b>{_COLOR_JP.get(c0, c0)}</b>のまま")
        # 50MA上比率
        ...
        if C is not None and len(C) >= 6:
            ...
            try:
                ...
                if tks:
                    ...
                    hold = ('<div class="mut" style="font-size:10.5px;margin:8px 0 3px;font-weight:800">保有12の週次</div>'
                            + "".join(f'<span class="chip" style="margin:2px 3px 0 0">{t} <b class="{"pos" if v>0 else "neg"}">{v:+.1f}%</b></span>' for t, v in hs))
            except Exception:
                pass
            board_html = (f'<div class="card"><h2>週次騰落ボード <span class="h2en">Weekly Movers</span></h2>'
                          f'<div class="sub">テーマ内中央値の5営業日リターン。RRG（中期RS）とのズレ＝ローテーション初動の観測。正式選定には使わない。</div>'
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    _pickup_note = ('<div class="card" style="border-color:rgba(148,163,184,.25);background:rgba(148,163,184,.04)">'
                    '<div class="sub" style="margin:0"><b style="color:#94a3b8">研究・観測用</b>：このタブのシグナルは<b>正式選定（RS上位・隔週月曜）には使わない</b>。'
                    'その日の裁量で仕掛けない、が正式ルール。ここは翌週以降の候補観察と学習のための面。</div></div>')
    today = (_liq_bar() + _pickup_note
    ...
    market = (
        ...
        + _mkt_section("② 温度計（上げの質）", "広がり・参加度・先導株", en="Thermometer")
        ...
        + _mkt_section("④ 転換シグナル（攻めの装填）", "FTDと復帰候補（調整中のみ注視）", en="Reversal Signals")
    ...
    mkt_weekly = (
        _wk_concl
        + (mkt.get("wk_diff") or "")
        + _mkt_section("① 今週の相場", "地合いの推移・NQ色の帯・指数リターンで1週間を答え合わせ", en="Market Week")
...
def selftest(html, picks, setups, sectors, mkt=None, W=None, cutdate=None):
    ...
    for sid in ["t-today","t-market","t-weekly","t-equity","t-port","t-alloc","t-rs","t-rotation","t-sector","t-rules"]:
...
def main():
    ...
    if do_self:
        ...
        if errs:
            ...
            sys.exit(1)
        bs_, bys_ = leaders
        print("SELFTEST OK (10 tabs (daily/weekly/equity), restored pickup layout, X-ready market digest, market health v2, emergency brake, RS compare+IN/OUT+continuity, N=%d, leaders=%d, buys=%d, sectors=%d, no None/nan)" %
