...
def compute_metrics(W, order, s2i=None):
    ...
    for t in order:
        ...
        recs.append(dict(
            ...
            maxabs1d=maxabs1d, lo10=lo10, ucr=ucr, tc3=tc3,
...
def track_holdings(prev, picks, color, asof_date, m=None):
    ...
    def _process(t, px, ema21=None, lo10=None):
        ...
        # 出口（2026-07-07確定）: トレール=10日安値×0.998、21EMA×0.99は建て時の初期ストップ成分。
        #   現値より上の線は無効（21EMA割れ時にema線が上に来るのを除外）→ 常に現値より下を採用。
        ...
        _cands = [x for x in (lo_stop, ema_stop) if x > 0 and x < px]
        ...
        try:
            ...
            days = 1
        _which = "10日安値" if (stop == lo_stop and lo_stop > 0) else ("21EMA" if (stop == ema_stop and ema_stop > 0) else "—")
...
def build_leader_breadth(m, macro=None):
    ...
    dropped_list = sorted(
        [(t, float(rank_now.get(t, 9999)), _dv(t)) for t in then24
...
def _leader_breadth_card(lb):
    ...
    drop_chips = ("".join(
        ...
        f'<span class="mut" style="font-size:10px">{"圏外" if nr>900 else str(int(nr))+"位"}</span></span>'
        for t, nr, dv in dl) if dl else '<span class="mut">なし</span>')
    fade_chips = ("".join(
        f'<span class="chip warn" data-liq="{dv/1e6:.1f}" data-tkone="{t}">{t} '
...
def _rotation_card(sectors, m):
    ...
    return (f'<div class="card"><h2>ローテーション時間軸（どこに資金が来ているか）</h2>'
            ...
            f'<table><tr><th class="l">サブテーマ</th><th>状態</th><th>短期</th><th>中期</th>'
            f'<th>長期</th><th>広がり</th><th class="l">主導株</th></tr>{rows}</table></div>')
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    for i, (t, r) in enumerate(cand.iterrows(), 1):
        ...
        rows.append(
            ...
            f'{_state_badge(code, nu_tag, nu_cls, _rsn)}{_er_badge(t, _er, mkt.get("asof_bar"))}{_adr_badge(r.get("adr"))}{_entry_badge(r.get("rs"), r.get("close"), r.get("ema21"))}{_fade_badge(r)}</div></td>'
    ...
    port = (f'<div class="card"><div class="hdr"><h2>ポートフォリオ（個別株スリーブ）</h2>'
            ...
            f'<div class="note rk-note"><b>買うのはリーダー（63日RS≥85・200MA上）の中で上位{N_PORT}だけ</b>。'
            f'<b>出口まで</b>＝トレール（10日安値/21EMA）までの距離＋<b>想定±Y%</b>（約1ヶ月1σ）。'
            f'<b class="shallow">浅</b>＝出口が想定変動の内側／<b class="r3b">+3R⅓利確</b>＝+3Rで1/3利確（残りは10日安値割れ）／'
            f'<b class="entb ent-good">◎ゾーン内</b>＝21EMA−1〜+4%の理想の拾い場（RS97+は伸びを追わない）／'
            f'<b class="fadeb fade-lo">勢い細り</b>＝189は強いが短期RS低下＝降りどき警戒／'
            f'<b>出遅れ</b>＝買う対象（タグのみ）・<b>選外（小型創薬/リーダー外）</b>＝グレーアウト。</div></details>'
    ...
    for rank, (t, r) in enumerate(deck.iterrows(), start=_last_pos+2):
        ...
        wrows.append(
            ...
            f'<td class="l tk">{t}{_risk_flag_badge(t, r.get("close"))}{_exbadge}'
    ...
    today = (_liq_bar()
             + _mkt_section("① 圧縮コイル（VCP）", "締まり＝エネルギー蓄積・ブレイクで放出") + _vcp_card(vcp_rows)
             + _mkt_section("② パターン別スキャナー", "ベース＆反転パターン（眼を鍛える用）") + _patterns_card(patterns)
             + _mkt_section("③ 本日の注目", "押し目・ブレイク近・保ち合いの総合抽出") + _buy_today_card(buytoday)
    ...
    body = (build_today_action(sar, mkt, asof)
            ...
            + _mkt_section("① ローテーション時間軸", "どの詳細サブテーマに資金が来ているか（短期・中期・長期）")
            + f'{_rotation_card(sectors, m)}'
            + _mkt_section("② 強い業種の主導株", "強いグループ×個別も強い＝順張りの一等地")
            + f'{sector_leaders}'
            + _mkt_section("③ サブテーマ別RS（詳細）", "見出しタップで並べ替え・行タップで構成銘柄")
            + f'{sector}'
            + _mkt_section("④ ローテーションの向き（RRG）", "改善→主導→弱化→停滞の時計回り")
