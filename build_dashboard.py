...
def fetch_live(order, period="3y", chunk=100, retries=3):
    ...
    _missed = [y2b.get(y, y) for y in yfs if y not in store]
    if _missed:
...
def _swing_planner_card():
    ...
    return (
        ...
        'onfocus="tickerSuggest(\'swPick\',\'swPickSuggest\',\'plan\')"><datalist id="swList"></datalist></div>'
        ...
        'onfocus="tickerSuggest(\'hldTk\',\'hldTkSuggest\',\'hold\')"></div>'
...
function _tickerMatches(q,mode,limit){
  ...
  return _tickerItems(mode).filter(function(x){return x.t.indexOf(q)>-1;}).sort(function(a,b){
    ...
    if(a.kind!==b.kind)return a.kind==='lev'?-1:1;return a.t<b.t?-1:1;
  }).slice(0,limit||8);
}
function tickerSuggest(inputId,boxId,mode){
  ...
  box.innerHTML=a.map(function(x){return '<button type="button" class="tksug" onclick="tickerPick(\''+mode+'\',\''+x.t+'\')">'
    ...
    +(x.kind==='lev'?'<span class="tksug-k">レバETF</span>':'')+'</button>';}).join('');box.classList.add('on');
}
...
function hldTickerInput(fromPick){
  ...
  if(hint){hint.className='tkhint'+(x?' ok':(t?' warn':''));hint.textContent=!t?'':(x?(x.kind==='lev'?x.label+'として確認済み・戦略を「レバETF」に設定':'ティッカー確認済み'):'候補から選択してください（未確認のまま登録不可）');}
}
