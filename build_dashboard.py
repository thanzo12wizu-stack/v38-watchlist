...
Command Center — dashboard builder (最終確定 2026-07-05)
  A. 個別スリーブ(70%): N=12, 189日RS上位 × 50日SMA>200日SMA × 出来高金額$10M/日 × 株価$5以上,
     ...
     出口(確定・二層): 層1=初期ハードストップ max(10日安値×0.998,21EMA×0.99)＋建て時istop→+3Rで1/3利確(建値移動なし)。
...
def compute_metrics(W, order, s2i=None):
    ...
    for t in order:
        ...
        recs.append(dict(
            ...
            ret63=ret63, ret189=ret189, ret189_l5=ret189_l5, ret189_l20=ret189_l20, ret21=ret21, hi52=hi52, dist52=dist52, pos52=pos52,
...
def _fade_badge(r):
    """防御アラート: 189日RSはまだ強い(≥80)のに短期(63日/21日)RSが細り＝勢い低下の初動。
       順張りの"降り遅れ"対策の早期警戒。表示専用（自動売却しない・降りは確定出口線で）。"""
    try:
...
def build_rrg(m, s2t, win=63, mom=10):
    """s2t大テーマ15個を、構成銘柄のRS189中央値(横=強さ)×RSランクΔ中央値(縦=勢い)で4象限配置。
       尾(tail)=5日前・20日前の位置も計算し、ローテの軌跡を可視化。表示専用。"""
    from collections import defaultdict
    ...
    for t in m.index:
        ...
        if v and len(v) >= 1 and v[0]:
            big[v[0]].append(t)
    # 各時点のRS189百分位（尾の座標用）: 現在・5日前・20日前
    def _rank_col(col):
        return m[col].rank(pct=True) * 100 if col in m.columns else None
    rk_now = _rank_col("ret189") if "ret189" in m.columns else (m["rs189"] if "rs189" in m.columns else None)
    rk_l5  = _rank_col("ret189_l5")
    rk_l20 = _rank_col("ret189_l20")
    pts = []
    for theme, tickers in big.items():
        ...
        if x != x:
            continue
        # 尾の座標: 各過去時点のRS百分位中央値（x）と、その時点の勢い（y=次時点との差）
        tail = []
        try:
            if rk_now is not None and rk_l5 is not None and rk_l20 is not None:
                xn = float(np.nanmedian(rk_now.loc[idxs]))
                x5 = float(np.nanmedian(rk_l5.loc[idxs]))
                x20 = float(np.nanmedian(rk_l20.loc[idxs]))
                # 各点の勢い = その時点のxと1つ前(古い)時点のxの差
                tail = [dict(x=x20, y=x20 - x20),   # 最古（勢いは0基準）
                        dict(x=x5,  y=x5 - x20),
                        dict(x=xn,  y=xn - x5)]
        except Exception:
            tail = []
        q = "主導" if (x >= 50 and y >= 0) else "弱化" if x >= 50 else "改善" if y >= 0 else "停滞"
        cross = bool(y >= 3 and q in ("改善", "主導"))
        disp = theme.split(". ", 1)[-1] if ". " in theme else theme
        pts.append(dict(tk=theme, ja=disp, x=x, y=y, q=q, cross=cross,
                        semis=("半導体" in theme), tail=tail))
...
def _rrg_svg(pts, cxv=50):
    ...
    def overlaps(bx):
        ...
        return False
    marks = ""; labels = ""; tails = ""
    # 尾（軌跡）: 各テーマの過去→現在をつなぐ線＝ローテの動きを可視化
    for p in pts:
        tl = p.get("tail") or []
        if len(tl) >= 2:
            col = _RRG_COL.get(p["q"], "#9aa4b2")
            pathpts = " ".join(f'{X(t["x"]):.1f},{Y(t["y"]):.1f}' for t in tl)
            tails += f'<polyline points="{pathpts}" fill="none" stroke="{col}" stroke-width="1.4" opacity="0.35"/>'
            # 尾の中間点に小さなドット
            for t in tl[:-1]:
                tails += f'<circle cx="{X(t["x"]):.1f}" cy="{Y(t["y"]):.1f}" r="2" fill="{col}" opacity="0.3"/>'
...
def _rrg_card(pts, pts_all=None, sub_data=None, cxv=50, title="テーマ・ローテーション（RS基準）", desc=None):
    ...
    if has_all:
        toggle = ('<div class="rrgtog">'
                  ...
                  '</div>')
    _desc = desc or ('大テーマを構成銘柄のRSで配置(横=RS189中央値・50が市場中央／縦=RSランクの1週間変化＝資金の向き)。<b>改善→主導→弱化→停滞</b>の時計回りに循環。'
                     '<b>尾＝20日前→5日前→現在の軌跡</b>（動きの向きでローテが見える）。<b>⚡＝勢いがプラス側</b>。■＝半導体。<b>› 付きチップをタップ→ニッチ業種と構成銘柄</b>。')
...
def build_pullback_screener(m, k=20):
    ...
    try:
        ...
        for t, pts, lit, r in scored:
            ...
            rows += (f'<tr data-tkone="{t}"><td class="l tk">{t}{_tkcopy(t)}</td>'
...
def build_regime_alerts(m, st=None):
    """レジーム先行警戒灯 F1-F3（表示専用・売買非連動）。"""
    if st is None:
    ...
    try:
        ...
        return (
            f'<div class="card reg-card"><div class="hdr"><h2>レジーム警戒灯</h2>'
            f'<span class="reg-hdr {hcls}">{hdr}</span></div>'
            ...
            f'</div>'
            f'<div class="note">相場全体の内部劣化を早期に捉える先行灯（表示専用・売買はしない）。<b>2灯以上で防御へ心構えを切替</b>。'
            ...
            f'生存バイアスで全指標は過小＝実際はもっと早く点く。個別「勢い細り」バッジのポート集約版。</div></div>')
    except Exception as e:
...
def _leader_run_card(lr):
    ...
    return (f'<div class="card"><h2>先導株モメンタム・ラン</h2>'
            ...
            f'<b class="mr-acc">加速</b>＝ラン拡大／<b class="mr-cru">巡航</b>＝高値維持／<b class="mr-fad">失速</b>＝ラン細り。表示専用の心構え材料。</div>'
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    for i, (t, r) in enumerate(cand.iterrows(), 1):
        ...
        rows.append(
            ...
            f'<td class="l tk">{t}{_tkcopy(t)}{_risk_flag_badge(t, r.get("close"))}{_badge}'
            f'<div class="rowsec">{_th} ・ {sth}</div>'
            f'<div class="rowbadges">'
            f'<span class="capb cap-{r.get("tier_key","none")}">{r.get("tier_lab","—")}</span>'
            f'{_state_badge(code, nu_tag, nu_cls, _rsn)}{_er_badge(t, _er, mkt.get("asof_bar"))}{_adr_badge(r.get("adr"))}{_fade_badge(r)}{_pq_badge(r, compact=True)}</div></td>'
    ...
    port = (f'<div class="card"><div class="hdr"><h2>ポートフォリオ（個別株スリーブ）</h2>'
            ...
            f'<div class="note rk-note"><b>買うのは189日RS≥85＆200MA上のリーダーから、63日RS≥85のもの上位{N_PORT}だけ</b>。'
            f'<b>出口まで</b>＝実効ストップ（初期=10日安値/21EMA・伸びたらピーク×0.70）までの距離＋<b>想定±Y%</b>（約1ヶ月1σ）。'
            f'<b class="shallow">浅</b>＝出口が想定変動の内側／<b class="r3b">+3R⅓利確</b>＝+3Rで1/3利確（残りはワイドトレール）／'
            f'<b class="fadeb fade-lo">勢い細り</b>＝189は強いが短期RS低下＝降りどき警戒／'
            f'<b>出遅れ</b>＝買う対象（タグのみ）・<b>選外（小型創薬/リーダー外/勢い落ち）</b>＝グレーアウト。</div></details>'
    ...
    for rank, (t, r) in enumerate(deck.iterrows(), start=_last_pos+2):
        ...
        wrows.append(
            ...
            f'<td class="l tk">{t}{_tkcopy(t)}{_risk_flag_badge(t, r.get("close"))}{_exbadge}'
    ...
    if _carry:
        ...
        for t in _carry:
            ...
            crows.append(
                ...
                f'<td class="l tk">{t}{_tkcopy(t)}{_risk_flag_badge(t, r.get("close"))}<span class="carryb">順位落ち・保有中</span>'
    ...
    _regime_panel = build_regime_alerts(m, _rst)
    ...
    port = (_regime_panel
            + _defense
            + _surface_transition
            + _beta_card(build_beta_view(m, picks)) + _liq_bar()
            + _mkt_section("① 現在の構成", "個別株スリーブ（N=12・等ウェイト）")
    ...
    today = (_liq_bar()
             + _mkt_section("① 押し目質スクリーナー", "検証で+EVだった押し目の形を全リーダーから抽出（実証ベース）") + build_pullback_screener(m)
             + _mkt_section("② スイング・プランナー", "候補を選ぶ → 株数・損切り・利確を円と%で") + _swing_planner_card()
             + _mkt_section("③ 眼を鍛えるスキャナー", "VCP・パターン・本日の注目（機械ゲートは無効・裁量の眼を養う用）")
             + _vcp_card(vcp_rows) + _patterns_card(patterns) + _buy_today_card(buytoday)
             + _mkt_section("④ リーダー監視", "RS上位群（RS≥85・200MA上）を状態別に") + leaders_card
    ...
    market = (
        ...
        + _leader_breadth_card(mkt.get("leader_breadth"))
    ...
    body = (build_today_action(sar, mkt, asof)
            ...
            + _mkt_section("① ローテーションの向き（RRG）", "改善→主導→弱化→停滞の時計回り（視覚的な全体像）")
            + f'{_rrg_card(mkt.get("rrg"), None, mkt.get("theme_hier"), 50, "テーマ・ローテーション（自ユニバースのRS）", None)}'
            + _mkt_section("② 大枠（セクターETF）", "市場全体のセクター資金フロー・大分類")
            + f'{_heatmap_card(mkt.get("sector_ranks"))}'
            + f'{_sector_rank_card(mkt.get("sector_ranks"))}</section>'
            f'<section id="t-sector">'
            + _mkt_section("① サブテーマ別RS（詳細）", "見出しタップで並べ替え・行タップで構成銘柄")
            + f'{sector}'
            + _mkt_section("② 強い業種の主導株", "強いグループ×個別も強い＝順張りの一等地")
