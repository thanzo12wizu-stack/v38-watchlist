...
def compute_metrics(W, order, s2i=None):
    ...
    for t in order:
        ...
        try:
            if len(c) >= 22 and v.notna().sum() >= 22:
                ...
                for back in range(0, 10):                       # 0=当日, 1=前日 ...
                    ...
                    if not bool(up.iloc[i]):
                        continue
                    win_c = c.iloc[i - 10:i]                    # 直前10営業日(当日除く)
                    win_v = v.iloc[i - 10:i]
                    dn = win_v[win_c < win_c.shift(1).reindex(win_c.index)]
        ...
        recs.append(dict(
            ...
            ret63=ret63, ret189=ret189, ret189_l1=ret189_l1, ret189_l5=ret189_l5, ret189_l10=ret189_l10, ret189_l20=ret189_l20, ret189_l30=ret189_l30, ret21=ret21, hi52=hi52, dist52=dist52, pos52=pos52,
...
def _rrg_svg(pts, cxv=50, period="now"):
    ...
    QFS = 30                                   # 象限ラベルのフォント（大きく・薄く）
    qlab = (f'<text x="{Wd-pad-10}" y="{pad+QFS+2}" fill="#4ade80" fill-opacity="0.55" font-size="{QFS}" font-weight="800" text-anchor="end">主導</text>'
            f'<text x="{pad+10}" y="{pad+QFS+2}" fill="#60a5fa" fill-opacity="0.55" font-size="{QFS}" font-weight="800">改善</text>'
            f'<text x="{Wd-pad-10}" y="{Ht-pad-12}" fill="#fbbf24" fill-opacity="0.55" font-size="{QFS}" font-weight="800" text-anchor="end">弱化</text>'
            f'<text x="{pad+10}" y="{Ht-pad-12}" fill="#f87171" fill-opacity="0.55" font-size="{QFS}" font-weight="800">停滞</text>')
    FS = 11.5; CH = FS * 1.02
    placed = []
    def overlaps(bx):
        for q in placed:
