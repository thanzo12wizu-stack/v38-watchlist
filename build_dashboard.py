...
Command Center — dashboard builder (最終確定 2026-07-05)
  ...
  C. NQ 4色ゲート(即応・確認日数ゼロ): 個別 青100/緑100/黄100(トレール15%に締める)/赤0(撤退), レバ 青100/緑50/黄0/赤0。執行は翌寄り。
...
def compute_metrics(W, order, s2i=None):
    ...
    for t in order:
        ...
        if len(c) >= 55:
            ...
            vr_5 = (v / v.rolling(50).mean()).iloc[-5:]
    ...
    if s2i:
        ...
        ind = pd.Series({t: s2i.get(t, "NA") for t in df.index})
...
def _equity_card(eq, at=None):
    setup = (
        ...
        '形式は1行1記録の「date,equity」または「date,equity,米株比率%」。桁区切りカンマは使わない。週1記録でも機能する。<br>'
...
def build_rrg(macro, extras, win=63, mom=10, themes=None):
    """テーマETFのSPY相対力(63日基準)×その10日モメンタムで4象限に分類。
       典型は 改善→主導→弱化→停滞 の時計回り。データはライブextras(9mo)。
       themes未指定なら主要16テーマ(RRG_THEMES)。"""
    b = macro.get("SPY")
    if b is None:
        return []
    bc = b["Close"].dropna()
...
def _rrg_svg(pts):
    ...
    xr = max(1.5, max(abs(min(xs) - 100), abs(max(xs) - 100)) * 1.28)
...
def _rrg_card(pts, pts_all=None):
    if not pts or len(pts) < 4:
        return ""
    def seeds_html(ps):
    ...
    def lists_html(ps):
        ...
        return "".join(
            ...
            + ("".join(f'<span class="chip">{p["ja"]}{"⚡" if p["cross"] else ""}</span>'
                       for p in sorted([x for x in ps if x["q"] == q], key=lambda z: -z["x"])) or '<span class="empty">なし</span>')
            + '</div>' for q in order)
    has_all = bool(pts_all and len(pts_all) >= 4)
    # 主要16
    view16 = (f'<div class="rrgview" id="rrg-16">'
              f'<div class="chart" style="height:auto">{_rrg_svg(pts)}</div>{seeds_html(pts)}{lists_html(pts)}</div>')
    # 全テーマ（あれば）
    view_all = ""
    if has_all:
        view_all = (f'<div class="rrgview" id="rrg-all" style="display:none">'
                    f'<div class="chart" style="height:auto">{_rrg_svg(pts_all)}</div>{seeds_html(pts_all)}{lists_html(pts_all)}</div>')
    toggle = ""
    if has_all:
        toggle = ('<div class="rrgtog">'
                  ...
                  '</div>')
    return (f'<div class="card"><div class="hdr"><h2>テーマ・ローテーション（SPY相対）</h2>{toggle}</div>'
            f'<div class="sub">業種テーマETFの相対力(横=SPY基準63日)×勢い(縦=10日変化)。<b>改善→主導→弱化→停滞</b>の時計回りに循環。'
            f'<b>白丸⚡＝勢いがプラス転換した直後＝次の主導の先回り候補</b>。■＝半導体。</div>'
...
def build_portfolio(m, s2t):
    ...
    # 出遅れ株除外: 業種内相対 <−20%(自業種より20%以上出遅れ)は選定から外す。
    #   図の相場サイクル理論の「出遅れ株」=業種内で独自に失速。検問5/6通過(運用者裁量採用)。
    #   ※セクターごと流出・トレンド崩壊・過去の栄光(直近失速)の除外は全て棄却済み(先行リターンむしろ高い)。
    if "sec_rel" in cand.columns:
        cand["laggard"] = cand["sec_rel"] < -0.20
        pool = cand[~cand["laggard"].fillna(False)]
    else:
        pool = cand
...
LEADER_RS = 85   # リーダー(=先導株)の唯一の定義: 63日RS≥85 かつ 200日線上。
                 # マーケットタブ「先導株一覧」もピックアップの母集団もこの1定義を参照(重複定義なし)。
...
def _leaders_watch_card(m, cap=80):
    """先導株一覧（RS≥85 かつ 200MA上）をRS順のチップで。タップで詳細。ウォッチ用。"""
    ...
    return (f'<div class="card"><h2>先導株一覧 '
            f'<button class="copybtn" onclick="copyList(this)" data-list="{tks}">📋 コピー</button></h2>'
            f'<div class="sub">RS≥{LEADER_RS}（63日）かつ200日線上＝<b>リーダー</b>(サイクルの先導グループ)・{n}銘柄。ピックアップの候補もこの母集団から。RS強い順・タップで詳細。</div>'
...
def build_breadth_ts(W, lookback=504):
...
def build_ratio_ts(macro, num, den_keys, lookback=504):
...
def build_vix_term_ts(macro, lookback=504):
...
def build_leader_temp(W, win=756):
    ...
    # 推移: 直近3年・%タイル（マーケットスコア形式）
    tail = pctile.iloc[-win:]
    ds = tail.iloc[::max(1, len(tail)//170)]
    ts = [(d.strftime("%Y-%m-%d"), float(v)) for d, v in ds.items()]
    return dict(cur=cur, ts=ts)

def _leader_temp_card(lt):
    """先導株の強さ: 温度バー＋%タイル推移(マーケットスコア形式)。正規分布は出さない。表示専用。"""
    if not lt or not lt.get("ts"):
        return ""
    cur = lt["cur"]; ts = lt["ts"]
    zone = ("枯渇（反発予兆）" if cur < 10 else "過熱（現状の強さ）" if cur >= 82 else
            "強（過熱手前）" if cur >= 65 else "並" if cur >= 30 else "やや枯渇")
    bar = (f'<div style="position:relative;height:26px;border-radius:13px;overflow:hidden;'
           f'background:linear-gradient(90deg,#1f6feb 0%,#22c55e 30%,#d29922 65%,#f97316 82%,#ef4444 100%)">'
           f'<div style="position:absolute;top:0;left:0;height:26px;width:10%;border-right:2px dashed rgba(255,255,255,.45)"></div>'
           f'<div style="position:absolute;top:-4px;left:{cur:.1f}%;width:4px;height:34px;background:#fff;'
           f'border-radius:2px;transform:translateX(-50%);box-shadow:0 0 6px rgba(0,0,0,.6)"></div></div>'
           f'<div style="display:flex;justify-content:space-between;font-size:10px;color:#6e7681;margin-top:6px">'
           f'<span>0<br>枯渇</span><span>25</span><span>50<br>並</span><span>75</span><span>100<br>過熱</span></div>')
    ...
    if n < 3:
        ...
        chart = (f'<div class="chart" style="margin-top:14px">{svg}'
                 f'<div class="cap cap-c"><span style="color:#e6edf3;font-weight:700">推移・直近3年</span></div>'
                 f'{_date_axis(ts)}</div>')
    return (
        f'<div class="card"><h2>先導株の強さ</h2>'
        f'<div class="sub">先導株(RS189上位10%)の勢いが過去分布の何%タイルか＝獲物の温度。露出には非接続・表示専用。</div>'
        f'<div style="text-align:center;font-size:30px;font-weight:900;margin:4px 0">{cur:.0f} %</div>'
        f'<div style="text-align:center;font-size:12px;color:#8b949e;margin-bottom:12px">現在：{zone}</div>'
        f'{bar}{chart}'
...
function copyEquityRow(){
  ...
  var row=ds+','+Math.round(v)+((uv||uv===0)&&uv>=0&&uv<=100?','+uv:'');
...
function _rebalCheck2(){
  ...
  if(isMon&&days>=13){
    el.style.display='';
    el.innerHTML='📅 今日は隔週リバランス日（月曜） → 継続条件（50&gt;200日線・RS上位'+(2*n)+'）を外れた銘柄を売って最新トップ'+n+'へ組み直し＋<b onclick="goTab(\'t-alloc\')" style="cursor:pointer;text-decoration:underline">配分タブ</b>で株数確認 <button class="nqb b-clear" onclick="rebalDone()" style="margin-left:6px">完了にする</button>';
...
def _svg_breadth(ts, n_uni=None):
    ...
    return (f'<div class="card"><h2>ブレッドス推移（200日線上の割合）</h2>'
            f'<div class="sub">200日線を上回る銘柄の割合（{uni_txt}・約2年）</div>'
            f'<div class="chart">{svg}'
...
def _svg_mri(ts):
    ...
    return (f'<div class="card"><h2>マーケットステータス推移</h2>'
            f'<div class="sub">地合いスコアの推移・直近約2年（75強気/60中立/45弱含み/30弱気）</div>'
            f'<div class="chart">{svg}'
...
def _alloc_tab(calc):
    ...
    return (
        ...
        '<div class="sub">今日の総資産（＋任意で米株比率%）を入れて<b>コピー</b> → GitHubの equity.csv 末尾に貼り付け（週1でOK）。カーブと露出バンドはマーケットタブ⑥に表示。</div>'
        '<div class="calc-in">'
        ...
        '<a class="memx" style="text-decoration:none" href="https://github.com/thanzo12wizu-stack/v38-watchlist/edit/main/equity.csv" target="_blank" rel="noopener">✏️ equity.csvを開く</a>'
        '</div>'
        '<div class="mut" style="font-size:11px;margin-top:4px">流れ: 📋コピー → ✏️で編集画面 → 末尾に貼り付け → Commit（3タップ）。日付は自動で付く。</div>'
        '</div></div>'
        + _mkt_section("② 配分計算", "NQ色 → 今日の株数に落とす")
        + '<div class="card">'
        '<div class="hdr"><h2>資金配分・株数計算</h2></div>'
        '<div class="sub">円の総資産から、現在のNQ色に応じた各銘柄の買付株数を出す。'
        '<b>隔週リバランス時点の計画値</b>（金曜クローズ判定→月曜寄りで執行）（実約定はギャップ・手数料でズレる）。<b>#</b>=RSランク・<b class="kf-skip" style="padding:0 4px">一服待ち</b>=急落局面で投げ未確認・<b class="kf-ok" style="padding:0 4px">解禁</b>=セリングクライマックス確認済み（満額）</div>'
        # 現在の色＋係数
        '<div class="gatebox">'
        ...
        '<b>現在損益・−25%初期ストップ・トレール(ピーク×0.70)</b>を表示する。'
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    today = _liq_bar() + _mkt_section("① 本日の注目", "セットアップ合致・裁量の参考（ルール外）") + _buy_today_card(buytoday) + _mkt_section("② リーダー監視", "RS≥85の常連＝次の主役候補") + leaders_card
    ...
    port = (f'<div class="card"><div class="hdr"><h2>ポートフォリオ（個別株スリーブ）</h2>'
            ...
            f'<th>前日比</th><th class="th-w">200MA<br>乖離</th><th class="th-w">トレール</th><th class="th-w">52週<br>高値差</th></tr>'
            + "".join(rows) + '</table>'
            + _risk_note(mkt) +
    ...
    port = _beta_card(build_beta_view(m, picks)) + _liq_bar() + _mkt_section("① 現在の構成", "個別株スリーブ（N=12・等ウェイト）") + port + _mkt_section("② 控え", "次の入替で上がってくる候補") + deck_card
    search_card = (
    ...
    watch = cap_filter_bar + _mkt_section("① 銘柄を調べる", "ティッカー検索 → タップで詳細") + search_card + _mkt_section("② セットアップで絞る", "複数条件ヒット＝注目度が高い") + overlap_card + setup_card + _mkt_section("③ 今日の値動き", "新高値圏＝需給が強い") + newhigh_card + _mkt_section("④ 先導株", "サイクルを引っ張るRS上位群") + _leaders_watch_card(m)
    ...
    rules = (
        ...
        f'<ul class="rules">'
        f'<li><b>選定</b>（隔週・月曜）：<b>189日RS</b>（≒9ヶ月の相対的な強さの順位）の上位{N_PORT}銘柄。条件は 50日線&gt;200日線・売買代金$10M/日以上・株価$5以上。<b>各1/{N_PORT}を等分</b>、テーマ上限なし。</li>'
    ...
    body = (build_today_action(sar, mkt, asof)
            ...
            f'<section id="t-alloc">{_alloc_tab(mkt.get("calc"))}'
            + _mkt_section("④ レバ玉の環境", "SOXLを入れる時のサイズ目安（表示専用）")
            + f'{_lev_env_card(mkt.get("lev_env"), mkt.get("rrg"))}</section>'
...
def selftest(html, picks, setups, sectors):
    ...
    if gate_coeffs("Yellow") != (1.0, 0.0): errs.append("gate: 黄=露出100(トレール15%)が正")
    ...
    if "セリングクライマックス" not in html and "投げ" not in html: errs.append("投げ確認ルールの説明欠落")
    if "出遅れ" not in html and "業種内" not in html: errs.append("出遅れ株除外の説明欠落")
    ...
    if "先導株の強さ" not in html: errs.append("leader temp gauge missing")
...
def main():
    ...
    mkt = {"distrib": build_distribution(macro), "ftd": build_ftd(macro), "perf": build_perf(macro, _extras),
           ...
           "trend_hist": _thist,
           "asof_bar": asof_bar,
           "mri_ts": [(d.strftime("%Y-%m-%d"), float(v)) for d, v in mri.iloc[-504:].items()],
           "leader_temp": build_leader_temp(W)}
    mkt["rrg"] = build_rrg(macro, _extras)
