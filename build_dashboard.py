...
def _validate_price_body(W, min_rows=2):
    """OHLCV価格本体の健全性を検証し、問題点リストを返す（#5）。空なら健全。
       全系列がDataFrame・非空・DatetimeIndex・昇順・index/columns重複なし・系列間でindex/columns一致・
       High≥Low・High≥Open/Close・Low≤Open/Close・Volume≥0 を確認する。"""
    probs = []
    ...
    if probs:
        return probs
    # OHLC整合とVolume非負（NaNは許容＝欠測。矛盾のみ検出）
    try:
        H, L, O, C, V = W["High"], W["Low"], W["Open"], W["Close"], W["Volume"]
        if bool((H < L).sum().sum()): probs.append("High<Low somewhere")
        if bool((H < O).sum().sum()): probs.append("High<Open somewhere")
        if bool((H < C).sum().sum()): probs.append("High<Close somewhere")
        if bool((L > O).sum().sum()): probs.append("Low>Open somewhere")
        if bool((L > C).sum().sum()): probs.append("Low>Close somewhere")
        if bool((V < 0).sum().sum()): probs.append("Volume<0 somewhere")
    except Exception as _e:
        probs.append(f"OHLC consistency check error {repr(_e)[:40]}")
    # #5 有限性・正値・充足率。大小関係だけだとNaN比較が全FalseになりNaN/Inf/0/負価格を見逃す。
    try:
        H, L, O, C, V = W["High"], W["Low"], W["Open"], W["Close"], W["Volume"]
        # (1) Inf/-Inf は価格・出来高いずれも不可（NaNは欠測として許容、ただし全欠測は下でNG）
        for _nm, _df in (("High", H), ("Low", L), ("Open", O), ("Close", C), ("Volume", V)):
            _arr = _df.to_numpy(dtype="float64", copy=False)
            if bool(np.isinf(_arr).any()):
                probs.append(f"{_nm}: contains Inf")
        # (2) 価格(OHLC)の有効値は必ず > 0（0や負は不正）。NaNは除外して判定。
        for _nm, _df in (("High", H), ("Low", L), ("Open", O), ("Close", C)):
            _arr = _df.to_numpy(dtype="float64", copy=False)
            _valid = _arr[~np.isnan(_arr)]
            if _valid.size and bool((_valid <= 0).any()):
                probs.append(f"{_nm}: non-positive price (<=0)")
        # (3) 出来高の有効値は有限かつ >= 0（負は上で別途、Infは(1)で検出）
        _v = V.to_numpy(dtype="float64", copy=False)
        _vv = _v[~np.isnan(_v)]
        if _vv.size and bool((_vv < 0).any()):
            probs.append("Volume: negative")
        # (4) 全欠測の行/銘柄を禁止（Closeで判定）。全NaN列(銘柄)・全NaN行(日付)は後段計算を壊す。
        _cnan = C.isna()
        _all_nan_cols = [c for c in C.columns if bool(_cnan[c].all())]
        if _all_nan_cols:
            probs.append(f"Close: all-NaN columns {list(_all_nan_cols)[:5]}")
        if bool(_cnan.all(axis=1).any()):
            probs.append("Close: an all-NaN row (date with no data)")
        # (5) 銘柄ごとの最低データ充足率（Close非NaN率）。極端に薄い系列は不正データの兆候。
        _cov = C.notna().mean(axis=0)
        _thin = [c for c in C.columns if float(_cov.get(c, 0)) < 0.30]
        if _thin:
            probs.append(f"Close: coverage <30% for {list(_thin)[:5]}")
        # (6) 最終行に必要なClose（少なくとも1銘柄）が存在すること
        if len(C.index) and bool(C.iloc[-1].isna().all()):
            probs.append("Close: last row is entirely NaN")
    except Exception as _e:
        probs.append(f"finite/positive/coverage check error {repr(_e)[:40]}")
    return probs
...
def load_inputs(use_cache=True):
    ...
    # #4/#10 sector JSONのschema検証: トップlevel dict・s2i/s2t/e2j がdict、さらに値の型まで検証。
    #   値が不正だと後段で unhashable type: 'list'/'dict' のcryptic errorになるため、入力箇所を示して停止する。
    ...
    # s2i: key=ticker, value=業種文字列
    for _kk, _vv in (sj.get("s2i") or {}).items():
        if not isinstance(_vv, str):
            raise SystemExit(f'[FATAL] sector JSON s2i["{_kk}"] must be str, got {type(_vv).__name__}: {SECTOR_JSON}')
    # s2t: key=ticker, value=文字列 または 1〜2要素の文字列配列
    for _kk, _vv in (sj.get("s2t") or {}).items():
        if isinstance(_vv, str):
            continue
        if isinstance(_vv, list):
            if not (1 <= len(_vv) <= 2) or not all(isinstance(x, str) for x in _vv):
                raise SystemExit(f'[FATAL] sector JSON s2t["{_kk}"] must be str or 1-2 str list: {SECTOR_JSON}')
        else:
            raise SystemExit(f'[FATAL] sector JSON s2t["{_kk}"] must be str or list, got {type(_vv).__name__}: {SECTOR_JSON}')
    # e2j: key/valueとも文字列
    for _kk, _vv in (sj.get("e2j") or {}).items():
        if not isinstance(_kk, str) or not isinstance(_vv, str):
            raise SystemExit(f'[FATAL] sector JSON e2j entry must be str->str at key {_kk!r}: {SECTOR_JSON}')
...
def load_state(strict=False, fail_closed=False):
    ...
    if strict:
        if env_p and os.path.exists(env_p):
            # 明示fixtureが破損/schema不一致なら停止する。空フォールバックはテスト条件を変えるため不可（#追加）。
    ...
    p = next((x for x in paths if x and os.path.exists(x)), None)
    if not p:
        # 主ファイルが無い場合、書き込み失敗などで .prev だけ残っていれば復元する（空stateで続行しない・#5）
        for _base in [env_p, os.path.join(os.path.dirname(CACHE), "state.json")]:
            if _base and os.path.exists(_base + ".prev"):
                sys.stderr.write("[state] main state missing; restored from .prev (%s)\n" % (_base + ".prev"))
                return _load_json_safe(_base + ".prev", {}, "state.json.prev", fail_closed=fail_closed, validator=_validate_state_schema)
        return {}
    return _load_json_safe(p, {}, "state.json", fail_closed=fail_closed, validator=_validate_state_schema)
...
def load_trend_history(cur_color, asof_date, persist=True, strict=False, fail_closed=False):
    ...
    hist = []
    if path:
        # strict(offline selftest)で明示fixtureが破損/schema不一致なら停止（空配列フォールバックはテスト条件を変える・#追加）
        _fc = True if strict else fail_closed
        hist = _load_json_safe(path, [], "trend_history.json", fail_closed=_fc, validator=_validate_trend_schema) or []
    elif not strict:
        for _base in [env_p, os.path.join(os.path.dirname(CACHE), "trend_history.json")]:
            if _base and os.path.exists(_base + ".prev"):
                hist = _load_json_safe(_base + ".prev", [], "trend_history.json.prev", fail_closed=False, validator=_validate_trend_schema) or []
                break
    today = str(asof_date)
    # #4 as-of より未来の日付は不正（未来日を含む履歴は日付降順を招く）。strict/本番は停止、その他は除外。
    ...
    if _future:
        ...
        sys.stderr.write(f"[trend] dropping {len(_future)} future-dated entries after as-of {today}\n")
        hist = [hc for hc in hist if hc and hc[0] <= today]
    prev = None
...
def main():
    ...
    W = guard_last_bar(W)
    # #3/#5/#14 重要系列(価格本体)の包括検証: OHLCV不整合は実行モードに関係なく停止する。
    #   本番でもselftest/integrationでも FAILED(非0)。純粋な診断(--diag)のみ警告で続行。
    _pb_probs = _validate_price_body(W)
    if _pb_probs:
        _msg = "[FATAL] 価格本体(OHLCV)が不正です: " + "; ".join(_pb_probs[:6])
