...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    today = role_note + _buy_today_card(buytoday) + buy_card + leaders_card
    ...
    # RSリーダー控え（22社）は下でdeck_card構築後に port へ追加
    ...
    deck_card = (f'<div class="card"><div class="hdr"><h2>RSリーダー控え</h2>{_cp(list(deck.index))}</div>'
             f'<div class="sub">候補プールの{N_PORT+1}位以降（200MA上×週足SAR非弱気）・次の月次入替で繰り上がる</div>'
             f'<table><tr><th class="l">#</th><th class="l">銘柄</th><th>RS</th>'
             f'<th>前日比</th><th>63日</th><th>52週高値差</th></tr>'
             + "".join(wrows) + '</table></div>')
    port = port + deck_card        # RSリーダー控え（22社）をポートフォリオタブへ
    search_card = (
    ...
    watch = search_card + overlap_card + setup_card + newhigh_card
