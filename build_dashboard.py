...
def build_rates_card(ctx, asof=None):
    """スマホでは要点を常時表示し、詳細はタップで展開する金利カード。"""
    def _bp(x):
    ...
    for sid, cat, lab, unit, colored in rows_def:
        ...
        if sid == "T10Y2Y" and not vals:
            ...
            if g10.get("vals") and g2.get("vals"):
                m10, m2 = dict(g10["vals"]), dict(g2["vals"])
        ...
        chg = _bp(chg20)
        if lvl is None:
            metrics.append(dict(sid=sid, cat=cat, lab=lab + alt, lvl=None, chg=None,
                                pct=None, spd=None, foot="未取得", cls="mut"))
            continue
        chg_cls = ("neg" if chg > 0 else "pos" if chg < 0 else "mut") if colored and chg is not None else "mut"
        ...
        metrics.append(dict(
            sid=sid, cat=cat, lab=lab + alt, lvl=float(lvl), chg=chg, pct=pct, spd=spd,
            foot=f"観測 {eff_ld or '—'}{stale_txt}・{src_lab}{cache_txt}", cls=chg_cls,
        ))

    def _value(m):
        if m["lvl"] is None:
            return "—"
        return f'{m["lvl"]:.2f}%' if m["sid"] != "T10Y2Y" else f'{m["lvl"]:.2f}pt'

    def _change(m):
        return "—" if m["chg"] is None else f'{m["chg"]:+.0f}bp'

    mini = "".join(
        '<div class="macro-mini">'
        f'<span>{m["lab"]}</span><div><b>{_value(m)}</b><em class="{m["cls"]}">{_change(m)}</em></div>'
        '</div>' for m in metrics
    )
    detail_parts = []
    for m in metrics:
        pct_label = "—" if m["pct"] is None else "%.0f%%" % m["pct"]
        spd_label = "" if m["spd"] is None else "・変化位置 %.0f%%" % m["spd"]
        detail_parts.append(
            '<div class="macro-detail-row">'
            f'<div><b>{m["lab"]}</b><span>{m["cat"]}</span></div>'
            f'<div class="macro-detail-num"><b>{_value(m)}</b>'
            f'<span class="{m["cls"]}">{_change(m)} / 20日</span></div>'
            f'<small>1年 {pct_label}{spd_label}　{m["foot"]}</small>'
            '</div>'
        )
    detail = "".join(detail_parts)
    core = next((m for m in metrics if m["sid"] == "DFII10"), None)
    if core and core["lvl"] is not None:
        lead = f'実質金利 {_value(core)}・20日 {_change(core)}'
    else:
        lead = "実質金利を中心に確認"
    return (
        '<details class="card macro-card">'
        '<summary class="macro-summary">'
        '<div class="macro-summary-main">'
        '<div class="macro-title-row"><h2>金利レジーム <span class="h2en">Rates</span></h2>'
        f'<span class="macro-lead">{lead}</span></div>'
        f'<div class="macro-mini-grid">{mini}</div>'
        '</div></summary>'
        f'<div class="macro-detail-list">{detail}</div>'
        '</details>'
    )

def build_rates_insight(fred_ctx, asof=None):
    """実質金利×HY OASの現在象限だけを常時表示し、4象限統計はタップで展開する。"""
    try:
        cache = _fred_load_cache()
        today = dt.date.today()
        long_vals = {}
        for sid in ("DFII10", "BAMLH0A0HYM2"):
            ...
            if cv.get("vals"):
                try:
                    use_cache = (today - pd.Timestamp(cv.get("cached_at")).date()).days <= 7
                except Exception:
                    use_cache = True
            if use_cache:
                long_vals[sid] = cv["vals"]
                continue
            vals, _src = _fred_fetch_one(
                sid, api_key=os.environ.get("FRED_API_KEY") or None,
                start="2003-01-01", timeout=8, retry=False,
                deadline=time.monotonic() + 12,
            )
            if vals:
                cache[key] = dict(vals=vals, cached_at=today.isoformat())
                long_vals[sid] = vals
            elif cv.get("vals"):
                long_vals[sid] = cv["vals"]

        qcv = cache.get("LONG:QQQ") or {}
        q = None
        if qcv.get("vals"):
            try:
        ...
        if q is None or not fresh:
            try:
                ...
                qq.index = pd.to_datetime(qq.index).tz_localize(None)
                q = qq.dropna().astype(float)
                cache["LONG:QQQ"] = dict(
                    vals=[(str(i.date()), float(v)) for i, v in q.items()],
                    cached_at=today.isoformat(),
                )
            except Exception:
                pass
        _fred_save_cache(cache)
        if not long_vals.get("DFII10") or not long_vals.get("BAMLH0A0HYM2") or q is None or len(q) < 500:
            return ""

        real = pd.Series({pd.Timestamp(d): v for d, v in long_vals["DFII10"]}).sort_index()
        oas = pd.Series({pd.Timestamp(d): v for d, v in long_vals["BAMLH0A0HYM2"]}).sort_index()
        if asof is not None:
            cut = pd.Timestamp(asof)
            q, real, oas = q[q.index <= cut], real[real.index <= cut], oas[oas.index <= cut]
        df = pd.DataFrame({"q": q}).join(real.rename("r"), how="left").join(oas.rename("o"), how="left").ffill().dropna()
        df["dr"] = (df["r"] - df["r"].shift(20)) * 100
        df["do"] = (df["o"] - df["o"].shift(20)) * 100
        df["fwd"] = df["q"].shift(-21) / df["q"] - 1
        hist = df.dropna(subset=["dr", "do", "fwd"])
        hist = hist.groupby(hist.index.to_period("M")).tail(1)
        cells = {}
        for rn, rc in (("実質金利↑", hist.dr > 0), ("実質金利↓", hist.dr <= 0)):
            for on, oc in (("OAS拡大", hist.do > 0), ("OAS縮小", hist.do <= 0)):
                seg = hist.loc[rc & oc, "fwd"]
                cells[(rn, on)] = (
                    float(seg.median()) * 100,
                    float((seg > 0).mean()) * 100,
                    len(seg),
                ) if len(seg) >= 20 else None
        cur = (
            "実質金利↑" if df.dr.iloc[-1] > 0 else "実質金利↓",
            "OAS拡大" if df.do.iloc[-1] > 0 else "OAS縮小",
        )
        cur_stat = cells.get(cur)
        cur_text = "n不足" if cur_stat is None else f'{cur_stat[0]:+.1f}%・勝率 {cur_stat[1]:.0f}%・n={cur_stat[2]}'
        cur_cls = "mut" if cur_stat is None else ("pos" if cur_stat[0] > 0 else "neg")
        ...
        for rn in ("実質金利↑", "実質金利↓"):
            for on in ("OAS拡大", "OAS縮小"):
                st = cells.get((rn, on))
                now = cur == (rn, on)
                body = '<span class="mut">n不足</span>' if st is None else (
                    f'<b class="{"pos" if st[0] > 0 else "neg"}">{st[0]:+.1f}%</b>'
                    f'<span>勝率 {st[1]:.0f}%・n={st[2]}</span>'
                )
                boxes.append(
                    f'<div class="macro-quadrant {"now" if now else ""}">'
                    f'<small>{rn} × {on}{"・現在" if now else ""}</small>{body}</div>'
                )
        return (
            '<details class="card macro-card">'
            '<summary class="macro-summary">'
            '<div class="macro-summary-main">'
            '<div class="macro-title-row"><h2>金利×信用 <span class="h2en">Rates × Credit</span></h2>'
            f'<span class="macro-lead">{cur[0]} × {cur[1]}</span></div>'
            f'<div class="macro-current"><span>現在</span><b class="{cur_cls}">{cur_text}</b></div>'
            '</div></summary>'
            f'<div class="macro-quadrants">{"".join(boxes)}</div>'
            '<div class="macro-note">月末観測・QQQ 21営業日先中央値 / 勝率</div>'
            '</details>'
        )
    except Exception as e:
...
def build_macro_pressure(fred_ctx, macro, asof=None):
    """信用・ドル・金融環境・債券ボラを、要点＋折りたたみ詳細で表示する。"""
    items = []

    def _item(cat, lab, vals, ld, src, unit, cached_at=None, is_bp=False,
        ...
        try:
            ...
            spd = None
        if lvl is None:
            return dict(cat=cat, lab=lab, value="—", change="—", cls="mut",
                        pct="—", spd="", foot="未取得", note=up_note or "")
        if is_bp:
            change = "—" if chg20 is None else f'{chg20 * 100:+.0f}bp'
            value = f'{lvl * 100:.0f}bp'
        else:
            change = "—" if chg20 is None else f'{chg20:+.2f}'
            value = f'{lvl:.2f}{unit}'
        cls = "mut" if chg20 is None else ("neg" if chg20 > 0 else "pos" if chg20 < 0 else "mut")
        src_lab = {
            "api": "API", "csv": "CSV", "cache": "前回値", "yf": "yfinance",
            "MOVE": "MOVE", "TLT実現ボラ": "TLT実現ボラ", "none": "—",
        }.get(src, src)
        stale_txt = "" if stale is None else (f"・{stale}営業日前" if stale > 0 else "・本日")
        cache_txt = f"・取得 {cached_at}" if src == "cache" and cached_at else ""
        return dict(
            cat=cat, lab=lab, value=value, change=change, cls=cls,
            pct="—" if pct is None else f"{pct:.0f}%",
            spd="" if spd is None else f"・変化位置 {spd:.0f}%",
            foot=f"観測 {eff or '—'}{stale_txt}・{src_lab}{cache_txt}",
            note=up_note or "", pct3_hint=pct3_hint,
        )

    oas = (fred_ctx or {}).get("BAMLH0A0HYM2", {})
    items.append(_item("信用", "HY OAS", oas.get("vals"), oas.get("last_date"),
                       oas.get("src"), "%", oas.get("cached_at"), is_bp=True,
                       up_note="拡大=信用悪化"))
    dv, dlab, dld, dsrc, dcached = _dollar_from_macro_or_fred(fred_ctx, macro)
    items.append(_item("ドル", dlab, dv, dld, dsrc, "", dcached,
                       up_note="ドル高=資金逼迫"))
    nfci = (fred_ctx or {}).get("NFCI", {})
    items.append(_item("金融環境", "NFCI", nfci.get("vals"), nfci.get("last_date"),
                       nfci.get("src"), "", nfci.get("cached_at"),
                       up_note="0超=平均より引き締まり"))
    bv, bsrc, bunit = _bondvol_from_macro(macro)
    blab = {"MOVE": "MOVE", "TLT実現ボラ": "TLT 20日実現ボラ",
            "none": "債券ボラ"}.get(bsrc, "債券ボラ")
    items.append(_item("金利ボラ", blab, bv, (bv[-1][0] if bv else None), bsrc, bunit,
                       up_note="上昇=金利市場不安定"))

    mini = "".join(
        '<div class="macro-mini">'
        f'<span>{x["cat"]}</span><div><b>{x["value"]}</b><em class="{x["cls"]}">{x["change"]}</em></div>'
        '</div>' for x in items
    )
    detail = "".join(
        '<div class="macro-detail-row">'
        f'<div><b>{x["lab"]}</b><span>{x["note"]}</span></div>'
        f'<div class="macro-detail-num"><b>{x["value"]}</b><span class="{x["cls"]}">{x["change"]} / 20日</span></div>'
        f'<small>{x.get("pct3_hint", "3年%ile")} {x["pct"]}{x["spd"]}　{x["foot"]}</small>'
        '</div>' for x in items
    )
    return (
        '<details class="card macro-card">'
        '<summary class="macro-summary">'
        '<div class="macro-summary-main">'
        '<div class="macro-title-row"><h2>マクロ圧力 <span class="h2en">Macro Pressure</span></h2>'
        '<span class="macro-lead">信用・ドル・金融環境・金利ボラ</span></div>'
        f'<div class="macro-mini-grid">{mini}</div>'
        '</div></summary>'
        f'<div class="macro-detail-list">{detail}</div>'
        '</details>'
...
/* 金利・マクロ: スマホ要点表示＋タップ展開 */
.macro-card{padding:11px 12px}
.macro-summary{list-style:none;cursor:pointer;position:relative;padding-right:22px}
.macro-summary::-webkit-details-marker{display:none}
.macro-summary:after{content:"›";position:absolute;right:1px;top:1px;color:#6f8198;font-size:22px;line-height:1;transition:transform .16s ease}
.macro-card[open] .macro-summary:after{transform:rotate(90deg)}
.macro-summary-main{min-width:0}
.macro-title-row{display:flex;align-items:baseline;justify-content:space-between;gap:8px}
.macro-title-row h2{margin:0;white-space:nowrap}
.macro-lead{font-size:10px;color:#8ea0b7;text-align:right;line-height:1.25}
.macro-mini-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px;margin-top:8px}
.macro-mini{background:#0b1220;border:1px solid #1b2636;border-radius:8px;padding:6px 7px;min-width:0}
.macro-mini>span{display:block;font-size:9px;color:#718096;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.macro-mini>div{display:flex;align-items:baseline;justify-content:space-between;gap:5px;margin-top:2px}
.macro-mini b{font-size:13px;color:#e6edf3;white-space:nowrap;font-variant-numeric:tabular-nums}
.macro-mini em{font-size:9px;font-style:normal;white-space:nowrap}
.macro-detail-list{display:grid;gap:6px;margin-top:9px;padding-top:8px;border-top:1px solid #1c2533}
.macro-detail-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:5px 10px;background:#0b1220;border:1px solid #1b2636;border-radius:8px;padding:7px 8px}
.macro-detail-row>div:first-child{min-width:0}
.macro-detail-row>div:first-child b{display:block;font-size:11px;color:#cbd5e1}
.macro-detail-row>div:first-child span{display:block;font-size:9px;color:#66758a;margin-top:1px;line-height:1.3}
.macro-detail-num{text-align:right;white-space:nowrap}
.macro-detail-num b{display:block;font-size:13px;color:#e6edf3}
.macro-detail-num span{display:block;font-size:9px;margin-top:1px}
.macro-detail-row>small{grid-column:1/-1;font-size:8.7px;color:#5f6b7e;line-height:1.35;border-top:1px solid rgba(255,255,255,.04);padding-top:4px}
.macro-current{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:8px;background:#0b1220;border:1px solid #24405f;border-radius:8px;padding:7px 9px}
.macro-current span{font-size:9px;color:#718096}
.macro-current b{font-size:11px;text-align:right}
.macro-quadrants{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px;margin-top:9px;padding-top:8px;border-top:1px solid #1c2533}
.macro-quadrant{background:#0b1220;border:1px solid #1b2636;border-radius:8px;padding:7px 8px;min-width:0}
.macro-quadrant.now{border-color:#38bdf8;background:rgba(56,189,248,.07)}
.macro-quadrant small{display:block;font-size:8.8px;color:#718096;line-height:1.25}
.macro-quadrant b{display:block;font-size:13px;margin-top:3px}
.macro-quadrant span{display:block;font-size:8.8px;color:#718096;margin-top:1px}
.macro-note{font-size:8.8px;color:#5f6b7e;margin-top:6px}
@media(min-width:760px){.macro-mini-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.macro-card{padding:13px 15px}.macro-detail-list{grid-template-columns:repeat(2,minmax(0,1fr))}.macro-detail-row>small{min-height:25px}}
...
def main():
    ...
    if do_self:
        ...
        if errs:
            ...
            sys.exit(1)
        bs_, bys_ = leaders
        print("SELFTEST OK (8 tabs, restored pickup layout, X-ready market digest, market health v2, emergency brake, RS compare+IN/OUT+continuity, N=%d, leaders=%d, buys=%d, sectors=%d, no None/nan)" %
