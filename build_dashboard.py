...
# ═══════════════════════════════════════════════════════════════════
W30X = dict(wret_min=0.06, ext_min=0.05, lo52_mult=1.50, ext_weak=0.01, px_min=5.0, recent_weeks=4)
...
W30X_STATS = {
    ...
    "PART":  dict(w30=26.2, w50=11.3, dbl=7.5,  fake=55, n=1830),
    "WEAK":  dict(w30=23.5, w50=8.9,  dbl=5.9,  fake=81, n=3487),
    "CROSS": dict(w30=26.6, w50=10.9, dbl=7.4,  fake=70, n=13906),
}

def w30_breakout(c, p=W30X):
    out = dict(w30x_grade=None, w30x_wk_ago=None, w30x_wret=np.nan,
               w30x_ext=np.nan, w30x_vslo52=np.nan, w30x_dist52h=np.nan,
               w30x_score=None, w30x_tier=None, w30x_confirmed=True, w30x_alive=True)
    try:
        w = c.resample("W-FRI").last().dropna()
    except Exception:
        return out
    ...
    for k in range(p["recent_weeks"] + 1):
        ...
        if not (px == px and mai == mai and loi == loi) or px < p["px_min"] or loi <= 0:
            continue
        wret = px / float(w.iloc[i - 1]) - 1; ext = px / mai - 1; vslo = px / loi
        hii = float(hi52.iloc[i]); d52h = px / hii - 1 if hii == hii and hii > 0 else np.nan
        if ext < p["ext_weak"]:
            g = "WEAK"
        elif wret >= p["wret_min"] and ext >= p["ext_min"]:
            g = "FULL" if vslo >= p["lo52_mult"] else "PART"
        else:
            g = "CROSS"
        ...
        out.update(w30x_grade=g, w30x_wk_ago=k, w30x_wret=wret, w30x_ext=ext,
                   ...
                   w30x_confirmed=confirmed, w30x_alive=alive)
        return out
    return out

_W30_LAB = {"FULL": "W30◎", "PART": "W30○", "WEAK": "W30△", "CROSS": "W30"}
_W30_CLS = {"FULL": "w30b w30b-full", "PART": "w30b w30b-part", "WEAK": "w30b w30b-weak", "CROSS": "w30b"}

def _w30x_badge(r):
    g = r.get("w30x_grade")
    if not (isinstance(g, str) and g in W30X_STATS):
        return ""
    s = W30X_STATS[g]; k = r.get("w30x_wk_ago") or 0
    wk = "今週" if k == 0 else f"{k}週前"
    pend = "" if r.get("w30x_confirmed", True) else "（週中・仮判定）"
    dead = "" if r.get("w30x_alive", True) else "（30WMA割れ済・失効）"
    desc = {"FULL": "本格: 週足+6%以上・30WMAを5%以上クリア・52週安値から+50%以上",
            "PART": "勢いはあるが安値からの戻り不足（伸びは平均並み・騙しは半減）",
            "WEAK": "ギリ抜け（クリア幅1%未満）: 騙し率81%の要警戒",
            "CROSS": "素の30週線上抜け（追加条件未達）"}[g]
    tier = r.get("w30x_tier"); tlab = f"[{tier}]" if g == "FULL" and tier else ""
    return (f'<span class="{_W30_CLS[g]}" title="W30ブレイク{tlab} {wk}{pend}{dead} — {desc}。'
            f'全期間実測(2016-26,N={s["n"]}): +30%到達{s["w30"]}% / +50% {s["w50"]}% / '
            f'52週2倍{s["dbl"]}% / 騙し(12週内30WMA割れ){s["fake"]}%">'
            f'{_W30_LAB[g]}{tlab}{"" if k == 0 else f" {k}w"}</span>')

def build_w30_breakouts(m, cap=12):
    rk = {"FULL": 0, "PART": 1, "CROSS": 2}
    rows, warns = [], []
    _valid = {"FULL", "PART", "WEAK", "CROSS"}
    for t, r in m.iterrows():
        g = r.get("w30x_grade")
        if not (isinstance(g, str) and g in _valid):   # None/nan/想定外は除外
            continue
        if g == "WEAK":
            warns.append((t, r)); continue
        if not r.get("w30x_alive", True):
            continue
        rows.append((rk[g], -float(r.get("w30x_ext") or 0), t, r))
    rows.sort()

    # ① シグナル一覧：各行がバッジ主役の独立ブロック（元モジュールの体裁）
    def _line(t, r):
        ...
        return (f'<div class="w30row" data-tkone="{t}"><div class="w30row-h"><b>{t}</b>{_w30x_badge(r)}</div>'
                f'<div class="w30row-m">週足 {100*r["w30x_wret"]:+.1f}%・30WMA +{100*r["w30x_ext"]:.1f}%・{s52}{turn}</div></div>')
    body = "".join(_line(t, r) for _, _, t, r in rows[:cap]) or '<div class="mut">直近4週のシグナルなし</div>'
    ...
    signal_card = (f'<div class="card"><div class="hdr"><h2>ブレイク一覧 <span class="h2en">Signals</span></h2>{_cp([t for _,_,t,_ in rows[:cap]])}</div>'
            f'<div class="sub">直近4週・30WMA上を維持中。◎本格→○勢いのみ→素クロスの順。</div>'
            f'{body}{warn_html}</div>')

    # ② グレード別の実測（ランクが一目でわかる表：元ネタの数値をそのまま）
    def _statrow(g, lab, cls):
        st = W30X_STATS[g]
        return (f'<tr><td class="l"><b class="{cls}">{lab}</b></td><td>{st["w30"]:.1f}%</td><td>{st["w50"]:.1f}%</td>'
                f'<td>{st["dbl"]:.1f}%</td><td class="neg">{st["fake"]}%</td><td class="mut">{st["n"]:,}</td></tr>')
    legend_card = (f'<div class="card"><h2>判定の意味と実測 <span class="h2en">Grades</span></h2>'
            f'<div class="sub">2016-06〜2026-07・790銘柄・13,906イベント。+30/+50＝以後26週内・2倍は52週内の週足終値到達率／騙し＝12週内に30WMA再割れ。</div>'
            f'<table><tr><th class="l">判定</th><th>+30%</th><th>+50%</th><th>2倍</th><th>騙し</th><th>N</th></tr>'
            f'{_statrow("FULL","◎ 本格","w30g-full")}{_statrow("PART","○ 勢いのみ","w30g-part")}'
            f'{_statrow("CROSS","素 クロス","mut")}{_statrow("WEAK","△ ギリ抜け","w30g-weak")}</table>'
            f'<details style="margin-top:8px"><summary class="mut" style="font-size:11px;cursor:pointer">判定条件と◎の格付け（S/A/B）</summary>'
            f'<div class="sub" style="margin-top:6px">クロス週に評価：<b>勢い</b>=週足≥+6%／<b>クリア幅</b>=終値/30WMA−1≥+5%／<b>位置</b>=終値≥52週安値×1.50。'
            f'◎=3条件すべて・○=位置のみ不足・△=クリア幅1%未満・素=それ以外。<br>'
            f'◎の格付け：52週高値−30%超下で+2、週足+10%以上で+1、クリア幅+10%以上で+1。'
            f'<b>S</b>=3〜4点（+30%到達66.4%・2倍41.3%）／<b>A</b>=1〜2点（42.2%・18.9%）／<b>B</b>=0点（35.2%・9.9%）。</div></details></div>')

    # ③ 手仕舞い：カード形式で1件ずつ（.kvは使わない＝崩れ回避）
    q = W30X_EXIT
    ...
    exit_card = ('<div class="card"><h2>手仕舞いの目安 <span class="h2en">Exits</span></h2>'
            ...
            + '</div>')
    return signal_card + legend_card + exit_card
...
def build_ema21_touch(m, W, s2t, cap=12):
    ...
    try:
        ...
        def _tr(t, th, r189, r63, dv, td, am, xadr):
            ...
            xa = "" if xadr is None else f'<span class="mut" style="font-size:9.5px">（{xadr:.1f}×ADR）</span>'
            return (f'<tr data-rs="{r189:.0f}" data-dev="{dv:.2f}" data-td="{td}" data-atr="{(am if am is not None else 999):.2f}">'
                    f'<td><b data-tkone="{t}">{t}</b></td><td style="font-size:11px;color:#94a3b8">{th}</td>'
                    f'<td>{r189:.0f} / {r63:.0f}</td><td>{dv:+.1f}%{xa}</td>'
                    f'<td class="{am_cls}">{am_txt}</td>'
                    f'<td class="mut" style="font-size:10.5px">{"今日" if td == 0 else str(td) + "日前"}</td></tr>')
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    body = (build_today_action(sar, mkt, asof)
            ...
            '<button onclick="tab(\'t-port\',this)">Portfolio</button>'
            '<button onclick="tab(\'t-alloc\',this)">Sizing</button>'
            ...
            f'<section id="t-alloc">{_alloc_body}</section>'
            ...
            + f'{build_w30_breakouts(m)}</section>'
