...
def compute_metrics(W, order, s2i=None, macro=None):
    ...
    r2 = _r_at("ret63_l2", _pool_l21)      # ~42d前（近似で21日前プール）
...
def _expected_move_card(em):
    ...
    return (f'<div class="card"><h2>オプション想定変動幅（VIX/VXN・約1ヶ月）</h2>'
            ...
            f'VIX/VXN＝S&P/Nasdaqのインプライド・ボラ。正規分布仮定の理論1σ（2σも同様）で、実分布は裾が厚め。'
            f'保有個別は<b>ADR（日中値幅ベースの簡易ボラ・ギャップ非算入）</b>で近似。</div>'
            f'{rows}</div>')
...
def _net_liq_read(nl):
    ...
    ch = nl.get("chg4w", 0.0)
    if ch >= 0.15:
        return ("追い風", "pos", "流動性がしっかり増加中で、株の支援材料。押し目は買われやすい局面")
    if ch >= 0.04:
        return ("やや追い風", "pos", "流動性は緩やかに増加しており、リスクオン寄りの地合い")
    if ch <= -0.15:
        return ("警戒", "neg", "流動性がはっきり減少中で、株の重石になりやすい。上値は重く、戻り売りに注意")
    if ch <= -0.04:
        return ("やや警戒", "neg", "流動性は緩やかに減少。単独では売り材料にならないが、他が崩れると効いてくる")
    return ("中立", "mut", "ほぼ横ばいで、今は気にしなくてよい範囲。方向が明確に出たら改めて注目したい")
...
def _transition_leaders_card(tl):
    ...
    return (f'<div class="card"><div class="hdr"><h2>転換初動リーダーボード</h2>{_cp([r["t"] for r in tl["leaders"][:16]])}</div>'
            ...
            f'今<b>21EMAを奪回＋出来高回復</b>したら初動候補。※底は<b>暫定安値</b>（後で安値更新すると回復率は変わる）。</div>'
...
def _leader_temp_card(lt):
    ...
    return (
        ...
        f'<details class="cxpl" open><summary>読み方</summary><div class="cxpl-b">リーダー群（RS189上位10%）の勢いを過去分布の%タイルで表示</div></details>'
        f'{chart}'
        f'</div>')
