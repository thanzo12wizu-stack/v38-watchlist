...
Command Center — dashboard builder (v2・確定)
  A. 個別スリーブ(40%): N=15, 189日RS上位 × 終値>50日線 × 週足SAR非弱気(W-FRI) × 出来高金額$10M/日 × 株価$5以上,
     ...
     ピーク×0.65トレールで利を伸ばし割れたら全売り, 初期ストップ建値×0.70(-30%), ストップ後2ヶ月クールダウン。
     赤明け=翌営業日の寄りで即トップ15再選定。月中退出の空き枠は再投下せず現金で持ち月末トゥルーアップで吸収。
...
def build_buy_today(m, s2i, e2j, s2t, k=20, pick_set=None):
    ...
    def classify(r):
        ...
        vsurge = (rvol is not None and rvol == rvol and rvol >= 1.3)
    ...
    for prio, _nr, t, r, tag, tcls in selected:
        ...
        out.append(dict(t=t, rs=r["rs"], tag=tag, tcls=tcls,
                        ...
                        rvol=rvol, vdry=vdry,
                        dry=(vdry is not None and vdry == vdry and vdry <= 0.90),
                        pivot=pivot, stop=float(r["close"]) * 0.70,
...
def _buy_today_card(buys):
    ...
    for b in buys:
        ...
        if b["tag"] == "ブレイク":                       # ブレイクは当日の出来高増(RVOL)
            ...
            vcls = "pos" if (rv == rv and rv >= 1.3) else "mut"
        else:                                            # それ以外は枯れ(vdry)
    ...
    return (f'<div class="card hot-card"><div class="hdr"><h2>本日のピックアップ</h2>'
            ...
            f'<b>押し目</b>=直近高値からの下げ幅（バーが短いほど高値に近い）／<b>★</b>=トップ15。</div>'
...
def _buy_card(buys, pick_set=None):
    ...
    rows = []
    for b in buys:
        held = '<span class="bt-held">★トップ15</span>' if (pick_set and b["t"] in pick_set) else ''
        rows.append(
            ...
            f'<div><span class="k">利確</span><span class="v mut">ピーク×0.65</span></div>'
    ...
    return (f'<div class="card"><div class="hdr"><h2>本日の押し目シグナル <span class="role-tag rt-act">仕掛ける</span></h2>'
            f'{_cp([b["t"] for b in buys])}</div>'
            f'<div class="sub">RS90以上の主力（リーダー）で押し目が成立した銘柄。<b>押し目</b>＝直近高値からの下げ幅、<b>ADR</b>＝1日の平均値幅（値動きの荒さ）、<b>RVOL</b>＝平常比の出来高（注目度）。エントリー幅と初期ストップ付き（利確はトレール・固定目標なし）。<b>★トップ15</b>＝機械システムの保有候補と一致。</div>'
            + "".join(rows) + '</div>')
...
var NQG={Blue:[1,1],Green:[0.5,0.5],Yellow:[0.5,0],Red:[0,0]};
...
function plHtml(n,e,tk){
  ...
  var pl=(n.px/e-1)*100, stop=e*0.70, pk=ccPeak(n.t,n.px,e), tr=pk?pk*0.65:null;
  ...
  return '<span class="'+(pl>=0?'pos':'neg')+'" style="font-weight:700">'+(pl>=0?'+':'')+pl.toFixed(1)+'%</span>'
       + '<div class="mut" style="font-size:10px">−30% $'+stop.toFixed(2)+sh+' ・ trail $'+(tr?tr.toFixed(2):'—')+trHit+'</div>';
}
function entSave(el){
...
function calcRun(){
  ...
  if(has){
    ...
    sum.innerHTML=s;
    // 構成C：月中の売却枠は再投下しない（トリム＝燃料が無く、実測で moot〜僅か悪化：45.3/−21.1/1.66 なし vs 45.0/−21.6/1.62 あり）。現金保持→月末トゥルーアップで吸収。
    if(freed>0){
      lev+='<div class="redep mut" style="padding:8px 10px">売却で空いた $'+freed.toFixed(0)+' は<b>現金のまま保持</b>し、次の月次トゥルーアップで新トップ15へ均等配分。月中の再投下はしない（トリムが無い構成Cでは再投下に妙味なしと実測で確認済み）。</div>';
    }
  } else { sum.innerHTML=''; }
...
def build_today_action(sar, mkt, asof):
    ...
    return (f'<div class="todayact ta-{ccls}" id="taCard" data-srv="{color or ""}">'
            ...
            f'<div class="ta-act ta-{acls}" id="taAct">{act}</div>{rebal_line}'
...
def _alloc_tab(calc):
    ...
    return (
        ...
        '<b>現在損益・−30%初期ストップ・トレール(ピーク×0.65)</b>を表示する。'
        'トレールまたはストップに当たった銘柄は<b>「売却済」</b>を押すと、その枠を空けて集計する。'
        '空いた枠は<b>月中は再投下せず現金で保持</b>し、次の月次トゥルーアップで新トップ15へ均等配分する（実測で月中再投下は改善せず）。'
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    port = (f'<div class="card"><div class="hdr"><h2>ポートフォリオ（個別株スリーブ）</h2>'
            ...
            f'ピーク×0.65トレールで利を伸ばす／建値−30%初期ストップ＋2ヶ月クールダウン／RS上位30の継続床で月次入替。'
    ...
    rules = (
        ...
        f'<li><b>トレール</b>：<b>ピーク×0.65のトレール</b>（ピーク更新で切上げ）で利を伸ばし、割れたら全部売り。固定の利確ラインは設けない。</li>'
        ...
        f'<li><b>赤入り</b>＝両スリーブ全現金。<b>赤明け＝翌営業日の寄りで即座に</b>最新トップ{N_PORT}を選び直す（月末を待たない・赤の間に主役交代するため）</li><li><b>月中の空き枠</b>：ストップ／トレールで出た現金は<b>月中は再投下せず現金で保持</b>し、翌月の月次トゥルーアップで新トップ15へ均等配分（実測で月中再投下は改善せず＝トリム廃止に伴い不要）</li>'
        f'<li>青⇄緑⇄黄の移動は同じ銘柄を持ったまま%だけ増減（選び直さない）</li>'
        f'<li>保有株が月中に弱っても、<b>トレール・ストップ以外</b>では動かさない（週足SAR弱気・200日割れ・RS低下だけでは売らず、月次トゥルーアップで処理）</li>'
        f'</ul>'
        ...
        f'NQの確定状態は手動(TradingView)が真。自動推定はOniMine復元FSM（実状態と約95%一致・方向99%）だが、'
        f'境界（青↔緑・赤↔黄）と反転直後は外し得るので、手動記録ボタンで上書きできる。</div></div>')
    ...
    body = (build_today_action(sar, mkt, asof)
            ...
            f'<section id="t-sector">{_sector_rank_card(mkt.get("sector_ranks"))}{sector}</section>'
