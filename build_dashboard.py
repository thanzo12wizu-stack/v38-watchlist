...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    for t, th, r in picks:
        ...
        nu_tag, _nun, nu_cls = momentum_nuance(r)
        # 組み込み待ち判定: 急落局面(20日安値割れ or 5日-15%)かつ投げ(capit5)未確認 → 一服待ち
        ...
        _wait_badge = '<span class="waitb">組み込み待ち</span>' if (_drop and not _clx) else (
                      '<span class="okb">投げ確認・解禁</span>' if (_drop and _clx) else "")
        _wtr = ' class="row-wait"' if (_drop and not _clx) else ""
        rows.append(
            f'<tr{_wtr} data-liq="{(r.get("dvol") or 0)/1e6:.1f}" data-tkone="{t}"><td class="l mut">{_raw_rank.get(t,"—")}</td>'
            f'<td class="l tk">{t}{_wait_badge}'
    ...
    for rank, (t, r) in enumerate(deck.iterrows(), start=N_PORT+1):
        ...
        _trcls = ' class="row-excluded"' if _exq else ""
        wrows.append(
    ...
    deck_card = (f'<div class="card"><div class="hdr"><h2>RSリーダー控え</h2>{_cp(list(deck.index))}</div>'
             ...
             f'並びでどの業種が強いかも分かる。<b>除外（バイオ）</b>＝思想的に選定対象外（表示のみ・繰り上がらない）</div>'
             f'<table><tr><th class="l">#</th><th class="l">銘柄</th><th>RS</th>'
