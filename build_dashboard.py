...
def _ov_fold_patch(tpl):
    ...
    js = ("<script>function ovFold(){try{"
          ...
          # コメント（連続する .cmt をまとめて1つに）
          "var c=document.querySelector('.cmt');"
          "if(c&&!c.closest('details.ovfold')){var d=mk('コメント（今日・今週）');c.parentNode.insertBefore(d,c);"
          "var nodes=[],x=c;while(x&&x.classList&&x.classList.contains('cmt')){var nx=x.nextElementSibling;nodes.push(x);x=nx;}"
...
def build_overview_cards(ctx):
    ...
    try:
        ...
        try:
            ...
            for s in series:
                lab = (s.get("label") or s.get("name") or "").upper()
                vals = s.get("vals") or s.get("values")
                if vals and "QQQ" in lab:
                    Dparts["p_qqq"] = _ov_resample([float(v) for v in vals], 28)
                elif vals and "SPY" in lab:
                    Dparts["p_spy"] = _ov_resample([float(v) for v in vals], 28)
        except Exception:
            pass
        if Dparts:
        ...
        try:
            ...
            if m is not None and hasattr(m, "sort_values"):
                col189 = "rs189" if "rs189" in m.columns else None
                if col189:
                    dd = m.copy()
                    dd = dd[dd[col189] >= 85].sort_values(col189, ascending=False).head(12)
                    rows = []
                        ...
                        def gi(*names):
                            for nm in names:
                                if nm in row and row[nm] == row[nm]:
                                    return row[nm]
                            return None
                        r21 = int(round(float(gi("rs21", "rs63", col189) or 0)))
                        r63 = int(round(float(gi("rs63", col189) or 0)))
                        r189 = int(round(float(gi(col189) or 0)))
    ...
    try:
        ...
        try:
            ...
            if isinstance(dv, dict):
                def _lastb(series):
                    ...
                    if vv:
                        try:
                            return float(vv[-1][1] if isinstance(vv[-1], (list, tuple)) else vv[-1])
                        except Exception:
                            return None
                    return None
                qqqb = _lastb("QQQ") or _lastb("qqq")
                spyb = _lastb("SPY") or _lastb("spy")
            if qqqb is not None:
        ...
        try:
            ...
            bot = sorted(secs, key=lambda s: _sw(s))[:1]
            pa50 = breadth.get("pa50")
            adv_txt = f"上昇銘柄{pa50*100:.0f}%" if isinstance(pa50, (int, float)) and pa50 <= 1.5 else (
                f"上昇銘柄{pa50:.0f}%" if isinstance(pa50, (int, float)) else "")
            if top:
                tnm = "・".join((t.get("ja") or "—") for t in top)
                lead_txt = f"主導は{tnm}"
                if isinstance(_sw(top[0]), (int, float)):
                    lead_txt += f"（{_sw(top[0])*100:+.1f}%）" if abs(_sw(top[0])) <= 1.5 else f"（{_sw(top[0]):+.1f}%）"
                bot_txt = f"、弱いのは{bot[0].get('ja')}" if bot else ""
                cx1 = lead_txt + bot_txt + ("。" + adv_txt if adv_txt else "。")
                doc1 = _re2.sub(r'(<div class="cl">今日</div><div class="cx">)[^<]*(</div>)',
                                r'\g<1>' + cx1 + r'\g<2>', doc1, count=1)
                # 今週
                regnm = (sar[0] if isinstance(sar, (list, tuple)) and sar else None) or "現状"
                cx2 = f"資金は{tnm}へ流入。レジームは{regnm}を継続。"
                doc1 = _re2.sub(r'(<div class="cl">今週</div><div class="cx">)[^<]*(</div>)',
                                r'\g<1>' + cx2 + r'\g<2>', doc1, count=1)
        except Exception:
            pass
...
def _market_comment(aux, mkt, sar, breadth):
    ...
    return (
        ...
        f'{note_html}<div class="mkt-post-tools"><span></span>'
