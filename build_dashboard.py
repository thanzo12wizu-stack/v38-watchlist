...
def compute_metrics(W, order, s2i=None):
    ...
    df["rs_w1"] = df["ret63_w1"].rank(pct=True) * 100 if "ret63_w1" in df else df["rs_l1"]  # 先週(~5d前)
    df["rs_w2"] = df["ret63_w2"].rank(pct=True) * 100 if "ret63_w2" in df else df["rs_l1"]  # 2週間前(~10d前)
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    deck_card = (f'<div class="card"><div class="hdr"><h2>RSリーダー控え</h2>{_cp(list(deck.index))}</div>'
             ...
             f'並びでどの業種が強いかも分かる。<b class="lagb" style="padding:0 4px">出遅れ（選外）</b>＝RS上位だが業種内で−20%劣後（表示のみ・選定/繰上げ対象外）。<b>除外（小型創薬）</b>＝臨床段階・中小型バイオ（ヘッドライン一発で乱高下）を選定対象外。大手バイオ/製薬/GLP-1/手術ロボは対象。</div>'
             f'<table><tr><th class="l">#</th><th class="l">銘柄</th><th>RS</th>'
