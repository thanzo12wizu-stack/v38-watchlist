...
def compute_metrics(W, order):
    ...
    for t in order:
        ...
        recs.append(dict(
            ...
            pchg_prev=(prev/c.iloc[-3] - 1) if (len(c) >= 3 and c.iloc[-3] and not np.isnan(c.iloc[-3])) else np.nan,
...
def build_buy_today(m, s2i, e2j, s2t, k=10):
    ...
    sub = sub[sub["ext21s"] < 8]                       # 過熱(21EMA+8%以上)を除外＝「非過熱」表記と一致
    if sub.empty:
        return []
...
def _buy_card(buys):
    ...
    return (f'<div class="card"><div class="hdr"><h2>本日の押し目シグナル</h2>'
            f'{_cp([b["t"] for b in buys])}</div>'
            f'<div class="sub">RS90以上のリーダーで押し目成立。エントリー幅・初期ストップ（利確はピーク×0.65トレール／固定目標なし）（{len(buys)}銘柄）</div>'
            + "".join(rows) + '</div>')
