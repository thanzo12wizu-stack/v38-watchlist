...
def build_rrg(macro, win=63, mom=10):
    ...
    for tk, ja in SECTOR_ETFS:
        ...
        pts.append(dict(tk=tk, ja=ja, x=x, y=y, q=q, cross=(cross and q in ("改善", "主導"))))
    return pts
...
def _rrg_card(pts):
    ...
    for p in pts:
        ...
        ring = f'<circle cx="{X(p["x"]):.1f}" cy="{Y(p["y"]):.1f}" r="10" fill="none" stroke="#fff" stroke-width="1.6" opacity="0.9"/>' if p["cross"] else ""
        dots += (f'{ring}<circle cx="{X(p["x"]):.1f}" cy="{Y(p["y"]):.1f}" r="5.5" fill="{col}"/>'
                 f'<text x="{X(p["x"]):.1f}" y="{Y(p["y"])-9:.1f}" fill="#dbe4ef" font-size="13" font-weight="700" text-anchor="middle">{p["ja"]}</text>')
