...
def _buy_card(buys):
    ...
    return (f'<div class="card"><div class="hdr"><h2>本日の押し目シグナル</h2>'
            f'{_cp([b["t"] for b in buys])}</div>'
            f'<div class="sub">ルールが押し目買いと判定したリーダー（正式な買い場）。エントリーレンジと+15%/+30%目標（{len(buys)}銘柄）</div>'
            + "".join(rows) + '</div>')
...
function showDet(tk){
  ...
  document.getElementById('dov-nm').textContent=d.nm||'';
...
def _det_json(m, names, tapset):
    ...
    for t in tapset:
        if t not in m.index:
            continue
        r = m.loc[t]
        out[t] = {"nm": names.get(t, ""), "rs": _r(r.get("rs"), 0),
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    mri_bd = (f'<div id="mri-bd" class="mri-bd" onclick="event.stopPropagation()">'
              ...
              + "".join(_bd_rows) + '<div class="mnote">もう一度タップで閉じる ▴</div></div>')
    banner = sar_pill + f"""
    ...
    today = comment + role_note + _buy_today_card(buytoday) + buy_card + leaders_card + newhigh_card
    ...
    det_json = _det_json(m, names, tapset)
    dov = ('<div id="dov-bg" class="dov-bg"><div class="dov" onclick="event.stopPropagation()">'
           ...
           '<div id="dov-nm" class="dov-nm"></div>'
