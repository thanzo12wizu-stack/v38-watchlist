...
def build_portfolio(m, s2t):
    ...
    pool = pool[~pool.index.map(lambda t: is_excluded_theme(t, s2t))]
...
EXCLUDE_THEMES = ("6. バイオ・ヘルスケア",)
def is_excluded_theme(t, s2t):
    v = s2t.get(t)
    th = (v[0] if isinstance(v, list) and v else v) or ""
    return any(th.startswith(x[:2]) and "バイオ" in th for x in EXCLUDE_THEMES) or th in EXCLUDE_THEMES
...
def build_leader_temp(W, win=756):
    ...
    # 推移: 利用可能な範囲(最大win)・%タイル（マーケットスコア形式）
...
def _leader_temp_card(lt):
    ...
    if n < 3:
        ...
        chart = (f'<div class="chart" style="margin-top:14px">{svg}'
                 f'<div class="cap cap-c"><span style="color:#e6edf3;font-weight:700">推移・{span}</span></div>'
                 f'{_date_axis(ts)}</div>')
    return (
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    # 保有テーブル = RS上位から連番で、選定12が埋まるまでの全銘柄(選外=出遅れ/バイオもバッジ付きで同じ表に)
    _pick_set = set(t for t, _, _ in picks)
    rows = []
    _filled = 0
    for i, (t, r) in enumerate(cand.iterrows(), 1):
        if _filled >= N_PORT and t not in _pick_set:
            break                                   # 12銘柄選定済み＝以降は控えへ
        sth = subtheme_of(t, s2t, e2j.get(s2i.get(t), "—"))
        ...
        if _selected:
            _filled += 1
        _exq = bool(r.get("excluded_theme")); _lag = bool(r.get("laggard"))
        # 選外バッジ / 組み込み待ち判定
        if _exq:
            _badge = '<span class="exq">除外（バイオ）</span>'; _trcls = ' class="row-excluded"'
        elif _lag:
            _badge = '<span class="lagb">出遅れ（選外）</span>'; _trcls = ' class="row-excluded"'
        else:
            ...
            _badge = ('<span class="waitb">組み込み待ち</span>' if (_drop and not _clx) else
    ...
    port = (f'<div class="card"><div class="hdr"><h2>ポートフォリオ（個別株スリーブ）</h2>'
            ...
            + alloc_bar +
            f'<div class="note rk-note"><b>RS</b>＝189営業日（≒9ヶ月）リターンの全市場パーセンタイル。<b>#</b>＝RS順位（保有＝上位・控え＝以降）。銘柄名の下にリーダー状態と値動きを表示。<b>リーダー</b>＝63日RS≥85 かつ 200MA上（欠けると「リーダー外」）。<b>選外（出遅れ/バイオ）も同じ表にRS順で表示</b>（グレーアウト・選定/繰上げ対象外）。</div>'
    ...
    setup_card = (f'<div class="card"><h2>セットアップ別</h2>'
                  ...
                  + "".join(setup_html) + '</div>')
    # 控え = 保有テーブル(選定12が埋まるまでのRS順)に出た範囲より下＝真に次の繰上げ候補。
    _pick_set = set(t for t, _, _ in picks)
    # 保有テーブルの最終行の位置(選定12個目のcand内index位置)を求める
    _last_pos = 0
    _cnt = 0
    ...
    for rank, (t, r) in enumerate(deck.iterrows(), start=_last_pos+2):
        ...
        _exbadge = ('<span class="exq">除外（バイオ）</span>' if _exq else
                    '<span class="lagb">出遅れ（選外）</span>' if _lag else "")
        _trcls = ' class="row-excluded"' if (_exq or _lag) else ""
        wrows.append(
    ...
    deck_card = (f'<div class="card"><div class="hdr"><h2>RSリーダー控え</h2>{_cp(list(deck.index))}</div>'
             ...
             f'並びでどの業種が強いかも分かる。<b class="lagb" style="padding:0 4px">出遅れ（選外）</b>＝RS上位だが業種内で−20%劣後（表示のみ・選定/繰上げ対象外）。<b>除外（バイオ）</b>＝思想的に選定対象外。</div>'
             f'<table><tr><th class="l">#</th><th class="l">銘柄</th><th>RS</th>'
    ...
    watch = cap_filter_bar + _mkt_section("① 銘柄を調べる", "ティッカー検索 → タップで詳細") + search_card + _mkt_section("② セットアップで絞る", "複数条件ヒット＝注目度が高い") + overlap_card + setup_card + _mkt_section("③ 今日の値動き", "新高値圏＝需給が強い") + newhigh_card + _mkt_section("④ リーダー", "サイクルを引っ張るRS上位群（RS≥85・200MA上）") + _leaders_watch_card(m)
