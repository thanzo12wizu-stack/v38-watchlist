...
def _market_comment(aux, mkt, sar):
    slope={"↑":"上向き","→":"横ばい","↓":"鈍化"}.get(aux.get("slope"),aux.get("slope")); band=mri_band(aux["hl"])[0].replace("（過熱・反落注意⚠）","（過熱気味）")
    def dirof(ts,up,dn):
        if not ts or len(ts)<60:return None
        st,_=_trend_state([v for _,v in ts],up,dn); return "横ばい" if st=="中立" else st
    bts=mkt.get("breadth_ts",[]); pa200=bts[-1][1] if bts else None; bd=dirof(bts,"拡大","縮小"); ad=dirof(mkt.get("adline_ts",[]),"改善","悪化")
    cr=dirof(mkt.get("credit_ts",[]),"良好","悪化"); vt=mkt.get("vixterm_ts",[]); vv=vt[-1][1] if vt else None; vx="平穏" if vv is not None and vv<.95 else "注意" if vv is not None and vv<=1 else "逆転" if vv is not None else "—"
    dv=mkt.get("dollarvol_ts"); ut=(dv.get("uni_trend") or {}) if isinstance(dv,dict) else {}; turn=ut.get("dir") if ut.get("sig") else "横ばい"
    warn=[k for k,v in (mkt.get("distrib") or {}).items() if v.get("n",0)>=6]
    rst=mkt.get("regime_state") or {}; pulse=mkt.get("lead_pulse") or {}
    det=sum([bd=="縮小",ad=="悪化",cr=="悪化",vx=="逆転",bool(warn),turn=="縮小"]); imp=sum([bd=="拡大",ad=="改善",cr=="良好",turn=="拡大"])
    ...
    if aux.get("hl",0)>=60 and sar and sar[0] in ("Yellow","Red"): verdict += " 指数トレンドを優先。"
    f1=lambda v: f'{v*100:.0f}%' if v is not None and v==v else '—'
    rows=[("内部",f'200MA上 {pa200:.0f}%・幅 {bd or "—"}・騰落 {ad or "—"}・売買代金 {turn}' if pa200 is not None else f'幅 {bd or "—"}・騰落 {ad or "—"}・売買代金 {turn}'),
          ("リーダー",f'三窓85+ {pulse.get("all3",0)}・定着 {pulse.get("stable",0)}・失速 {pulse.get("fade",0)}・中心 {pulse.get("top_theme") or "—"}'),
          ("リスク",f'信用 {cr or "—"}・VIX構造 {vx}'+(('・売り抜け '+"/".join(warn)) if warn else '')),
          ("先行灯",f'F1 {f1(rst.get("f1"))}・F2 {f1(rst.get("f2"))}・F3 {f1(rst.get("f3"))}')]
    note=mkt.get("broad_note"); note_html=f'<div class="cmt-note cmt-{note[1]}">{note[0]}</div>' if note else ''
    return f'<div class="card cmt"><h2>今日のマーケット</h2>{note_html}<div class="lead-head"><b>{verdict}</b><span>{band}・傾き {slope}</span></div><div class="lead-grid">'+''.join(f'<div class="lead-row"><span>{k}</span><b>{v}</b></div>' for k,v in rows)+'</div></div>'
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    market = (
        ...
        _mkt_section("① 結論と行動", "青/緑=運用・黄=新規停止・赤=撤退", en="Verdict & Action")
        + banner
        + comment
        + rotation
