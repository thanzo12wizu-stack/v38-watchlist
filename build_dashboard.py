...
def mri_auxiliary(mri, vals, metrics):
    ...
    last = vals.iloc[-1]
    # 11 bearish conditions (labeled, for display of which are lit) — all independent of MRI itself
    bvals = [
...
def _svg_ratio_card(ts, title, sub, up_label, dn_label, accent, gid):
    """期間先頭=100リベース系列の推移カード（クレジット・攻守ローテーション）。"""
    if not ts or len(ts) < 5:
        return ""
    ys = [v for _, v in ts]; last = ys[-1]
    lo, hi = min(ys), max(ys)
    ...
    svg = _svg_inner(ys, accent, gid, grid, "", lo_pad=pad, hi_pad=pad, ymin=0, ymax=100000)
    up = last >= 100
    col = "#7ff0a8" if up else "#fca5a5"
    state = up_label if up else dn_label
    return (f'<div class="card"><h2>{title}</h2>'
            f'<div class="sub">{sub}</div>'
            f'<div class="chart">{svg}'
            f'<div class="cap"><span>{ts[0][0]}</span>'
            f'<span style="color:{col};font-weight:700">現在 {last:.1f}（{state}）</span>'
            f'<span>{ts[-1][0]}</span></div></div></div>')
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    market = (banner
              ...
              + _svg_ratio_card(mkt.get("credit_ts", []),
                                "クレジット推移（HYG / LQD）",
                                "ハイイールド債÷投資適格債・6カ月前=100。低下＝信用悪化で株より先に割れる先行警報",
                                "リスクオン", "信用悪化・警戒", "#fbbf24", "cg")
              + _svg_ratio_card(mkt.get("defensive_ts", []),
                                "攻守ローテーション（一般消費財 / ディフェンシブ）",
                                "RSPD÷(RSPS必需品・RSPU公益)・6カ月前=100。上昇＝攻め優勢／低下＝守りへ資金逃避(隠れリスクオフ)",
                                "攻め優勢", "守り優勢", "#a78bfa", "dg")
