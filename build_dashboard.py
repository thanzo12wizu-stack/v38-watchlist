...
Command Center — dashboard builder (最終確定 2026-07-05)
  ...
  E. 8 tabs (マーケット/テクニカル/ポートフォリオ/配分/セクターローテ/業種RS/ルール) + 今日の運用ヘッダ + 隔週リバランス点検（月曜）。
...
# ----------------------------------------------------------------------------- live fetch (CI)
MACRO_TICKERS = ["^VIX", "^VIX3M", "^VVIX", "^VXN", "QQQ", "QQQE", "SPY", "HYG", "LQD", "IEI",
                 "RSP", "IWM", "^TNX", "^FVX", "^TYX",
...
def compute_metrics(W, order, s2i=None):
    ...
    for t in order:
        ...
        recs.append(dict(
            ...
            maxabs1d=maxabs1d, lo10=lo10, lo10_prev=lo10_prev, ema21_10ago=ema21_10ago, ret126=ret126, ucr=ucr, tc3=tc3,
            ret63=ret63, ret189=ret189, ret189_l5=ret189_l5, ret189_l10=ret189_l10, ret189_l20=ret189_l20, ret189_l30=ret189_l30, ret21=ret21, hi52=hi52, dist52=dist52, pos52=pos52,
...
def _sentiment_card(senti):
    ...
    return (
        ...
        f'<b>逆張りが効くのは極値のみ</b>（過熱≥85／総悲観≤15）・中間帯はノイズ。<b>参考指標・売買ルールには不関与</b></div>'
        + "".join(comp_rows)
...
def _equity_card(eq, at=None):
    setup = (
        ...
        '<div class="sub">口座資産の推移に21EMAを引き、割れたら新規停止・サイズ半減の判断材料にする（参考指標＝売買には使わない）。</div>'
...
def track_holdings(prev, picks, color, asof_date, m=None):
    ...
    def _process(t, px, ema21=None, lo10=None):
        ...
        # 出口（確定・二層）:
        #   層1=初期ハードストップ max(10日安値×0.998, 21EMA×0.99)＋建て時istop → R・+3R⅓利確。建て初期の下方防御。
        #   層2=ワイドトレール ピーク×0.70（黄も0.70・締めなし）。利が乗った局面のランナー。
        #   実効ストップ = 両者の高い方。初期はハード、伸びたらワイドが上回り自然に引き継ぐ。
        #   （現値より上の線は無効＝21EMA割れ時にema線が上に来るのを除外）。
        ...
        hard_cands = [x for x in (lo_stop, ema_stop) if x > 0 and x < px]
        hard_stop  = max(hard_cands) if hard_cands else px * 0.90
        wide_stop  = peak * tf if (peak and peak > 0) else 0.0     # 層2: ピーク×0.70（tfは上で0.70固定）
        _cands = [x for x in (hard_stop, wide_stop) if x > 0 and x < px]
        stop = max(_cands) if _cands else px * 0.90
        # 初期ストップ（建て時に確定）→ R と +3R 目標。istop は state に保存し引き継ぐ。
        istop = float(ph.get("istop") or hard_stop)
        if istop >= px:                      # 保険: 建値情報欠落等で不正なら現値基準に矯正
            istop = min(hard_stop, px * 0.97)
        R = ep - istop
        t3 = ep + 3.0 * R if R > 0 else None                 # +3R で 1/3 利確（建値移動なし）
        hit3 = bool(t3 is not None and px >= t3)
        # どの線が実効ストップかを表示用に判定
        if   stop == wide_stop and wide_stop > 0: _which = "ピーク×0.70"
        elif stop == lo_stop   and lo_stop   > 0: _which = "10日安値"
        elif stop == ema_stop  and ema_stop  > 0: _which = "21EMA"
        else:                                     _which = "—"
...
def _fade_badge(r):
    """防御アラート: 189日RSはまだ強い(≥80)のに短期(63日/21日)RSが細り＝勢い低下の初動。
       順張りの"降り遅れ"対策の早期警戒。参考指標（自動売却はしない・降りは確定出口線で）。"""
    try:
...
def build_theme_hierarchy(m, s2t):
    """大テーマ(表示名)→サブテーマ群のローテ+構成銘柄。RRGチップ(大テーマ)のjaキーと一致。
       全367サブテーマが必ず親を持つ(取りこぼしゼロ)。63×189日RS中央値で粒度統一。<b>参考指標</b>＝この数字で売買はしません（見て相場観をつくる用）。"""
...
def build_rrg(m, s2t, win=63, mom=10):
    ...
    rk = {"now": _rank("ret189"), "w1": _rank("ret189_l5"),
          "w2": _rank("ret189_l10"), "m1": _rank("ret189_l20"), "m0": _rank("ret189_l30")}
    pts = []
    for theme, tickers in grp.items():
        ...
        per = {}
        xs = {}
        for pk, src in rk.items():
            if src is None:
                continue
            xs[pk] = float(np.nanmedian(src.loc[idxs]))
        if "now" not in xs or xs["now"] != xs["now"]:
            continue
        # 各期間の勢い y = そのxと1つ前の期間のxの差(5日あたり)
        def _y(cur, prev):
            if cur in xs and prev in xs and xs[cur] == xs[cur] and xs[prev] == xs[prev]:
                d = xs[cur] - xs[prev]
                return d if cur == "now" else d
            return 0.0
        order = {"now": ("now", "w1"), "w1": ("w1", "w2"), "w2": ("w2", "m1"), "m1": ("m1", "m0")}
        for pk in ("now", "w1", "w2", "m1"):
            if pk not in xs:
                continue
            cur, prev = order[pk]
            x = xs[pk]; y = _y(cur, prev)
            q = "主導" if (x >= 50 and y >= 0) else "弱化" if x >= 50 else "改善" if y >= 0 else "停滞"
            per[pk] = dict(x=round(x, 1), y=round(y, 1), q=q, cross=bool(y >= 3 and q in ("改善", "主導")))
        if "now" not in per:
            continue
        pn = per["now"]
...
def _rrg_svg(pts, cxv=50, period="now"):
    ...
    xr = max(8.0, max(abs(min(xs) - cxv), abs(max(xs) - cxv)) * 1.20)
    yr = max(2.0, max(abs(min(ys)), abs(max(ys))) * 1.10)
...
def _lev_env_card(le, rrg):
    ...
    return (
        ...
        f'<div class="sub">SOXLを新規で入れる時の環境チェック。<b>投入帯＝SOXXが50MAの+0〜3%上</b>のときだけ段階投入、それ以外はTQQQのみ（MAE実測でテール半減）。<b>参考指標</b>＝この数字で売買はしません（見て相場観をつくる用）。</div>'
...
def build_pullback_screener(m, k=20):
    ...
    try:
        ...
        return (f'<div class="card"><h2>押し目質スクリーナー <span class="pqbadge">裁量参考</span></h2>'
                ...
                f'※これは執行タイミングの質の目安であって機械エントリー条件ではない（統合検証でタイト選定は原型に劣後）。'
                f'生存バイアス上限・この出口固有。</div></div>')
    except Exception as e:
...
def compute_regime_state(m):
    ...
    try:
        ...
        f3 = np.nan; qn = 0
        try:
            e21 = pool["ema21"]; d21q = (pool["close"] / e21 - 1)
            qual = pool[(pool["rs189"] >= LEADER_RS) & (pool["close"] > pool["sma200"])
                        & (pool.get("rs63", pool["rs189"]) >= LEADER_RS)
                        & (d21q >= -0.01) & (d21q < 0.04)]
            qn = len(qual)
            if qn >= 5:
        ...
        return dict(f1=f1, f2=f2, f3=f3, qn=qn, c1=c1, c2=c2, c3=c3,
                    n_bad=n_bad, n_warn=n_warn, f1_names=f1_names, f2_names=f2_names)
    except Exception as e:
        print("[warn] regime state failed:", e)
        return None

def build_regime_alerts(m, st=None, collapsible=False):
    """レジーム先行警戒灯 F1-F3（参考指標・売買には使わない）。"""
    if st is None:
        st = compute_regime_state(m)
    if not st:
        return ""
    try:
        ...
        n_bad = st["n_bad"]
        if   n_bad >= 2: hdr, hcls = "⚠ 防御モード（2灯以上点灯）", "reg-hdr-bad"
        elif n_bad == 1: hdr, hcls = "注意（1灯点灯）", "reg-hdr-warn"
        else:            hdr, hcls = "平常（内部は健全）", "reg-hdr-ok"
        chips1 = " ".join(f'<span class="rgchip">{t}</span>' for t in f1_names) or '<span class="mut">—</span>'
        ...
        return (
            ...
            f'<div class="reg-cell {c1}"><div class="reg-k">F1 リーダー脱落率</div>'
            f'<div class="reg-v">{v1}</div><div class="reg-l">{l1}</div>'
            f'<div class="reg-chips">{chips1}</div></div>'
            f'<div class="reg-cell {c2}"><div class="reg-k">F2 勢い細り率</div>'
            f'<div class="reg-v">{v2}</div><div class="reg-l">{l2}</div>'
            f'<div class="reg-chips">{chips2}</div></div>'
            f'<div class="reg-cell {c3}"><div class="reg-k">F3 キュー崩れ</div>'
            f'<div class="reg-v">{v3}</div><div class="reg-l">{l3}</div>'
            f'<div class="reg-chips mut">—</div></div>'
            f'</div>'
            f'<div class="note">相場全体の内部劣化を早期に捉える先行灯（<b>参考指標</b>＝売買には使わない）。<b>2灯以上で防御へ心構えを切替</b>。'
            f'F1=20日前の上位24→現在36位外の割合（天井≈45日前・≥30%警戒）／F2=上位24のうち63日RS&lt;85（≈26日前・≥40%警戒）／'
            f'F3=待機キュー内の反発待ち比率（≈22日前・≥60%で下落1.9x・母数{qn}）。序列=脱落→勢い細り→高値途絶。'
            f'生存バイアスで全指標は過小＝実際はもっと早く点く。'
            f'<b>3つの警戒系の関係</b>: ①この<b>レジーム警戒灯</b>＝相場全体の内部劣化を3つの%で集約（旧「リーダーブレッドス警戒灯」はF1F2と同一計算だったため本灯に統合）。'
            f'②<b>先導株モメンタム・ラン</b>＝同じ勢い細り(F2)を<b>個別銘柄ごと</b>に加速/巡航/失速で見た版（%集約 vs 一銘柄ずつ）。'
            f'③保有行の<b>勢い細りバッジ</b>＝自分が持っている銘柄がF2に該当したときの個別表示。'
            f'＝全部「189は強いが短期RSが細る」という<b>同じ現象を、粒度違いで見ている</b>。この灯で相場全体、ランで個別群、バッジで自玉を確認する。</div></div></div>')
    except Exception as e:
...
def build_dollarvol_ts(W, macro=None, lookback=CHART_LB, smooth=21):
    """売買代金推移: ユニバース合算(終値×出来高の総和)＋QQQ/SPYの売買代金を、
       ...
       あわせてユニバース売買代金のトレンド（拡大/縮小）を判定してコメント用に返す。<b>参考指標</b>＝この数字で売買はしません（見て相場観をつくる用）。"""
...
def build_updown_vol_ts(W, macro=None, lookback=CHART_LB, win=20):
    """上げ日出来高÷下げ日出来高（20日・ユニバース合算$売買代金）。O'Neil式の集積/分散。
       市場の方向はQQQ日次で判定（取れなければ等加重ユニバース平均リターン）。>1=買い集め優勢。<b>参考指標</b>＝この数字で売買はしません（見て相場観をつくる用）。"""
    C = W.get("Close"); V = W.get("Volume")
...
def _updown_vol_card(ts):
    ...
    return (f'<div class="card"><h2>集積／分散（上げ日÷下げ日 売買代金）</h2>'
            ...
            f'ブレッドスやディストリビューション・デイとは独立した「本物か」の裏取り。<b>参考指標</b>＝この数字で売買はしません（見て相場観をつくる用）。</div>'
            f'<div class="chart">{svg}'
...
def _expected_move_card(em):
    ...
    return (f'<div class="card"><h2>オプション想定変動幅（VIX/VXN・約1ヶ月）</h2>'
            ...
            f'保有個別の想定下値は<b>ADR（実現ボラ）</b>ベースでポート表に反映（出口線がノイズ内なら印）。<b>参考指標</b>＝この数字で売買はしません（見て相場観をつくる用）。</div>'
            f'{rows}</div>')
...
def build_ftd(macro, idxs=(("QQQ", "NASDAQ100"), ("SPY", "S&P500"))):
    ...
    for tk, lab in idxs:
        ...
        # FTDの有効窓: Day1から25日以内に出たものだけを有効な確認とみなす（IBDの実務窓）
        ftd_valid = (ftd_ago is not None and day1 is not None and (att_day - ftd_ago) <= 25)
        ...
        above200 = bool(sma200 is not None and sma200 == sma200 and float(c.iloc[-1]) >= float(sma200))
        if ftd_valid and ftd_ago <= 40:
            # 確認済み：40日以内なら点灯表示（15日固定を撤廃＝古い点灯を無視しない）
            st, cls = f"点灯 {ftd_ago}日前 +{ftd_pct * 100:.1f}%", "pos"
        elif above50 and above200:
            # 既に明確な上昇トレンド＝FTDの枠組みは非対象。試行カウントは出さない
            st, cls = "上昇継続中", "mut"
        elif above50 and days_low >= 12:
            st, cls = "上昇継続中", "mut"
        elif att_day > 25:
            # 25日超えても未確認＝有効な試行でない（だらだら反発・押し戻し）
            st, cls = "試行長期化・未確認（だまし警戒）", "warnt"
        elif att_day >= 1:
...
def _market_comment(aux, mkt, sar):
    ...
    if rst:
        ...
        if nb >= 2:
            rtxt = (f"<b>内部の先行灯が2つ以上点灯（防御寄り）</b>——リーダー脱落{f1s}・勢い細り{f2s}・キュー崩れ{f3s}。"
...
def build_defense_checklist(n_bad):
    """F1-F3が2灯以上点灯時のみ表示する防御チェックリスト（心構え・売買非連動）。"""
    if n_bad < 2:
        return ""
    return (f'<div class="card def-card"><h2>⚠ 防御チェックリスト <span class="def-badge">警戒灯2灯以上</span></h2>'
            f'<div class="sub">内部劣化が複数の先行灯で確認された局面。<b>売買ルールは変えず</b>、以下で心構えと執行の質を上げる。</div>'
            f'<ul class="def-list">'
            f'<li><b>新規サイズを控える</b>— 通常0.75%リスク/件を、確信度の低いものは半分に。枠を無理に埋めない</li>'
            f'<li><b>レバの途中参入を止める</b>— SOXL/TQQQの新規トランシェ投入を保留（SOXX 50MA上0-3%帯でも見送り）</li>'
            f'<li><b>利確を早める意識</b>— +3R到達玉は⅓利確を確実に（不敗化）。ただし残りのワイドトレールは動かさない</li>'
            f'<li><b>出口線を再確認</b>— 全保有のピーク×0.70距離をチェック。近い玉から目視レビュー</li>'
            f'<li><b>旧リーダーを記録</b>— この局面のリーダー群を控えておく（転換時に+28.8%で回帰する候補）</li>'
            f'</ul>'
            f'<div class="note">序列=脱落(F1)→勢い細り(F2)→キュー崩れ(F3)→高値途絶。'
            f'これは「防御モードに心構えを切替える」トリガーであって自動売買ではない（短期タイミング予測力はゼロ・検証済み）。'
            f'2灯以上が同時点灯した時のみ表示。1灯なら振るい落としの可能性が残る。</div></div>')
...
def _leader_run_card(lr):
    ...
    order = {"加速": 0, "巡航": 1, "失速": 2}
    rows = sorted(lr["rows"][:20], key=lambda r: order.get(r["phase"], 3))
    chips = ""
    for r in rows:
    ...
    return (f'<div class="card"><h2>先導株モメンタム・ラン</h2>'
            ...
            f'<b class="mr-acc">加速</b>＝ラン拡大／<b class="mr-cru">巡航</b>＝高値維持／<b class="mr-fad">失速</b>＝ラン細り。<b>参考指標</b>＝心構え用（売買には使わない）。</div>'
...
def build_leader_temp(W, win=CHART_LB):
    """先導株の強さ温度計: 先導株(RS189上位10%)の63日リターン平均の、過去分布に対する%タイル系列。
       予測でなく現状描写。非対称=左端(枯渇)のみNQ底に中央値18日先行(的中6割)/右端(過熱)は先取りせず。<b>参考指標</b>＝この数字で売買はしません（見て相場観をつくる用）。"""
    closes = W.get("Close")
...
def _leader_temp_card(lt):
    """先導株の強さ: %タイル推移（マーケットステータスと同形式のゾーン塗り折れ線）。<b>参考指標</b>＝この数字で売買はしません（見て相場観をつくる用）。"""
    ...
    return (
        ...
        f'<div class="sub">リーダー群(RS189上位10%)の勢いが過去分布の何%タイルか＝獲物の温度。露出には非接続・<b>参考指標</b>（売買には使わない）。</div>'
        f'{chart}'
...
def _patterns_card(pats):
    ...
    return (f'<div class="card"><h2>テクニカル・パターン別</h2>'
            ...
            f'<b>眼を鍛える用</b>（仕掛けはブレイク実現を確認して裁量で）。タップで詳細。</div>{blocks}</div>')
...
.mrchip{display:flex;align-items:center;gap:7px;padding:6px 9px;border-radius:8px;background:#0e1420;border:1px solid #1e2836;cursor:pointer}
...
.mrtk{font-weight:800;font-size:12.5px;min-width:44px}
.mrph{font-size:9.5px;font-weight:700;padding:1px 5px;border-radius:4px;background:#121a29;color:#9fb0c5}
.mrchip.run-acc .mrph{color:#43c98a}.mrchip.run-cru .mrph{color:#8fb3ff}.mrchip.run-fade .mrph{color:#e5645e}
.mrtr{font-family:ui-monospace,Menlo,monospace;font-size:11px;color:#7e8ca0;margin-left:auto}
.mrtr b{color:#dbe4ef}
...
function swPickFill(){
  ...
  if(c){
    ...
    document.getElementById('swStopHint').textContent='自動: max(10日安値×0.998, 21EMA×0.99)';
...
function swingCalc(){
  ...
  out.innerHTML=
    ...
    +'<div class="mut" style="font-size:11px;margin-top:8px">Rは「1株の損切り幅」。株数は<b>1回の負けが資産の'+(risk*100).toFixed(2)+'%に収まる</b>ように自動決定。'
      +'ストップは確定出口（10日安値/21EMA）。<b>+3Rで1/3利確→残りは10日安値割れで全売り・建値移動なし</b>。R:R比は初期3.0。</div>';
}
...
function calcRun(){
  ...
  if(has&&levPool>0){
    ...
    if(sw>0){
      ...
      if(tr>0){ lev+='<div class="soxl-tr"><span class="mut">既存 '+tr+'/3 は継続（帯外では買い増さない・保有は地合い/建値ルールで管理）</span><span class="soxl-btns"><button class="nqb b-clear" onclick="soxlTrSet(0)">リセット</button></span></div>'; }
    }
    lev+='</div>';
  }
  if(has){
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    for i, (t, r) in enumerate(cand.iterrows(), 1):
        ...
        rows.append(
            ...
            f'{_state_badge(code, nu_tag, nu_cls, _rsn)}{_er_badge(t, _er, mkt.get("asof_bar"))}{_adr_badge(r.get("adr"))}{_fade_badge(r)}{_chase_alert(r)}{_ent_btn(t)}</div></td>'
    ...
    port = (f'<div class="card"><div class="hdr"><h2>ポートフォリオ（個別株スリーブ）</h2>'
            ...
            f'<div class="note rk-note"><b>買うのは189日RS≥85＆200MA上のリーダーから、63日RS≥85のもの上位{N_PORT}だけ・位置不問</b>（エントリー精緻化は全検証で原型に劣後）。'
            f'<b>出口</b>＝初期ストップ建値×0.75（ワイド）→伸びたらピーク×0.70トレール（黄も0.70）。'
            f'<b>赤明けは青/緑復帰の翌寄りに即再構築</b>（B1）・黄締めなし（B2）＝Fableゲート通過済(+8.6pt/+0.42)。'
    ...
    if _carry:
        ...
        for t in _carry:
            ...
            crows.append(
                ...
                f'<td class="l tk">{t}{_risk_flag_badge(t, r.get("close"))}<span class="carryb">順位落ち・保有中</span>'
    ...
    _carry_sec = (_mkt_section("② 順位落ち保有", "トップ12外だが継続OK＝月曜まで監視", en="Demoted Holdings") + carry_card) if carry_card else ""
    _deck_num = "③" if carry_card else "②"
    _rst = compute_regime_state(m)
    _regime_panel = build_regime_alerts(m, _rst, collapsible=True)
    _defense = build_defense_checklist(_rst["n_bad"] if _rst else 0)
    ...
    today = (_liq_bar()
             + _mkt_section("① 銘柄を調べる", "ティッカー検索 → タップで詳細", en="Ticker Search") + search_card
             + _mkt_section("② リーダー監視", "RS上位群（RS≥85・200MA上）を状態別に", en="Leaders") + leaders_card
             + _mkt_section("③ 圧縮・ベース", "VCP・保ち合い・ブレイク近", en="Setups: VCP / Base / Breakout")
             + _vcp_card(vcp_rows) + _patterns_card(patterns) + _buy_today_card(buytoday)
             + _mkt_section("④ ポジション・サイザー", "候補を選ぶ → 株数・損切り・利確を円と%で", en="Position Sizer") + _swing_planner_card())
    ...
    market = (
        ...
        + _mkt_section("③ 先行灯（崩れの予兆）", "種類=先行3〜8週。指数がまだ高値のうちに内部劣化を捉える",
                       "<b>F1 リーダー脱落率</b>＝20日前に上位24だった強い銘柄のうち、今36位より下に落ちた割合（≥30%で警戒・天井の≈45日前に点く最速の灯）。"
                       "<b>F2 勢い細り率</b>＝今の上位24のうち、63日RSが85未満に細った割合（≥40%で警戒・≈26日前）。"
                       "<b>F3 キュー崩れ</b>＝買い候補キュー内で「反発待ち（深く押した）」銘柄の比率（≥60%で下落1.9倍・≈22日前）。"
                       "崩れは<b>この順番</b>で来る: 脱落(F1)→勢い細り(F2)→キュー崩れ(F3)→高値途絶。"
                       "<b>2灯同時=防御チェックリスト起動</b>（新規サイズ半減・レバ途中参入停止・+3R利確確実）——売買ルールは変えない、構えだけ変える。"
                       "クレジット・VIX期間構造・売り抜け日・センチメントは灯の<b>裏取り用</b>（単体で動かない）。"
                       "1灯なら振るい落としの可能性が残る。生存バイアスで全灯は過小＝実際はもっと早く点く。", en="Early-Warning Lights")
        + build_regime_alerts(m)
        + _svg_ratio_card(mkt.get("credit_ts", []),
                          ("クレジット推移（HYG / IEI・金利中立化）" if mkt.get("credit_den") == "IEI"
                           else "クレジット推移（HYG / LQD）"),
                          ("ハイイールド債 ÷ <b>デュレーション近接の米国債(IEI 3-7年)</b>・平均からの偏差σ（2年）。"
                           "HYG/LQDのデュレーション差（金利汚染）を排し<b>信用リスクを純粋化</b>。0=平常・プラス=リスクオン"
    ...
    rules = (
        ...
        f'<li><b>出口</b>（Fableゲート差戻し 2026-07-08）：<b>初期ストップ = 建値×0.75（ワイド）</b> → <b>ピーク×0.70トレール（黄も0.70）</b>。二層出口（タイト初期＋3R利確）は統合検証で−6.6pt CAGR＝ランナーバグ由来の誤採用を棄却。+3R⅓利確は心理オプション（未再検証・任意）。固定利確ラインは置かない。<b>入口注意はただ一つ: 52週高値−10%以内を追わない（−0.28R・唯一の生存要素）＝指値で下に置く。</b>'
        f'<span class="mut">※+3R利確＝触れた玉を数学的に不敗化、裾を担ぐ2/3は最も裾を殺さない10日安値に任せる役割分担。21EMA×0.93は出口に使わない（判定定規の警戒帯＝深さ−5%超×ADR≥6.5% のみ）。黄=新規停止・保有はトレール継続、赤=全撤退。</span></li>'
        ...
        f'<li><b>SOXL投入帯</b>（MAE実測ベース）：SOXLは<b>SOXXが50MAの+0〜3%上</b>にある日だけ新規投入する（青/緑の日）。この帯は建て直後の初期逆行（MAE）が最も浅く、<b>−25%級の大事故が半分</b>（50MA割れ直下25%→12%）。理由＝支持線がすぐ下＝invalidationが近く、割れたら即撤退できるので深傷になる前に逃げられる。'
        f'<b>投入は1/3ずつ×3トランシェ</b>（各サイズはボラターゲットでスケール）。<b>invalidation＝SOXXが50MAを終値で割ったら新規投入停止</b>（保有は地合いルールに従う）。'
...
def selftest(html, picks, setups, sectors):
    errs = []
    for sid in ["t-today","t-market","t-port","t-alloc","t-rotation","t-sector","t-rules"]:
...
def main():
    ...
    mkt["regime_state"] = compute_regime_state(m)
    ...
    if do_self:
        ...
        if errs:
            ...
            sys.exit(1)
        bs_, bys_ = leaders
        print("SELFTEST OK (8 tabs, status banner, N=%d, leaders=%d, buys=%d, sectors=%d, no None/nan)" %
