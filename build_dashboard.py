...
def _w30x_badge(r):
    ...
    return (f'<span class="{_W30_CLS[g]}" title="W30ブレイク {wk}{pend}{dead} — {desc}。'
            f'全期間実測(2016-26,N={s["n"]}): +30%到達{s["w30"]}% / +50% {s["w50"]}% / '
            f'52週2倍{s["dbl"]}% / 騙し(12週内30WMA割れ){s["fake"]}%">'
            f'{_W30_LAB[g]}{"" if k == 0 else f" {k}w"}</span>{tbadge}')
...
def build_w30_breakouts(m, cap=12):
    ...
    for t, r in m.iterrows():
        ...
        if not r.get("w30x_alive", True):
            continue
        _tr = {"S": 0, "A": 1, "B": 2}.get(r.get("w30x_tier"), 3)   # ◎内はS→A→Bで並べる
        rows.append((rk[g], _tr, -float(r.get("w30x_ext") or 0), t, r))
    rows.sort()
    ...
    body = "".join(_line(t, r) for _, _, _, t, r in rows[:cap]) or '<div class="mut">直近4週のシグナルなし</div>'
    ...
    signal_card = (f'<div class="card"><div class="hdr"><h2>ブレイク一覧 <span class="h2en">Signals</span></h2>{_cp([t for _,_,_,t,_ in rows[:cap]])}</div>'
