...
def _lev_env_card(le, rrg):
    ...
    return (
        ...
        f'<div class="sub">SOXLを新規で入れる時の環境チェック。<b>投入帯＝SOXXが50MAの+0〜3%上</b>のときだけ段階投入、それ以外はTQQQのみ（MAE実測でテール半減）。</div>'
...
def _new_entry_badge(r):
    ...
    return (f'<span class="newb" title="{lab}、36位圏（3N）の外から入ってきた銘柄。'
            f'新陳代謝を見るための表示で、買い条件そのものではない">NEW {lab}</span>')
...
def compute_regime_history(W, days=112):
    """F1/F2/F3の日次系列を過去days日分再計算し、各灯が注意/警戒に入った日を返す。
       閾値・母集団・定義は compute_regime_state と完全に同一（発火順の新名: F1脱落/F2キュー崩れ/F3勢い細り）。
    ...
    try:
        ...
        pool = (ma50 > ma200) & (C >= 5) & (dv >= DVOL_FLOOR)
        split_bad = (C.pct_change().abs() > 1.5).rolling(189).max().fillna(0).astype(bool)
        pool = pool & ~split_bad
        ...
        f1 = (was24 & (rk > 36)).sum(axis=1) / was24.sum(axis=1).replace(0, np.nan)
        # F2(新)=キュー崩れ: 適格母集団の反発待ち比率
        qual = (rs189p >= LEADER_RS) & (C > ma200) & pool
        f2 = (qual & ((ret20 <= 0) | (d52 < -0.15))).sum(axis=1) / qual.sum(axis=1).replace(0, np.nan)
        # F3(新)=勢い細り: 上位24のうち rs63<85
        top24 = (rk <= 24)
        f3 = ((rs63 < LEADER_RS) & top24).sum(axis=1) / top24.sum(axis=1).replace(0, np.nan)
        idx = C.index[-days:]
        out = {}
...
def compute_regime_state(m):
    """F1-F3の値と点灯数を計算して返す（パネルと条件カードで共用）。
       ...
       これにより「上位24リーダー」がポート表示のランキングと完全一致する。"""
    try:
...
def build_regime_alerts(m, st=None, collapsible=False, hist=None):
    ...
    try:
        ...
        return (
            ...
            f'</div>'
            f'<div class="note">'
            f'<p><b>3灯は足し算しない。</b>それぞれ別の問いに答える計器。複合ルール（2灯以上）は時期分割で基準を割るため採用しない（前半2.12x→後半0.69x）。</p>'
            ...
            f'<p class="mut">Fable監査 2026-07-08：灯ごとに元の検証指標（F1・F2＝赤転換までの先行日数と的中率／F3＝条件付きDD倍率）で再測定し、先行順に番号と並びを揃えた。'
            f'3灯を同一の物差しで測ると役割を取り違える。母数3N=36への拡大は信号希釈で不合格。'
            f'裏取りにはクレジット・VIX期間構造・売り抜け日・センチメントを使う（単体では動かない）。生存バイアスで全灯は過小＝実際はもっと早く点く。</p></div></div></div>')
    except Exception as e:
...
def build_portfolio(m, s2t):
    ...
    # 新規参入: 5日前(l5)/20日前(l20)の同一プール内RS189順位が3N=36位の外 → 今36位内に入った銘柄。
    #   ランクは cand と同一母集団(50>200×$10M×$5)内で付け直す＝表示中の順位と同じ土俵。
    _CAP = 3 * N_PORT
    def _rank_of(col):
        if col not in cand.columns:
            return None
        v = cand[col]
        return v.rank(ascending=False, na_option="bottom") if v.notna().sum() >= 30 else None
    _rk_now = cand["rs189"].rank(ascending=False)
    _entry = {}
    for col, lab in (("rs189_l5", "今週"), ("rs189_l20", "今月")):
        r = _rank_of(col)
        if r is None:
            continue
        for t in cand.index:
            ...
            if _rk_now.get(t, 999) <= _CAP and r.get(t, 999) > _CAP:
                _entry[t] = lab
...
def build_updown_vol_ts(W, macro=None, lookback=CHART_LB, win=20):
    """上げ日出来高÷下げ日出来高（20日・ユニバース合算$売買代金）。O'Neil式の集積/分散。
       市場の方向はQQQ日次で判定（取れなければ等加重ユニバース平均リターン）。>1=買い集め優勢。"""
    C = W.get("Close"); V = W.get("Volume")
...
def build_defense_checklist(st):
    """点灯した灯ごとに、その灯が実測で答える問いに対応する行動だけを出す。
       Fable監査(2026-07-08): 複合ルール(2灯以上)は時期分割で基準割れ→不採用。
    ...
    return (f'<div class="card def-card"><h2>⚠ 防御チェックリスト <span class="def-badge">{"・".join(lit)} 点灯</span></h2>'
            ...
            f'<p class="mut">Fable監査 2026-07-08：「2灯以上」ルールは時期分割で基準割れ（前半2.12x→後半0.69x）のため廃止し、灯ごとの単独運用に変更。</p>'
            f'</div></div>')
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    port = (f'<div class="card"><div class="hdr"><h2>ポートフォリオ（個別株スリーブ）</h2>'
            ...
            f'<b class="newb">NEW</b>＝36位圏（3N）に新しく入ってきた銘柄（今週/今月）＝ランキングの新陳代謝／'
    ...
    market = (
        ...
        + _mkt_section("③ 先行灯（崩れの予兆）", "種類=先行3〜8週。指数がまだ高値のうちに内部劣化を捉える",
                       ...
                       "複合ルール（2灯以上）は時期分割で基準を割るため廃止した（前半2.12x→後半0.69x）。</p>"
        ...
        + build_regime_alerts(m, hist=mkt.get("reg_hist"))
