...
def build_overview_cards(ctx):
    ...
    try:
        tpl1 = _b64.b64decode(OV1_TPL_B64).decode("utf-8")
    except Exception:
        tpl1 = "<html><body>Overview 1</body></html>"
    try:
        tpl2 = _b64.b64decode(OV2_TPL_B64).decode("utf-8")
    except Exception:
    ...
    try:
        ...
        try:
            if sectors:
                ...
                rows = []
                for s in ss:
                    base = float(s.get("score") or s.get("med") or 50)
                    rot = float(s.get("rot") or 0)
                    def clamp(v): return max(1, min(99, int(round(v))))
                    r21 = clamp(base + rot * 4); r63 = clamp(base + rot * 2)
                    r126 = clamp(base + rot); r189 = clamp(base)
                    wk = _ov_pctstr(s.get("w1"))
                    n = int(s.get("n") or 0)
                    top2 = s.get("top2") or []
                    ldr = (top2[0] if top2 else (s.get("leader") or "—"))
                    nm = s.get("ja") or "—"
    ...
    try:
        ...
        try:
            ...
            pass
        # SUB（小分類）: [name, en, parent, base, trend]
        try:
            en_map = globals().get("_OV_MICRO_EN", {})
            par_map = globals().get("_OV_MAJOR_EN", {})
            if sectors:
                ...
                for s in ss:
                    nm = s.get("ja") or "—"
                    en = en_map.get(nm, "")
                    parent = s.get("parent") or "テック"
                    base = int(round(float(s.get("score") or s.get("med") or 50)))
                    tr = round(float(s.get("rot") or 0), 1)
