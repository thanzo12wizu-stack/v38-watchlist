...
def build_portfolio(m, s2t):
    # 確定ルール: 189日RS上位15 × 200MA上 × 週足SAR非弱気 × 出来高金額$10M/日以上（テーマ上限なし＝上から純粋に）
    cand = m[(m["close"] > m["sma200"]) & (m["sar_bull"] == True)
             & (m["dvol"] >= DVOL_FLOOR)].copy()
...
def _perf_card(p):
    ...
    # 等ウェイト（RSP / QQQE）+ 21EMA・50SMA からの乖離
    be = p.get("breadth_etf") or []
    if be:
        blocks.append('<div class="perf-div">等ウェイト</div>')
        for ix in be:
            px, e21, s50 = ix.get("px"), ix.get("e21"), ix.get("s50")
            d21 = (px / e21 - 1) if (px and e21) else None
            d50 = (px / s50 - 1) if (px and s50) else None
            blocks.append(_strip(ix, [
                ("1週", fmt_pct(ix["w1"]), _pcls(ix["w1"])),
                ("1カ月", fmt_pct(ix["m1"]), _pcls(ix["m1"])),
                ("21EMA乖離", fmt_pct(d21), _pcls(d21)),
                ("50SMA乖離", fmt_pct(d50), _pcls(d50))]))
    # VIXカーブ
    vc = p.get("vixcurve"); vixhtml = ""
    if vc:
        r = vc.get("ratio")
        if r is None:
            vs, vcl = "—", "mut"
            ...
            vs, vcl = "バックワーデーション", "neg"
        v6 = f' / 6M {vc["v6"]:.1f}' if vc.get("v6") else ""
        vixhtml = (f'<div class="vixc"><span class="vixc-l">VIXカーブ</span> '
                   f'1M <b>{vc["v1"]:.1f}</b> / 3M {vc["v3"]:.1f}{v6} '
                   f'<span class="{vcl}">{vs}</span></div>')
    elif p.get("vix") is not None:
        vixhtml = f'<div class="vixc"><span class="vixc-l">VIX</span> <b>{p["vix"]:.1f}</b></div>'
    dv = p.get("divergence")
...
def _dd_card(dd):
    ...
    return (f'<div class="card"><h2>ディストリビューション・デイ（直近25営業日）</h2>'
            f'<div class="sub">前日比 −0.2%以下かつ出来高が前日超の「売り抜け日」。指数が+5%上昇すると解消。6本以上で調整警戒</div>'
...
def _ftd_card(ftd):
    ...
    return (f'<div class="card"><h2>フォロースルー・デイ</h2>'
            f'<div class="sub">下落後のラリー試行の確認日（安値から4日目以降に+1.5%×出来高増）。新規上昇トレンドの先行サイン</div>'
            f'{rows}</div>')
...
function _liqDefault(){
  document.querySelectorAll('section .lqf-rec').forEach(function(b){ liqFilter(10, b); });
}
...
def _sector_rank_card(ranks):
    ...
    return (f'<div class="card"><h2>セクター強弱</h2>'
            f'<div class="sub">S&amp;P11セクター（大分類）と業種テーマ（小分類）の騰落率</div>'
            f'<div class="sectog">'
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    for t, th, r in picks:
        ...
        nu_tag, _nun, nu_cls = momentum_nuance(r)
        rows.append(
            f'<tr data-liq="{(r.get("dvol") or 0)/1e6:.1f}"><td class="l mut">{_raw_rank.get(t,"—")}</td>'
    ...
    port = (f'<div class="card"><div class="hdr"><h2>ポートフォリオ（個別株スリーブ）</h2>'
            ...
            f'古典的な12-1モメンタムに基づく9ヶ月ルックバックが研究で最適。月1回フル入替。<b>#</b>は189日RS順位（テーマ上限なしなので保有=1〜15位／控え=16位以降と連番）。'
            ...
            + "".join(rows) + '</table>'
            f'<div class="note">選定：189日RS上位{N_PORT} × 200MA上 × 週足SAR非弱気 × 出来高金額$10M以上。<b>テーマ上限なし</b>（リーダーを上から純粋にRS順／同一テーマに偏り得る）・各1/{N_PORT}均等（＝1銘柄{per:.2f}%）。'
            f'通常損切りなし／毎月フル入替／利確ラダー無し（唯一の例外＝建値−30%でその銘柄のみ現金化）。'
    ...
    for rank, (t, r) in enumerate(deck.iterrows(), start=N_PORT+1):
        ...
        wrows.append(
            f'<tr data-liq="{(r.get("dvol") or 0)/1e6:.1f}"><td class="l mut">{rank}</td>'
            f'<td class="l tk" data-tkone="{t}">{t}'
    ...
    sector = (f'<div class="card"><h2>サブテーマ別RS（ユニバース内）</h2>'
              ...
              f'上から強い順。行をタップで構成銘柄をRS順に展開。※銘柄選定には不使用の参考指標</div>'
              f'<table><tr><th class="l">#</th><th class="l">サブテーマ（社数・RS）</th>'
    ...
    market = (banner
              ...
              f'<div class="sub">200日線を上回る銘柄の比率（≒$S5TH）</div>'
              f'<div class="kv"><span class="k">200MA上 <span class="mut" style="font-size:10px">≒$S5TH</span></span><span class="v">{breadth["pa200"]:.0f}% '
    ...
    rules = (
        f'<div class="card"><h2>システムルール（確定版 2026-06-26）</h2>'
        ...
        f'<ul class="rules">'
        f'<li><b>選定（月初に1回）</b>：<b>189日RS（≒9ヶ月モメンタム）</b>で上位{N_PORT} × 200日線上 × 週足SAR非弱気 × <b>出来高金額$10M/日以上</b>。<b>テーマ上限なし</b>（リーダーを上から純粋にRS順）・各1/{N_PORT}均等（古典的な12-1モメンタム）</li>'
