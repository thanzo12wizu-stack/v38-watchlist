...
def build_overview_cards(ctx):
    ...
    try:
        ...
        try:
            ...
            if nl and nl.get("vals") and _nlr:
                s3.append(f"ネット流動性は{_nlr[0]}（${nl['last']:.2f}T・4週{nl.get('chg4w',0):+.2f}）＝{_nlr[2]}")
            if top:
                tnm = "・".join((t.get("ja") or "—") for t in top)
                fl = f"資金は{tnm}へ"
                if bot:
    ...
    try:
        ...
        sc0 = sc
        # MAJ（大分類 11）: [jp, en, base_rs, trend]
        try:
            macro = ((mkt.get("sector_ranks") or {}).get("macro")) or []
            ...
            if macro:
                ...
                for x in macro[:11]:
                    nm = x.get("ja") or x.get("name") or "—"
                    en = en_map.get(nm, "")
                    base = x.get("rs") or x.get("score") or x.get("med")
                    if base is None:
                        base = 50
                    tr = 0.0
                    try:
                        tr = (float(x.get("w1") or 0) - float(x.get("m1") or 0)) * 100
                    except Exception:
                        tr = 0.0
                    rows.append([nm, en, int(round(float(base))), round(tr, 1)])
                if rows:
                    inner = ",".join("['%s','%s',%d,%s]" % (r[0], r[1], r[2], r[3]) for r in rows)
                    lit = ("[" + inner + "]\n    .map(function(r){return {name:r[0],en:r[1],col:PC[r[0]],"
                           "rs:{r21:clamp(r[2]+r[3]*5),r63:clamp(r[2]+r[3]*3),"
                           "r126:clamp(r[2]+r[3]*1.5),r189:clamp(r[2])}};})")
...
def compute_net_liquidity(fred_all):
    """ネット流動性 = FRB総資産 − リバースレポ − TGA（全て$兆換算・週次）。
       WALCL:百万$ / RRPONTSYD,WTREGEN:十億$。日付はWALCL基準でRRP/TGAをasof結合。
       リスク資産ベータと相関が高く、バランスシート単体より地合い判断に効く。"""
    try:
        import bisect
        fa = fred_all or {}
        def _map(sid, div):
            d = fa.get(sid) or {}
            return sorted(((pd.Timestamp(dd).normalize(), float(v) / div)
                           for dd, v in (d.get("vals") or []) if v == v), key=lambda x: x[0])
        walcl = _map("WALCL", 1e6); rrp = _map("RRPONTSYD", 1e3); tga = _map("WTREGEN", 1e3)
        if len(walcl) < 4:
            return None
        ...
        vals = [(dd.strftime("%Y-%m-%d"), w - _asof(dd, rd, rv) - _asof(dd, td, tv)) for dd, w in walcl]
        last = vals[-1][1]
        ...
        return dict(vals=vals, last=last, chg4w=last - prev4, chg13w=last - prev13,
                    walcl=walcl[-1][1], rrp=_asof(walcl[-1][0], rd, rv), tga=_asof(walcl[-1][0], td, tv))
    except Exception:
        return None
...
def build_net_liquidity_card(nl, asof=None):
    ...
    return (f'<div class="card"><h2>ネット流動性 <span class="h2en">Net Liquidity</span></h2>'
            f'<div class="sub">FRB総資産 − リバースレポ − TGA（$兆）。リスク資産のベータと連動＝地合いの本命。</div>'
            f'<div class="pulse"><div class="ptop"><span class="pnm">ネット流動性</span>'
            f'<span class="pbig">${last:.2f}T</span>'
            f'<span class="pchg {dcls}">{chg:+.2f}T<span class="mut">/4週</span></span></div>'
            f'<div class="pcat">4週トレンド：<b class="{dcls}">{dirt}</b>（13週 {nl.get("chg13w",0):+.2f}T）</div>'
            f'{sp}'
...
def _net_liq_read(nl):
    ...
    ch = nl.get("chg4w", 0.0)
    if ch >= 0.15:
        return ("追い風", "pos", "流動性がしっかり増加中。株の支援材料で、押し目は買われやすい")
    if ch >= 0.04:
        return ("やや追い風", "pos", "流動性は緩やかに増加。リスクオン寄りの地合い")
    if ch <= -0.15:
        return ("警戒", "neg", "流動性がはっきり減少中。株の重石になりやすく、上値は重い・戻り売りに注意")
    if ch <= -0.04:
        return ("やや警戒", "neg", "流動性は緩やかに減少。単独では売り材料でないが、他が崩れると効いてくる")
    return ("中立", "mut", "ほぼ横ばい。今は気にしなくてよい範囲（方向が出たら注目）")
...
def _market_comment(aux, mkt, sar, breadth, cat_html=""):
    ...
    _ddT = sum((v.get("n") or 0) for v in (mkt.get("distrib") or {}).values())
    _pnh = breadth.get("pnh")
    _inner = []
    if _ddT >= 8:
        _inner.append(f"売り抜け{_ddT}日で内部は明確に劣化")
    elif _ddT >= 5:
        _inner.append(f"売り抜け{_ddT}日でやや売り圧力")
    if finite(_pnh):
    ...
    if isinstance(_all3, (int, float)):
        _lead.append(f"RS3期間85超の質の高いリーダー{_all3}銘柄で層は" + ("薄い" if _all3 < 8 else "厚い"))
...
def build_categorized_commentary(mkt, m, breadth, sar, aux):
    ...
    total_dd = sum((v.get("n") or 0) for v in distrib.values())
    ftd = mkt.get("ftd") or []
    ...
    _ddq = ("内部が劣化（要警戒）" if total_dd >= 8 else ("やや重い" if total_dd >= 5 else "健全"))
    blocks.append(("マーケット", "Regime", [
        ...
        ("週次", f'スコアは週{dnum(sd5)}＝{scdir(sd5)}。売り抜け(distribution)は指数合計{total_dd}日で{_ddq}。'),
    ...
    blocks.append(("指数", "Indices", [
        ...
        ("週次", f'QQQ {pct(qqq.get("w1"))}・SPY {pct(spy.get("w1"))}。' + (f'売り抜けQQQ {qqq_dd}日/SPY {spy_dd}日＝' + ("6日以上で内部劣化・戻りは慎重" if (qqq_dd or 0) >= 6 or (spy_dd or 0) >= 6 else "許容範囲で押し目は機能") + '。' if qqq_dd is not None else '')),
    ...
    _layer = ""
    if fin(all3):
        _layer = f'RS3期間すべて85超の質の高いリーダーは{all3}銘柄で層は' + ("薄い（トレンドの土台が細い）" if all3 < 8 else "厚い") + '。'
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    body = (build_today_action(sar, mkt, asof)
            ...
            + _mkt_section("Overview 1 — マーケット概略", "", en="Market Overview")
            + '__POSTCARD1__</section>'
            f'<section id="t-post2">'
            + _mkt_section("Overview 2 — セクター・ローテーション", "", en="Sector Rotation")
...
def main():
    ...
    mkt["net_liq"] = compute_net_liquidity(_fred_all)
    mkt["net_liq_card"] = build_net_liquidity_card(mkt["net_liq"], asof=asof_bar)
    mkt["econ_cal"] = build_econ_calendar(asof=asof_bar)
    mkt["weekly_macro"] = build_weekly_macro_board(_fred_all, mkt["net_liq"], asof=asof_bar)
