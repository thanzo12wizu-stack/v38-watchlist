...
Command Center — dashboard builder (テクニカル構造強化 2026-07-11)
  ...
  D. 地合いスコア=14指標加重和(raw 0-100, 表示専用・ゲート不関与)。先導株の強さ温度計/センチメント/レバ環境も
...
def _leader_temp_card(lt):
    ...
    return (
        ...
        f'<div class="sub">リーダー群(RS189上位10%)の勢いが過去分布の何%タイルか＝獲物の温度。露出には非接続。</div>'
        f'{chart}'
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    today = (_liq_bar()
             + _mkt_section("① 構造で絞る", "ブレイク・VCP・カップ・転換・20/4", en="Price Structure")
             + build_structure_scanner(m)
             + _mkt_section("② 重なりで絞る", "兆候が4つ以上そろった銘柄", en="Signal Confluence")
             + build_confluence_watch(m, cand)
             + _mkt_section("③ 出来高とタイミング", "ポケットピボット・本日の候補", en="Volume & Timing")
             + build_pocket_pivots(m) + _buy_today_card(buytoday)
             + _mkt_section("④ ベースと反転", "VCP・タイト化・反転パターン", en="Bases & Reversals")
             + _vcp_card(vcp_rows) + _patterns_card(patterns)
...
def selftest(html, picks, setups, sectors, mkt=None, W=None, cutdate=None):
    ...
    if "構造スキャナー" not in html: errs.append("technical structure scanner missing")
    for _lab in ("真のブレイク", "VCP準備", "カップ・ハンドル", "転換確認", "20/4進行", "ブレイク失敗"):
        if _lab not in html: errs.append(f"technical structure block missing: {_lab}")
    if "収縮回数・安値切上げ・出来高枯れ・ピボット距離" not in html:
        errs.append("VCP structure fields missing")
...
def main():
    ...
    if do_self:
        ...
        if errs:
            ...
            sys.exit(1)
        bs_, bys_ = leaders
        print("SELFTEST OK (8 tabs, technical structure, emergency brake, RS compare+IN/OUT+continuity, N=%d, leaders=%d, buys=%d, sectors=%d, no None/nan)" %
