...
def build_lev_env(macro):
    ...
    ※Fable裁定: ボラは『可否』でなく『覚悟の目盛り』。強度スロットルも配分昇格しない。"""
    d = macro.get("SOXX")
    if d is None:
        return None
    c = d["Close"].dropna()
...
def _market_comment(aux, mkt, sar, breadth):
    """20-second market digest.

    The compact rows remain visible. Context and interpretation live inside a native
    <details> element so mobile users can expand or collapse the explanation.
    """
    def finite(v):
    ...
    def period_sector(key):
        ...
        wm = rank_one(mac, key, False)
        tops = [lead_text(x, key) for x in (tm, tn) if x]
        return ("・".join(tops) if tops else "—", lead_text(wm, key) if wm else "—")
    def names(xs, n=2):
    ...
    # Trend-change sentence: distinguish rebound, short pause, persistent improvement and persistent deterioration.
    ...
    rot_main = f'主導 {names(center)}／初動 {names(early)}／弱化 {names(weakening)}'
    ...
    q_today = f'QQQ {dpct(qqq.get("d1"))}・SPY {dpct(spy.get("d1"))}・上昇 {breadth.get("adpct", 0):.0f}%'
    q_week = f'QQQ {dpct(qqq.get("w1"))}・SPY {dpct(spy.get("w1"))}'
    q_month = f'QQQ {dpct(qqq.get("m1"))}・SPY {dpct(spy.get("m1"))}'
    score_line = f'Health {score:.0f}（1日 {dnum(sd1)}／1週 {dnum(sd5)}／1カ月 {dnum(sd21)}）'
    breadth_line = f'200MA上 {pa200:.0f}%（1週 {dnum(bd5, "pt")}／1カ月 {dnum(bd21, "pt")}）' if finite(pa200) else '200MA上 —'

    rows = [
        ("今日", q_today, f'主役 {today_top}／下位 {today_weak}'),
        ("1週間", q_week, f'主役 {week_top}／下位 {week_weak}'),
        ("1カ月", q_month, f'主役 {month_top}／下位 {month_weak}'),
        ("変化", score_line, f'{change}・{breadth_line}'),
        ("ローテ", rot_main, f'NQ {nq_col}・{nq_phrase}'),
    ]
    ...
    deep = [
        ...
        ("リスク", f'信用 {cr or "—"}・VIX構造 {vx}・{emtxt}' + (f'・売り抜け {"/".join(warn)}' if warn else '')),
    ...
    visible = ''.join(
        f'<div class="mkt20-row"><span class="mkt20-k">{k}</span><div><b>{main}</b><small>{sub}</small></div></div>'
        for k, main, sub in rows
    )
    detailed = ''.join(
        f'<div class="mkt20-deep"><span>{k}</span><b>{v}</b></div>' for k, v in deep
    )
    return (
        ...
        f'{note_html}<div class="mkt20-label">20秒サマリー</div><div class="mkt20-grid">{visible}</div>'
        f'<details class="mkt-fold"><summary><span>詳しい説明</span><span class="mkt-fold-state"></span></summary>'
...
def build_technical_board(m, cand=None, buytoday=None, cap_each=16):
    """One ticker, one primary state. Secondary evidence is shown as tags instead of repeated cards."""
    liq = (m["close"] >= 5) & (m["dvol"] >= DVOL_FLOOR)
    ...
    for t, r in m[liq.fillna(False)].iterrows():
        ...
        if not finite(rs189) or (float(rs189) < 70 and not failed):
            continue
        truebr = yes(r, "true_breakout")
        cupbr = yes(r, "cup_breakout")
        p204 = yes(r, "power20_4")
        vcp = yes(r, "vcp_ready")
        cup = yes(r, "cup_ready")
        turn = yes(r, "trend_change")
        ucr = yes(r, "ucr")
        ...
        ppd = r.get("pp_days")
        pp_now = finite(ppd) and int(ppd) == 0
        btag = buy_map.get(t)
        pbq, _ = pullback_quality(r)

        bucket = status = cls = None
        quality = 0.0
        if failed:
            bucket, status, cls = "回避", "ブレイク失敗", "st-fail"
            quality = abs(float(r.get("brk_ret") or 0.0)) * 100
        elif truebr:
            bucket, status, cls = "今", "ブレイク", "st-break"
            quality = float(r.get("breakout_score") or 0.0)
        elif cupbr:
            bucket, status, cls = "今", "カップ突破", "st-break"
            quality = float(r.get("breakout_score") or r.get("vcp_score") or 0.0)
        elif p204:
            bucket, status, cls = "伸長", "20/4", "st-high"
            quality = float(r.get("brk_ret") or 0.0) * 100.0
        elif btag == "押し目":
            bucket, status, cls = "今", "押し目", "st-pull"
            quality = float(pbq or 0.0)
        elif pp_now and yes(r, "stage2") and float(rs189) >= LEADER_RS:
            bucket, status, cls = "今", "PP", "st-break"
            quality = min(100.0, 50.0 + max(0.0, float(r.get("pp_ratio") or 1.0) - 1.0) * 20.0)
        elif vcp:
            bucket, status, cls = "形成", "VCP", "st-base"
            quality = float(r.get("vcp_score") or 0.0)
        elif cup:
            bucket, status, cls = "形成", "カップ", "st-base"
            quality = float(r.get("vcp_score") or 0.0)
        elif turn:
            bucket, status, cls = "形成", "転換", "st-turn"
            quality = float(rs189)
        elif flat:
            bucket, status, cls = "形成", "フラット", "st-base"
            quality = float(r.get("vcp_score") or 0.0)
        elif tight:
            bucket, status, cls = "形成", "3タイト", "st-base"
            quality = float(r.get("vcp_score") or 0.0)
        elif ucr:
            bucket, status, cls = "形成", "U&R", "st-turn"
            quality = float(rs189)
        elif htf:
            bucket, status, cls = "形成", "HTF", "st-high"
            quality = float(rs189)
        else:
            continue

        tags = []
        if pp_now and status != "PP": tags.append("PP")
        ...
        if tight and status != "3タイト": tags.append("3タイト")
        if flat and status != "フラット": tags.append("フラット")
        if cup and status not in ("カップ", "カップ突破"): tags.append("カップ")
        if turn and status != "転換": tags.append("転換")
        ...
        if status in ("ブレイク", "カップ突破"):
            vol = r.get("rvol_pre")
            voltxt = f'{float(vol):.1f}×' if finite(vol) else "—"
        else:
            vol = r.get("vdry_pre")
            voltxt = f'{float(vol):.2f}×' if finite(vol) else "—"
        rows.append(dict(t=t, bucket=bucket, status=status, cls=cls, q=quality,
    ...
    order = {"今": 0, "形成": 1, "伸長": 2, "回避": 3}
    rows.sort(key=lambda x: (order[x["bucket"]], -x["q"], -x["rs"], x["t"]))
    selected = []
    counts = {}
    for row in rows:
        b = row["bucket"]
        if counts.get(b, 0) >= cap_each:
            continue
    ...
    for row in selected:
        if row["bucket"] != cur:
            cur = row["bucket"]
            nall = sum(1 for x in rows if x["bucket"] == cur)
            body.append(f'<tr class="scr-gh"><td colspan="6"><b>{cur}</b><span class="scr-gn">{nall}件</span><span class="scr-gd">{group_desc[cur]}</span></td></tr>')
            body.append('<tr class="scr-ch"><td class="l">銘柄</td><td>状態</td><td>RS189</td><td>ピボット差</td><td>出来高</td><td class="l">補強材料</td></tr>')
        ...
        body.append(
            ...
            f'<td class="scr-tk">{row["t"]}</td>'
            f'<td><span class="bt-tag {row["cls"]}">{row["status"]}</span></td>'
    ...
    return (f'<div class="card hot-card"><div class="hdr"><h2>テクニカル候補</h2>{_cp(sorted(tickers))}</div>'
            f'<div class="sub">同じ銘柄は最も優先度の高い状態に1回だけ掲載。ほかの形は補強材料へまとめる。</div>'
...
def render(names, m, mri, breakdown, dropped, aux, setups, picks, cand,
    ...
    today = (_liq_bar()
             + _mkt_section("① テクニカル候補", "1銘柄1行で優先状態に整理", en="Technical Setups")
             + tech_board
...
def selftest(html, picks, setups, sectors, mkt=None, W=None, cutdate=None):
    ...
    if "1銘柄1行で優先状態に整理" not in html: errs.append("technical deduplication label missing")
    ...
    if "20秒サマリー" not in html: errs.append("20-second market digest missing")
...
def main():
    ...
    if do_self:
        ...
        if errs:
            ...
            sys.exit(1)
        bs_, bys_ = leaders
        print("SELFTEST OK (8 tabs, unified technical board, market health v2, emergency brake, RS compare+IN/OUT+continuity, N=%d, leaders=%d, buys=%d, sectors=%d, no None/nan)" %
