...
def _w30x_badge(r):
    ...
    return (f'<span class="{_W30_CLS[g]}" title="W30ブレイク {wk}{pend}{dead} — {desc}。'
            ...
            f'{_W30_LAB[g]}</span>{tbadge}{wk_lab}')
...
def build_w30_breakouts(m, cap=12):
    ...
    legend_card = ('<div class="card"><h2>定義・グレード・格付け <span class="h2en">Grades</span></h2>'
            ...
            + _trow(_b("w30b w30b-part","○ PART"), "[1][2]のみ", W30X_STATS["PART"])
            + _trow(_b("w30b","・CROSS"), "素の上抜け", W30X_STATS["CROSS"])
            ...
            'スコア0→4で+30%到達35→58〜79%の単調性を全期間で確認済み。</div></div>')
