...
def test_report_publisher_serializes_and_retries_after_rebase() -> None:
    ...
    assert workflow.index("git rebase origin/main") < workflow.index("if git push origin HEAD:main")
