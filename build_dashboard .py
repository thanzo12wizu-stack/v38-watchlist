...
.rrow{{display:flex;flex-wrap:wrap;align-items:baseline;justify-content:space-between;gap:2px 10px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:9px 12px}}
...
.rrow .rgt{{text-align:right;flex:0 0 auto;max-width:100%}}
...
def build_rates_card(ctx, asof=None):
    ...
    for sid,cat,lab,unit,colored in rows_def:
        ...
        speed_txt=f'・速度 {spd:.0f}%' if spd is not None else ''
        cells.append(f'<div class="rrow"><div class="lft"><div class="nm">{lab}{alt}</div><div class="cat">{cat}</div></div>'
                     ...
                     f'<span class="hint">水準(1年) {pct_txt}{speed_txt}</span></div><div class="foot">観測 {eff_ld or "—"}{stale_txt}・{src_lab}{cache_txt}</div></div>')
...
def build_rates_insight(fred_ctx, asof=None):
    ...
    try:
        ...
        hist=df.dropna(subset=["dr","do","fwd"]); hist=hist.groupby(hist.index.to_period("W")).tail(1)
        cells={}
        for rn,rc in (("実質金利↑",hist.dr>0),("実質金利↓",hist.dr<=0)):
            for on,oc in (("OAS拡大",hist.do>0),("OAS縮小",hist.do<=0)):
                seg=hist.loc[rc&oc,"fwd"]; cells[(rn,on)]=(float(seg.median())*100,float((seg>0).mean())*100,len(seg)) if len(seg)>=30 else None
        ...
        return f'<div class="card"><h2>金利×信用 <span class="h2en">Rates × Credit</span></h2><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:6px">{"".join(boxes)}</div><div class="rmeta mut" style="margin-top:5px">週次観測 {hist.index[0].strftime("%Y-%m")}〜{hist.index[-1].strftime("%Y-%m")}・n={len(hist):,}・QQQ 21営業日先の中央値/勝率</div></div>'
    except Exception as e:
