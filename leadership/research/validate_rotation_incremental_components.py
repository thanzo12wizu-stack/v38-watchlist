...
def strict_events(df,mask):
    ...
    for _,g in z.groupby('sector',sort=False):
        ...
        for i,r in g.iterrows():
            a=bool(r._s)
            if a and not prev and i-last>=COOLDOWN:rows.append(r.drop(labels='_s').to_dict());last=i
            prev=a
...
def cluster_diff(ev,group,a,b,outcome,reps=5000,seed=1):
    ...
    obs=calc(z);rng=np.random.default_rng(seed);vals=[]
    if len(sectors)>=3:
        for _ in range(reps):
            parts=[z[z.sector==s] for s in rng.choice(sectors,len(sectors),replace=True)];v=calc(pd.concat(parts,ignore_index=True))
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--panel',type=Path,required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args();a.output.mkdir(parents=True,exist_ok=True)
    df=pd.read_csv(a.panel,parse_dates=['date']).sort_values(['sector','date'])
    df['internal_delta20']=df.groupby('sector').internal_score.diff(20)
    ...
    report={'schema':1,'research_only':True,'method':'Transition-only high-price entry events; 20-session cooldown; 2024+; sector-cluster bootstrap. Tests incremental components rather than absolute state returns.','comparisons':{}}
    for h in (20,40):
    ...
    (a.output/'incremental_component_report.json').write_text(json.dumps(safe(report),ensure_ascii=False,indent=2),encoding='utf-8')
    lines=['# Rotation Incremental Component Validation','', 'This asks whether Internal / Flow / deterioration add information after entering the same high-price state.','', '| Comparison | Difference | cluster 95% CI |','|---|---:|---|']
    for k,v in report['comparisons'].items():lines.append(f"| {k} | {100*v['difference']:+.2f}% | [{'' if v['sector_cluster_ci95'][0] is None else f'{100*v[\"sector_cluster_ci95\"][0]:+.2f}%'}, {'' if v['sector_cluster_ci95'][1] is None else f'{100*v[\"sector_cluster_ci95\"][1]:+.2f}%'}] |")
    # Avoid relying on this markdown if a Python version rejects nested formatting; JSON is authoritative.
    try:(a.output/'README.md').write_text('\n'.join(lines)+'\n',encoding='utf-8')
    except Exception:pass
    print('DONE incremental component validation')
