...
def audit_current_tree(root: Path) -> dict:
    ...
    if private.exists():
        for path in private.rglob("*"):
            ...
            allowed = relative.endswith(".enc.json")
            if not allowed:
                private_plaintext.append(relative)

    passed = not forbidden_paths and not missing_public and not public_marker_hits and not private_plaintext
    return {
        "schema_version": "1.0",
        ...
        "locked_dashboard_error_count": 0,
