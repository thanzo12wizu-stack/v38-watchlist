...
# Strict LOO: keep adjusted Open in a transient cache and recover missing
# yfinance batch symbols via crumb-free Yahoo chart / FMP fallback.
path = "build_v38_strict_loo_live.py"
replace_once(
    path,
...
new_downloader = '''def _write_price_cache(path: str | Path, open_: pd.DataFrame, close: pd.DataFrame,
...
'''
replace_block(path, "def download_adjusted_close(", "def arithmetic_returns", new_downloader)
replace_once(
    path,
...
# Sleeve: consume the same transient cache, only downloading genuinely absent symbols.
path = "build_v38_sleeve_live.py"
replace_once(
    path,
    "MIN_THEME_MEMBERS = 3\n",
    'MIN_THEME_MEMBERS = 3\nPRICE_CACHE_SCHEMA = "v38-sleeve-price-cache-1"\n',
)
cache_helpers = '''def load_shared_price_cache(path: str | Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
...
'''
replace_once(path, "def yahoo_symbol(symbol: str) -> str:\n", cache_helpers + "def yahoo_symbol(symbol: str) -> str:\n")
replace_once(
    path,
...
# Wire the transient cache through the canonical workflow.
path = ".github/workflows/dashboard.yml"
replace_once(
    path,
    "            --history v38-strict-loo-history.json \\\n            --out v38-strict-loo-live.json",
    "            --history v38-strict-loo-history.json \\\n            --price-cache /tmp/v38-sleeve-price-cache.pkl.gz \\\n            --out v38-strict-loo-live.json",
)
replace_once(
    path,
...
# Network-free round-trip regression test.
test_path = Path("tests/test_v38_strict_loo_live.py")
text = test_path.read_text(encoding="utf-8")
if "def test_shared_price_cache_roundtrip_for_sleeve(" not in text:
    text += '''
