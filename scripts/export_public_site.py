...
# Optional source -> public-target mappings. Leadership and live Rotation data
# are produced independently, so the normal dashboard exporter preserves them
# when present without making Command Center generation depend on them.
OPTIONAL_PUBLIC_FILES = (
    ...
    # Standalone research UI. It reads the existing Options positioning snapshot
    # and is intentionally not wired into V38 ranking or trading rules.
    ("options-intelligence.html", "options-intelligence.html"),
    # Rotation live data is generated by its own isolated workflow. Until the
    # producer succeeds the UI must fail closed rather than require stale data.
    ("rotation/data/rotation-theme56.json", "rotation/data/rotation-theme56.json"),
    ("rotation/data/rotation-theme56-stock-context.json", "rotation/data/rotation-theme56-stock-context.json"),
    ("rotation/data/status.json", "rotation/data/status.json"),
)
