...
def sub_once(pattern, repl, name, flags=re.S):
    global s
    s2, n = re.subn(pattern, lambda m: repl, s, count=1, flags=flags)
    if n != 1:
        raise SystemExit(f'patch {name}: expected 1 match, got {n}')
    s = s2

old_doc = '  D. Market Conditions=中長期の市場構造（短期15・中期55・長期20・Damage10）。'
new_doc = '  D. Market Conditions=57ETF × 12指標の完全等加重（方向対称・EMA2）。Breadth 10日変化は別軸表示。'
if old_doc not in s:
    raise SystemExit('top doc marker not found')
s = s.replace(old_doc, new_doc, 1)

universe = '''# Market Conditions v4: fixed, non-levered cross-sectional ETF universe.
...
sub_once(
    r'# Market Conditions v3: fixed, non-levered cross-sectional ETF universe\.\n.*?assert len\(MC_MARKET_TICKERS\) == 43',
    universe, 'universe')
...
sub_once(r'STATUS_DEF = \[\n.*?\n\]\n# Pillar weights:[^\n]*', status, 'status-def')

participation = '''def _mc_participation(mask):
...
def _mc_stratified_median'''
sub_once(r'def _mc_participation\(mask\):\n.*?\n\n\ndef _mc_stratified_median', participation, 'participation')

mri = '''def mri_frame(macro, W=None):
...
sub_once(r'def mri_frame\(macro, W=None\):\n.*?\n\n\ndef mri_auxiliary', mri + '\n\n\ndef mri_auxiliary', 'mri-frame')

aux = '''def mri_auxiliary(mri, vals, metrics):
...
sub_once(r'def mri_auxiliary\(mri, vals, metrics\):\n.*?\n\n\ndef mri_band', aux + '\n\n\ndef mri_band', 'mri-aux')

old_kj = '_kj = {"trend":"トレンド","breadth":"広がり","risk":"ボラティリティ","credit":"信用"}'
new_kj = '_kj = {"short":"モメンタム","medium":"広がり","long":"トレンド","damage":"Damage","trend":"トレンド","breadth":"広がり","risk":"ボラティリティ","credit":"信用"}'
if old_kj not in s:
    raise SystemExit('render label map not found')
s = s.replace(old_kj, new_kj, 1)

old_head = 'MARKET CONDITIONS 内訳（短期15 / 中期55 / 長期20 / Damage10）'
new_head = 'MARKET CONDITIONS 内訳（57ETF × 12指標・完全等加重）'
if old_head not in s:
    raise SystemExit('breakdown header not found')
s = s.replace(old_head, new_head, 1)

old_aux = '''      <div class="aux">
        <div class="a">傾き <b>{aux['slope']}</b></div>
        <div class="a">警戒 <b>{aux['bear_n']}</b>/4 {_led(aux['bear_n'], 4)}</div>
      </div>'''
new_aux = '''      <div class="aux">
        <div class="a">傾き <b>{aux['slope']}</b></div>
        <div class="a">内部10D <b>{aux['breadth_arrow']} {aux['breadth_delta10']:+.1f}pt</b></div>
        <div class="a">警戒 <b>{aux['bear_n']}</b>/4 {_led(aux['bear_n'], 4)}</div>
      </div>'''
if old_aux not in s:
    raise SystemExit('banner aux block not found')
...
if 'raw = short*.15 + medium*.55 + long*.20 + damage*.10' in s:
    raise SystemExit('old asymmetric weighting remains')

p.write_text(s, encoding='utf-8')
print('patched build_dashboard.py')
