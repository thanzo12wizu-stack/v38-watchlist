...
# Backend: preserve same-session scan metadata and verify only trustworthy current scan rows.
...
'''
    if old not in s:
        raise SystemExit("backend _day anchor not found")
...
def _hist_obs(row, source="HISTORY"):
    ...
    return {"date": _day(row.get("date")),
            ...
            "options_observed_at": row.get("observed_at"),
...
s = sub_once(
    s,
    r'def _time_quality\(cur, session_date, source\):\n.*?\n\n\ndef _direction_bias',
    tq_new.rstrip() + '\n\n\ndef _direction_bias',
    "backend time quality",
    re.S,
)
old_build = 'latest,previous=sh.get(tk) or dh.get(tk) or (None,None); cur=_hist_obs(latest); prev=_hist_obs(previous); source="SCAN" if tk in sh else "HISTORY"'
new_build = 'latest,previous=sh.get(tk) or dh.get(tk) or (None,None); source="SCAN" if tk in sh else "HISTORY"; cur=_hist_obs(latest,source); prev=_hist_obs(previous,source)'
if old_build not in s:
    raise SystemExit("backend build source order not found")
s = s.replace(old_build, new_build, 1)
p.write_text(s, encoding="utf-8")


# Frontend: broad scan is current only for swing 7-21DTE, never for other period tabs.
p = Path("options-intelligence.js")
s = p.read_text(encoding="utf-8")
if "const boolish =" not in s:
    old = "const day = v => String(v || '').slice(0, 10);"
    new = old + "\nconst boolish = v => v === true || ['1', 'true', 'yes', 'y'].includes(String(v || '').trim().toLowerCase());"
    if old not in s:
        raise SystemExit("frontend day anchor not found")
    s = s.replace(old, new, 1)
...
s = sub_once(
    s,
    r'function timeQuality\(cur, source, u\) \{.*?\n\}\n\nfunction localSignal',
    tq_js.rstrip() + '\n\nfunction localSignal',
    "frontend time quality",
    re.S,
)
old_else = textwrap.dedent('''\
    } else {
      const p = sh[t] || dh[t];
      if (!p) continue;
      cur = histObs(p[0]);
...
if old_else not in s:
    raise SystemExit("frontend broad-scan branch not found")
s = s.replace(old_else, new_else, 1)
p.write_text(s, encoding="utf-8")


# Regression tests for the exact time-quality boundary being fixed.
...
print("patched same-session SCAN verification")
